...
!
! !SYSTEM ROUTINES:
!     MPI_TYPE_GET_EXTENT, MPI_TYPE_CREATE_HVECTOR, MPI_TYPE_COMMIT
!
! !REVISION HISTORY:
...
! !LOCAL VARIABLES:
      ...
      INTEGER  Disp, Length, I, J, lb, extent, ub, method
...
!
        ...
        DO Ipe=1, GroupSize
          IF ( PatternIn%SendDesc(Ipe)%type /= MPI_DATATYPE_NULL ) THEN
            CALL MPI_TYPE_GET_EXTENT( PatternIn%SendDesc(Ipe)%type, lb, extent, ierror )
            ub = lb + extent
            Stride_S = max(Stride_S,ub)
          ENDIF
          IF ( PatternIn%RecvDesc(Ipe)%type /= MPI_DATATYPE_NULL ) THEN
            CALL MPI_TYPE_GET_EXTENT( PatternIn%RecvDesc(Ipe)%type, lb, extent, ierror )
            ub = lb + extent
...
!
        DO Ipe=1, GroupSize
          IF ( PatternIn%SendDesc(ipe)%type /= MPI_DATATYPE_NULL ) THEN
            CALL MPI_TYPE_CREATE_HVECTOR( Mult, 1, Stride_S, PatternIn%SendDesc(Ipe)%type,&
          ...
          ENDIF
          IF ( PatternIn%RecvDesc(Ipe)%type /= MPI_DATATYPE_NULL ) THEN
            CALL MPI_TYPE_CREATE_HVECTOR( Mult, 1, Stride_R, PatternIn%RecvDesc(Ipe)%type,&
