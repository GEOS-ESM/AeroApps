...
VNAMES_SU = ['SO4']
VNAMES_NI = ['NH3','NH4a','NO3an1','NO3an2','NO3an3']
...
#---
def writeNC ( stations, lons, lats, tyme, isotimeIn, MieVars, MieVarsNames,tUnits,
...
#   Replaced to be consistent with Orbit  time. 
...
#   --------------------------
    ...
    # Get Variables
    # --------------------------
    (Vars, tUnits) = getVars(options.inFile)
...
#           MieVars = computeMie(Vars,channelIn,VNAMES,options.rcFile,options)
#       except:
#           # No Nitrates/Brown Carbon in Aer File
#           # ------------------------------------
#           VNAMES = [ 'DU001', 'DU002', 'DU003', 'DU004', 'DU005',
#                      'SS001', 'SS002', 'SS003', 'SS004', 'SS005',
#                      'BCPHOBIC', 'BCPHILIC',
...
#           MieVars = computeMie(Vars,channelIn,VNAMES,options.rcFile,options)
    ...
    writeNC(StnNames,Vars.LONGITUDE,Vars.LATITUDE,Vars.TIME,Vars.ISOTIME,tUnits,
            MieVars,MieVarsNames,MieVarsUnits,options.inFile,options.outFile,options)

    outstring = options.outFile.split('.')
    filebase = ''
    for i in range(0,len(outstring)-1):
        filebase = filebase+outstring[i]+'.'

    if options.dust:
        #outFile = options.outFile+'.dust'
        
        outFile = filebase+'du.nc'
