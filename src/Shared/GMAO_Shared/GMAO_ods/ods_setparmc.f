...
!BOP
!
! !ROUTINE:      ODS_SetParmC --- Sets an ODS character parameter for creating files
! 
! !DESCRIPTION: 
!    \label{ODS:SetParmC}
...
*        ------------------------------
         ParmC_nlist = ParmC_nlist + 1

*        Return with an error message if the list size
*        exceeds the allocated space
*        ---------------------------------------------
         if ( ParmC_nlist .gt. Parm_mlist ) then
            ierr = NCSysErr
            write ( stderr, 901 )
            return
         end if

*        Store the NetCDF file parameter name and string
*        -----------------------------------------------
         ParmC_list   ( ParmC_nlist ) = ParmName
         ParmC_string ( ParmC_nlist ) = ParmString

      else

*        Store the NetCDF file parameter string
*        --------------------------------------
         ParmC_string ( list_entry  ) = ParmString
...
*     ------
 ...
 901  format ( /, ' ODS_SetParmC : The number of parameters to ',
