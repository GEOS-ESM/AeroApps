...
*...................................................................
      ...
      subroutine ODS_NCP_RtoI4 ( ncid,  varid,
     .                           start, count,
     .                           nval,  values, ierr )
...
!-------------------------------------------------------------------------
!
! !ROUTINE:  ODS_NCP_RtoI4
! 
! !DESCRIPTION: 
!     This routine converts native real numbers to four byte
!     integers and calls the NetCDF routine, NCVPT, to write the
!     four byte integers to an ODS/HDF file.  This routine insures
!     that the data are correctly passed into the NetCDF routine,
!     irrespective of the machine on which the software is running. 
!
! !INTERFACE:  ODS_NCP_RtoI4 ( ncid,  varid,
!                                     start, count,
!                                     nval,  values, ierr )
!
! !INPUT PARAMETERS: 
      ...
      real               values ( * ) ! Values to be written
!
! !OUTPUT PARAMETER:
...
!
! !SEE ALSO:
!     ODS_NCG_I4toR  ( The inverse companion of this routine )
!
! !REVISION HISTORY: !
...
*     -----------------------------------------------------
      do 10, ival = 1, nval
         val      = dble ( values ( ival ) )
         if ( val  .lt. I4_MinD .or.
     .        val  .gt. I4_MaxD ) ierr = NCSysErr
 10   continue

*     If a value is out of range then return with an error message
*     ------------------------------------------------------------
      if ( ierr .ne. NCNoErr  ) then
         write ( stderr, 901) 'NCLong'
         return
      end if

*     Convert to 2-byte integers and store in work space
*     --------------------------------------------------
      do 20, ival = 1, nval
         I4_Val ( ival ) = nint ( values ( ival ) )
 20   continue
...
*     ------
 ...
 901  format ( /, ' ODS_NCP_RtoI4: Value to be written as the ',
