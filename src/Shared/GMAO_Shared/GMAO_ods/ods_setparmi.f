...
!BOP
!
! !ROUTINE:      ODS_SetParmI --- Sets a NetCDF integer parameter for creating files
! 
! !DESCRIPTION: 
!    \label{ODS:SetParmI}
...
!     contain any delimters.  Some usefule parameters are as
!     follows:
!
! \bv
...
*        ----------------------------------------------
         ParmI_list ( ParmI_nlist ) = ParmName
         ParmI_val  ( ParmI_nlist ) = ParmVal

      else

*        Store the NetCDF file parameter value
*        -------------------------------------
         ParmI_val  ( list_entry  ) = ParmVal
...
*     ------
 ...
 901  format ( /, ' ODS_SetParmI : The number of parameters to ',
