...
!BOP
!
! !ROUTINE: ODS_PutC() --- Writes data in a character array to file
! 
! !DESCRIPTION:
!  \label{ODS:PutC}
!     Stores the contents of an {\tt character} variable for a full
...
!    
!  \item This routine checks to determine if the string length declared
!     in the ODS file exceeds the length of each string in the input
!     array to ensure that the entire string is written to the ODS file.
!
!  \item Once data from the specified date and hour has been written,
!     the routine will allow overwriting of data from a previous
!     date and hour.  The only restriction is that the size of the 
!     input data block does not exceed the space already allocated
!     in the ODS file.  Only the first {\tt nval} values will be
!     overwritten.
!
!  \end{enumerate}
!
! !INTERFACE: 
!
      subroutine ODS_PutC ( id, VarName, julian_day, syn_hour,
     .                          nval,    values,     ierr )
!
! !INPUT PARAMETERS:
      implicit        NONE
      ...
      integer         nval            ! number of values for this
                                      !  synoptic time.
      character * (*) values ( nval ) ! Character strings to be written
!
! !OUTPUT PARAMETERS:
...
*     If allocated space in file is insufficient to
*     store each string then exit with error status
*     ---------------------------------------------
      if ( str_len .lt. len ( values ( 1 ))) then
         write ( stderr, 907 ) len ( values ( 1 )), str_len, VarName
         ierr  = ODS_BadLen
         return
      end if
...
*     -------------------------------
      ...
      count ( 2 ) =   first_nval
      count ( 3 ) =   1
      call ODS_NCVPTC ( nc_id, varid, start, count, values, ierr )
      if ( ierr .ne. NCNoErr ) return
...
*     ----------------------------------------------------
      ...
      do 10, ibatch = first_batch + 1, last_batch - 1
         ...
         count ( 2 ) = batch_len
         count ( 3 ) = 1
         call ODS_NCVPTC ( nc_id, varid, start, count,
     .                     values ( ival ), ierr )
         if ( ierr .ne. NCNoErr ) return
         ival        = ival + batch_len
 10   continue
...
*     ------
 ...
 901  format ( /, ' ODS_PutC: File handle id number does not ',
     .         /, '           correspond to an opened ODS file' )
 902  format ( /, ' ODS_PutC: Cannot write to file unless mode ',
     .         /, '           is set to write.  Filename is: ', a )
 903  format ( /, ' ODS_PutC: Inappropriate variable for this ',
     .         /, '           routine.  Variable name is: ', a ) 
 904  format ( /, ' ODS_PutC: The number of values to be written ',
     .         /, '           must be a non-negative integer. ',
     .         /, '           Variable name is: ', a ) 
 905  format ( /, ' ODS_PutC: Cannot increase the size of a ',
     .         /, '           block of data that corresponds ',
     .         /, '           to a date and time earlier than ',
     .         /, '           the latest date and time for ',
     .         /, '           which there is data. ' )
 906  format ( /, ' ODS_PutC: Must use ODS_Append to increase the '
     .         /, '           size of a block of data that corre-'
     .         /, '           sponds to the latest date and time '
     .         /, '           for which there is data. ' )
 907  format ( /, ' ODS_PutC: The length of each string in the given ',
     .         /, '           input array (= ', i5, ') exceeds the ',
     .         /, '           length of each string declared in the ',
     .         /, '           ODS file array (= ', i4, ').',
     .         /, '           Variable name is: ', a )
