...
def append_lml(ncin, options, zlib=False):
    ...
    for v in ['NH4','so4_a1','so4_a2','so4_a3','pom_a4',
       ...
       'dst_a2','dst_a3','ncl_a1','ncl_a2','ncl_a3','CO','NO2','NO','O3','SO2','rho']:
       #'dst_a2','dst_a3','ncl_a1','ncl_a2','ncl_a3','CO','CO01','NO2','NO','O3','SO2','rho']:
...
def append_vars(ncin, options, zlib=False):
    ...
    rho = ncin.variables['rho'][:,:,:,:]  # kg m-3 
    nt, nz, ny, nx = co.shape
    oc = npy.zeros((nt,nz,ny,nx))

    for v in ['pom_a1','pom_a4','soa_a1','soa_a2']: 
        oc = oc + ncin.variables[v][:,:,:,:]/1.4 # 1.4 is the ratio of pom emissions vs co emissions 
...
#   --------------------------
    ...
    # copy original file 
    # ------------------
    os.system("cp "+cam_File+" "+cam_File+".interm.nc")

    # Open the input file
    # -------------------
    nc0 = Dataset(cam_File, "r", format=options.format)
    nc = Dataset( cam_File+".interm.nc", 'a', format=options.format)
    ...
    os.system("rm "+cam_File+".interm.nc")
