#!/usr/bin/env python3
"""
    Sampler for CASTNET surface HNO3 obs for a modeling experiment
...
if __name__ == '__main__':
    ...
    station,lon,lat = sites['SITE_ID'].values, sites['LONGITUDE'].values, sites['LATITUDE'].values
    ...
    outdir = config['sampled_outdir'] + '/castnet'
    ...
    with LocalCluster(**kwargs) as cluster, Client(cluster) as client:
        logger.info(f"+++++++ CASTNET Sampling on {site_path}")
        ...
        for ctl in ctls:
            ...
            outFile = f'{outdir}/castnet.{ctl}.nc4'
            First = True
