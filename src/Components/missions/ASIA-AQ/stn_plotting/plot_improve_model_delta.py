...
from ts_plot_delta import ts_plot_delta
...
#    spc_mapping = {'ammNO3f': 'NI'}
    ...
    for spc in spc_mapping:
        ...
        df_baseline = monthly_mean.to_dataframe(name="PM25").reset_index()
        df_baseline["Source"] = "Baseline"
        
        monthly_mean = imp_data.groupby('time.month').mean(skipna=True).rename({'month': 'time'})
        df_imp = monthly_mean.to_dataframe(name="PM25").reset_index()
        ...
        df_baseline_diff = monthly_mean.to_dataframe(name="PM25").reset_index()
        ...
        df_oxh_diff= monthly_mean.to_dataframe(name="PM25").reset_index()
        ...
        df_oxm_diff = monthly_mean.to_dataframe(name="PM25").reset_index()
        df_oxm_diff['Source'] = 'OxM - IMPROVE'
        
        # Create dataframe for bottom subplot
        plot_df_bottom = pd.concat([df_baseline_diff, df_oxh_diff, df_oxm_diff], ignore_index=True)
        
        # Create the plot
        if config.get('do_ts_delta', False):
            ts_plot_delta(plot_df_top, plot_df_bottom, spc, plot_outdir)
