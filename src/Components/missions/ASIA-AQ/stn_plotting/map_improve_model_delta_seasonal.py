...
#    spc_mapping = {'ammNO3f': 'NI'}
    ...
    for seas in seasons:
        for spc in spc_mapping:
            ...
            df_baseline = ds_baseline.sel(time=ds_baseline.time.dt.season == seas).to_dataframe(name="PM25").reset_index()
            df_baseline["Source"] = "Baseline"
             
            monthly_mean = imp_data.groupby('time.month').mean(skipna=True).rename({'month': 'time'})
            df_imp = imp_data.sel(time=imp_data.time.dt.season == seas).to_dataframe(name="PM25").reset_index()
            ...
            df_baseline_diff = ds_baseline_diff.sel(time=ds_baseline_diff.time.dt.season == seas).to_dataframe(name="PM25").reset_index()
            ...
            df_oxh_diff= ds_oxh_diff.sel(time=ds_oxh_diff.time.dt.season == seas).to_dataframe(name="PM25").reset_index()
            ...
            df_oxm_diff = ds_oxm_diff.sel(time=ds_oxm_diff.time.dt.season == seas).to_dataframe(name="PM25").reset_index()
            ...
            titles  = ['OX GMI '+seas,'OXM '+seas,'OXH '+seas]

            os.makedirs(plot_outdir+'/'+seas,exist_ok=True) 
            # Create the plot
            if config.get('do_map_mae', False):
                plot_station_map_mae(df_list,spc,outdir=plot_outdir+'/'+seas,titles=titles)
            if config.get('do_map_mb', False):
                plot_station_map_mb(df_list,spc,outdir=plot_outdir+'/'+seas,titles=titles)
