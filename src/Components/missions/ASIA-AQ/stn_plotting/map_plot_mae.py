...
def plot_station_map_mae(df_list, spcname, lon_col='Longitude', lat_col='Latitude', 
...
#        vmax = max(sm['abs_mean'].max() for sm in station_means)
    ...
    cbar.set_label(f'Mean Absolute Error {spcname} ({units})', fontsize=11)
...
#    plt.tight_layout()
    ...
    if outdir is not None:
        outfile = f'{outdir}/{spcname}_map_mae.png'
