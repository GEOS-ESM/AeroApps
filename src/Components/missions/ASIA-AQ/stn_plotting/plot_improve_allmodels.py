...
#    spc_mapping = {'ammNO3f': 'NI'}
    ...
    for spc in spc_mapping:
        ...
        df_oxh= monthly_mean.to_dataframe(name="PM25").reset_index()
        df_oxh['Source'] = 'OxH'
        
        monthly_mean = ds_oxm.groupby('time.month').mean(skipna=True).rename({'month': 'time'})
        df_oxm = monthly_mean.to_dataframe(name="PM25").reset_index()
        df_oxm['Source'] = 'OxM'
        
        # Create dataframe for plot
        plot_df_top = pd.concat([df_imp, df_baseline, df_oxm, df_oxh], ignore_index=True)
