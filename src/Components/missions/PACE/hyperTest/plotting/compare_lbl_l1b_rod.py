...
if __name__ == "__main__":
    ...
    icross = 1000
    inRoot = '../outputs'
    lblFile = '{}/hyperTest_LBL_Thuillier_g5nr_rayleigh_sleavefree.nc4'.format(inRoot)
    ...
    ckFile = '{}/pace-g5nr.lc.gas.rayleigh_sleavefree.20060324_005000.nc4'.format(inRoot)
    ...
    t_blue = ck.variables['TRANS_RAY_blue'][iscan,icross,:]
    t_red  = ck.variables['TRANS_RAY_red'][iscan,icross,:]
    t_swir = ck.variables['TRANS_RAY_SWIR'][iscan,icross,:]

    r_blue = ck.variables['ROD_blue'][iscan,icross,:]
    r_red  = ck.variables['ROD_red'][iscan,icross,:]
    r_swir = ck.variables['ROD_SWIR'][iscan,icross,:]

    ck_smooth = np.ma.append(t_blue,t_red)
    ck_smooth = np.ma.append(ck_smooth,t_swir)

    ck_smooth = np.ma.append(r_blue,r_red)
    ck_smooth = np.ma.append(ck_smooth,r_swir)
    ...
    for ich in range(istart_lbl,noci):
        ...
        if norm != 0:
            lbl_smooth[ich] = np.trapz(alpha_lbl*rsr_int[ich,:],wav_lbl)/norm
