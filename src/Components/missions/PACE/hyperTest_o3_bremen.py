...
def get_abs_o3(inFile, te):
    """
    read temperature dependent ozone lbl cross sections
    interpolate to temperature profile (te)
    """
    nc = Dataset(inFile)
    # lbl interpolate o3xsec
    wav_lbl = np.array(nc.variables['wav_lbl'][:])
    xsec_o3_lbl = nc.variables['xsec_o3_lbl'][:]
    # original o3 spectra
    wav_o3 = np.array(nc.variables['wav_o3'][:])
    ...
    wav = np.append(wav_o3[i],wav_lbl)
    xsec = np.append(xsec_o3[i,:],xsec_o3_lbl,axis=0)

    # get xsecstion for te temperature profile
    xsec_f = interp1d(temp,xsec,kind='linear',fill_value='extrapolate')
...
if __name__ == "__main__":
    ...
    # Read in LBL ozone cross section profile [m2/molecule]
    # interpolate xsec to te profile
    inFile = '{}/o3_bremen/xsec_o3_lbl.nc'.format(outRoot)
    wav_o3, abs_o3_z = get_abs_o3(inFile,te)
