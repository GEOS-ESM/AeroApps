...
if __name__ == "__main__":
    ...
    # OZONE Stuff
    # ---
    # Read in mcclams ozone mixing ratio profile [molecules/m3]
    ...
    # weighted by rsr
    inFile = '{}/xsec_o3_rsr_weighted_V0.nc'.format(outRoot)
    wav_abs, abs_o3 = get_abs_o3_rsr(inFile,te)
    abs_o3.shape = (1,len(wav_abs))
    for k in range(km-1):
...
#    nwl   = 2
    ...
    for ich in np.arange(nwl):
        ...
        rod,depol_ratio_table = get_rod_rsr(inFile,ch)

        # this checks if wavelength was in the table
        if len(rod) > 0:
            # Scale ROT to ROD
            ROT = ROT*rod/ROT.sum()
            depol_ratio = depol_ratio_table

        ROD.append(ROT.sum(axis=0))
