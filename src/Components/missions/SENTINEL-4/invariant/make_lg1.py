...
def set_basemap(projection,clon,clat,elon,elat,lon_0=0.0,satellite_height=35785831.0):
		
	# Default is SENTINEL-4
	# GEO disk parked at 0
	# -----------------------
...
def map_(fig,m,data,cmap,minval,maxval,title,norm=None,format=None,elon=None,elat=None,
		...
		meridian  = np.arange(-180.,180.,5.)
		parallels = np.arange(30.,80.,5.)
		...
		for i in np.arange(len(meridian)):
			plt.annotate(np.str(meridian[i]),xy=m(meridian[i],40),xycoords='data')
		for i in np.arange(len(parallels)):
			plt.annotate(np.str(parallels[i]),xy=m(0,parallels[i]),xycoords='data')
...
if __name__ == '__main__':
	...
	outfile    = 'sentinel-4.lg1.invariant.nc4'
	projection = 'geos'
...
#	for s in latmax:
	...
	# Save to netcdf file
	ncOut 								 = Dataset('sentinel-4.lg1.invariant.nc4','w',format='NETCDF4_CLASSIC')
	...
	ncOut.comment     		 = "This file contains SENTINEL-4 geolocation information at native resolution (before coadding)"
	...
	plt.savefig('sentinel-4_EW_pixelsize_nX={}.png'.format(nEW), transparent='true')
	plt.close(fig)  
	...
	cbaxes = fig.add_axes([0.91, 0.147, 0.02, 0.73]) 
	fig.colorbar(im,ax=ax, cax = cbaxes) 
	plt.savefig('sentinel-4_NS_pixelsize_nY={}.png'.format(nNS), transparent='true')
	plt.close(fig)  	
	...
	# cbaxes = fig.add_axes([0.91, 0.147, 0.02, 0.73]) 
	# fig.colorbar(im,ax=ax, cax = cbaxes) 
	# plt.savefig('sentinel-4_pixelarea.png', transparent='true')
	# plt.close(fig)  		