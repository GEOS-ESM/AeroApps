...
!    1. Linearized Output replaced by Lw_Basic term, and 4 scalings (K_iso, K_dir, K_mj and K_mv)
...
!  SIF output (Isotropic). Isotropy flag must be set.
      ...
      REAL(fpk), dimension ( MAX_SLEAVEWFS, MAXSTOKES, MAXBEAMS ) :: SL_LS_FLUORESCENCE

!  Derivatives of Basic LW

      REAL(fpk), dimension ( MAX_SLEAVEWFS, MAXSTOKES, MAXBEAMS ) :: SL_LS_LW_BASIC

!  Scaling factors for Isotropic Surface leaving term (if flag set), derivatives

      REAL(fpk), dimension ( MAX_SLEAVEWFS, MAXSTOKES, MAXBEAMS ) :: SL_LS_KSCALE_ISOTROPIC
...
!  Derivatives of Scaling factors for Exact Surface-Leaving term

      REAL(fpk), dimension ( MAX_SLEAVEWFS, MAXSTOKES, MAX_USER_STREAMS, &
        MAX_USER_RELAZMS, MAXBEAMS ) :: SL_LS_KSCALE_USERANGLES

!  Derivatives of Scaling factors for Fourier components of Surface-leaving terms:
