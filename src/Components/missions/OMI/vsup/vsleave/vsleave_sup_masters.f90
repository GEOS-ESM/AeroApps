...
!    ==> Outputs: Add Azimuth-dependent direct water-leaving term (WLKscale_SVA)
...
!
      ...
      SUBROUTINE VSLEAVE_MAINMASTER ( AOS_MODEL, ABTOT_CONTROL, &
...
!  Basic LW term
!   4/28/22. .Has no solar dependence
...
!  6/17/22. Scaling for Solar angle, vertical view. Fast calculation

      REAL(fpk)    :: WLKscale_S00 ( MAXBEAMS )
...
!  Help variables
      ...
      INTEGER   :: I, IB, K, UM, LUM, LUA, NMOMENTS, nazmquads, localm, status_sub
...
!  6/17/22. Scaling for Solar angle, vertical view. Added to argument list
         ...
         Call WaterLeaving_2EEE &
          ( Maxbeams, Max_User_Streams, Max_User_Relazms, Maxstreams,            & ! Dimensions
            ...
            WLBasic, WLKscale_ISO, WLKscale_S00, WLKscale_SDF, WLKscale_SVF, WLKscale_SVA, fail, status_sub, message )       

!  12/10/21. Version 2.8.5. Old code Call to WaterLeaving_2E removed, WaterLeaving_2EE commented out
...
!mick mod 9/19/2017 - exception handling upgraded for Version 2.8
           ...
           STATUS = VLIDORT_SERIOUS
           NMESSAGES = NMESSAGES + 1
           MESSAGES(NMESSAGES) = 'Fatal - WaterLeaving_2EEE failed in VSLEAVE_MAINMASTER'
           NMESSAGES = NMESSAGES + 1
...
!  01/05/22. Version 2.8.5. Add Status 3 warning for log-linear extrapolation to C = 0.01
         ...
         if ( status_sub .gt. 0 ) then
           ...
           NMESSAGES = NMESSAGES + 1
           MESSAGES(NMESSAGES) = 'Traced to WaterLeaving_2EEE call in VSLEAVE_MAINMASTER '
         endif
...
!  12/10/21. Version 2.8.5. Beta Version for NASA GSFC, PACE Studies
!    1. Output replaced by Lw_Basic, and 4 scalings (WLKscale_ISO, WLKscale_SDF, WLKscale_SVF, WLKscale_SVA)
!  4/28/22. LW_Basic now has no solar angle dependence. Same for all beams

!  6/17/22. Scaling for Solar angle, vertical view. Added WLKscale_S00 to output

         VSLEAVE_Sup_Out%SL_LW_BASIC        (1,1:NBEAMS) = WLBasic
         VSLEAVE_Sup_Out%SL_KSCALE_ISOTROPIC(1,1:NBEAMS) = WLKscale_ISO(1:NBEAMS)
         VSLEAVE_Sup_Out%SL_KSCALE_SOLARVERT(1,1:NBEAMS) = WLKscale_S00(1:NBEAMS)
