...
!  SIF output (Isotropic). Isotropy flag must be set.
      ...
      REAL(fpk), dimension ( MAXSTOKES, MAXBEAMS ) :: SL_FLUORESCENCE

!  Basic LW. 4/28/22. Now with no SZA dependence. Same for all BEAMS

      REAL(fpk), dimension ( MAXSTOKES, MAXBEAMS ) :: SL_LW_BASIC

!  Scaling factor for Isotropic Surface leaving term (if flag set)

      REAL(fpk), dimension ( MAXSTOKES, MAXBEAMS ) :: SL_KSCALE_ISOTROPIC

!  Scaling factor for Solar-vertical Surface leaving term, always generated

      REAL(fpk), dimension ( MAXSTOKES, MAXBEAMS ) :: SL_KSCALE_SOLARVERT

!  Scaling factor for Exact Surface-Leaving term

      REAL(fpk), dimension ( MAXSTOKES, MAX_USER_STREAMS, MAX_USER_RELAZMS, MAXBEAMS ) :: SL_KSCALE_USERANGLES
...
!    Every solar direction, SL-transmitted user streams
      ...
      REAL(fpk), dimension ( 0:MAXMOMENTS, MAXSTOKES, MAXSTREAMS, MAXBEAMS )       :: SL_KSCALE_F_0
      REAL(fpk), dimension ( 0:MAXMOMENTS, MAXSTOKES, MAX_USER_STREAMS, MAXBEAMS ) :: SL_KSCALE_USER_F_0
