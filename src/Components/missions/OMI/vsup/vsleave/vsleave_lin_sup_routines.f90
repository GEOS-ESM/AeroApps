...
! #                                                             #
! #         Linearized_WaterLeaving_2EEE (Top-level)            #
! #                                                             #
! #           - Lin_Reflectance_generator                       #
! #           - Lin_Ocean_Reflectance_First                     #
...
!      7. Added "status" output from Linearized_WaterLeaving_2EEE, deals with foQ C-value out-of-range warnings
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
      ...
      PUBLIC  :: Linearized_WaterLeaving_2EEE

      CONTAINS

subroutine Linearized_WaterLeaving_2EEE &
     ( Maxszas, Maxvzas, Maxazms, Maxstreams, Maxmoments, Maxazmquads, Maxaqhalf, & ! Dimensions
       ...
       WLBasic, WLKscale_ISO, WLKscale_SDF, WLKscale_SVF, WLKscale_SVA,                & ! Main output
       dC_WLBasic, dC_WLKscale_ISO, dC_WLKscale_SDF, dC_WLKscale_SVF, dC_WLKscale_SVA, & ! Linearized output
       dW_WLBasic, dW_WLKscale_ISO, dW_WLKscale_SDF, dW_WLKscale_SVF, dW_WLKscale_SVA, & ! Linearized output
       fail, status, message )                                           

!  1/31/21, Version 2.8.3. This routine has been re-written, several new things
!    ==> Azimuth dependence in the water-leaving radiance terms
!    ==> Proper estimation of the diffuse-term Fourier componnets
...
!  Basic LW term
   ...
   REAL(fpk), intent(out)    :: WLBasic    ( Maxszas )
   REAL(fpk), intent(out)    :: dC_WLBasic ( Maxszas )
   REAL(fpk), intent(out)    :: dW_WLBasic ( Maxszas )

!  Scaling factor for Isotropic value. Fast calculation

   REAL(fpk), intent(out)    :: WLKscale_ISO    ( Maxszas )
   REAL(fpk), intent(out)    :: dC_WLKscale_ISO ( Maxszas )
   REAL(fpk), intent(out)    :: dW_WLKscale_ISO ( Maxszas )
...
!    Scaling factors for FOURIER COMPONENTS: Input solar, output stream angles
   ...
   REAL(fpk), intent(out)    :: dC_WLKscale_SDF ( 0:Maxmoments, Maxszas, Maxstreams )
   REAL(fpk), intent(out)    :: dW_WLKscale_SDF ( 0:Maxmoments, Maxszas, Maxstreams )
...
!   Scaling factors for  FOURIER COMPONENTS: input solar, output view angles
   ...
   REAL(fpk), intent(out)    :: dC_WLKscale_SVF ( 0:Maxmoments, Maxszas, Maxvzas )
   REAL(fpk), intent(out)    :: dW_WLKscale_SVF ( 0:Maxmoments, Maxszas, Maxvzas )
...
!    ==> Scaling factors for Direct term output: input solar, output view angles, output view azimuths
   ...
   REAL(fpk), intent(out)    :: dC_WLKscale_SVA ( Maxszas, Maxvzas, Maxazms )
   REAL(fpk), intent(out)    :: dW_WLKscale_SVA ( Maxszas, Maxvzas, Maxazms )
...
!      ( 0 = success, 1 = warning low-end, 2 = warning high-end ) 
...
!  1/9/22.  Version 2.8.5. Add do_lowerlimit_01 variable (hard-wired), C_Limit_Lower, C_Limit_Upper
   ...
   logical   :: noWL, do_lowerlimit_01
   integer   :: J, I, naqhalf, nmoments, localm
   REAL(fpk) :: Refrac_R, Refrac_I, Refrac_sq
   real(fpk) :: dtr, pi, Albedo, Const0, Const1, Const_Basic, denom, Rwprime, eta, f
   real(fpk) :: SZA_cosines(Maxszas), mu0, help1, dC_help1, dW_Help1, TBm, dW_TBm
   real(fpk) :: dW_Const1, dC_Rwprime, dW_Const_Basic, dC_Const_Basic, dC_eta, dC_f
...
!    -- 1/31/21, Version 2.8.3. Add Ocean_SVA_Reflecs: input solar, output view angles, azimuths
!    ==> Add dC_Ocean_SVQ_Reflecs: input solar, output view angles, azimuths

!    -- 12/10/21, Version 2.8.5. Add Ocean_Bas_Reflecs, rename Ocean_Reflec_First

   real(fpk) :: Ocean_Reflec_First
   real(fpk) :: Ocean_Bas_Reflecs  ( Maxszas )
   ...
   real(fpk) :: dC_Ocean_Reflec_First
   real(fpk) :: dC_Ocean_Bas_Reflecs  ( Maxszas )
...
!    -- 12/10/21, Version 2.8.5. Add Trans_basic and its derivative

   real(fpk) :: Trans_basic(Maxszas), dW_Trans_basic(Maxszas)
...
!  Zero the output
!    - 1/31/21, Version 2.8.3. Initialize new azimuth-dependent output WLeaving_SVA/SVF. Remove TaSav
!    - 12/10/21. Version 2.8.5. Initialize new output (Lw_Basic, and 4 scalings)

   fail = .false. ; message = ' '
   WLBasic      = zero ; dC_WLBasic      = zero ; dW_WLBasic      = zero
   WLKscale_ISO = zero ; dC_WLKscale_ISO = zero ; dW_WLKscale_ISO = zero
   WLKscale_SDF = zero ; dC_WLKscale_SDF = zero ; dW_WLKscale_SDF = zero
   WLKscale_SVF = zero ; dC_WLKscale_SVF = zero ; dW_WLKscale_SVF = zero
   WLKscale_SVA = zero ; dC_WLKscale_SVA = zero ; dW_WLKscale_SVA = zero
!   TaSav        = zero
...
!  12/10/21. Version 2.8.5. Ocean_Reflectance_First renamed subroutine
   ...
   call  Lin_Ocean_Reflectance_First &
     ( AOS_MODEL, ABTOT_CONTROL, Wavelength, PigmentConc, noWL, &
       Ocean_Reflec_First, eta, dC_Ocean_Reflec_First, dC_eta )
...
!  Call the routine
!  12/10/21. Version 2.8.5. Add Ocean_Bas_Reflecs (and linearization) to the argument outputs
!  01/05/22. Version 2.8.5. Add do_lowerlimit_01 flag for lowering C-value limit to 0.01 as the end value.
   ...
   Call Lin_Reflectance_generator &
     ( Maxszas, Maxvzas, Maxazms, Maxstreams, Maxmoments, Maxazmquads, Maxaqhalf,                &
       ...
       Wavelength, PigmentConc, refrac_R, Ocean_Reflec_First, dC_Ocean_Reflec_First,             &
       nszas, nvzas, nazms, nstreams, nmoments, nazmquads, naqhalf, szas, vzas, azms, streams,   &
       Ocean_Bas_Reflecs,    Ocean_Iso_Reflecs,    Ocean_SDF_Reflecs,    Ocean_SVF_Reflecs,    Ocean_SVA_Reflecs,    &
       dC_Ocean_Bas_Reflecs, dC_Ocean_Iso_Reflecs, dC_Ocean_SDF_Reflecs, dC_Ocean_SVF_Reflecs, dC_Ocean_SVA_Reflecs, &
       fail, message )
...
!  12/10/21. Version 2.8.5. Add TRANS_BASIC output (and dW linearization)
   ...
   if ( do_rough_surface ) then
     Call Lin_RoughSurface_Transmittances &
       ( Maxszas, Maxvzas, Maxstreams, do_isotropy, Ocean_Iso_Reflecs, & ! Input dimensioning/Isotropic input
         ...
         TRANS_BASIC,    TRANS_SOLAR,    TRANS_VIEWING,    TRANS_STREAM,      & ! Regular    OUTPUTS
         dW_TRANS_BASIC, dW_TRANS_SOLAR, dW_TRANS_VIEWING, dW_TRANS_STREAM )    ! Linearized OUTPUTS
   else
     Call FlatSurface_Transmittances &
       ( Maxszas, Maxvzas, Maxstreams, do_isotropy, Refrac_R,          & ! Input dimensions/numbers 
         nszas, nvzas, nstreams, szas, vzas, streams,                  & ! Input geometry
         TRANS_BASIC, TRANS_SOLAR, TRANS_VIEWING, TRANS_STREAM )         ! OUTPUTS
     dW_TRANS_BASIC   = zero ; dW_TRANS_SOLAR  = zero 
     dW_TRANS_VIEWING = zero ; dW_TRANS_STREAM = zero
   endif
...
!  12/10/21. Version 2.8.5. Add WLBasic to output, and calculate Scaling Factors. Add Linearizations
!     -- Trans_basic, Trans_solar, Trans_Stream, Trans_Viewing are all 1.0 for the flat surface (default)
!     -- Alternative. Should really use the F-term in ( one - Albedo * f * Ocean_Reflec_First )  slightly larger values
...
!    -- R. Spurr and A. Sayer, 11-12 April 2018.  PATCH, Add Mu0 factor
      ...
      mu0 = SZA_cosines(J)
      TBm = mu0 * TRANS_BASIC(J)
      f    = 0.6279 - (0.2227*eta) - (0.0513*eta*eta) + (0.2465*eta -0.3119 ) * mu0
      denom =  ( one - Albedo * f * Ocean_Reflec_First )
      Rwprime = one / denom

      Const0          = ( one / refrac_sq ) * trans_solar(J)
      Const1          = Const0 * Foam_correction
      Const_Basic     = Const1 * Rwprime
      WLBasic(J)      = Const_Basic * Ocean_Bas_Reflecs(J) * TBm

!  Basic term. C-derivatives
  
      dC_f = ( - 0.2227 - 0.1026*eta + 0.2465* mu0 ) * dC_eta
      dC_Rwprime = Rwprime * Rwprime * Albedo &
                  * ( dC_f * Ocean_Reflec_First + f * dC_Ocean_Reflec_First )
      dC_Const_Basic = Const1 * dC_Rwprime
      dC_WLBasic(J)  = ( dC_Const_Basic * Ocean_Bas_Reflecs(J) + Const_Basic * dC_Ocean_Bas_Reflecs(J) ) * TBm

!  Basic term. W-derivatives. Flat Case, all derivatives are zero.

      if ( do_rough_surface ) then
        dW_TBm    = mu0 * dW_TRANS_BASIC(J)
        dW_Const1 = ( one / refrac_sq ) * ( dW_trans_solar(J) * Foam_correction + trans_solar(J) * dW_Foam_correction )
        dW_Const_Basic = dW_Const1 * Rwprime
        dW_WLBasic(J)  = ( dW_Const_Basic * TBm + Const_Basic * dW_TBm ) *  Ocean_Bas_Reflecs(J)
      else
        dW_Const_Basic = zero
      endif

!  Isotropy scaling and linearizations

      help1           = one / Ocean_Bas_Reflecs(J) / TRANS_BASIC(J)
      WLKScale_Iso(J) = Ocean_Iso_Reflecs(J) * Help1
      dC_Help1 =  - help1 * dC_Ocean_Bas_Reflecs(J) / Ocean_Bas_Reflecs(J)
      dC_WLKScale_Iso(J) = Ocean_Iso_Reflecs(J) * dC_Help1 + dC_Ocean_Iso_Reflecs(J) * Help1
      if ( do_rough_surface ) then
        dW_Help1 =  - help1 * dW_TRANS_BASIC(J) / TRANS_BASIC(J)
        dW_WLKScale_Iso(J) = Ocean_Iso_Reflecs(J) * dW_Help1
      else
        dW_Help1 = zero
      endif
...
!  Directional terms : Isotropy - Just copy the Isotropic value
      ...
      if ( do_isotropy ) then
         ...
         dC_WLKscale_SDF(0,J,1:nstreams) = dC_WLKScale_Iso(J)
         dW_WLKscale_SDF(0,J,1:nstreams) = dW_WLKScale_Iso(J)
         do I = 1, nvzas
            WLKScale_SVA(J,I,1:nazms) = WLKScale_Iso(J) 
            WLKscale_SVF(0,J,I)       = WLKScale_Iso(J)
            dC_WLKScale_SVA(J,I,1:nazms) = dC_WLKScale_Iso(J) 
            dC_WLKscale_SVF(0,J,I)       = dC_WLKScale_Iso(J)
            dW_WLKScale_SVA(J,I,1:nazms) = dW_WLKScale_Iso(J) 
            dW_WLKscale_SVF(0,J,I)       = dW_WLKScale_Iso(J)
         enddo
      endif

!  Directional terms : Non-Isotropy
...
!    -- R. Spurr and A. Sayer, 11-12 April 2018.  PATCH, Add Mu0 factor
!    -- 1/31/21,  Version 2.8.3. include azimuth-dependent outputs WLeaving_SVA, dW_WLeaving_SVA
!    -- 1/31/21,  Version 2.8.3. include Fourier dependent output WLeaving_SVF, WLeaving_SDF, dW_WLeaving_SVF, dW_WLeaving_SDF
!    -- 12/10/21, Version 2.8.5. Output WLKSCALE Values instead

      if ( .not. do_isotropy ) then

         do I = 1, nstreams
            WLKscale_SDF(0:Localm,J,I)    = Help1 * Trans_Stream(I,J)  * Ocean_SDF_Reflecs(0:Localm,J,I)
            dC_WLKscale_SDF(0:Localm,J,I) = ( dC_help1 *    Ocean_SDF_Reflecs(0:Localm,J,I) &
                                               + Help1 * dC_Ocean_SDF_Reflecs(0:Localm,J,I) ) * Trans_Stream(I,J)
            dW_WLKscale_SDF(0:Localm,J,I) = ( dW_help1 * Trans_Stream(I,J) + &
                                                 Help1 * dW_Trans_Stream(I,J) ) * Ocean_SDF_Reflecs(0:Localm,J,I)
         enddo
         do I = 1, nvzas
            WLKscale_SVF(0:Localm,J,I)    = help1 * Ocean_SVF_Reflecs(0:Localm,J,I) * Trans_Viewing(I,J) 
            dC_WLKscale_SVF(0:Localm,J,I) = ( dC_help1 *    Ocean_SVF_Reflecs(0:Localm,J,I) &
                                               + help1 * dC_Ocean_SVF_Reflecs(0:Localm,J,I) ) * Trans_Viewing(I,J)
            dW_WLKscale_SVF(0:Localm,J,I) = ( dW_help1 * Trans_Viewing(I,J) + &
                                                 Help1 * dW_Trans_Viewing(I,J) ) * Ocean_SVF_Reflecs(0:Localm,J,I)
         enddo
         do I = 1, nvzas
            WLKscale_SVA(J,I,1:nazms)   = help1 * Ocean_SVA_Reflecs(J,I,1:nazms) * Trans_Viewing(I,J) 
            dC_WLKscale_SVA(J,I,1:nazms) = ( dC_help1 *   Ocean_SVA_Reflecs(J,I,1:nazms) &
                                              + help1 * dC_Ocean_SVA_Reflecs(J,I,1:nazms)) * Trans_Viewing(I,J)
            dW_WLKscale_SVA(J,I,1:nazms) = ( dW_help1 * Trans_Viewing(I,J) + &
                                                Help1 * dW_Trans_Viewing(I,J) ) * Ocean_SVA_Reflecs(J,I,1:nazms)
         enddo
...
!88 continue
!   fail = .true.
!   message = 'Openfile error in Linearized_WaterLeaving_2EEE; file not found: ' // Trim(TaRayFile)

end subroutine Linearized_WaterLeaving_2EEE
...
subroutine Lin_Ocean_Reflectance_First &
       ( AOS_MODEL, ABTOT_CONTROL, Wavelength, PigmentConc, noWL, Ocean_Reflec, eta_out, dC_Ocean_Reflec, dC_eta_out )
...
!           Actually better for the other models too.
      ...
      ELSE
        IF ( ABTOT_CONTROL ) THEN
          R2  = b_tot/(b_tot+a_tot) ! Alternative. Better choice after talking w/ Sasha on 4/12/18 wqin
          dR2 = ( db_tot - R2*(db_tot + da_tot) ) / (b_tot+a_tot)
        ELSE
          R2  = ( b_tot / a_tot )       ! Sasha's suggestion on 11/28/18, Implemented 3/18/19 Version 2.8a
...
!  Finish
      ...
      return
end subroutine Lin_Ocean_Reflectance_First
...
!  Basic reflectance
      ...
      real(fpk), intent(in)   :: Ocean_Reflec_First, dC_Ocean_Reflec_First
...
!  12/10/21. Version 2.8.5. Include the basic output Ocean_Bas_Reflecs
      ...
      real(fpk), intent(out) :: Ocean_Bas_Reflecs  ( Maxszas )
      ...
      real(fpk), intent(out) :: dC_Ocean_Bas_Reflecs ( Maxszas )
...
!  output
!  12/10/21. Version 2.8.5. Initialize the basic output Ocean_Bas_Reflecs + linearization

      Ocean_Bas_Reflecs = zero ; dC_Ocean_Bas_Reflecs = zero
      ...
      if ( do_Isotropy ) then
         call Interpolate_Lin_fOQ_BS1 &
           ( Maxszas, nszas, szas, FoQFile, do_lowerlimit_01, Wavelength, PigmentConc, &
             fOQ_Int_B, foQ_Int_1, dC_foQ_Int_B, dC_foQ_Int_1, fail, message )
         if ( fail ) return
         do j = 1, nszas
            Ocean_Bas_Reflecs(j)    = Ocean_Reflec_First * foQ_Int_B(j)
            dC_Ocean_Bas_Reflecs(J) = dC_Ocean_Reflec_First *    foQ_Int_B(J) &
                                     +   Ocean_Reflec_First * dC_foQ_Int_B(J)
            Ocean_Iso_Reflecs(j)    = Ocean_Reflec_First * foQ_Int_1(j)
            dC_Ocean_Iso_Reflecs(J) = dC_Ocean_Reflec_First *    foQ_Int_1(J) &
                                     +   Ocean_Reflec_First * dC_foQ_Int_1(J)
      ...
      if ( do_Azimuth_output ) then
        call Interpolate_Lin_fOQ_BS3 &
           ( Maxszas, Maxstreams, Maxvzas, Maxazms, nszas, nvzas, nazms, nstreams,                    &
             ...
             fOQ_Int_B, foQ_Int_1, foQ_Int_SV, foQ_Int_SVA, foQ_Int_SD,                               &
             dC_foQ_Int_B,dC_foQ_Int_1, dC_foQ_Int_SV, dC_foQ_Int_SVA, dC_foQ_Int_SD,                 &
             fail, message ) 
      else
        call Interpolate_Lin_fOQ_BS2 &
           ( Maxszas, Maxstreams, Maxvzas, nszas, nvzas, nstreams,                              &
             ...
             fOQ_Int_B, foQ_Int_1, foQ_Int_SV, foQ_Int_SD,                                      &
             dC_foQ_Int_B, dC_foQ_Int_1, dC_foQ_Int_SV, dC_foQ_Int_SD,                          &
             fail, message ) 
      endif
      if ( fail ) return
...
!  1/31/21, Version 2.8.3. Fill out Exact case (either with or without azimuth dependence)
      ...
      if ( do_Azimuth_output ) then
         do j = 1, nszas ; do i = 1, nvzas
            ...
            dC_Ocean_SVA_Reflecs(j,i,1:nazms) = dC_Ocean_Reflec_First *    foQ_Int_SVA(J,I,1:nazms) &
                                                 + Ocean_Reflec_First * dC_foQ_Int_SVA(J,I,1:nazms)
         enddo ; enddo
      else
         do j = 1, nszas ; do i = 1, nvzas
            Ocean_SVA_Reflecs(j,i,1:nazms)    = Ocean_Reflec_First * foQ_Int_SV(J,I)
            dC_Ocean_SVA_Reflecs(j,i,1:nazms) = dC_Ocean_Reflec_First *    foQ_Int_SV(J,I) &
                                                 + Ocean_Reflec_First * dC_foQ_Int_SV(J,I)
         enddo ; enddo
      endif

!  1/31/21, Version 2.8.3. Fill out azimuth-averaged reflectances (non-Fourier case), and exit

      if ( .not. do_Fourier_output ) then
         do j = 1, nszas ; do i = 1, nstreams 
            Ocean_SDF_Reflecs(0,j,i)    = Ocean_Reflec_First * foQ_Int_SD(J,I)
            dC_Ocean_SDF_Reflecs(0,j,i) = dC_Ocean_Reflec_First *    foQ_Int_SD(J,I) &
                                           + Ocean_Reflec_First * dC_foQ_Int_SD(J,I)
         enddo ; enddo
         do j = 1, nszas ; do i = 1, nvzas
            Ocean_SVF_Reflecs(0,j,i)    = Ocean_Reflec_First * foQ_Int_SV(J,I)
            dC_Ocean_SVF_Reflecs(0,j,i) = dC_Ocean_Reflec_First *    foQ_Int_SV(J,I) &
                                           + Ocean_Reflec_First * dC_foQ_Int_SV(J,I)
         enddo ; enddo
      endif
...
!  Basic factor, short hand
        ...
        help    = help0 * Ocean_Reflec_First
        dc_help = help0 * dC_Ocean_Reflec_First
...
!  12/10/21. Version 2.8.5. Add TRANS_BASIC output
   ...
   real(fpk), intent(out) :: Trans_basic(Maxszas), Trans_solar(Maxszas)
   real(fpk), intent(out) :: Trans_stream(Maxstreams,Maxszas), Trans_Viewing(Maxvzas,Maxszas)
   real(fpk), intent(out) :: dW_Trans_basic(Maxszas), dW_Trans_solar(Maxszas)
...
!  12/10/21. Version 2.8.5. Add TRANS_BASIC initialization
   ...
   trans_norm    = one
   Trans_basic   = one ; dW_Trans_basic   = zero
...
!  12/10/21. Version 2.8.5. New section for TRANS_BASIC
!  Upward transmittances into Nadir directions
           ...
           if ( Iso_Reflecs(J).gt.Minimum_Reflectance ) then
              ...
              call Lin_Water_Transmittance &
                  ( Max_PolarQuads, Max_AzimQuads,                          & ! Input
                    ...
                    WINDSPEED, SUNGLINT_COEFFS, dSUNGLINT_COEFFS,           & ! Input
                    PHI_W, CPHI_W, SPHI_W, TRANS_NORM,                      & ! Inpu
                    TRANS_BASIC(J), dW_TRANS_BASIC(J) )
           endif
         endif
...
!    -- 12/10/21. Version 2.8.5. Add FoQ_Int_B

   real(fpk)    , intent(out)  :: foQ_Int_B(Maxszas)
   real(fpk)    , intent(out)  :: foQ_Int_1(Maxszas)
   real(fpk)    , intent(out)  :: dC_foQ_Int_B(Maxszas)
...
!    -- 12/10/21. Version 2.8.5. Add FoQ_Bas0
...
!    -- 12/10/21. Version 2.8.5. Add FoQ_Bas1 and its derivative

   real(spk) :: dtr, fw1, fw2, fs1, fs2, cszas(Maxszas), yspline(6), bbas(6), cbas(6), dbas(6)
   real(spk) :: Wave, csza, C, LogC, lam, sun, pigc, fval, Logder, foQ_suns(6), dC_foQ_suns(6), FoQ_Bas1(6), dC_foQ_Bas1(6)
...
!    12/10/21. Version 2.8.5. Avoid spline extrapolation by pinning to end values.
!                             Out of range C-derivatives are zero.

   LogC = log(Real(PigmentConc)) ; if ( FoQ_status .eq. 4 ) logC = log(0.01)
   do j = 1, 6
      if ( FoQ_status .eq. 1 ) then
         foQ_suns(j) = fw1*foQ_averaged(w1,j,1) + fw2*foQ_averaged(w2,j,1) ; dC_foQ_suns(j) = 0.0_fpk
      else if ( FoQ_status .eq. 2 ) then
         foQ_suns(j) = fw1*foQ_averaged(w1,j,6) + fw2*foQ_averaged(w2,j,6) ; dC_foQ_suns(j) = 0.0_fpk
      else 
         do k = 1, 6 !pigc
...
!  12/10/21. Version 2.8.5. Repeat for the foQ_Bas0 values --> foQ_Bas1
!  12/10/21. Version 2.8.5. Avoid spline extrapolation by pinning to end values. Out of range C-derivatives are zero
!  01/05/22. Version 2.8.5. Add do_lowerlimit_01 flag for lowering C-value limit to 0.01 as the end value.
!                           Set status 0 for in-range spline interpolation    ; 10.0 > C > 0.03. INTERPOLATION
!                           Set status 1 for Pinning Value at Lower End limit of 0.03, C < 0.03. NO EXTRAPOLATION
!                           Set status 2 for Pinning value at Upper end limit of 10.0, C > 10.0. NO EXTRAPOLATION
...
!                           Set status 4 for Pinning Value at Lower End 0.01, C < 0.01 (if flagged). Use EXTRAPOLATED VALUE at 0.01
   ...
   do j = 1, 6 !sun
      if ( FoQ_status .eq. 1 ) then
         foQ_Bas1(j) = fw1*foQ_Bas0(w1,j,1) + fw2*foQ_Bas0(w2,j,1) ; dC_foQ_suns(j) = 0.0_fpk
      else if ( FoQ_status .eq. 2 ) then
         foQ_Bas1(j) = fw1*foQ_Bas0(w1,j,6) + fw2*foQ_Bas0(w2,j,6) ; dC_foQ_suns(j) = 0.0_fpk
      else 
         do k = 1, 6 !pigc
            yspline(k) = fw1*foQ_Bas0(w1,j,k) + fw2*foQ_Bas0(w2,j,k) 
         enddo
         Call bspline(6,6,LogPigs,yspline,bbas,cbas,dbas)
         Call dSeval(6,6,LogC,LogPigs,yspline,bbas,cbas,dbas,foQ_Bas1(j),LogDer) ; dC_foQ_Bas1(j) = LogDer / C
      endif
   enddo

!  Solar angles
!  12/10/21. Version 2.8.5. Interpolate foQ_Bas1 for the Basic Output values FoQ_Int_B (also derivative)
   ...
   do n = 1, nszas
      ...
      FoQ_Int_B(n)    = dble ( fs1 * foQ_Bas1(s1)    + fs2 * foQ_Bas1(s2) )
      FoQ_Int_1(n)    = dble ( fs1 * foQ_suns(s1)    + fs2 * foQ_suns(s2)    )
      dC_FoQ_Int_B(n) = dble ( fs1 * dC_foQ_Bas1(s1) + fs2 * dC_foQ_Bas1(s2) )
...
!  output is double precision
!    -- 12/10/21. Version 2.8.5. Add FoQ_Int_B and its derivative
...
!    -- 12/10/21. Version 2.8.5. Add FoQ_Bas1 and its derivative
   ...
   real(spk) :: foQ_Bas1(6), foQ_suns(6), foQ_nads(6,17), dC_foQ_Bas1(6), dC_foQ_suns(6), dC_foQ_nads(6,17)
...
!  initialize
!    -- 12/10/21. Version 2.8.5. initialize FoQ_Int_B and its derivative
...
!                           Set status 4 for Pinning Value at Lower End 0.01, C < 0.01 (if flagged). Use EXTRAPOLATED VALUE at 0.01
   ...
   do j = 1, 6
      ...
      do m = 1, 17
         if ( FoQ_status .eq. 1 ) then
            foQ_nads(j,m) = fw1*foQ_averaged_2(w1,j,1,m) + fw2*foQ_averaged_2(w2,j,1,m) ; dC_foQ_nads(j,m) = 0.0_fpk
         else if ( FoQ_status .eq. 2 ) then
            foQ_nads(j,m) = fw1*foQ_averaged_2(w1,j,6,m) + fw2*foQ_averaged_2(w2,j,6,m) ; dC_foQ_nads(j,m) = 0.0_fpk
         else 
            do k = 1, 6
...
!                         
   do j = 1, 6 !sun
      if ( FoQ_status .eq. 1 ) then
         foQ_Bas1(j) = fw1*foQ_Bas0(w1,j,1) + fw2*foQ_Bas0(w2,j,1) ; dC_foQ_Bas1(j) = 0.0_fpk
      else if ( FoQ_status .eq. 2 ) then
         foQ_Bas1(j) = fw1*foQ_Bas0(w1,j,6) + fw2*foQ_Bas0(w2,j,6) ; dC_foQ_Bas1(j) = 0.0_fpk
      else 
         do k = 1, 6 !pigc
            yspline(k) = fw1*foQ_Bas0(w1,j,k) + fw2*foQ_Bas0(w2,j,k) 
         enddo
...
!  Solar angles
!  12/10/21. Version 2.8.5. Interpolate foQ_Bas1 for the Basic Output values FoQ_Int_B (also derivatives)
...
!  output is double precision
!  1/31/21, Version 2.8.3. Add foQ_Int_SVA, dC_foQ_Int_SV with azimuth dependence
!  12/10/21. Version 2.8.5. Add FoQ_Int_B and its derivative
...
!  12/10/21. Version 2.8.5. Add FoQ_Bas1 and its derivative
   ...
   real(spk) :: foQ_Bas1(6), foQ_suns(6), foQ_nads(6,17), foQ_nadazs(6,17,13)
   real(spk) :: dC_foQ_Bas1(6), dC_foQ_suns(6), dC_foQ_nads(6,17), dC_foQ_nadazs(6,17,13)
...
!  1/31/21, Version 2.8.3. Initialize new variables FoQ_Int_SVA, dC_FoQ_Int_SVA
!    -- 12/10/21. Version 2.8.5. initialize FoQ_Int_B and its derivative
...
!         foQ_suns(j) = y1 + ( LogC - x1 ) * ( y2 - y1 ) / (x2 - x1)
   ...
   do j = 1, 6
      ...
      do m = 1, 17
         ...
         do z = 1, 13
            if ( FoQ_status .eq. 1 ) then
               foQ_nadazs(j,m,z) = fw1*foQ_Full(w1,j,1,m,z) + fw2*foQ_Full(w2,j,1,m,z) ; dC_foQ_nadazs(j,m,z) = 0.0_fpk
            else if ( FoQ_status .eq. 2 ) then
               foQ_nadazs(j,m,z) = fw1*foQ_Full(w1,j,6,m,z) + fw2*foQ_Full(w2,j,6,m,z) ; dC_foQ_nadazs(j,m,z) = 0.0_fpk
            else 
               do k = 1, 6
