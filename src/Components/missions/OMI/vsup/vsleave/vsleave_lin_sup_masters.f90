...
!  Linearizations
!   1/31/21. Version 2.8.3. Remove DO_ISO_JACOBIANS
...
!  Number of Stokes components, discrete ordinate streams

      INTEGER   :: NSTOKES, NSTREAMS
...
!  Overall Jacobian control. 1/31/21. Version 2.8.3. DO_ISO_JACOBIANS has gone.
...
!   1/31/21. Version 2.8.3. Remove this DO_ISO_JACOBIANS call. NO LONGER REQUIRED 
...
!   1/31/21. Version 2.8.3. Remove the DO_ISO_JACOBIANS part of this condition
...
!
      ...
      SUBROUTINE VSLEAVE_LIN_MAINMASTER ( AOS_MODEL, ABTOT_CONTROL, &
...
!    ==> Explicit treatment of doublet goemetry
      ...
      USE VLIDORT_PARS_m, Only : MAX_USER_RELAZMS, MAX_USER_STREAMS, MAX_USER_OBSGEOMS,          &
                                 MAXSTREAMS, MAXBEAMS, MAX_MESSAGES, MAXMOMENTS, FPK, ZERO, ONE, TWO, &
                                 VLIDORT_SUCCESS, VLIDORT_WARNING, VLIDORT_SERIOUS
      ...
      USE vsleave_sup_routines_2_m     , only : WaterLeaving_2EEE,         &
                                                get_fluorescence_755,      &
                                                solar_spec_irradiance

      USE vsleave_lin_sup_routines_2_m , only : Linearized_WaterLeaving_2EEE
...
!  Number of discrete ordinate streams, stokes components andquadrature
!   Version 3.7, added the qudrature arrays

      INTEGER   :: NSTREAMS, NSTOKES
...
!    1. Output replaced by WLBasic, and 4 scalings (WLKscale_ISO, WLKscale_SDF, WLKscale_SVF, WLKscale_SVA)
!    2. Add derivatives

!  Basic LW term

      REAL(fpk)    :: WLBasic         ( MAXBEAMS )
      REAL(fpk)    :: dC_WLBasic      ( MAXBEAMS )
      REAL(fpk)    :: dW_WLBasic      ( MAXBEAMS )

!  Scaling for Isotropic value. Fast calculation

      REAL(fpk)    :: WLKscale_ISO    ( MAXBEAMS )
      REAL(fpk)    :: dC_WLKscale_ISO ( MAXBEAMS )
      REAL(fpk)    :: dW_WLKscale_ISO ( MAXBEAMS )

!  Scaling for input solar, output stream angles. 1/31/21. Version 2.8.3. ALL FOURIERS!

      REAL(fpk)    :: WLKscale_SDF    ( 0:MAXMOMENTS, MAXBEAMS, MAXSTREAMS )
      REAL(fpk)    :: dC_WLKscale_SDF ( 0:MAXMOMENTS, MAXBEAMS, MAXSTREAMS )
      REAL(fpk)    :: dW_WLKscale_SDF ( 0:MAXMOMENTS, MAXBEAMS, MAXSTREAMS )

!  Scaling for input solar, output view angles. 1/31/21. Version 2.8.3. ALL FOURIERS!

      REAL(fpk)    :: WLKscale_SVF    ( 0:MAXMOMENTS, MAXBEAMS, MAX_USER_STREAMS )
      REAL(fpk)    :: dC_WLKscale_SVF ( 0:MAXMOMENTS, MAXBEAMS, MAX_USER_STREAMS )
      REAL(fpk)    :: dW_WLKscale_SVF ( 0:MAXMOMENTS, MAXBEAMS, MAX_USER_STREAMS )

!  1/31/21. Version 2.8.3. Azimuth dependence in the water-leaving radiance
!    ==> Add Azimuth-dependent term WLKscale_SVA, controlled by flag do_Azimuth_Output

!  Scaling for input solar, output view angles and azimuths 

      REAL(fpk)    :: WLKscale_SVA    ( MAXBEAMS, MAX_USER_STREAMS, MAX_USER_RELAZMS )
      REAL(fpk)    :: dC_WLKscale_SVA ( MAXBEAMS, MAX_USER_STREAMS, MAX_USER_RELAZMS )
      REAL(fpk)    :: dW_WLKscale_SVA ( MAXBEAMS, MAX_USER_STREAMS, MAX_USER_RELAZMS )
...
!  Help variables
      ...
      INTEGER   :: IB, K, nslpars, I, M, Q, UM, NAZ, LUM, LUA, NMOMENTS, nazmquads, localm, status_sub
...
!   2/28/21. Version 2.8.3. Remove DO_ISO_JACOBIANS
...
!  Geometry proxy settings. 2/28/21. Version 2.8.3. Rewritten
!  -----------------------
...
!  2/28/21, Version 2.8.3. Add settings for the Doublet geometry option
...
!  2/28/21. Version 2.8.3. Fourier dependence in the water-leaving radiance
...
!  12/10/21. Version 2.8.5. 
!     -- Water-leaving output replaced by Lw_Basic term, and 4 scalings (K_iso, K_dir, K_mj and K_mv)
!     -- Separately declared Fluorescence

      VSLEAVE_Sup_Out%SL_FLUORESCENCE      = ZERO
      VSLEAVE_Sup_Out%SL_LW_BASIC          = ZERO
      VSLEAVE_Sup_Out%SL_KSCALE_ISOTROPIC  = ZERO
      VSLEAVE_Sup_Out%SL_KSCALE_USERANGLES = ZERO
      VSLEAVE_Sup_Out%SL_KSCALE_F_0        = ZERO
      VSLEAVE_Sup_Out%SL_KSCALE_USER_F_0   = ZERO
...
!mick fix 9/19/2017 - removed N_SLEAVE_WFS as an output variable: strictly an input now
      ...
      VSLEAVE_LinSup_Out%SL_LS_FLUORESCENCE      = ZERO
      VSLEAVE_LinSup_Out%SL_LS_LW_BASIC          = ZERO
      VSLEAVE_LinSup_Out%SL_LS_KSCALE_ISOTROPIC  = ZERO
      VSLEAVE_LinSup_Out%SL_LS_KSCALE_USERANGLES = ZERO
      VSLEAVE_LinSup_Out%SL_LS_KSCALE_F_0        = ZERO
      VSLEAVE_LinSup_Out%SL_LS_KSCALE_USER_F_0   = ZERO
...
!  12/10/21. Version 2.8.5. Renamed Fluorescence output

          help2 = FF * FsSum / FL_SunSpec
          Fluor_ISOTROPIC = help2 * Fs755(IB)
          VSLEAVE_Sup_Out%SL_FLUORESCENCE(1,IB) = FL_Amplitude755 * Fluor_ISOTROPIC
...
!  12/10/21. Version 2.8.5. Renamed Fluorescence output
          ...
          if (FL_F755_JACOBIANS ) then
             NSLPARS = 1 ; VSLEAVE_LinSup_Out%SL_LS_FLUORESCENCE(1,1,IB) = Fluor_ISOTROPIC
          endif
          ...
          if ( .not. FL_DO_DataGaussian ) then
             ...
             do m = 1, 6
                IF ( FL_GAUSS_JACOBIANS(m) ) then
                   NSLPARS = NSLPARS + 1
                   VSLEAVE_LinSup_Out%SL_LS_FLUORESCENCE(NSLPARS,1,IB) = help1 * d_Fssum(m)
                ENDIF
             enddo
          endif
...
!    2. Add linearizations of these new outputs

         if ( .not. DO_CHLORCONC_WF .and. .not. DO_WINDSPEED_WF ) then

            Call WaterLeaving_2EEE &
             ( Maxbeams, Max_User_Streams, Max_User_Relazms, Maxstreams,            & ! Dimensions
               Maxmoments, Maxazmquads, Maxaqhalf, AOS_MODEL, ABTOT_CONTROL,        & ! Dimensions
               ...
               WLBasic, WLKscale_ISO, WLKscale_SDF, WLKscale_SVF, WLKscale_SVA, fail, status_sub, message )       
...
!mick mod 9/19/2017 - exception handling upgraded for Version 2.8
           !write(*,'(A)')Trim(message) ; Stop 'WaterLeaving_2 failed'
               STATUS = VLIDORT_SERIOUS
               NMESSAGES = NMESSAGES + 1
               MESSAGES(NMESSAGES) = 'Fatal - WaterLeaving_2EEE failed in VSLEAVE_LIN_MAINMASTER'
               NMESSAGES = NMESSAGES + 1
               MESSAGES(NMESSAGES) = Trim(message)
              GO TO 899
            endif

!  12/10/21. Version 2.8.5. Status warnings on use of foQ interpolation
!  01/05/22. Version 2.8.5. Add Status 3 warning for log-linear extrapolation to C = 0.01

            if ( status_sub .gt. 0 ) then
               STATUS = VLIDORT_WARNING
               NMESSAGES = NMESSAGES + 1
               if ( status_sub.eq.1 ) then
                  MESSAGES(NMESSAGES) = 'Warning: Pigment C-value out of bounds for foQ interpolation, pin to lowest value'
               else if ( status_sub.eq.2 ) then
                  MESSAGES(NMESSAGES) = 'Warning: Pigment C-value out of bounds for foQ interpolation, pin to highest value'
               else if ( status_sub.eq.3 ) then
                  MESSAGES(NMESSAGES) = 'Warning: Pigment C-value using Log-linear extrapolation of foQ at lowest end'
               endif
               NMESSAGES = NMESSAGES + 1
               MESSAGES(NMESSAGES) = 'Traced to WaterLeaving_2EEE call in VSLEAVE_LIN_MAINMASTER '
            endif

         else

            Call Linearized_WaterLeaving_2EEE &
             ( Maxbeams, Max_User_Streams, Max_User_Relazms, Maxstreams,            & ! Dimensions
               Maxmoments, Maxazmquads, Maxaqhalf, AOS_MODEL, ABTOT_CONTROL,        & ! Dimensions
               FoQFile, do_Isotropic, do_Azimuth_Output, do_Fourier_Output,         & ! Flags and file
               Do_RoughSurface, Do_FoamOption, Do_GlintShadow, Do_FacetIsotropy,    & ! Glitter flags
               ...
               nbeams, n_user_streams, n_user_relazms, nstreams, nazmquads,         & ! Geometry
               beam_szas, user_angles, user_relazms, streams,                       & ! geometry
               WLBasic, WLKscale_ISO, WLKscale_SDF, WLKscale_SVF, WLKscale_SVA,     & ! Regular output
...
!mick mod 9/19/2017 - exception handling upgraded for Version 2.8
           !write(*,'(A)')Trim(message) ; Stop 'WaterLeaving_2 failed'
               STATUS = VLIDORT_SERIOUS
               NMESSAGES = NMESSAGES + 1
               MESSAGES(NMESSAGES) = 'Fatal - Lin_WaterLeaving_2EEE failed in VSLEAVE_LIN_MAINMASTER'
               NMESSAGES = NMESSAGES + 1
               MESSAGES(NMESSAGES) = Trim(message)
              GO TO 899
            endif
...
!  2/28/21, Version 2.8.3. Azimuth dependence in the water-leaving radiance
...
!    1. Output replaced by Lw_Basic, and 4 scalings (WLKscale_ISO, WLKscale_SDF, WLKscale_SVF, WLKscale_SVA)
         ...
         VSLEAVE_Sup_Out%SL_LW_BASIC        (1,1:NBEAMS) = WLBasic     (1:NBEAMS)
         VSLEAVE_Sup_Out%SL_KSCALE_ISOTROPIC(1,1:NBEAMS) = WLKscale_ISO(1:NBEAMS)
         localm = nmoments ; if ( do_Isotropic.or..not.do_Fourier_output ) localm = 0
         do ib = 1, nbeams
            if ( do_user_obsgeoms ) then
              VSLEAVE_Sup_Out%SL_KSCALE_USERANGLES(1,lum,lua,ib)     = WLKscale_SVA(ib,ib,ib)
              VSLEAVE_Sup_Out%SL_KSCALE_USER_F_0  (0:localm,1,ib,ib) = WLKscale_SVF(0:localm,ib,ib)
            else if ( do_doublet_geometry ) then
              do um = 1, n_user_streams
                VSLEAVE_Sup_Out%SL_KSCALE_USERANGLES(1,um,lua,ib)      = WLKscale_SVA(ib,um,um)
                VSLEAVE_Sup_Out%SL_KSCALE_USER_F_0  (0:localm,1,um,ib) = WLKscale_SVF(0:localm,ib,um)
              enddo
            else
              do um = 1, n_user_streams
                VSLEAVE_Sup_Out%SL_KSCALE_USERANGLES(1,um,1:n_user_relazms,ib) = WLKscale_SVA(ib,um,1:n_user_relazms)
                VSLEAVE_Sup_Out%SL_KSCALE_USER_F_0  (0:localm,1,um,ib)         = WLKscale_SVF(0:localm,ib,um)
              enddo
            endif
            do i = 1, nstreams
              VSLEAVE_Sup_Out%SL_KSCALE_F_0(0:nmoments,1,i,ib) = WLKscale_SDF(0:nmoments,ib,i)
            enddo
         enddo
...
!   Pigment
         ...
         if ( DO_CHLORCONC_WF ) then
           Q = Q + 1
           VSLEAVE_LinSup_Out%SL_LS_LW_BASIC         (Q,1,1:NBEAMS) = dC_WLBasic(1:NBEAMS)
           VSLEAVE_LinSup_Out%SL_LS_KSCALE_ISOTROPIC(Q,1,1:NBEAMS) = dC_WLKscale_ISO(1:NBEAMS)
           do ib = 1, nbeams
             if ( do_user_obsgeoms ) then
               VSLEAVE_LinSup_Out%SL_LS_KSCALE_USERANGLES(Q,1,lum,lua,ib)      = dC_WLKscale_SVA(ib,ib,ib)
               VSLEAVE_LinSup_Out%SL_LS_KSCALE_USER_F_0  (Q,0:localm,1,lum,ib) = dC_WLKscale_SVF(0:localm,ib,ib)
             else if ( do_doublet_geometry ) then
               do um = 1, n_user_streams
                 VSLEAVE_LinSup_Out%SL_LS_KSCALE_USERANGLES(Q,1,um,lua,ib)      = dC_WLKscale_SVA(ib,um,um)
                 VSLEAVE_LinSup_Out%SL_LS_KSCALE_USER_F_0  (Q,0:localm,1,um,ib) = dC_WLKscale_SVF(0:localm,ib,um)
               enddo
             else
               do um = 1, n_user_streams
                 VSLEAVE_LinSup_Out%SL_LS_KSCALE_USERANGLES(Q,1,um,1:naz,ib)    = dC_WLKscale_SVA(ib,um,1:naz)
                 VSLEAVE_LinSup_Out%SL_LS_KSCALE_USER_F_0  (Q,0:localm,1,um,ib) = dC_WLKscale_SVF(0:localm,ib,um)
               enddo
             endif
             do i = 1, nstreams
               VSLEAVE_LinSup_Out%SL_LS_KSCALE_F_0(Q,0:localm,1,i,ib) = dC_WLKscale_SDF(0:localm,ib,i)
             enddo
           enddo
         endif
...
!   Windspeed
         ...
         if ( DO_WINDSPEED_WF ) then
           Q = Q + 1
           VSLEAVE_LinSup_Out%SL_LS_LW_BASIC        (Q,1,1:NBEAMS) = dW_WLBasic(1:NBEAMS)
           VSLEAVE_LinSup_Out%SL_LS_KSCALE_ISOTROPIC(Q,1,1:NBEAMS) = dW_WLKscale_ISO(1:NBEAMS)
           do ib = 1, nbeams
             if ( do_user_obsgeoms ) then
               VSLEAVE_LinSup_Out%SL_LS_KSCALE_USERANGLES(Q,1,lum,lua,ib)      = dW_WLKscale_SVA(ib,ib,ib)
               VSLEAVE_LinSup_Out%SL_LS_KSCALE_USER_F_0  (Q,0:localm,1,lum,ib) = dW_WLKscale_SVF(0:localm,ib,ib)
             else if ( do_doublet_geometry ) then
               do um = 1, n_user_streams
                 VSLEAVE_LinSup_Out%SL_LS_KSCALE_USERANGLES(Q,1,um,lua,ib)      = dW_WLKscale_SVA(ib,um,um)
                 VSLEAVE_LinSup_Out%SL_LS_KSCALE_USER_F_0  (Q,0:localm,1,um,ib) = dW_WLKscale_SVF(0:localm,ib,um)
               enddo
             else
               do um = 1, n_user_streams
                 VSLEAVE_LinSup_Out%SL_LS_KSCALE_USERANGLES(Q,1,um,1:naz,ib)    = dW_WLKscale_SVA(ib,um,1:naz)
                 VSLEAVE_LinSup_Out%SL_LS_KSCALE_USER_F_0  (Q,0:localm,1,um,ib) = dW_WLKscale_SVF(0:localm,ib,um)
               enddo
             endif
             do i = 1, nstreams
               VSLEAVE_LinSup_Out%SL_LS_KSCALE_F_0(Q,0:localm,1,i,ib) = dW_WLKscale_SDF(0:localm,ib,i)
             enddo
           enddo
         endif
...
!  Consistency check for total number of SLEAVE weighting functions
         ...
         IF ( Q .ne. N_SLEAVE_WFS ) then
            ...
            NMESSAGES = NMESSAGES + 1 ; STATUS = VLIDORT_SERIOUS ;  GO TO 899 
         ENDIF
