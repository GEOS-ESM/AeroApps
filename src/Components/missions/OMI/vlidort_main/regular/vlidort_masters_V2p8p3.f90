...
!  12/10/21. Version 2.8.5. Change name to SLTERM_USER_F_0 (throughout code). Introduce LW basic (local variable)

      DOUBLE PRECISION ::   SLTERM_LWBASIC  ( MAXSTOKES, MAXBEAMS )
      DOUBLE PRECISION ::   SLTERM_F_0      ( MAXSTOKES, MAXSTREAMS, MAXBEAMS )
      DOUBLE PRECISION ::   SLTERM_USER_F_0 ( MAXSTOKES, MAX_USER_STREAMS, MAXBEAMS )
...
!  Local quantities for the water-leaving configuration
      ...
      DOUBLE PRECISION :: CUMSOURCE_DB, SLTERM_LOCAL, TRANS, TFACTOR, LWBASIC
...
!  12/10/21. Version 2.8.5. This section rewritten to encompass new way of expressing surface-leaving
!     -- Distinguish between water-leaving and Fluorescence
!     -- (Water-leaving) set LWBasic and use Scaling.

      IF ( DO_SURFACE_LEAVING ) THEN
        IF ( DO_FLUORESCENCE ) THEN
          SLTERM_ISOTROPIC(1:NSTOKES,1:N_SZANGLES) = &
               VLIDORT_Sup%SLEAVE%TS_FLUORESCENCE(1:NSTOKES,1:N_SZANGLES)
        ELSE IF ( DO_WATER_LEAVING ) THEN
          DO O1 = 1, NSTOKES
            DO IB = 1, N_SZANGLES
              SLTERM_LWBASIC(O1,IB) = VLIDORT_Sup%SLEAVE%TS_LW_BASIC(O1,IB)
              SLTERM_ISOTROPIC(O1,IB) = SLTERM_LWBASIC (O1,IB) * VLIDORT_Sup%SLEAVE%TS_KSCALE_ISOTROPIC(O1,IB)
              IF ( DO_USER_VZANGLES ) THEN
                SLTERM_USERANGLES(O1,1:N_USER_VZANGLES,1:N_USER_RELAZMS,IB) = SLTERM_LWBASIC (O1,IB) * &
                       VLIDORT_Sup%SLEAVE%TS_KSCALE_USERANGLES(O1,1:N_USER_VZANGLES,1:N_USER_RELAZMS,IB)


!write(*,*)O1,IB,SLTERM_LWBASIC(O1,IB),SLTERM_USERANGLES(O1,1:N_USER_VZANGLES,1:N_USER_RELAZMS,IB)
...
!             SLTERM_USER_F_0(0:NMOMS,1:NSTOKES,1:N_USER_VZANGLES,IBEAM) = &
!             SLTERM_USER_F_0(0:NMOMS,1:NSTOKES,1:N_USER_VZANGLES,IBEAM) * TFACTOR
!           enddo
!         endif
...
!     -- (Water-leaving), use LWBasic and Scaling. Only for water leaving

         IF ( DO_SURFACE_LEAVING .and. DO_WATER_LEAVING ) THEN
            DO O1 = 1, NSTOKES
               DO IB = 1, N_SZANGLES
                  SLTERM_F_0(O1,1:NSTREAMS,IB) = SLTERM_LWBASIC(O1,IB) * &
                        VLIDORT_Sup%SLEAVE%TS_KSCALE_F_0(FOURIER,O1,1:NSTREAMS,IB)
                  IF ( DO_USER_VZANGLES ) THEN
                     SLTERM_USER_F_0(O1,1:N_USER_VZANGLES,IB) = &
                      VLIDORT_Sup%SLEAVE%TS_KSCALE_USER_F_0(FOURIER,O1,1:N_USER_VZANGLES,IB)
                  ENDIF
               ENDDO
            ENDDO


!    write(*,*)'INPUT',FOURIER,1,SLTERM_F_0(1,1:3,1)
...
!   -- Send in the SLTERM_LWBASIC argument
         ...
         CALL VLIDORT_FOURIER ( FOURIER, DO_FOURIER0_NSTOKES2, &
            ...
            Misc, Therm, Mult, ALBEDO, BRDF_F, BRDF_F_0, USER_BRDF_F, USER_BRDF_F_0,                              & !Input
            SLTERM_LWBASIC, SLTERM_ISOTROPIC, SLTERM_F_0, SLTERM_USER_F_0,                                        & !Input
...
!  Useful debug
!       write(*,*)FOURIER,SLTERM_F_0(1,1:4,1)
!       write(*,*)FOURIER,STOKES_F(1,1,1,1:3,1)
!       write(*,*)FOURIER,STOKES_F(2,1,1,1:3,1)
!       write(*,*)FOURIER,STOKES_F(2,1,1,1:3,2)
...
!  12/10/21. Version 2.8.5. Probably going to scrap this. Change logic
!    -- Instead, just add TRANS_ATMOS_FINAL to the VLIDORT output type structure

         IF ( DO_WATER_LEAVING ) THEN
           IF ( FOURIER.eq.0 ) THEN
             VLIDORT_Out%Main%TS_TRANS_ATMOS_FINAL(1:NBEAMS) = TRANS_ATMOS_FINAL(1:NBEAMS)
           ENDIF
           IF ( DO_WLADJUSTED_OUTPUT ) THEN
             IF ( FOURIER.eq.0 ) THEN
               ...
               DO IB = 1, NBEAMS
                 ...
                 IF ( DO_USER_VZANGLES ) THEN
                   VLIDORT_Out%WLOut%TS_WLADJUSTED_F_User_0(FOURIER,O1,1:N_USER_VZANGLES,IB) = &
                                          TFACTOR * SLTERM_USER_F_0(O1,1:N_USER_VZANGLES,IB)
                 ENDIF
               ENDDO
             ENDIF
           ENDIF
...
!  12/10/21. Version 2.8.5. Do not need this.
...
!            VLIDORT_Sup%SLEAVE%TS_SLTERM_USER_F_0(FOURIER,O1,1:N_USER_VZANGLES,1:N_SZANGLES) = &
!                                  SLTERM_USER_F_0(O1,1:N_USER_VZANGLES,1:N_SZANGLES)
!         ENDIF
...
!  12/10/21. Version 2.8.5. Completely rewritten to accommodate new-style SLEAVE definitions

      IF (DO_SURFACE_LEAVING) THEN
        CALL VLIDORT_WRITE_SUP_SLEAVE_INPUT ( &
          DO_USER_VZANGLES, NSTOKES, NSTREAMS, N_SZANGLES, N_USER_VZANGLES, N_USER_RELAZMS, &
          VLIDORT_Sup%SLEAVE%TS_LW_BASIC, VLIDORT_Sup%SLEAVE%TS_KSCALE_ISOTROPIC,           &
          VLIDORT_Sup%SLEAVE%TS_KSCALE_USERANGLES, VLIDORT_Sup%SLEAVE%TS_KSCALE_F_0, VLIDORT_Sup%SLEAVE%TS_KSCALE_USER_F_0 )
      END IF
...
!    -- 1/31/21.  Version 2.8.3. Local Fourier-component dimension (0:MAXMOMENTS) has been dropped.
!    -- 12/10/21. Version 2.8.5. Include the LWBasic argument
 
      DOUBLE PRECISION, INTENT (IN) ::   SLTERM_LWBASIC   ( MAXSTOKES, MAXBEAMS )
      ...
      DOUBLE PRECISION, INTENT (IN) ::   SLTERM_USER_F_0  ( MAXSTOKES, MAX_USER_STREAMS, MAXBEAMS )
      ...
      IF ( DO_SOLAR_SOURCES ) THEN
        IF ( .NOT.DO_FOCORR_ALONE ) THEN
          CALL VLIDORT_DIRECTRADIANCE ( DO_USER_STREAMS, DO_REFRACTIVE_GEOMETRY,           & ! Input flags (general)
                ...
                TRANS_ATMOS_FINAL, SLTERM_ISOTROPIC, SLTERM_F_0, SLTERM_USER_F_0,          & ! input surface leaving
...
!  Useful debug
!      do IBEAM = 1, nbeams
!         write(*,*)FOURIER,TRANS_ATMOS_FINAL(IBEAM)
!         write(*,*)FOURIER,SL_QUADTERM(1,IBEAM,1), SL_USERTERM(1,IBEAM,1)
!     enddo
...
!  1/31/21. Version 2.8.3. SLTERM_USER_F_0 defined locally, drop FOURIER index.
               ...
               IF ( FOURIER.EQ.0 ) then
                  ...
                  ELSE
                     DO LUM = 1, N_PPSTREAMS
                        ...
                        SL_USERTERM(UM,IBEAM,O1) = SLTERM_USER_F_0(O1,UM,IBEAM) * TFACTOR
                     ENDDO
                  ENDIF
               ELSE
                  DO LUM = 1, N_PPSTREAMS
