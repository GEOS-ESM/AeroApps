...
!
      ...
      SUBROUTINE VLIDORT_WRITE_SUP_SLEAVE_INPUT ( &
        ...
        LW_BASIC, KSCALE_ISOTROPIC, KSCALE_USERANGLES, KSCALE_F_0, KSCALE_USER_F_0)
...
!  12/10/21. Version 2.8.5. Completely rewritten to accommodate new-style SLEAVE definitions (LWBASIC + 4 KSCALE)
...
!      INTEGER, INTENT(IN) ::   NMOMENTS
      ...
      DOUBLE PRECISION, INTENT(IN) :: LW_BASIC          ( MAXSTOKES, MAXBEAMS )
      DOUBLE PRECISION, INTENT(IN) :: KSCALE_ISOTROPIC  ( MAXSTOKES, MAXBEAMS )
      DOUBLE PRECISION, INTENT(IN) :: KSCALE_USERANGLES ( MAXSTOKES, MAX_USER_STREAMS, MAX_USER_RELAZMS, MAXBEAMS  )

!  1/31/21. Version 2.8.3. (2/16/21). SLEAVE arrays directly from type structure. Restore MAXMOMENTS

      DOUBLE PRECISION, INTENT(IN) :: KSCALE_F_0      ( 0:MAXMOMENTS, MAXSTOKES, MAXSTREAMS, MAXBEAMS )
      DOUBLE PRECISION, INTENT(IN) :: KSCALE_USER_F_0 ( 0:MAXMOMENTS, MAXSTOKES, MAX_USER_STREAMS, MAXBEAMS )
...
!  Write all surface-leaving input to file
      ...
      DO IB=1,N_SZANGLES
        DO S=1,NSTOKES
          WRITE(OUTUNIT,DWFR2) &
            'IB = ',IB,' S = ',S,' LW_BASIC(S,IB) = ',LW_BASIC(S,IB)
        END DO
      END DO
      ...
      DO IB=1,N_SZANGLES
        DO S=1,NSTOKES
          WRITE(OUTUNIT,DWFR2) &
            'IB = ',IB,' S = ',S,' KSCALE_ISOTROPIC(S,IB) = ',KSCALE_ISOTROPIC(S,IB)
        END DO
      END DO
            ...
            DO USTRM=1,N_USER_VZANGLES
              DO S=1,NSTOKES
                WRITE(OUTUNIT,DWFR4) &
                  ...
                  ' KSCALE_USERANGLES(S,USTRM,URA,IB) = ',KSCALE_USERANGLES(S,USTRM,URA,IB)
              END DO
            END DO
          END DO
        END DO
      ...
      DO IB=1,N_SZANGLES
        DO STRM=1,NSTREAMS ; DO S=1,NSTOKES ; DO MOM = 0, NMOMS
          WRITE(OUTUNIT,DWFR4) &
            ...
            ' KSCALE_F_0(MOM,S,STRM,IB) = ',KSCALE_F_0(MOM, S,STRM,IB)
        END DO ; END DO ; END DO
      END DO
          ...
          DO S=1,NSTOKES
             WRITE(OUTUNIT,DWFR4) &
               'IB = ',IB,' USTRM = ',USTRM,' S = ',S,' MOM = ',MOM,&
                  ' KSCALE_USER_F_0(MOM,S,USTRM,IB) = ',KSCALE_USER_F_0(MOM,S,USTRM,IB)
          END DO
        END DO ; END DO ; END DO
      END IF
