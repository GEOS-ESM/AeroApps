...
def getRandom(args):
    varname,ndim,idim,index,inFiled = args
...
class MERGE(object):
    ...
    def __init__(self,date,randomFile,args):
        ...
        if args.ext is None:
            ...
            do_ext_spc = False
            do_ext_fine = False
            do_ext_spc_fine = False
        else:
            if (args.fine is None) and (args.spc is None):
                do_ext = True
            else:
                do_ext = False
            ...
            if (args.fine is None) and (args.spc is None):
                ...
                do_ext_fine = False
                do_ext_spc_fine = False
            elif (args.fine) and (args.spc is None):
                do_ext_spc = False
                do_ext_fine = True
                do_ext_spc_fine = False
            elif (args.fine) and (args.spc):
                do_ext_spc = False
                do_ext_spc_fine = False
                do_ext_spc_fine = True
            elif (args.fine is None) and (args.spc):
                do_ext_spc = True
                do_ext_fine = False
                do_ext_spc_fine = False                
...
#        cols = []
        ...
        # total 
        inTemplate    = cf('outDir')    + '/' + '%orbitname-{}.lc.ext.%nymd_%hour00z.%chnm.nc4'.format(args.exp)
        outDir       = cf('outDir').split('/')[:-1]
        outDir         = '/'.join(outDir)
        outTemplate    = outDir + '/' + '%orbitname-{}.lc.ext.random.%nymd_%hour00z.%chnm.nc4'.format(args.exp)
        ...
        # total speciated
        if do_ext_spc:
            inTemplate    = cf('outDir')    + '/' + '%orbitname-{}.lc.ext.%nymd_%hour00z.%chnm.%spc.nc4'.format(args.exp)
            outDir       = cf('outDir').split('/')[:-1]
            outDir         = '/'.join(outDir)
            outTemplate    = outDir + '/' + '%orbitname-{}.lc.ext.random.%nymd_%hour00z.%chnm.%spc.nc4'.format(args.exp)
        ...
        # fine mode
        inTemplate    = cf('outDir')    + '/' + '%orbitname-{}.lc.ext.finemode.%nymd_%hour00z.%chnm.nc4'.format(args.exp)
        outDir       = cf('outDir').split('/')[:-1]
        outDir         = '/'.join(outDir)
        outTemplate    = outDir + '/' + '%orbitname-{}.lc.ext.finemode.random.%nymd_%hour00z.%chnm.nc4'.format(args.exp)
        ...
        # finemode speciated
        inTemplate    = cf('outDir')    + '/' + '%orbitname-{}.lc.ext.finemode.%nymd_%hour00z.%chnm.%spc.nc4'.format(args.exp)
        outDir       = cf('outDir').split('/')[:-1]
        outDir         = '/'.join(outDir)
        outTemplate    = outDir + '/' + '%orbitname-{}.lc.ext.finemode.random.%nymd_%hour00z.%chnm.%spc.nc4'.format(args.exp)
        ...
        # lidar merged files
        inTemplate  = cf('outDir')    + '/' + '%orbitname-lidar-{}.lc2.%nymd_%hour00z.%chnm.nc4'.format(args.exp)
        outDir       = cf('outDir').split('/')[:-1]
        outDir         = '/'.join(outDir)
        outTemplate    = outDir + '/' + '%orbitname-lidar-{}.lc2.random.%nymd_%hour00z.%chnm.nc4'.format(args.exp)
        ...
        for varname in nco.variables:
            ...
            if 'time' in dim:
                ...
                for irandom,dd in enumerate(self.random_times):                    
                    ...
                    aa = [varname,ndim,idim,index,inFiled]
                    args.append(aa)

                p = Pool(25)
                varout = p.map(getRandom,args)
                p.close()
                p.join()
...
#   --------------------------
    ...
    parser.add_argument( "--levelb",action="store_true",
                        help="do levelb merging (default=False).")

    parser.add_argument( "--surface",action="store_true",
                        help="do surface merging (default=False).")

    parser.add_argument( "--merged",default=None,
                        help="do lidar merged files merging (default=None).")

    parser.add_argument( "--ext",default=None,
                        help="channel for ext merging (default=None).")

    parser.add_argument( "--spc",default=None,
                        help="species for ext merging (default=None).")

    parser.add_argument( "--fine",action="store_true",
                        help="do ext fine mode merging (default=False).")

    parser.add_argument( "--vlidort",action="store_true",
                        help="do vlidort merging (default=False).")
