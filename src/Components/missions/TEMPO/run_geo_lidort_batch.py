#!/bin/python3
# -*- coding: utf-8 -*-
""" Runscript for geo_lidort episodes"""
...
def make_workspace(date,ch,outdir):
    dirname = str(date.date())+'T'+str(date.time())+'.'+ch+'.lidort'
    outdir = outdir + '/Y'+str(date.year)+'/M'+str(date.month).zfill(2)+'/D'+str(date.day).zfill(2)
    if not os.path.exists(dirname):
        os.makedirs(dirname)
    
    os.chdir(dirname)
    if not os.path.isfile('geo_lidort.x'):
        os.symlink('../geo_lidort.x','geo_lidort.x')
    ...
    source = open('../geo_lidort_run.j','r')
    destination = open('geo_lidort_run.j','w')
    for line in source:
...
def make_maiac_rcfile(dirname,indir,date,ch,interp,i_band=None):
    os.chdir(dirname)

    rcfile = open('geo_lidort.rc','w')
...
def make_ler_rcfile(dirname,indir,date,ch,interp,i_band=None):
    os.chdir(dirname)
...
if __name__ == "__main__":
    ...
    while (startdate <= enddate):
        for i, ch in enumerate(channels):
            ...
            workdir = make_workspace(startdate,ch,outdir)

            if (surface.upper() == 'MAIACRTLS'):
                make_maiac_rcfile(workdir,indir,startdate,ch,interp,i_band=band_i)
            else:
                make_ler_rcfile(workdir,indir,startdate,ch,interp,i_band=i_band[i])

            runstring = np.append(runstring,'./'+workdir+'/geo_lidort_run.j')
