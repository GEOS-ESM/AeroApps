...
def getCoords(geoFile,verbose):
    ...
    if not hasattr(lon,'mask'):
        lon = np.ma.array(lon)
        lon.mask = lon == missing

    if not hasattr(lat,'mask'):
        lat = np.ma.array(lat)
        lat.mask = lat == missing
...
if __name__ == "__main__":
    ...
    outFile = 'tempo.cld.mcd43c1_climatology.413.nc4'
    geoFile = 'tempo.lg1.cld.invariant.413.nc4'
    ...
    #-------------------------------------------
    filelist = glob(path + 'Y2006/M*/*.hdf')
    a = glob(path + 'Y2007/M*/*.hdf')
    for f in a: filelist.append(f)
