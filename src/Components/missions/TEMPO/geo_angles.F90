...
! --------
  ...
  character(len=256)                    :: satname, indir, outdir
...
! -------------------------------
  if (myid == 0) then
    write(*,*) 'Porcessing ', lower_to_upper(trim(satname)),' domain on ',date,' ', time, 'Z'
...
! Create OUTFILE
!-------------------------
  call mp_create_outfile(date,time,ncid,szaVarID, vzaVarID, raaVarID)
...
! -----------------------------------------
  ...
  if (myid == 0) then
    write(*,*) '<> Finished calculating sensor and solar angles for '//trim(lower_to_upper(satname))//' domain'
      ...
      if (myid == p) then
        ...
        do j = startj, endj  
          if (CLAT(i,j) .ne. MISSING) then      
            ...
            ! Write to outfile
            call check(nf90_put_var(ncid,szaVarID,(/sat_angles(4)/),start=(/i,j/),count=(/1,1/)), "writing out sza")
            call check(nf90_put_var(ncid,vzaVarID,(/sat_angles(2)/),start=(/i,j/),count=(/1,1/)), "writing out vza")
            call check(nf90_put_var(ncid,raaVarID,(/sat_angles(1) - sat_angles(3)/),start=(/i,j/),count=(/1,1/)), "writing out raa")
          end if
        end do
...
! OUTFILES
  write(OUT_file,'(14A)') trim(outdir),'/LevelB/Y',date(1:4),'/M',date(5:6),'/D',date(7:8),'/',trim(satname),'.lb2.angles.',date,'_',time,'z.nc4'
...
!;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  subroutine mp_create_outfile(date, time, ncid, szaVarID, vzaVarID, raaVarID)
    ...
    integer                            :: clonVarID, clatVarID, timeVarID
    ...
    call check(nf90_put_att(ncid,NF90_GLOBAL,'title','Satellite and solar position angles for '//lower_to_upper(satname)),"title attr")
    ...
    ! one processor writes out clon, clat, scantime
    if (myid == 0) then
    ...
    call check(nf90_var_par_access(ncid, szaVarID, nf90_independent),"SZA writes independently")
    call check(nf90_var_par_access(ncid, vzaVarID, nf90_independent),"VZA writes independently")
    call check(nf90_var_par_access(ncid, raaVarID, nf90_independent),"RAAA writes independently")
