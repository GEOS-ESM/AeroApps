...
if __name__ == "__main__":
   ...
   #yyyy, mm, dd = '2020', '09', '10' ; cal_orb_time=[21,4];out_label='AGU' # AGU smoke case 
   #yyyy, mm, dd = '2021', '08', '28' ; cal_orb_time=[14,33];out_label='AGU' # AGU dust case
#    yyyy, mm, dd = '2021', '08', '28' ; cal_orb_time=[21,7] ;out_label='case2'# Post AGU
   yyyy, mm, dd = '2020', '08', '21' ; cal_orb_time=[19,58] ;out_label='test'# Post AGU
   ...
   print('Get CALIPSO Level 2 data')
#    print(cal_full_path)
   ### Sort files in ascending order in date and time
...
#    sys.exit()
   ...
   for i in range(CAL_NORBS):
      print(f"   Orbit {i+1} starts at {CAL2.time[b[i]]} , end in {CAL2.time[b[i+1]-1]}") 
   ...
   for i in range(TRO_NORBS):
      print(f"  {i} , time {IWFILES[i][15:21]}  , array size lines x cols {np.shape(IWVAR['lat'][i])} ")
   ...
   print("Now match temporarly each CALIOP orbit with closest TROPomi\n") 
   #### now match orbits
...
#       IOW_indexes = [differences[0][1], differences[1][1]]
   ...
   base_date = TROP_time[TROP_indx[0][0]].date()  # Get the date from first TROPO time
   ...
   print(f" Matching TROPO time: {TROP_time[TROP_indx[closest_index][0]]}")   
   ...
   ### Case 2020-09-10 (very first case)
   ical = closest_index# 2  # For CAL2.time[b[2] (2020-09-10 21:04:18), closest TROPO times are: 2020-09-10 20:34:03
   ### Case  2021-08-28 13:49:32 (Sahara dust)
   ###ical =3
   
   ###### Get index for overlap TROP file  
   itro = TROP_indx[ical][0] # for selected CAL orbit, select the closes TROPMI orbit
   ### Set output file string with a label set by user
   output_file = 'IOWA-CALIOP_' + CAL2.filelist[ical][35:-4] + '_' + out_label + '.nc' 
   print(f"\nWorking with CAL orbit ({CAL2.time[b[ical]]}), with closest TROPO time : {TROP_time[TROP_indx[ical][0]]}" )
   print("    Selected CALIOP L2 file :" , CAL2.filelist[ical])
