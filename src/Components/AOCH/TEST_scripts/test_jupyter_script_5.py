...
string_out='TEST'
#output_filename = 'AlongTrack_MERRA-IOWA-CALIOP_' + caldata.cal_filename[35:-15] + '_3.nc'
output_filename = 'MERRA2_over_mplsite_' + string_out + '.txt'
...
time_range = [time_array[0]-timedelta(hours=1),time_array[0]+timedelta(hours=1)]
...
for i in range(N):
    ...
    #stn = STATION(['GSFC'],[lon],[lat],m2data,time_range=time_range,verbose=False)
...
with open(output_filename, 'w', newline='') as csvfile:
    ...
    writer.writerow(header)  # header in row 0
    # Format and write each row
    for row in data_output_list:
        formatted_row = [
            f"{int(row[0]):6d}",      # integer with 6 characters width
            f"{float(row[1]):6.2f}",   # float with 6 total width, 2 decimal places
            f"{float(row[2]):6.2f}"    # float with 6 total width, 2 decimal places
        ]
