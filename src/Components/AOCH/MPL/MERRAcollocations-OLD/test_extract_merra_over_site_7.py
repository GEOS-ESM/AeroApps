...
-------------------------------------
May/2/2025 - ver7 - test another approache. After traj_ds is computed , loop in time and subset the traj_ds array,
and then save in output file and then load as it in the previous version. 
...
#traj_ds_i = traj_ds.isel(time=0) # method 1
traj_ds_i = traj_ds.isel(time=slice(0, 0+1)) #method 2

### Save optical properties?
if save_merra == 'True':
   out_MERRA = 'm2_test_method1.nc'
   print('\n Saving MERRA data file.... ', out_MERRA)
   traj_ds_i.to_netcdf(out_MERRA)
   print(" Done Savings!\n")
