...
!  LIDORT module dependencies
   ...
   USE lidort_lc_corrections, only : LIDORT_LC_SSCORR_NADIR, LIDORT_LC_SSCORR_OUTGOING, &
   ...
   USE lidort_lc_miscsetups        ! Need 2 public routines
   ...
   USE lidort_lc_bvproblem,   only : LC_BVP_SOLUTION_MASTER, LC_BVPTEL_SOLUTION_MASTER

   USE lidort_intensity            ! Need all these

   USE lidort_lc_wfatmos           ! Need all these
...
!PRIVATE :: LIDORT_LCS_FOURIER_MASTER
...
!  Mean-intensity and flux weighting functions
      ...
      REAL(fpk) :: MINT_COLUMNWF ( MAX_ATMOSWFS, MAX_USER_LEVELS, MAXBEAMS, MAX_DIRECTIONS )
      REAL(fpk) :: FLUX_COLUMNWF ( MAX_ATMOSWFS, MAX_USER_LEVELS, MAXBEAMS, MAX_DIRECTIONS )
...
!  Mean-intensity and weighting functions
...
!  Fourier-component column weighting functions at user angles
      ...
      REAL(fpk)       :: COLUMNWF_F ( MAX_ATMOSWFS,     MAX_USER_LEVELS, &
...
!  New 15 March 2012
      ...
      MINT_COLUMNWF(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:NBEAMS,:)  = ZERO
      FLUX_COLUMNWF(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:NBEAMS,:)  = ZERO
      ...
      IF ( DO_THERMAL_EMISSION ) THEN
        IF ( DO_COLUMN_LINEARIZATION ) THEN
          ...
          ( DO_UPWELLING, DO_DNWELLING, DO_THERMAL_TRANSONLY, DO_MSMODE_THERMAL, &
            DO_USER_STREAMS, DO_COLUMN_LINEARIZATION,               & ! input
...
!     LC_SSCORR is now a "Plus" routine (Intensity and Jacobians)

        IF ( DO_SSCORR_NADIR .AND. .NOT. DO_SS_EXTERNAL ) THEN

!  Intensity only or Intensity+Column Linearization
...
!  ----------------------------------
        ...
        IF ( DO_SSCORR_OUTGOING .AND. .NOT. DO_SS_EXTERNAL ) THEN
          ...
          CALL LIDORT_LC_SSCORR_OUTGOING                                                  &
              ( DO_UPWELLING, DO_DNWELLING, DO_COLUMN_LINEARIZATION,                      & ! Input
                ...
                NBEAMS, N_USER_STREAMS, N_USER_RELAZMS, N_USER_LEVELS, N_TOTALCOLUMN_WFS, & ! Input
          ...
          IF ( STATUS_SUB .ne. LIDORT_SUCCESS ) THEN
            TRACE_2 = 'Error from LIDORT_LC_SSCORR_OUTGOING'
            TRACE_3 = ' ** Called in LIDORT_LCS_MASTER'
            STATUS_CALCULATION = LIDORT_SERIOUS
            LIDORT_Out%Status%TS_STATUS_CALCULATION = STATUS_CALCULATION
            LIDORT_Out%Status%TS_MESSAGE = MESSAGE
...
!mick mod 3/19/2015 - consolidated if conditions
      IF ( .NOT.DO_SOLAR_SOURCES .OR. DO_ISOTROPIC_ONLY &
...
!  @@@@@@@@@@@ Robfix 10 October 2013 - Add TAYLOR_ORDER argument (Line 1)
          ...
          CALL LIDORT_LCS_FOURIER_MASTER &
          ( FOURIER_COMPONENT, TAYLOR_ORDER, DO_OBSERVATION_GEOMETRY,                                                      & !Input
            ...
            INITIAL_TRANS, ITRANS_USERM, AVERAGE_SECANT, LC_INITIAL_TRANS, LC_AVERAGE_SECANT,                              & !Input
            ...
            EMULT_UP, EMULT_DN, UT_EMULT_UP, UT_EMULT_DN, LC_EMULT_UP, LC_EMULT_DN, LC_UT_EMULT_UP, LC_UT_EMULT_DN,        & !Input
            DO_COLUMN_LINEARIZATION, DO_SURFACE_LINEARIZATION, N_TOTALCOLUMN_WFS, LAYER_VARY_FLAG,LAYER_VARY_NUMBER,       & !Input
            ...
            COLUMNWF_F,  MINT_COLUMNWF,  FLUX_COLUMNWF, SURFACEWF_F, MINT_SURFACEWF, FLUX_SURFACEWF,                       & !Output
...
!  Fourier summation of Column jacobians
                ...
                IF ( DO_COLUMN_LINEARIZATION ) THEN
                  ...
                  ( DO_UPWELLING, DO_SS_EXTERNAL, DO_SSFULL,            & ! input
                    ...
                    IBEAM, FOURIER_COMPONENT, UMOFF,                    & ! input
...
!  Fourier summation of Column jacobians
                ...
                IF ( DO_COLUMN_LINEARIZATION ) THEN
                  ...
                  ( DO_UPWELLING, DO_SS_EXTERNAL, DO_SSFULL,                 & ! input
                    DO_SSCORR_NADIR, DO_SSCORR_OUTGOING, DO_NO_AZIMUTH,      & ! input
                    AZMFAC, N_USER_LEVELS, IBEAM, FOURIER_COMPONENT,         & ! input
                    N_DIRECTIONS, WHICH_DIRECTIONS, N_TOTALCOLUMN_WFS,       & ! input
                    COLUMNWF_F, COLUMNWF_SS, COLUMNWF_DB,                    & ! input
                    COLUMNWF )                                                 ! output
                ENDIF
...
!          IF ( DO_COLUMN_LINEARIZATION ) THEN
!            CALL LIDORT_LC_WRITEFOURIER ( DO_INCLUDE_SURFACE, FUNIT, FOURIER_COMPONENT )
!          ENDIF
...
!  Column Jacobians
      ...
      LIDORT_LinOut%Atmos%TS_COLUMNWF&
        (1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:N_GEOMETRIES,:) = &
          COLUMNWF(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:N_GEOMETRIES,:)
      LIDORT_LinOut%Atmos%TS_MINT_COLUMNWF&
        (1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:NBEAMS,:)       = &
          MINT_COLUMNWF(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:NBEAMS,:)
      LIDORT_LinOut%Atmos%TS_FLUX_COLUMNWF&
        (1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:NBEAMS,:)       = &
          FLUX_COLUMNWF(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:NBEAMS,:)
...
!   IF SS results already available, no need to copy them !
      ...
      IF ( .NOT. DO_SS_EXTERNAL ) THEN
         !SS atmosphere column weighting functions
...
!  Finish
      ...
      CALL LIDORT_WRITE_STD_INPUT ( &
        ...
        NSTREAMS,NLAYERS,NFINELAYERS,N_THERMAL_COEFFS,&
...
SUBROUTINE LIDORT_LCS_FOURIER_MASTER &
      ( FOURIER_COMPONENT, TAYLOR_ORDER, DO_OBSERVATION_GEOMETRY,                                                      & !Input
        DO_UPWELLING, DO_DNWELLING, DO_SOLAR_SOURCES, DO_BRDF_SURFACE, DO_THERMAL_EMISSION, DO_SURFACE_EMISSION,       & !Input
        DO_THERMAL_TRANSONLY, DO_PLANE_PARALLEL, DO_USER_STREAMS, DO_SOLUTION_SAVING, DO_BVP_TELESCOPING,              & !Input
        DO_ADDITIONAL_MVOUT, DO_MVOUT_ONLY, DO_REFRACTIVE_GEOMETRY, DO_MSMODE_LIDORT, DO_MULTIBEAM, DO_MSMODE_THERMAL, & !Input
...
!  Mean-intensity and Flux weighting functions
      ...
      REAL(fpk), intent(inout) :: MINT_COLUMNWF ( MAX_ATMOSWFS, MAX_USER_LEVELS, MAXBEAMS, MAX_DIRECTIONS )
      REAL(fpk), intent(inout) :: FLUX_COLUMNWF ( MAX_ATMOSWFS, MAX_USER_LEVELS, MAXBEAMS, MAX_DIRECTIONS )
...
!  .. error tracing
         ...
         IF ( STATUS_SUB .NE. LIDORT_SUCCESS ) THEN
           ...
           TRACE_2 =  'Called in LIDORT_LCS_FOURIER_MASTER, Fourier component '//CF
       ...
       STATUS_SUB, MESSAGE, TRACE_1 )                                             ! Output
         ...
         IF ( STATUS_SUB .NE. LIDORT_SUCCESS ) THEN
           ...
           TRACE_2 = 'Error from BVP_MATRIXSETUP_MASTER, '//   &
                'Called in LIDORT_LCS_FOURIER_MASTER, Fourier # '//CF
           STATUS = LIDORT_SERIOUS
           RETURN
         ENDIF
...
!     I     omega_moms(1,0), L_omega_moms(1,1,0),     ! replaced line
        ...
        ELSE
          ...
          ( DO_UPWELLING, DO_DNWELLING, DO_THERMAL_TRANSONLY,    & ! Input
            DO_MVOUT_ONLY, DO_ADDITIONAL_MVOUT,                  & ! Input
            NSTREAMS, N_THERMAL_COEFFS,                          & ! Input
            NLAYERS, N_PARTLAYERS, PARTLAYERS_LAYERIDX,          & ! Input
...
!         @@@ Rob Fix, 02.05/11 Do not need this, Commented out
!                     done instead in LC_BVP_SOLUTION_MASTER
...
!  Column-Linearized BOA source term
            ...
            ( DO_INCLUDE_SURFACE, DO_BRDF_SURFACE, DO_INCLUDE_DIRECTBEAM,   & ! Input
              ...
              IBEAM, N_TOTALCOLUMN_WFS, QUAD_STRMWTS, QUAD_WEIGHTS,         & ! Input
...
!  Linearized upwelling Column weighting functions
            ...
            ( DO_USER_STREAMS, DO_SOLAR_SOURCES,                         & ! Input
              ...
              PARTLAYERS_OUTFLAG, PARTLAYERS_OUTINDEX,                   & ! Input
              UTAU_LEVEL_MASK_UP, PARTLAYERS_LAYERIDX,                   & ! Input
              FOURIER_COMPONENT, IBEAM, N_TOTALCOLUMN_WFS,               & ! Input
...
!  TOA source term linearized
            ...
            CALL GET_LC_TOASOURCE &
              ( DO_OBSERVATION_GEOMETRY, N_USER_STREAMS, IBEAM, N_TOTALCOLUMN_WFS, &
                LC_TOA_SOURCE )
...
!  Rob Fix, 01/06/14, Reorganized calling arguments, including TAYLOR_ORDER
           ...
           ( DO_LAYER_SCATTERING, TAYLOR_ORDER, FOURIER_COMPONENT,          & ! Input, Flags, Order
             IBEAM, NLAYERS, NSTREAMS, NSTREAMS_2, N_TOTALCOLUMN_WFS,       & ! Input, Numbers
...
!  ######
      ...
      RETURN
END SUBROUTINE LIDORT_LCS_FOURIER_MASTER

end MODULE LIDORT_LCS_masters
