...
!  module, dimensions and numbers
      ...
      USE LIDORT_pars, only : fpk, MAXLAYERS, MAXMOMENTS_INPUT, MAXMOMENTS,    &
...
!  module, dimensions and numbers
      ...
      USE LIDORT_pars, only : fpk, MAXLAYERS, MAX_PARTLAYERS, MAX_USER_LEVELS, &
                              MAXMOMENTS_INPUT, MAXMOMENTS, MAXBEAMS, ZERO, ONE, MAX_TAU_SPATH
...
!  Rob fix 11/27/14. Added Argument
...
!  Observational Geometry input control
        ...
        IF ( .not. DO_BRDF_SURFACE ) THEN
          ...
          IF ( DO_USER_STREAMS ) THEN
            IF ( .not. DO_OBSERVATION_GEOMETRY ) THEN
              ...
              ENDDO
            ELSE
              USER_DIRECT_BEAM(LUI,IB) = REFL_ATTN
            ENDIF
          ENDIF
        ENDIF
        ...
        IF ( DO_BRDF_SURFACE ) THEN
          ...
          IF ( DO_USER_STREAMS ) THEN
            IF ( .not. DO_OBSERVATION_GEOMETRY ) THEN
              ...
              ENDDO
            ELSE
              USER_DIRECT_BEAM(LUI,IB) = ATTN * USER_BRDF_F_0(M,LUI,IB)
            ENDIF
          ENDIF
        ENDIF
          ...
          IF ( DO_SL_ISOTROPIC .and. M.EQ.0 ) THEN
            ...
            IF ( DO_USER_STREAMS ) THEN
              IF ( .not. DO_OBSERVATION_GEOMETRY ) THEN
                ...
                ENDDO
              ELSE
                USER_DIRECT_BEAM(LUI,IB) = USER_DIRECT_BEAM(LUI,IB) + SL
              ENDIF
            ENDIF
          ELSE
            DO I = 1, NSTREAMS
            ...
            IF ( DO_USER_STREAMS ) THEN
              IF ( .not. DO_OBSERVATION_GEOMETRY ) THEN
                ...
                ENDDO
              ELSE
                SL = USER_SLTERM_F_0(M,LUI,IB)* HELP
                USER_DIRECT_BEAM(LUI,IB) = USER_DIRECT_BEAM(LUI,IB) + SL
              ENDIF
            ENDIF
          ENDIF
...
!     Rob  Fix 01/06/14  - Use of PPSTREAM and mask to deal with Obsgeom/Lattice
      ...
      USE LIDORT_pars, only : fpk, MAX_USER_STREAMS, MAXLAYERS, &
