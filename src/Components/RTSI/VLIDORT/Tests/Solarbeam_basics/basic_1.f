...
C   number of layers will be set from the atmosphere data read
     ...
     &    ('vlidort_v2p4rt_testing.inp_1',
     &     'vlidort_v2p4rt_testing.log_1',status_inputread)
      if ( status_inputread.ne.vlidort_success)
     &     stop'BAD READ of file: vlidort_v2p4_testing.inp_1'
...
C  Progress
      ...
      write(lu,'(a,50i3)')
     &    '   --- Done baseline calculation 1; # Fourier terms = ', 
     &        (FOURIER_SAVED(IB),IB = 1, N_SZANGLES)

C  Exception handling

      if (status_inputcheck.eq.vlidort_serious ) then
        write(*,*)'baseline calculation 1, input check failed'
...
c      SUBROUTINE GET_ELAPSED_TIME (ELAPSED_TIME)
C       ----------------------------------------------------------------
C       output arguments
C       ----------------------------------------------------------------
c        REAL            ELAPSED_TIME
...
C       local variables
C       ----------------------------------------------------------------
c        REAL            USER_SYS_TIME
C       ----------------------------------------------------------------
C       get elapsed time
C       ----------------------------------------------------------------
c        ELAPSED_TIME = ETIME(USER_SYS_TIME)
c
C       ----------------------------------------------------------------
C       return
C       ----------------------------------------------------------------
