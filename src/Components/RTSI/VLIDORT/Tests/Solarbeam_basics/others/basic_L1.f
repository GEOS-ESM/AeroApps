...
C   number of layers will be set from the atmosphere data read
     ...
     &    ('vlidort_v2p4_testing.inp_L1',
     &     'vlidort_v2p4_testing.log_L1',status_inputread)
      if ( status_inputread.ne.vlidort_success)
     &     stop'BAD READ of file: vlidort_v2p4_testing.inp_L1'
...
c      call get_elapsed_time(e1)
      CALL VLIDORT_L_MASTER_LAMBERTIAN
...
C  Progress
      ...
      write(lu,'(a,50i3)')
     &    '   --- Done baseline calculation L1; # Fourier terms = ', 
     &        (FOURIER_SAVED(IB),IB = 1, N_SZANGLES)

C  Exception handling

      if (status_inputcheck.eq.vlidort_serious ) then
        write(*,*)'baseline calculation L1, input check failed'
