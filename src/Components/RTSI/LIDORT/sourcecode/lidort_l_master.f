...
C  Include file of input and bookkeeping variables (regular)
...
C  ---------------
      ...
      INTEGER          UA, UM, TESTCONV, L, COUNT, KL, KQ
...
C  Standard input variables check
      ...
      IF ( STATUS_SUB .EQ. LIDORT_SERIOUS ) THEN
        ...
        TRACE = ' Called in LIDORT_L_MASTER '
...
C  Extended input variables check
      ...
      IF ( DO_LINEARIZATION ) THEN
        ...
        ELSE IF ( STATUS_SUB .EQ. LIDORT_WARNING ) THEN
          STATUS_INPUTCHECK = LIDORT_WARNING
          MAIL = ' LIDORT_L_CHECK_INPUT warning message'
          TRACE = ' Called in LIDORT_L_MASTER '
          CALL LIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS_INPUTCHECK )
        ENDIF
      ENDIF
...
C  Automatically 1 in the Lambertian case
      ...
      ELSE
       ...
       DO KL = 1, N_BRDF_KERNELS
        IF ( DO_KERNEL_FACTOR_WFS(KL) ) COUNT = COUNT + 1
        DO_KPARAMS_DERIVS(KL) = .FALSE.
        DO KQ = 1, N_BRDF_PARAMETERS(KL)
          IF ( DO_KERNEL_PARAMS_WFS(KL,KQ) ) THEN
            COUNT = COUNT + 1
            DO_KPARAMS_DERIVS(KL) = .TRUE.
          ENDIF
        ENDDO
       ENDDO
       N_TOTALBRDF_WFS = COUNT
      ENDIF

C  standard + linearized BRDF inputs, both together

      IF ( .NOT. DO_LAMBERTIAN_SURFACE ) THEN
        CALL LIDORT_L_BRDF_MASTER
      ENDIF
...
C  error handling
        ...
        IF ( STATUS_SUB .NE. LIDORT_SUCCESS ) THEN
          ...
          MAIL = 'Error from LIDORT_L_FOURIER_MASTER, Fourier = '//CF
          TRACE = 'Called by LIDORT_L_MASTER'
          CALL LIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS_CALCULATION )
          CLOSE(LIDORT_ERRUNIT)
          RETURN
        ENDIF
...
C  ----------------------------
      ...
      IF ( DO_WRITE_RESULTS ) THEN
        ...
        IF ( DO_LINEARIZATION ) THEN
          CALL LIDORT_L_WRITERESULTS ( RUNIT, LOCAL_N_USERAZM )
        ENDIF
        CLOSE(RUNIT)
      ENDIF
