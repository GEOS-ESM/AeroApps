...
C ###############################################################
C #                                                             #
C #  This is LIDORT_AUX.f. This is a collection of utility      #
C #  modules compiled from a variety of sources. This is the    #
C #  only part of the LIDORT package not written by the Author. #
C #  The subroutines in LIDORT_AUX are listed below with their  #
C #  source of origin (order of appearance).                    #
C #                                                             #
C #      lcstring:              E. Mikusch, DLR 1994            #
C #      len_string:            E. Mikusch, DLR 1994            #
C #      lidort_status:         R.J.D. Spurr, RTS, 2005         #
C #      lidort_lapack_error:   R.J.D. Spurr, CFA, 1999         #
C #      lidort_error_trace:    R.J.D. Spurr, CFA, 1999         #
C #      asymtx:                adapted by RJDS 1999 from the   #
C #                             DISORT module ASYMTX            #
...
C  --------------------
      ...
      IF ( STATUS_INPUTCHECK. EQ. LIDORT_WARNING ) THEN
        WRITE(*,*)' WARNING: Bad input from LIDORT input check'
        WRITE(*,*)'  ------ LIDORT has executed with internal defaults'
        WRITE(*,*)'  ------ Check the Logfile for details'        
      ELSE IF ( STATUS_INPUTCHECK. EQ. LIDORT_SERIOUS ) THEN
        WRITE(*,*)' FATAL: Bad input from LIDORT input check'
        WRITE(*,*)'  ------ LIDORT will not execute'
        WRITE(*,*)'  ------ Check the Logfile for details'
      ELSE
c        WRITE(*,*)' SUCCESS with LIDORT input check'
      ENDIF

      IF ( STATUS_INPUTCHECK. NE. LIDORT_SERIOUS ) THEN
       IF ( STATUS_CALCULATION. NE. LIDORT_SUCCESS ) THEN
        WRITE(*,*)' FATAL: LIDORT execution failure'
        WRITE(*,*)'  ------ Check the Logfile for details'
       ELSE
c        WRITE(*,*)' SUCCESS with LIDORT execution'
       ENDIF
      ENDIF
...
C
      ...
      SUBROUTINE LIDORT_LAPACK_ERROR
...
C
      ...
      SUBROUTINE LIDORT_ERROR_TRACE ( MESSAGE, TRACE, STATUS )
...
C  first call - open file
      ...
      IF ( LIDORT_ERROR_INIT ) THEN
        OPEN ( LIDORT_ERRUNIT,
     &           FILE = LIDORT_ERROR_FILENAME, STATUS = 'UNKNOWN' )
      ENDIF

C  write messages

      IF ( STATUS .NE. LIDORT_SUCCESS ) THEN
        IF ( TRACE .EQ. ' ' ) THEN
          WRITE(LIDORT_ERRUNIT,'(/a)')MESSAGE
        ELSE
          WRITE(LIDORT_ERRUNIT,'(/a)')MESSAGE
          WRITE(LIDORT_ERRUNIT,'(a)')TRACE
        ENDIF
      ENDIF

C  turn off error initialization flag

      LIDORT_ERROR_INIT = .FALSE.
...
C  input/output arguments
      ...
      DOUBLE PRECISION  AAD(MAXSTREAMS,MAXSTREAMS), WKD(MAXSTREAMS_2),
     &           EVALD(MAXSTREAMS), EVECD(MAXSTREAMS,MAXSTREAMS)
