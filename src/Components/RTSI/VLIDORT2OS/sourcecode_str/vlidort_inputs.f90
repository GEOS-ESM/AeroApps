...
!     DO_2OS_CORRECTION flag passed out. "Rob Change 10/20/14. Version 2.7E, 2OSCorr implementation"

      CALL VLIDORT_INIT_CONTROL_VARS ( &
        DO_FULLRAD_MODE, DO_SSCORR_NADIR, &
        DO_SSCORR_OUTGOING, DO_FO_CALC, DO_SSCORR_TRUNCATION, &
        DO_SS_EXTERNAL, DO_SSFULL, DO_2OS_CORRECTION, DO_DOUBLE_CONVTEST, &
...
!     DO_2OS_CORRECTION flag passed out. "Rob Change 10/20/14. Version 2.7E, 2OSCorr implementation"
      ...
      CALL VLIDORT_READ_INPUTS ( &
        ...
        DO_DOUBLE_CONVTEST, DO_SOLAR_SOURCES, DO_PLANE_PARALLEL, &
...
!    "Rob Change 10/20/14. Version 2.7E, 2OSCorr implementation"
      ...
      LOGICAL, INTENT (OUT) ::             DO_2OS_CORRECTION
...
!    "Rob Change 10/20/14. Version 2.7E, 2OSCorr implementation"
      ...
      LOGICAL, INTENT (INOUT) ::             DO_2OS_CORRECTION
...
!    "Rob Change 10/20/14. Version 2.7E, 2OSCorr implementation"
      ...
      PAR_STR = 'Do 2OS correction to scalar output?'
      IF (GFINDPAR ( FILUNIT, PREFIX, ERROR, PAR_STR)) &
           READ (FILUNIT,*,ERR=998) DO_2OS_CORRECTION
      CALL FINDPAR_ERROR &
        ( ERROR, PAR_STR, STATUS, NM, MESSAGES, ACTIONS )
...
!    "Rob Change 10/20/14. Version 2.7E, 2OSCorr implementation"
      ...
      LOGICAL , intent(in) ::             DO_2OS_CORRECTION
...
!  FATAL Checks on 2OS correction flag. This will control the output from VLIDORT
!    ** Must have nstokes = 1, upwelling field at TOA, observational geometry, solar sources
!    "Rob Change 10/20/14. Version 2.7E, 2OSCorr implementation"

      IF ( DO_2OS_CORRECTION ) THEN
         IF ( .not. DO_SOLAR_SOURCES ) then
            NM = NM + 1
            MESSAGES(NM) = 'Bad input, for 2OS correction, must have Solar sources'
            ACTIONS(NM)  = 'Re-set DO_SOLAR_SOURCES flag'
            STATUS = VLIDORT_SERIOUS
         ENDIF
         IF ( .not. DO_UPWELLING ) then
            NM = NM + 1
            MESSAGES(NM) = 'Bad input, for 2OS correction, must have Upwelling output'
            ACTIONS(NM)  = 'Re-set DO_UPWELLING flag'
            STATUS = VLIDORT_SERIOUS
         ENDIF
         IF ( .not. DO_OBSERVATION_GEOMETRY ) then
            NM = NM + 1
            MESSAGES(NM) = 'Bad input, for 2OS correction, must have Observational Geometry flag on'
            ACTIONS(NM)  = 'Re-set DO_OBSERVATION_GEOMETRY flag'
            STATUS = VLIDORT_SERIOUS
         ENDIF
         IF ( NSTOKES.gt.1 ) then
            NM = NM + 1
            MESSAGES(NM) = 'Bad input, for 2OS correction, must have a scalar calculation (NSTOKES=1)'
            ACTIONS(NM)  = 'Re-set NSTOKES=1 or turn off DO_2OS_CORRECTION'
            STATUS = VLIDORT_SERIOUS
         ENDIF
         IF ( USER_LEVELS(1) .ne. zero) then
            NM = NM + 1
            MESSAGES(NM) = 'Bad input, for 2OS correction, must have TOA Upwelling output for first user level'
            ACTIONS(NM)  = 'Re-set USER_LEVELS(1)=0.0d0 for DO_2OS_CORRECTION'
            STATUS = VLIDORT_SERIOUS
         ENDIF
       ENDIF

!  WARNING Checks on 2OS correction flag. This will control the output from VLIDORT
!    ** Turn off flux flags, N_USER_LEVELS should be 1, no FO calculation
!    "Rob Change 10/20/14. Version 2.7E, 2OSCorr implementation"

      IF ( DO_2OS_CORRECTION.and.STATUS.ne.VLIDORT_SERIOUS ) THEN
         IF ( DO_MVOUT_ONLY .or. DO_ADDITIONAL_MVOUT) then
            NM = NM + 1
            MESSAGES(NM) = 'Warning, for 2OS correction, You do not need flux output'
            ACTIONS(NM)  = 'DO_MVOUT_ONLY and DO_ADDITIONAL_MVOUT turned off internally'
            DO_MVOUT_ONLY       = .FALSE.
            DO_ADDITIONAL_MVOUT = .FALSE.
            STATUS = VLIDORT_WARNING
         ENDIF
         IF ( DO_FO_CALC ) then
            NM = NM + 1
            MESSAGES(NM) = 'Warning, for 2OS correction, FO calculation not enabled yet!!!!'
            ACTIONS(NM)  = 'DO_FO_CALC turned off internally'
            DO_FO_CALC   = .FALSE.
            STATUS = VLIDORT_WARNING
         ENDIF
         IF ( N_USER_LEVELS .gt. 1) then
            NM = NM + 1
            MESSAGES(NM) = 'Warning, for 2OS correction, You have more than one User level for output'
            ACTIONS(NM)  = 'Consider setting N_USER_LEVELS = 1'
            STATUS = VLIDORT_WARNING
         ENDIF
      ENDIF
...
!  FATAL Check on 2OS correction flag. This will control the output from VLIDORT
!    ** NEED VECTOR Greek-matrix input for 2OS, even though VLIDORT is running in Scalar mode
!    "Rob Change 10/20/14. Version 2.7E, 2OSCorr implementation"

      IF ( DO_2OS_CORRECTION ) THEN
         L = 0
         DO N  = 1, NLAYERS
            DO M = 0, NGREEK_MOMENTS_INPUT
               IF ( GREEKMAT_TOTAL_INPUT(M,N,2).ne.zero ) L = L + 1
            ENDDO
         ENDDO
         IF ( L.eq.0 ) then
            STATUS_SUB = VLIDORT_SERIOUS
            MESSAGE = 'For 2OS correction, MUST prepare polarized GREEKMAT input for SSCORR'
            RETURN
         ENDIF
      ENDIF
