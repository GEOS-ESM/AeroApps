...
! #                                                             #
! #            VLIDORT_2OSCORR_MASTER (master)                  #
! #                                                             #
! ###############################################################


      MODULE vlidort_2OScorr_master_m
      ...
      PRIVATE
      PUBLIC :: VLIDORT_2OSCORR_MASTER

      CONTAINS

      SUBROUTINE VLIDORT_2OSCORR_MASTER ( &
        VLIDORT_FixIn, &
        VLIDORT_ModIn, &
        VLIDORT_Sup,   &
        VLIDORT_Out )
...
!  Local output from "Calculate_Zmatrix"
!  -------------------------------------

   real(fpk) :: Ptc(2,MAXSTREAMS,4,4),Pts(2,MAXSTREAMS,4,4), &
...
!  help
   ...
   logical   :: almost
   logical   :: nextm
   integer   :: m, ndcoeffs, N, n1, L, i, k, o1
   real(fpk) :: xa, xv, xs, muv, mus, phi, W1, W2
...
!  Debug testing February 25th 2015. Validate against VIJAY's latest code.
!    logical, parameter :: do_debug_25feb15 = .true.
   logical, parameter :: do_debug_25feb15 = .false.
...
!  Debug -- This was used to check Coeff indexing.
!   do n = 1, nlayers
!      n1 = nlayers + 1 - n
!      do l = 0, maxmoments
!         write(96,55)n1,l,(coeffs(l,n1,k),k=1,6)
!      enddo
!   enddo
!55 format(2i4,1p6e15.6)
...
!  Get the Dumped data input
   ...
   enddo
   do n = 1, nlayers
!      read(87,*)omegas_1(n),omegas_2(n),opdeps(n),ncoeffs(n)
      read(87,*)omegas_1(n),opdeps(n),ncoeffs(n)
      do l = 0, 2*nstreams-1
         read(87,*)nd,ld,(coeffs(l,n,k),k=1,6)
!         write(95,55)n,l,(coeffs(l,n,k),k=1,6) ! This was used to check Coeff indexing
      enddo
   enddo
   close(87)
...
!   Get the dumped chapman factos
   ...
   open(88,file='debugfiles/fort.88',status='old')
       do n = 1, nlayers
         do k = 1, n
           read(88,*)nd,kd,SunChapman(n,k,1)
         enddo
       enddo
   close(88)
...
!   do n = 1, nlayers
!       do L = 1, nstreams
!          write(66,*)O1_QVTrans(n,L,1),O1_QVMult(n,L,1), O1_QATrans(n,L,1), O1_QAMult(n,L,1)
!      enddo
!   enddo
!   do n = 1, nlayers
!       do L = 1, nstreams
!          write(66,*)O2_AVTrans(n,1),O2_QVMult_d(n,L,1), &
...
!            write(999,*)L,N,M,Ptc,Pts,Prc,Prs !  Check on alternative code
...
!            write(998,*)L,N,M,Ptc,Pts,Prc,Prs !  Check on alternative code

!  debug----------------
!             if ( m.eq.1 .and. n.eq.30 ) then
!             do k = 1, nstreams
!               write(94,*)k,Ptc(1,k,1,1),Ptc(1,k,2,1),Ptc(1,k,1,3),Ptc(1,k,2,3)
!             enddo
!             do k = 1, nstreams
!               write(94,*)k,Prc(2,k,1,1),Prc(2,k,2,1),Ptc(2,k,3,1),Ptc(2,k,4,1)
!             enddo
!             pause 'zmat 1'
!             endif
...
!            if(m.eq.1)write(*,*)m,n,R2c(1),R2cscal
...
!            if(m.eq.1)write(*,*)m,n,R1c(1:2,1,1,1),R1cscal(1:2,1)
...
!  Add fourier component
        ...
        call Add_Fourier_Component &
             (m,nstokes,phi,R2c,R2s,R2cscal,R2(:,L),Icorr(L))
...
!  Apply 2OS correction to VLIDORT Radiance output
!  ===============================================

!   2OS results are MS-only, must be added to FO Radiance results
!   R2 and Icorr must be multiplied by FLUXFAC * PI/mu0 for Radiance output 
...
end subroutine VLIDORT_2OSCORR_MASTER

!  End module

END MODULE vlidort_2OScorr_master_m
