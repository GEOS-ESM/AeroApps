...
! ##########################################################
      ...
      DOUBLE PRECISION, INTENT (IN) :: LC_UPAR_UP_2 ( MAX_USER_STREAMS, &
...
!  Linearized output (Ostensibly output)
      ...
      DOUBLE PRECISION, INTENT (INOUT) :: COLUMNWF_F &
              ...
              DO UM = LOCAL_UM_START, N_USER_STREAMS
                DO O1 = 1, NSTOKES
                  DO Q = 1, NV_PARAMETERS
                    ...
                    COLUMNWF_F(Q,UTA,UM,IB,O1,UPIDX) = &
            ...
            ELSE
              DO O1 = 1, NSTOKES
                DO Q = 1, NV_PARAMETERS
                  ...
                  COLUMNWF_F(Q,UTA,LUM,IB,O1,UPIDX) = &
                       FLUX_MULTIPLIER * L_FINAL_SOURCE
                ENDDO
              ENDDO
            ENDIF
...
!
      ...
      DOUBLE PRECISION, INTENT (IN) :: L_UPAR_DN_1 &
          ( MAX_USER_STREAMS, MAXSTOKES, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, INTENT (IN) :: LC_UPAR_DN_2 ( MAX_USER_STREAMS, &
          MAXSTOKES, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, INTENT (IN) :: L_HMULT_1 &
          ( MAXEVALUES, MAX_USER_STREAMS, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, INTENT (IN) :: L_HMULT_2 &
          ( MAXEVALUES, MAX_USER_STREAMS, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, INTENT (IN) :: LC_EMULT_DN ( MAX_USER_STREAMS, &
              ...
              DO UM = LOCAL_UM_START, N_USER_STREAMS
                DO O1 = 1, NSTOKES
                  DO Q = 1, NV_PARAMETERS
                    ...
                    COLUMNWF_F(Q,UTA,UM,IB,O1,DNIDX) = &
                      FLUX_MULTIPLIER * L_FINAL_SOURCE
                  ENDDO
                ENDDO
              ENDDO
            ELSE
              DO O1 = 1, NSTOKES
                DO Q = 1, NV_PARAMETERS
                  ...
                  COLUMNWF_F(Q,UTA,LUM,IB,O1,DNIDX) = &
                    FLUX_MULTIPLIER * L_FINAL_SOURCE
                ENDDO
              ENDDO
            ENDIF
...
!  Add direct beam if flagged
        ...
        IF ( DO_INCLUDE_DIRECTBEAM.AND..NOT.DO_DBCORRECTION ) THEN
          ...
          ELSE
            DO O1 = 1, NELEMENTS
              FAC = - USER_DIRECT_BEAM(LUM,IB,O1)
              DO Q = 1, NV_PARAMETERS
                L_BEAM = ZERO
                DO K1 = 1, N
                  L_BEAM = L_BEAM + L_DELTAU_VERT(Q,K1) &
                                   * DELTAU_SLANT(N,K1,IB)
                ENDDO
                L_BOA_DBSOURCE(IB,O1,Q) = L_BEAM * FAC
              ENDDO
            ENDDO
          ENDIF
        ENDIF
...
!
      ...
      DOUBLE PRECISION, INTENT (IN) :: L_UPAR_UP_1 &
          ( MAX_USER_STREAMS, MAXSTOKES, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, INTENT (IN) :: LC_UPAR_UP_2 &
          ( MAX_USER_STREAMS, MAXSTOKES, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, INTENT (IN) :: L_UT_HMULT_UU &
          ( MAXEVALUES, MAX_USER_STREAMS, MAX_PARTLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, INTENT (IN) :: L_UT_HMULT_UD &
...
!
      ...
      DOUBLE PRECISION, INTENT (IN) :: L_UPAR_DN_1 &
          ( MAX_USER_STREAMS, MAXSTOKES, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, INTENT (IN) :: LC_UPAR_DN_2 &
          ( MAX_USER_STREAMS, MAXSTOKES, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, INTENT (IN) :: L_UT_HMULT_DU &
          ( MAXEVALUES, MAX_USER_STREAMS, MAX_PARTLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, INTENT (IN) :: L_UT_HMULT_DD &
...
!   &          LC_UPAR_DN_2(UM,O1,N,Q),LC_UT_EMULT_DN(UM,UT,IB,Q)
          ENDDO
         ENDDO
        ENDDO
...
!   &         LC_UPAR_DN_2(IB,O1,N,Q),LC_UT_EMULT_DN(LUM,UT,IB,Q)
         ENDDO
        ENDDO
          ...
          DO Q = 1, NV_PARAMETERS
           DO UTA = 1, N_USER_LEVELS
            DO O1 = 1, NSTOKES
              ...
              ENDDO
              MINT_COLUMNWF(Q,UTA,IBEAM,O1,WDIR) = SMI * HALF
              FLUX_COLUMNWF(Q,UTA,IBEAM,O1,WDIR) = SFX * PI2
            ENDDO
           ENDDO
          ENDDO
...
!  New 15 March 2012
      ...
      DOUBLE PRECISION, INTENT (IN) :: COLUMNWF_F &
          ( MAX_ATMOSWFS, MAX_USER_LEVELS, MAX_USER_VZANGLES, &
            MAX_SZANGLES, MAXSTOKES, MAX_DIRECTIONS )
...
!  Full single scatter calculation is initialized to zero here
...
!     &         fourier_component,ut,v,COLUMNWF_F(Q,UT,I,IBEAM,O1,W)
               ENDDO
              ENDDO
             ENDDO
            ENDDO
        ...
        IF ( .NOT. DO_SSFULL ) THEN
          DO Q = 1, N_TOTALCOLUMN_WFS
           DO IDIR = 1, N_DIRECTIONS
            ...
            DO UT = 1, N_USER_LEVELS
             DO O1 = 1, NSTOKES
               COLUMNWF(Q,UT,IB,O1,W) = &
                        COLUMNWF_F(Q,UT,LUM,IB,O1,W)
             ENDDO
            ENDDO
           ENDDO
          ENDDO
          ...
          DO Q = 1, N_TOTALCOLUMN_WFS
            DO IDIR = 1, N_DIRECTIONS
             ...
             DO UT = 1, N_USER_LEVELS
              DO O1 = 1, NSTOKES
                COLUMNWF(Q,UT,IB,O1,W) = &
                  COLUMNWF(Q,UT,IB,O1,W) + COLUMNWF_SS(Q,UT,IB,O1,W)
              ENDDO
             ENDDO
            ENDDO
          ENDDO
...
!  New 15 March 2012, Introduced DO_SS_EXTERNAL flag
        ...
        IF ( ((DO_SSFULL.OR.DO_DBCORRECTION).OR.DO_SS_EXTERNAL) &
          ...
          DO Q = 1, N_TOTALCOLUMN_WFS
            DO UT = 1, N_USER_LEVELS
              DO O1 = 1, NSTOKES
                COLUMNWF(Q,UT,IB,O1,UPIDX) = &
                COLUMNWF(Q,UT,IB,O1,UPIDX) + COLUMNWF_DB(Q,UT,IB,O1)
              ENDDO
            ENDDO
          ENDDO
        ENDIF
...
!       if(q.eq.3.and.ut.ne.5)write(*,*)
!     &         fourier_component,ut,ib,COLUMNWF_F(Q,UT,LUM,IB,O1,W)
               ENDDO
             ENDDO
           ENDDO
         ENDDO
