...
!  Derived Solar-beam Transmittance at BOA (Diagnostic output). Rob Fix 10/24/14
...
!  Scaled SSAs and phase function moments. Diagnostic output.

      DOUBLE PRECISION :: OMEGA_TOTAL    ( MAXLAYERS )
      DOUBLE PRECISION :: GREEKMAT_TOTAL &
          ( 0:MAXMOMENTS, MAXLAYERS, MAXSTOKES_SQ )

!  Linearized Input optical depths after delta-M scaling. Diagnostic output.
...
!  Linearized truncation factor (Diagnostic output).
...
!  Input optical properties after delta-M scaling. Deltau_vert is Diagnostic output
...
!  Linearized input optical properties after delta-M scaling. L_Deltau_vert is Diagnostic output
...
!  ---------------
      ...
      INTEGER ::          UM, UA, TESTCONV, L, LUM, LUA, NWF, N, O1
...
!  Local Nstokes values for the SSCORR and MISCSETUPS routines
!       "Rob Change 10/20/14. Version 2.7E, 2OSCorr implementation"

      INTEGER ::          LOCAL_NSTOKES, LOCAL_NSTOKES_SQ
...
!       "Rob Change 10/20/14. Version 2.7E, 2OSCorr implementation"
      ...
      LOCAL_NSTOKES_SQ = LOCAL_NSTOKES * LOCAL_NSTOKES
...
!  New 28 March 2014. BLACKBODY Linearization
!  ------------------------------------------
...
!    "Rob Change 10/20/14. Version 2.7E, 2OSCorr implementation"
      ...
      IF ( STATUS_SUB .EQ. VLIDORT_SERIOUS ) THEN
        ...
        VLIDORT_Out%Status%TS_ACTIONS(NCHECKMESSAGES) = ' Serious error from VLIDORT_DERIVE_INPUT, called in VLIDORT_LCS_MASTERS'
        RETURN
      ENDIF
...
!  Local variables "LOCAL_NSTOKES" and "LOCAL_NSTOKES_SQ" appear instead
!    of "NSTOKES" and "NSTOKES_SQ" in MISCSETUPS and LAC_MISCSETUPS calls
!    "Rob Change 10/20/14. Version 2.7E, 2OSCorr implementation"
...
!  With linearization.
      ...
      IF ( DO_ATMOS_LINEARIZATION ) THEN
        ...
        CALL VLIDORT_LAC_MISCSETUPS ( &
          ...
          DELTAU_VERT_INPUT, GREEKMAT_TOTAL_INPUT, &
          NMOMENTS, LOCAL_NSTOKES_SQ, NBEAMS, &
...
!  Use LOCAL_NSTOKES in the arguments to
!       SSCORR_NADIR,    LC_SSCORR_NADIR,,
!       SSCORR_OUTGOING, LC_SSCORR_OUTGOING, 
!       DBCORRECTION,    LAC_DBCORRECTION, LS_DBCORRECTION, LSSL_DBCORRECTION 
!  Also: VFO_LCS Master + local copy of FO results to SS output
!       "Rob Change 10/20/14. Version 2.7E, 2OSCorr implementation"
          ...
          IF ( DO_SSCORR_NADIR  .AND. .NOT.DO_FO_CALC ) THEN
            ...
            IF ( DO_COLUMN_LINEARIZATION ) THEN
              CALL VLIDORT_LC_SSCORR_NADIR ( &
                ...
                LOCAL_NSTOKES, NLAYERS, NGREEK_MOMENTS_INPUT, &
                N_USER_RELAZMS, USER_RELAZMS, &
...
!
            IF ( DO_SURFACE_LEAVING .and. DO_SLEAVE_WFS ) then
              CALL VLIDORT_LSSL_DBCORRECTION ( &
                ...
                DO_OBSERVATION_GEOMETRY,                                 &
                LOCAL_NSTOKES, NLAYERS, NBEAMS, N_SLEAVE_WFS, N_SURFACE_WFS,   &
...
!  New first-order code added 02 Jul 2013
          ...
          IF ( DO_FO_CALC ) THEN
            ...
            IF ( FAIL ) THEN
              TRACE_2 = 'VFO_LCS_MASTER_INTERFACE failed, VLIDORT_LCS_MASTER'
              STATUS_SUB  = VLIDORT_SERIOUS
              RETURN
            ENDIF
            ...
            COLUMNWF_SS(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,&
                        1:NBEAMS,1:LOCAL_NSTOKES,:) = &
              FO_COLUMNWF_SS(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,&
                        1:NBEAMS,1:LOCAL_NSTOKES,:)
            COLUMNWF_DB(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,&
                        1:NBEAMS,1:LOCAL_NSTOKES) = &
              FO_COLUMNWF_DB(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,&
                        1:NBEAMS,1:LOCAL_NSTOKES)
...
!        write(*,*)' ..calculating fourier component',FOURIER_COMPONENT
        ...
        CALL VLIDORT_LCS_FOURIER ( FOURIER_COMPONENT, &
          ...
          DO_SIMULATION_ONLY, DO_ATMOS_LINEARIZATION, DO_COLUMN_LINEARIZATION, DO_SURFACE_LINEARIZATION,                  & !Input
          DO_ATMOS_LBBF, DO_SURFACE_LBBF, NSTOKES_SQ, N_TOTALCOLUMN_WFS, LAYER_VARY_FLAG, LAYER_VARY_NUMBER,              & !Input
          EMISSIVITY, USER_EMISSIVITY, LS_USER_EMISSIVITY, LS_EMISSIVITY,                                                 & !Input
          Misc, Therm, Mult, LAC_Misc, L_Therm, LC_Mult,                                                                  & !Input
          ...
          MEAN_STOKES, FLUX_STOKES, MEAN_DIRECT, FLUX_DIRECT, MINT_COLUMNWF, FLUX_COLUMNWF,                               & !InOut
          MINT_COLUMNWF_DIRECT, FLUX_COLUMNWF_DIRECT, MINT_SURFACEWF, FLUX_SURFACEWF,                                     & !InOut
...
!  Added 16 October 2014, for diagnostic use only
...
!  Linearized Scaled optical properties

      NWF = N_TOTALCOLUMN_WFS
      VLIDORT_LinOut%Diag%TS_L_DELTAU_VERT  (1:NWF,1:NLAYERS) = L_DELTAU_VERT  (1:NWF,1:NLAYERS)
      VLIDORT_LinOut%Diag%TS_L_OMEGA_TOTAL  (1:NWF,1:NLAYERS) = L_OMEGA_TOTAL  (1:NWF,1:NLAYERS)
      VLIDORT_LinOut%Diag%TS_L_TRUNC_FACTOR (1:NWF,1:NLAYERS) = L_TRUNC_FACTOR (1:NWF,1:NLAYERS)
      VLIDORT_LinOut%Diag%TS_L_GREEKMAT_TOTAL(1:NWF,0:NMOMENTS,1:NLAYERS,1:16) = &
                L_GREEKMAT_TOTAL(1:NWF,0:NMOMENTS,1:NLAYERS,1:16)
...
!  Truncation factors (and linearized). Replaced with Diagnostic output.
!      VLIDORT_Sup%SS%TS_TRUNCFAC_SS           = zero
!      VLIDORT_LinSup%SS%Col%TS_LC_TRUNCFAC_SS = zero
!      IF ( VLIDORT_ModIn%MBool%TS_DO_2OS_CORRECTION ) THEN
!         VLIDORT_Sup%SS%TS_TRUNCFAC_SS(1:NLAYERS)           = TRUNC_FACTOR(1:NLAYERS)
!         VLIDORT_LinSup%SS%Col%TS_LC_TRUNCFAC_SS(:,1:NLAYERS) = L_TRUNC_FACTOR(:,1:NLAYERS)
!      ENDIF
...
!
      ...
      SUBROUTINE VLIDORT_LCS_FOURIER ( FOURIER_COMPONENT, &
        DO_OBSERVATION_GEOMETRY, DO_SSCORR_NADIR, DO_SSFULL, DO_SOLAR_SOURCES, DO_PLANE_PARALLEL,                       & !Input
        DO_REFRACTIVE_GEOMETRY, DO_SOLUTION_SAVING, DO_UPWELLING, DO_DNWELLING,                                         & !Input
        DO_QUAD_OUTPUT, DO_ADDITIONAL_MVOUT, DO_MVOUT_ONLY, DO_TOA_CONTRIBS, DO_DIRECT_BEAM, DO_CLASSICAL_SOLUTION,     & !Input
...
!  ##############
      ...
      IF ( DO_DEBUG_WRITE ) THEN
        write(*,*)
        write(*,*) 'FOURIER_COMPONENT = ' ,FOURIER_COMPONENT
      END IF
...
!                        'Called in VLIDORT_LCS_FOURIER_MASTER, Fourier # '//CF
!            STATUS = VLIDORT_SERIOUS
!            RETURN
!         ENDIF
...
!          IF ( ( DO_ATMOS_LBBF .or. DO_SURFACE_LBBF ) .and. &
!                      IBEAM.eq.1 .and. Fourier_component.eq.0 )  THEN
!           if ( n_partlayers .gt. 0 ) then
!             call vlidort_lbbf_jacobians_wpartials &
!               ( DO_ATMOS_LBBF, DO_SURFACE_LBBF, DO_THERMAL_TRANSONLY,      & ! Input
!                 DO_UPWELLING, DO_DNWELLING, DO_SOLAR_SOURCES,              & ! Input
