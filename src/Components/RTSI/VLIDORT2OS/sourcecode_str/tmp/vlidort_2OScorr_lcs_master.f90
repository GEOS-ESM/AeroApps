...
! #                                                             #
! #            VLIDORT_2OSCORR_LCS_MASTER (master)              #
! #                                                             #
! ###############################################################


      MODULE vlidort_2OScorr_lcs_master_m

      USE VLIDORT_PARS
      USE vlidort_2OScorr_utilities
      USE vlidort_2OScorr_routines
      USE vlidort_2OScorr_la_routines
      USE vlidort_2OScorr_lcs_routines

      PRIVATE
      PUBLIC :: VLIDORT_2OSCORR_LCS_MASTER

      CONTAINS

      SUBROUTINE VLIDORT_2OSCORR_LCS_MASTER ( &
        VLIDORT_FixIn, &
        VLIDORT_ModIn, &
        VLIDORT_Sup, &
        VLIDORT_Out, &
...
!  R2 reflectance and Icorr. The Main results!
   ...
   real(fpk) :: LC_R2   (4,MAX_ATMOSWFS,  MaxGeoms)
   real(fpk) :: LS_R2   (4,MAX_SURFACEWFS,MaxGeoms)

   real(fpk) :: LC_Icorr(MAX_ATMOSWFS,  MaxGeoms)
...
!  Average secant output
   ...
   real(fpk) :: LC_avg_secants(MAXLAYERS,MAX_ATMOSWFS,MaxGeoms)

!  Local output from subroutine "Calculate_Multipliers"
!  ----------------------------------------------------
...
!  Linearized First-order Transmittances and multipliers
   ...
   real(fpk)  :: LC_O1_QVTrans(MAXLAYERS,MAXSTREAMS,MAX_ATMOSWFS,MaxGeoms)
   real(fpk)  :: LC_O1_QATrans(MAXLAYERS,MAXSTREAMS,MAX_ATMOSWFS,MaxGeoms)

   real(fpk)  :: LC_O1_QVMult(MAXLAYERS,MAXSTREAMS,MAX_ATMOSWFS,MaxGeoms)
   real(fpk)  :: LC_O1_QAMult(MAXLAYERS,MAXSTREAMS,MAX_ATMOSWFS,MaxGeoms)

!  Linearized Second-order Transmittances and multipliers

   real(fpk)  :: LC_O2_AVTrans(MAXLAYERS,MAX_ATMOSWFS,MaxGeoms)
   real(fpk)  :: LC_O2_AVMult (MAXLAYERS,MAX_ATMOSWFS,MaxGeoms)

   real(fpk)  :: LC_O2_QVMult_d(MAXLAYERS,MAXSTREAMS,MAX_ATMOSWFS,MaxGeoms)
   real(fpk)  :: LC_O2_QAMult_d(MAXLAYERS,MAXSTREAMS,MAX_ATMOSWFS,MaxGeoms)
   real(fpk)  :: LC_O2_QAVMult_dd(MAXLAYERS,MAXSTREAMS,MAX_ATMOSWFS,MaxGeoms)
   real(fpk)  :: LC_O2_QAVMult_du(MAXLAYERS,MAXSTREAMS,MAX_ATMOSWFS,MaxGeoms)
...
!  Linearized source phase matrices
   ...
   real(fpk)  :: LC_Ptc(2,MAXSTREAMS,4,4,MAX_ATMOSWFS)
   real(fpk)  :: LC_Pts(2,MAXSTREAMS,4,4,MAX_ATMOSWFS)
   real(fpk)  :: LC_Prc(2,MAXSTREAMS,4,4,MAX_ATMOSWFS)
   real(fpk)  :: LC_Prs(2,MAXSTREAMS,4,4,MAX_ATMOSWFS)
...
!  First-order Fourier component variables
   ...
   real(fpk)  :: LC_R1cscal(2,MAXSTREAMS,    MAX_ATMOSWFS)
   real(fpk)  :: LC_R1c    (2,MAXSTREAMS,4,4,MAX_ATMOSWFS)
   real(fpk)  :: LC_R1s    (2,MAXSTREAMS,4,4,MAX_ATMOSWFS)
...
!  Second-order Fourier component variables
   ...
   real(fpk)  :: R2cscal, LC_R2cscal  ( MAX_ATMOSWFS ), LS_R2cscal  ( MAX_SURFACEWFS )
   real(fpk)  :: R2c(4),  LC_R2c   ( 4, MAX_ATMOSWFS ), LS_R2c   ( 4, MAX_SURFACEWFS )
   real(fpk)  :: R2s(4),  LC_R2s   ( 4, MAX_ATMOSWFS ), LS_R2s   ( 4, MAX_SURFACEWFS )
...
!  help
   ...
   logical   :: almost
   logical   :: nextm
   integer   :: m, ndcoeffs, N, N1, L, i, k, p, o1
   real(fpk) :: xa, xv, xs, muv, mus, phi, W1, W2, LC_xa(MAX_ATMOSWFS)
...
!   logical, parameter :: do_debug_23oct14 = .true.
   ...
   integer            :: nd, ld, kd
   real(fpk)          :: dtr, dn, z, aa, ray1, ray2, col0
...
!  Linearization control
   ...
   do_LC_Jacobians = VLIDORT_LinModIn%MCont%TS_DO_COLUMN_LINEARIZATION
   npars  = VLIDORT_LinFixIn%Cont%TS_N_TOTALCOLUMN_WFS
...
!  Linearized bulk optical properties. Single normalized quantities for 2OS.
   ...
   if ( do_LC_Jacobians ) then
     if ( do_deltam_scaling ) then
       do n = 1, nlayers
         n1 = nlayers + 1 - n
         do p = 1, npars
            LA_opdeps(n1,p)   = opdeps(n1) * VLIDORT_LinOut%Diag%TS_L_DELTAU_VERT(p,n)
            LA_omegas(n1,p)   = omegas(n1) * VLIDORT_LinOut%Diag%TS_L_OMEGA_TOTAL(p,n)
            LA_truncfac(n1,p) = VLIDORT_LinOut%Diag%TS_L_TRUNC_FACTOR(p,n) ! Already single normalized
            LA_omegas_1(n1,p) = LA_omegas(n1,p) 
            LA_omegas_2(n1,p) = LA_omegas(n1,p) * truncfac(n1) + omegas(n1) * LA_truncfac(n1,p)  ! Not needed, probably.
         enddo
       enddo
     else
       do n = 1, nlayers
         n1 = nlayers + 1 - n
         do p = 1, npars
            LA_opdeps(n1,p)   = opdeps(n1) * VLIDORT_LinFixIn%Optical%TS_L_DELTAU_VERT_INPUT(p,n)
            LA_omegas(n1,p)   = omegas(n1) * VLIDORT_LinFixIn%Optical%TS_L_OMEGA_TOTAL_INPUT(p,n)
            LA_omegas_1(n1,p) = LA_omegas(n1,p) 
         enddo
       enddo
     endif
   ...
   if ( do_LC_Jacobians ) then
     if ( do_deltam_scaling ) then
       do n = 1, nlayers
         ...
         do p = 1, npars
           do m = 0, MAXMOMENTS
             LA_coeffs(m,n1,1,p) = coeffs(m,n1,1) * VLIDORT_LinOut%Diag%TS_L_GREEKMAT_TOTAL(p,m,n,1)
             LA_coeffs(m,n1,5,p) = coeffs(m,n1,5) * VLIDORT_LinOut%Diag%TS_L_GREEKMAT_TOTAL(p,m,n,2)
             LA_coeffs(m,n1,2,p) = coeffs(m,n1,2) * VLIDORT_LinOut%diag%TS_L_GREEKMAT_TOTAL(p,m,n,6)
             LA_coeffs(m,n1,3,p) = coeffs(m,n1,3) * VLIDORT_LinOut%Diag%TS_L_GREEKMAT_TOTAL(p,m,n,11)
             LA_coeffs(m,n1,6,p) = coeffs(m,n1,6) * VLIDORT_LinOut%Diag%TS_L_GREEKMAT_TOTAL(p,m,n,15)
             LA_coeffs(m,n1,4,p) = coeffs(m,n1,4) * VLIDORT_LinOut%Diag%TS_L_GREEKMAT_TOTAL(p,m,n,16)
           enddo
         enddo
       enddo
     else
       do n = 1, nlayers
         ...
         do p = 1, npars
           do m = 0, MAXMOMENTS
             LA_coeffs(m,n1,1,p) = coeffs(m,n1,1) * VLIDORT_LinFixIn%Optical%TS_L_GREEKMAT_TOTAL_INPUT(p,m,n,1)
             LA_coeffs(m,n1,5,p) = coeffs(m,n1,5) * VLIDORT_LinFixIn%Optical%TS_L_GREEKMAT_TOTAL_INPUT(p,m,n,2)
             LA_coeffs(m,n1,2,p) = coeffs(m,n1,2) * VLIDORT_LinFixIn%Optical%TS_L_GREEKMAT_TOTAL_INPUT(p,m,n,6)
             LA_coeffs(m,n1,3,p) = coeffs(m,n1,3) * VLIDORT_LinFixIn%Optical%TS_L_GREEKMAT_TOTAL_INPUT(p,m,n,11)
             LA_coeffs(m,n1,6,p) = coeffs(m,n1,6) * VLIDORT_LinFixIn%Optical%TS_L_GREEKMAT_TOTAL_INPUT(p,m,n,15)
             LA_coeffs(m,n1,4,p) = coeffs(m,n1,4) * VLIDORT_LinFixIn%Optical%TS_L_GREEKMAT_TOTAL_INPUT(p,m,n,16)
           enddo
         enddo
       enddo
     endif
...
!  debug
!     do n =  20, 26
!        write(*,*)avg_secants(n,1),LC_avg_secants(n,1,1)
!     enddo
!     pause'111'
...
!  --------------------------------------------------------------
   ...
   if ( do_LC_Jacobians ) then
      call Calculate_Multipliers_LCPlus &
        ( MaxGeoms, nstreams, nlayers, ngeoms, npars, do_LC_Jacobians, & ! Inputs
          qstreams, avg_secants, geoms, opdeps, omegas_1,              & ! Inputs
          LC_avg_secants, LA_opdeps, LA_omegas_1,                      & ! Inputs
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
!   do n = 1, nlayers
!      write(67,*)n,O2_QAVMult_du(n,1:nstreams,1),O2_QAVMult_dd(n,1:nstreams,1)
!   enddo
!   do n = 1, nlayers
!      write(68,*)n,LC_O2_QAVMult_du(n,1:nstreams,1,1),LC_O2_QAVMult_dd(n,1:nstreams,1,1)
!   enddo

!  Debug, 
!   do n = 1, nlayers
!       write(91,'(a,i3)')'FO, Layer # ',n
!       do L = 1, nstreams
!          write(91,*)O1_QVTrans(n,L,1),O1_QVMult(n,L,1), O1_QATrans(n,L,1), O1_QAMult(n,L,1)
!      enddo
!   enddo!  FD debug 04 february 2013
!   do n = 1, nlayers
!      write(67,*)n,O2_QAVMult_du(n,1:nstreams,1),O2_QAVMult_dd(n,1:nstreams,1)
!   enddo
!   do n = 1, nlayers
...
!  Debug, 25 September 2012, and again............
!   do n = 1, nlayers
!       write(91,'(a,i3)')'FO, Layer # ',n
!       do L = 1, nstreams
!          write(91,*)O1_QVTrans(n,L,1),O1_QVMult(n,L,1), O1_QATrans(n,L,1), O1_QAMult(n,L,1)
...
!   enddo
!   do n = 1, nlayers
!       write(92,*)'SO, Layer # ',n,O2_AVTrans(n,1)
!       do L = 1, nstreams
!          write(92,*)O2_QVMult_d(n,L,1), O2_QAMult_d(n,L,1),O2_QAVMult_du(n,L,1),O2_QAVMult_dd(n,L,1)
!      enddo
!   enddo
!   do n = 1, nlayers
!       write(92,*)'SO, Layer # ',n,O2_AVTrans(n,1)
...
!  initialise the Fourier components for R1 and R2 (first/second order). Very Important
         ...
         LC_R2c = zero ; LC_R2s = zero ; LC_R2cscal = zero
         LS_R2c = zero ; LS_R2s = zero ; LS_R2cscal = zero
         R1c    = zero ; R1s    = zero ; R1cscal    = zero
         LC_R1c = zero ; LC_R1s = zero ; LC_R1cscal = zero
...
!  Legendre setups for the thread-safe alternative
...
!  Average secants
            ...
            xa = avg_secants(n,L)
            if ( do_LC_Jacobians ) LC_xa(1:npars) = LC_avg_secants(n,1:npars,L)
...
!                  ( MaxGeoms,m,N,L,do_LC_Jacobians,            & ! Inputs
!                    nstreams,nstokes,ndcoeffs,npars,           & ! Inputs
!                    qstreams, geoms, coeffs(0:MAXMOMENTS,N,:), & ! Inputs
!                    LA_coeffs(0:MAXMOMENTS,N,:,:),             & ! Inputs
!                    Ptc,Pts,Prc,Prs,LC_Ptc,LC_Pts,LC_Prc,LC_Prs )
!            else
!               call Calculate_Zmatrix &
!                  ( MaxGeoms,m,N,L,nstokes,nstreams,ndcoeffs,   & !Inputs
!                    qstreams, geoms, coeffs(0:MAXMOMENTS,N,:),  & !Inputs
!                    Ptc,Pts,Prc,Prs)
!            endif
...
!         ALTERNATIVE Thread-Safe Treatment
            ...
            if ( do_LC_Jacobians ) then
               call Calculate_Zmatrix_LAPlus_ALT &
                ( do_LC_Jacobians, m, nstokes, nstreams, Tns1, Tns2,       & ! Inputs
                  ndcoeffs, npars, Trl, Tr2Lp1, Tsql4, Tsqlm, Tbinfac,     & ! Inputs
...
!  Second order calculation
            ...
            if ( do_LC_Jacobians .or. do_LS_Jacobians ) then
               call Calculate_SecondOrder_LCSPlus &
                ( do_LC_Jacobians,do_LS_Jacobians,m,n,nstreams,nstokes,npars,nspars,  & ! Input Control
                  qweights,omegas_1(n),xv,xa,   LA_omegas_1(n,:), LC_xa,              & ! Input Basics
                  O2_AVTrans(n,L),              LC_O2_AVTrans(n,:,L),                 & ! Input R2 Multiplier
                  O2_QVMult_d(n,:,L),           LC_O2_QVMult_d(n,:,:,L),              & ! Input R2 Multipliers 
                  O2_QAMult_d(n,:,L),           LC_O2_QAMult_d(n,:,:,L),              & ! Input R2 Multipliers
                  O2_QAVMult_du(n,:,L),         LC_O2_QAVMult_du(n,:,:,L),            & ! Input R2 Multipliers 
                  O2_QAVMult_dd(n,:,L),         LC_O2_QAVMult_dd(n,:,:,L),            & ! Input R2 Multipliers 
...
!            if(m.eq.0)write(*,*)m,n,R2c(1),R2cscal

!  Get the first order of scattering (using previous layer)

            if ( do_LC_Jacobians .or. do_LS_Jacobians ) then
               call Calculate_FirstOrder_LCSPlus &
                ( do_LC_Jacobians, do_LS_Jacobians,m,nstreams,npars,nspars, & ! Input Control
                  Prc,Prs,            LC_Prc,LC_Prs,               & ! Scattering input
                  O1_QVTrans(n,:,L),  LC_O1_QVTrans(n,:,:,L),      & ! First-order multiplier input
                  O1_QVMult(n,:,L),   LC_O1_QVMult(n,:,:,L),       & ! First-order multiplier input
                  O1_QATrans(n,:,L),  LC_O1_QATrans(n,:,:,L),      & ! First-order multiplier input
                  O1_QAMult(n,:,L),   LC_O1_QAMult(n,:,:,L),       & ! First-order multiplier input
...
!  Add fourier component
         ...
         if ( do_LC_Jacobians.or.do_LS_Jacobians ) then
            call Add_Fourier_Component_LCSPlus &
             ( m, nstokes, phi, do_lc_jacobians, do_ls_jacobians, npars, nspars,       & ! Inputs
               R2c,R2s,R2cscal,LC_R2c,LC_R2s,LC_R2cscal,LS_R2c,LS_R2s,LS_R2cscal,      & ! Inputs
               R2(:,L), Icorr(L), LC_R2(:,:,L), LC_Icorr(:,L), LS_R2(:,:,L), LS_Icorr(:,L) )
         else
...
!  End answers (multiply by SZA cosine)
      ...
      if ( do_LC_Jacobians ) then
         do p = 1, npars
            LC_R2(1:nstokes,p,L) = LC_R2(1:nstokes,p,L) * mus
            LC_Icorr(p,L)        = LC_Icorr(p,L)        * mus
         enddo
      endif
      if ( do_LS_Jacobians ) then
...
!  Copy to vlidort output
   ...
   do L = 1, ngeoms
      ...
      if ( do_LC_Jacobians ) then
         do p = 1, npars
            VLIDORT_LinOut%Col%TS_2OSCORR_LC_R2(p,L,1:nstokes) = LC_R2(1:nstokes,p,L)
            VLIDORT_LinOut%Col%TS_2OSCORR_LC_ICORR(p,L)        = LC_Icorr(L,p)
         enddo
      endif
      if ( do_LS_Jacobians ) then
         do p = 1, nspars
...
!  Repeat process for Column Jacobians

      if ( do_LC_Jacobians ) then
         do p = 1, npars
            VLIDORT_LinOut%Col%TS_COLUMNWF(p,1,L,1,upidx) = VLIDORT_LinOut%Col%TS_COLUMNWF(p,1,L,1,upidx)&
                                                          + VLIDORT_LinOut%Col%TS_2OSCORR_LC_ICORR(p,L) * Conversion
            do o1 = 2, nstokes
               VLIDORT_LinOut%Col%TS_COLUMNWF(p,1,L,o1,upidx) = VLIDORT_LinSup%SS%Col%TS_COLUMNWF_SS(p,1,L,O1,upidx) &
                                                              + VLIDORT_LinSup%SS%Col%TS_COLUMNWF_DB(p,1,L,O1)       &
                                                              + VLIDORT_LinOut%Col%TS_2OSCORR_LC_R2(p,L,O1) * Conversion
            enddo
         enddo
      endif
...
end subroutine VLIDORT_2OSCORR_LCS_MASTER

!  End module

END MODULE vlidort_2OScorr_lcs_master_m
