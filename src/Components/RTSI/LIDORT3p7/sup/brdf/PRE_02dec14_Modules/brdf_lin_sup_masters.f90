...
!   Flags for the Direct-bounce term, replaces former "EXACT" variables 
!   Exact flag (!@@) and Exact only flag --> no Fourier term calculations
!      LOGICAL   :: DO_EXACT
!      LOGICAL   :: DO_EXACTONLY
      LOGICAL   :: DO_DIRECTBOUNCE_ONLY
      LOGICAL   :: DO_DIRECTBOUNCE_TRUNCATED
...
!      DO_EXACTONLY        = .FALSE.
      DO_DIRECTBOUNCE_ONLY      = .false.
      DO_DIRECTBOUNCE_TRUNCATED = .false.
...
!  **********************************************************************
!  Rob Fix 9/25/14. New Variables and more explicit character strings
...
!   Normally this would be .FALSE., if set, then no multiple-scatter BRDFs will be done
        ...
        IF (GFINDPAR ( FILUNIT, PREFIX, ERROR, PAR_STR)) THEN
           READ (FILUNIT,*,ERR=998)DO_DIRECTBOUNCE_ONLY
        ENDIF
        CALL FINDPAR_ERROR ( ERROR, PAR_STR, STATUS, NM, MESSAGES, ACTIONS )

!  New flag for turning on the Truncated (non-exact) flag for the direct-bounce BRDF
!     This should only be turned on for debug purposes. Default for most cases = .FALSE.
!     If this has been turned on, the DO_DIRECTBOUNCE_ONLY shoudl be turned off.

        PAR_STR = 'Do truncated direct-bounce BRDF?'
        IF (GFINDPAR ( FILUNIT, PREFIX, ERROR, PAR_STR)) THEN
           READ (FILUNIT,*,ERR=998)DO_DIRECTBOUNCE_TRUNCATED
        ENDIF
        CALL FINDPAR_ERROR ( ERROR, PAR_STR, STATUS, NM, MESSAGES, ACTIONS )
...
!      BRDF_Sup_In%BS_DO_EXACTONLY           = DO_EXACTONLY
      BRDF_Sup_In%BS_DO_DIRECTBOUNCE_ONLY      = DO_DIRECTBOUNCE_ONLY
      BRDF_Sup_In%BS_DO_DIRECTBOUNCE_TRUNCATED = DO_DIRECTBOUNCE_TRUNCATED
...
!  help
      ...
      LOGICAL    :: ADD_FOURIER, LOCAL_MSR, DO_EXACT, DO_EXACTONLY
...
!  Rob Fix 9/25/14. Two variables replaced
      ...
      DO_DIRECTBOUNCE_ONLY      = BRDF_Sup_In%BS_DO_DIRECTBOUNCE_ONLY
      DO_DIRECTBOUNCE_TRUNCATED = BRDF_Sup_In%BS_DO_DIRECTBOUNCE_TRUNCATED

!  Rob Fix 9/25/14. Set local flags

      DO_EXACTONLY = DO_DIRECTBOUNCE_ONLY .and..not.DO_DIRECTBOUNCE_TRUNCATED
      DO_EXACT     = .not.DO_DIRECTBOUNCE_TRUNCATED
        ...
        IF ( WHICH_BRDF(K) .NE. NewCMGLINT_IDX ) THEN
          IF ( .not.DO_KPARAMS_DERIVS(K) ) THEN
           ...
           ( DO_LOCAL_WSA, DO_LOCAL_BSA,                          & ! New line, Version 3.7
             ...
             WHICH_BRDF(K), DO_EXACT, DO_EXACTONLY,               & ! Inputs !@@
             LOCAL_MSR, DO_MSRCORR_EXACTONLY,                     & ! Inputs
...
!   !@@ Observational Geometry, Optionalities 12/31/12
        ...
        IF ( DO_USER_OBSGEOMS ) THEN
          DO IB = 1, NBEAMS
            EXACTDB_BRDFUNC(LUM,LUA,IB) = EXACTDB_BRDFUNC(LUM,LUA,IB) &
...
!  Linearization w.r.t Kernel Factor
        ...
        IF ( DO_KERNEL_FACTOR_WFS(K) ) THEN
          ...
          IF ( DO_USER_OBSGEOMS ) THEN 
            DO IB = 1, NBEAMS
              LS_EXACTDB_BRDFUNC(Q,LUM,LUA,IB) = DBKERNEL_BRDFUNC(LUM,LUA,IB)
            ENDDO
          ELSE
            DO IA = 1, N_USER_RELAZMS
              DO IB = 1, NBEAMS
...
!  Linearization w.r.t Kernel parameters
        ...
        DO P = 1, LOCAL_BRDF_NPARS
          IF ( LOCAL_BRDF_DERIVS(P) ) THEN
            ...
            IF ( DO_USER_OBSGEOMS ) THEN 
              DO IB = 1, NBEAMS
                LS_EXACTDB_BRDFUNC(Q,LUM,LUA,IB) = &
...
!  Observational  Geometries
         ...
         IF ( DO_USER_OBSGEOMS ) THEN
            DO IB = 1, NBEAMS
               T0 = EXACTDB_BRDFUNC(LUM,LUA,IB) ; T00 = SCALING * T0
               EXACTDB_BRDFUNC(LUM,LUA,IB) = T00
               IF ( DO_WSAorBSA_Jacobian ) THEN
                  LS_EXACTDB_BRDFUNC(WBSA,LUM,LUA,IB) = SCALING_0 * T0
               ELSE IF ( N_SURFACE_WFS .gt. 0) then
                  DO W = 1, N_SURFACE_WFS
                     T1 = LS_EXACTDB_BRDFUNC(W,LUM,LUA,IB)
                     T2 = T00 * D_TOTAL_ALBEDO_CALC(W)
                     LS_EXACTDB_BRDFUNC(W,LUM,LUA,IB) = SCALING * T1 - SCALING_0 * T2
                  ENDDO
               ENDIF
            ENDDO
         ENDIF
...
!  Lattice Geometries
         ...
         IF (.not. DO_USER_OBSGEOMS ) THEN
            DO IA = 1, N_USER_RELAZMS
               DO IB = 1, NBEAMS
                  DO UM = 1, N_USER_STREAMS
                    T0 = EXACTDB_BRDFUNC(UM,IA,IB) ; T00 = SCALING * T0
                    EXACTDB_BRDFUNC(UM,IA,IB) = T00
                    IF ( DO_WSAorBSA_Jacobian ) THEN
                      LS_EXACTDB_BRDFUNC(WBSA,UM,IA,IB) = SCALING_0 * T0
                    ELSE IF ( N_SURFACE_WFS .gt. 0) then
                      DO W = 1, N_SURFACE_WFS
                        T1 = LS_EXACTDB_BRDFUNC(W,UM,IA,IB)
                        T2 = T00 * D_TOTAL_ALBEDO_CALC(W)
                        LS_EXACTDB_BRDFUNC(W,UM,IA,IB) = SCALING * T1 - SCALING_0 * T2
                      ENDDO
                    ENDIF
                  ENDDO
               ENDDO
