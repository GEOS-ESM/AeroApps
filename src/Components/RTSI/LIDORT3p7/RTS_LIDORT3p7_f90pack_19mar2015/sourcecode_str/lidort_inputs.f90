...
!  module, dimensions and numbers
      ...
      USE LIDORT_pars, only : fpk, MAX_USER_STREAMS, MAX_USER_RELAZMS, MAX_USER_OBSGEOMS, &
...
!  Order of Taylor series (including terms up to EPS^n). Introduced 10/10/13 for Version 3.7
      ...
      INTEGER, intent(out) ::   TAYLOR_ORDER
...
!   New variable, 25 OCtober 2012, for Observational Geometry input
      ...
      REAL(fpk), intent(out) :: USER_OBSGEOMS (MAX_USER_OBSGEOMS,3)
...
SUBROUTINE LIDORT_READ_INPUTS &
      ( DO_SOLAR_SOURCES,        DO_THERMAL_EMISSION,         & ! input/output
        ...
        DO_FULLRAD_MODE,         DO_USER_STREAMS,             & ! input/output
        DO_SSCORR_NADIR,         DO_SSCORR_OUTGOING,          & ! input/output
        DO_SS_EXTERNAL,          DO_OBSERVATION_GEOMETRY,     & ! input/output (line modified 10/25/12)
        DO_SSFULL,               DO_SSCORR_TRUNCATION,        & ! input/output
        DO_PLANE_PARALLEL,       DO_THERMAL_TRANSONLY,        & ! input/output
        DO_BRDF_SURFACE,         DO_SURFACE_EMISSION,         & ! input/output
        DO_ADDITIONAL_MVOUT,     DO_MVOUT_ONLY,               & ! input/output
...
!  Order of Taylor series (including terms up to EPS^n). Introduced 10/10/13 for Version 3.7
      ...
      INTEGER  , intent(inout) :: TAYLOR_ORDER
...
!   New variable, 25 OCtober 2012, for Observational Geometry input
      ...
      REAL(fpk), intent(inout) :: USER_OBSGEOMS (MAX_USER_OBSGEOMS,3)
...
!  TAYLOR_ORDER parameter added, 10/10/13
...
!     ---- Skip other goeometry input reads
...
!  module, dimensions and numbers
      ...
      USE LIDORT_pars, only : MAX_USER_STREAMS, MAX_USER_RELAZMS, &
...
!  1b. Basic dimensions - conditionally checked
      ...
      IF ( LIDORT_ModIn%MBool%TS_DO_SSCORR_OUTGOING .or. &
           LIDORT_FixIn%Bool%TS_DO_SSFULL ) THEN
...
SUBROUTINE LIDORT_CHECK_INPUT &
      ( DO_UPWELLING, DO_DNWELLING, DO_THERMAL_EMISSION,                      & ! Fixed inputs
        ...
        TAYLOR_ORDER, NSTREAMS, N_USER_LEVELS, NLAYERS, HEIGHT_GRID,          & ! Fixed Inputs (line cleaned up)
        N_USER_OBSGEOMS, USER_OBSGEOMS, NBEAMS, BEAM_SZAS,                    & ! Input, possibly modified
        N_USER_STREAMS, N_USER_RELAZMS, USER_ANGLES, USER_RELAZMS,            & ! Input, possibly modified
...
!  Set number of sources NBEAMS to 1 for the thermal-only default
      ...
      IF ( .NOT.DO_SOLAR_SOURCES.AND.DO_THERMAL_EMISSION ) THEN
        IF ( N_USER_RELAZMS .NE. 1 ) THEN
         NM = NM + 1
         MESSAGES(NM) = 'Bad input: N_USER_RELAZMS set to 1 for thermal-only'
...
!  =======================================
      ...
      IF ( .NOT.DO_SOLAR_SOURCES.AND.DO_OBSERVATION_GEOMETRY ) THEN
        NM = NM + 1
        MESSAGES(NM) = &
...
!  module, dimensions and numbers
      ...
      USE LIDORT_pars, only : fpk, MAXLAYERS, MAXMOMENTS_INPUT, MAX_MESSAGES, &
                              LIDORT_SUCCESS, LIDORT_SERIOUS, &
...
!  module, dimensions and numbers
      ...
      USE LIDORT_pars, only : fpk, MAX_USER_STREAMS, MAX_USER_LEVELS, MAX_DIRECTIONS, &
