...
!  Internal thread dimension removed, 02 May 2014
...
!  BRDF Supplement Inputs
      ...
      EMISSIVITY(1:NSTREAMS)            = &
        LIDORT_Sup%BRDF%TS_EMISSIVITY(1:NSTREAMS)
      USER_EMISSIVITY(1:N_USER_STREAMS) = &
        LIDORT_Sup%BRDF%TS_USER_EMISSIVITY(1:N_USER_STREAMS)
...
!  New outgoing sphericity correction
        ...
        ( DO_UPWELLING, DO_DNWELLING, DO_OBSERVATION_GEOMETRY,           & ! Input
          ...
          IF ( STATUS_SUB .ne. LIDORT_SUCCESS ) THEN
            TRACE_2 = 'Failure from LIDORT_SSCORR_OUTGOING'
            TRACE_3 = ' ** Called in LIDORT_MASTER'
            STATUS_CALCULATION = LIDORT_SERIOUS
            LIDORT_Out%Status%TS_STATUS_CALCULATION = STATUS_CALCULATION
            LIDORT_Out%Status%TS_MESSAGE = MESSAGE
...
!  @@@@@@@@@@@ Robfix 10 October 2013 - Add TAYLOR_ORDER argument (Line 1)
          ...
          ( FOURIER_COMPONENT, TAYLOR_ORDER, DO_OBSERVATION_GEOMETRY,                                              & ! input
            ...
            T_DELT_USERM, T_UTUP_USERM, T_UTDN_USERM,                                                              & ! input
            EMULT_UP, EMULT_DN, UT_EMULT_UP, UT_EMULT_DN, SIGMA_P, SIGMA_M, ITRANS_USERM,                          & ! input
...
!                     before call to LIDORT_FOURIER_MASTER to here
...
!  Radiance and mean
      ...
      LIDORT_Out%Main%TS_MEAN_INTENSITY(1:N_USER_LEVELS,1:NBEAMS,:)  = MEAN_INTENSITY(1:N_USER_LEVELS,1:NBEAMS,:)
      LIDORT_Out%Main%TS_FLUX_INTEGRAL(1:N_USER_LEVELS,1:NBEAMS,:)   = FLUX_INTEGRAL(1:N_USER_LEVELS,1:NBEAMS,:)
      LIDORT_Out%Main%TS_DNMEAN_DIRECT(1:N_USER_LEVELS,1:NBEAMS) = DNMEAN_DIRECT(1:N_USER_LEVELS,1:NBEAMS) 
...
!  @@@@@@@@@@@ Robfix 10 October 2013 - Add TAYLOR_ORDER argument (Line 1)
   ...
   SUBROUTINE LIDORT_FOURIER_MASTER &
      ( FOURIER_COMPONENT, TAYLOR_ORDER, DO_OBSERVATION_GEOMETRY, &
        DO_UPWELLING, DO_DNWELLING, DO_SOLAR_SOURCES, DO_BRDF_SURFACE, DO_THERMAL_EMISSION,                    & ! input
        DO_SURFACE_EMISSION, DO_THERMAL_TRANSONLY, DO_PLANE_PARALLEL, DO_USER_STREAMS,                         & ! input
        DO_SOLUTION_SAVING, DO_BVP_TELESCOPING, DO_ADDITIONAL_MVOUT, DO_MVOUT_ONLY,                            & ! input
        DO_REFRACTIVE_GEOMETRY, DO_MSMODE_LIDORT, DO_MULTIBEAM, DO_MSMODE_THERMAL,                             & ! input
...
!  start layer loop
          ...
          DO LAYER = 1, NLAYERS
            ...
            INITIAL_TRANS, AVERAGE_SECANT, T_DELT_MUBAR, DELTAU_VERT,   & ! Input (Rob Fix 5/6/13, New argument)
