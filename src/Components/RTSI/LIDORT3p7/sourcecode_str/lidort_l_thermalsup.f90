...
MODULE LIDORT_L_THERMALSUP

CONTAINS

!  @@@@@@@@@@@ Robfix 13 January 2012.
!              Add argument to THERMAL_SETUP_PLUS
...
SUBROUTINE THERMAL_SETUP_PLUS                                    &
      ( DO_UPWELLING, DO_DNWELLING, DO_THERMAL_TRANSONLY,        & !input
        ...
        DO_USER_STREAMS, DO_ATMOS_LINEARIZATION,                 & !input
        LAYERMASK_UP, LAYERMASK_DN,                              & !input
        NLAYERS, N_PARTLAYERS, PARTLAYERS_LAYERIDX,              & !input
        LAYER_VARY_FLAG, LAYER_VARY_NUMBER,                      & !input
        N_USER_STREAMS, N_THERMAL_COEFFS, USER_STREAMS, BB_INPUT,& !input
...
!  Linearized Transmittance factors for user-defined stream angles
      ...
      REAL(fpk), intent(in)  :: L_T_DELT_USERM &
         (MAXLAYERS,      MAX_USER_STREAMS, MAX_ATMOSWFS)
      REAL(fpk), intent(in)  :: L_T_UTUP_USERM &
         (MAX_PARTLAYERS, MAX_USER_STREAMS, MAX_ATMOSWFS)
      REAL(fpk), intent(in)  :: L_T_UTDN_USERM &
         (MAX_PARTLAYERS, MAX_USER_STREAMS, MAX_ATMOSWFS)
...
!  Linearized Thermal coefficients
      ...
      REAL(fpk), intent(out) :: L_THERMCOEFFS &
         (MAXLAYERS,MAX_THERMAL_COEFFS,MAX_ATMOSWFS)
...
!  Tranmsittance solutions
      ...
      REAL(fpk), intent(out) :: T_UT_DIRECT_UP &
         (MAX_USER_STREAMS,MAX_PARTLAYERS)
      REAL(fpk), intent(out) :: T_UT_DIRECT_DN &
         (MAX_USER_STREAMS,MAX_PARTLAYERS)

!  Linearized Tranmsittance solutions

      REAL(fpk), intent(out) :: L_T_DIRECT_UP &
         ( MAX_USER_STREAMS, MAXLAYERS, MAX_ATMOSWFS )
      REAL(fpk), intent(out) :: L_T_DIRECT_DN &
         ( MAX_USER_STREAMS, MAXLAYERS, MAX_ATMOSWFS )

      REAL(fpk), intent(out) :: L_T_UT_DIRECT_UP &
         ( MAX_USER_STREAMS, MAX_PARTLAYERS, MAX_ATMOSWFS )
      REAL(fpk), intent(out) :: L_T_UT_DIRECT_DN &
         ( MAX_USER_STREAMS, MAX_PARTLAYERS, MAX_ATMOSWFS )
...
!  initial set of coefficients
      ...
      DO n = 1, nlayers
       ...
       help(n) = (bb_input(n)-bb_input(n-1)) / deltau_vert(n)
      END DO
...
!  @@@@@@@@@@@ Robfix 13 January 2012.
!              Zero Direct solutions for MSMODE only, then return
...
!  direct solution: LINEARIZED partial layer source terms
        ...
        if ( do_atmos_linearization .and. n_partlayers .ne. 0 ) then
         DO ut = 1, n_partlayers
          n  = partlayers_layeridx(ut)
          IF ( layer_vary_flag(n) ) THEN
           npars = layer_vary_number(n)
...
!  optical depth powers
      ...
      REAL(fpk), intent(in)  :: DELTAU_POWER &
         (MAXLAYERS,     MAX_THERMAL_COEFFS)
      REAL(fpk), intent(in)  :: XTAU_POWER &
         (MAX_PARTLAYERS,MAX_THERMAL_COEFFS)
...
!     These depend on eigensolutions and will change for each Fourier
      ...
      REAL(fpk), intent(in)  :: L_T_DELT_EIGEN &
         (MAXSTREAMS,MAXLAYERS,MAX_ATMOSWFS)
      REAL(fpk), intent(in)  :: L_T_UTUP_EIGEN &
         (MAXSTREAMS,MAX_PARTLAYERS,MAX_ATMOSWFS)
      REAL(fpk), intent(in)  :: L_T_UTDN_EIGEN &
         (MAXSTREAMS,MAX_PARTLAYERS,MAX_ATMOSWFS)
...
!  Saved quantities for the Green function solution
      ...
      REAL(fpk), intent(out) :: T_C_MINUS &
         (MAXSTREAMS,MAXLAYERS,0:MAX_THERMAL_COEFFS)
      REAL(fpk), intent(out) :: T_C_PLUS &
         (MAXSTREAMS,MAXLAYERS,0:MAX_THERMAL_COEFFS)

!  Linearized Solutions to the Thermal RT equations

      REAL(fpk), intent(out) :: L_T_WUPPER &
         (MAXSTREAMS_2,MAXLAYERS,MAX_ATMOSWFS)
      REAL(fpk), intent(out) :: L_T_WLOWER &
         (MAXSTREAMS_2,MAXLAYERS,MAX_ATMOSWFS)
      REAL(fpk), intent(out) :: L_UT_T_PARTIC &
         (MAXSTREAMS_2,MAX_PARTLAYERS,MAX_ATMOSWFS)

!  Linearized Saved quantities for the Green function solution

      REAL(fpk), intent(out) :: L_TTERM_SAVE &
         (MAXSTREAMS,MAXLAYERS,MAX_ATMOSWFS)
      REAL(fpk), intent(out) :: L_T_C_MINUS &
         (MAXSTREAMS,MAXLAYERS,0:MAX_THERMAL_COEFFS,MAX_ATMOSWFS)
      REAL(fpk), intent(out) :: L_T_C_PLUS &
         (MAXSTREAMS,MAXLAYERS,0:MAX_THERMAL_COEFFS,MAX_ATMOSWFS)
...
!  Linearized Eigenvectors defined at user-defined stream angles
      ...
      REAL(fpk), intent(in)  :: L_U_XPOS &
         (MAX_USER_STREAMS,MAXSTREAMS,MAXLAYERS,MAX_ATMOSWFS)
...
!  Linearized homogeneous solution multipliers (upwelling, part-layer)
      ...
      REAL(fpk), intent(in)  :: L_UT_HMULT_UU &
         (MAXSTREAMS,MAX_USER_STREAMS,MAX_PARTLAYERS,MAX_ATMOSWFS)
      REAL(fpk), intent(in)  :: L_UT_HMULT_UD &
         (MAXSTREAMS,MAX_USER_STREAMS,MAX_PARTLAYERS,MAX_ATMOSWFS)

!  Saved quantities for the Green function solution

      REAL(fpk), intent(in)  :: TTERM_SAVE (MAXSTREAMS,MAXLAYERS)
      REAL(fpk), intent(in)  :: T_C_MINUS &
         (MAXSTREAMS,MAXLAYERS,0:MAX_THERMAL_COEFFS)
      REAL(fpk), intent(in)  :: T_C_PLUS &
         (MAXSTREAMS,MAXLAYERS,0:MAX_THERMAL_COEFFS)

!  Linearized Saved quantities for the Green function solution

      REAL(fpk), intent(in)  :: L_TTERM_SAVE &
         (MAXSTREAMS,MAXLAYERS,MAX_ATMOSWFS)
      REAL(fpk), intent(in)  :: L_T_C_MINUS &
         (MAXSTREAMS,MAXLAYERS,0:MAX_THERMAL_COEFFS,MAX_ATMOSWFS)
      REAL(fpk), intent(in)  :: L_T_C_PLUS &
         (MAXSTREAMS,MAXLAYERS,0:MAX_THERMAL_COEFFS,MAX_ATMOSWFS)

!  Tranmsittance solutions

      REAL(fpk), intent(in)  :: T_DIRECT_UP (MAX_USER_STREAMS,MAXLAYERS )
      REAL(fpk), intent(in)  :: T_UT_DIRECT_UP &
         (MAX_USER_STREAMS,MAX_PARTLAYERS)
...
!  Layer source terms (direct + diffuse)
      ...
      REAL(fpk), intent(out) :: LAYER_TSUP_UTUP &
         (MAX_USER_STREAMS,MAX_PARTLAYERS)

!  Linearized Layer source terms (direct + diffuse)

      REAL(fpk), intent(out) :: L_LAYER_TSUP_UP &
         (MAX_USER_STREAMS, MAXLAYERS,      MAX_ATMOSWFS)
      REAL(fpk), intent(out) :: L_LAYER_TSUP_UTUP &
         (MAX_USER_STREAMS, MAX_PARTLAYERS, MAX_ATMOSWFS)
...
!  Linearized homogeneous solution multipliers (upwelling, part-layer)
      ...
      REAL(fpk), intent(in)  :: L_UT_HMULT_DU &
         (MAXSTREAMS,MAX_USER_STREAMS,MAX_PARTLAYERS,MAX_ATMOSWFS)
      REAL(fpk), intent(in)  :: L_UT_HMULT_DD &
         (MAXSTREAMS,MAX_USER_STREAMS,MAX_PARTLAYERS,MAX_ATMOSWFS)
...
!  Tranmsittance solutions
      ...
      REAL(fpk), intent(in)  :: T_UT_DIRECT_DN &
         (MAX_USER_STREAMS,MAX_PARTLAYERS)
...
!  Layer source terms (direct + diffuse)
      ...
      REAL(fpk), intent(out)  :: LAYER_TSUP_UTDN &
         (MAX_USER_STREAMS,MAX_PARTLAYERS)

!  Linearized Layer source terms (direct + diffuse)

      REAL(fpk), intent(out)  :: L_LAYER_TSUP_DN &
         ( MAX_USER_STREAMS, MAXLAYERS,      MAX_ATMOSWFS)
      REAL(fpk), intent(out)  :: L_LAYER_TSUP_UTDN &
         ( MAX_USER_STREAMS, MAX_PARTLAYERS, MAX_ATMOSWFS)
...
END MODULE LIDORT_L_THERMALSUP
