...
!  initialize linearization variables
      ...
      CALL VLIDORT_L_INIT_VARS ( &
        ...
        DO_ATMOS_LINEARIZATION, DO_LTE_LINEARIZATION, &
        N_TOTALPROFILE_WFS, N_TOTALCOLUMN_WFS, &
        LAYER_VARY_FLAG, LAYER_VARY_NUMBER, &
        DO_SURFACE_LINEARIZATION, DO_SURFBB_LINEARIZATION, &
...
!
      ...
      LOGICAL, INTENT (INOUT) ::            DO_LTE_LINEARIZATION
      ...
      LOGICAL, INTENT (INOUT) ::            DO_SURFBB_LINEARIZATION
...
!  New section, LTE linearization flag
...
!  Check LTE linearization

      IF ( DO_LTE_LINEARIZATION ) THEN
        IF ( .NOT. DO_THERMAL_TRANSONLY ) THEN
          NM = NM + 1
          MESSAGES(NM) = &
             'Must have THERMAL_transonly for LTE linearization'
...
!mick fix
        ...
        ELSE
          IF( DO_PROFILE_LINEARIZATION .OR. DO_COLUMN_LINEARIZATION )THEN
            NM = NM + 1
            MESSAGES(NM) = &
              'No other kind of atmospheric linearization allowed'
            ACTIONS(NM)  = 'Re-set input value'
            STATUS  = VLIDORT_SERIOUS
!mick fix
            NMESSAGES = NM
...
!mick fix - added DO_LTE_LINEARIZATION, PROFILEWF_NAMES, COLUMNWF_NAMES
      ...
      LOGICAL, INTENT (OUT) :: DO_LTE_LINEARIZATION
      ...
      LOGICAL, INTENT (OUT) :: DO_SURFBB_LINEARIZATION
...
!  Check total number of BRDF weighting functions is not out of bounds
      ...
      IF ( DO_SURFACE_LINEARIZATION ) THEN
        IF ( N_SURFACE_WFS .GT. MAX_SURFACEWFS ) THEN
         ...
         ACTIONS(NM)  = &
             'Action: increase MAX_SURFACE_WFS in VLIDORT.PARS!'
         STATUS = VLIDORT_SERIOUS
        ENDIF
      ENDIF
