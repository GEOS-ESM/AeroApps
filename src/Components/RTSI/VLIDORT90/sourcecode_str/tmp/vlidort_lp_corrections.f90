...
! #                                                             #
! #            VLIDORT_LP_SSCORR_OUTGOING (master)              #
! #              LP_OUTGOING_INTEGRATION_UP                     #
! #              LP_OUTGOING_INTEGRATION_DN                     #
! #                                                             #
                  ...
                  DO IA = 1, N_USER_RELAZMS
                   ...
                   DO Q = 1, NV_PARAMETERS
                    DO O1 = 1, NSTOKES
                     SUM = ZERO
                     DO O2 = 1, NSTOKES
                      HELP = ZMAT_UP(V,N,O1,O2) * LP_EMULT_UP(UM,N,NV,IB,Q) &
                ...
                DO V = 1, N_GEOMETRIES
                 DO Q = 1, NV_PARAMETERS
                  DO O1 = 1, NSTOKES
                   SUM = ZERO
                   DO O2 = 1, NSTOKES
                    HELP = ZMAT_UP(V,N,O1,O2) * LP_EMULT_UP(LUM,N,NV,V,Q) &
                  ...
                  DO IA = 1, N_USER_RELAZMS
                   ...
                   DO Q = 1, NV_PARAMETERS
                    DO O1 = 1, NSTOKES
                     ...
                     DO O2 = 1, NSTOKES
                      HELP = &
                          ZMAT_UP(V,N,O1,O2) * LP_UT_EMULT_UP(UM,UT,NV,IB,Q) &
                ...
                DO V = 1, N_GEOMETRIES
                 DO Q = 1, NV_PARAMETERS
                  DO O1 = 1, NSTOKES
                   ...
                   DO O2 = 1, NSTOKES
                    HELP = &
                        ZMAT_UP(V,N,O1,O2) * LP_UT_EMULT_UP(LUM,UT,NV,V,Q) &
                  ...
                  DO IA = 1, N_USER_RELAZMS
                   ...
                   DO Q = 1, NV_PARAMETERS
                    DO O1 = 1, NSTOKES
                     SUM = ZERO
                     DO O2 = 1, NSTOKES
                      HELP = ZMAT_DN(V,N,O1,O2) * LP_EMULT_DN(UM,N,NV,IB,Q) &
                ...
                DO V = 1, N_GEOMETRIES
                  DO Q = 1, NV_PARAMETERS
                    DO O1 = 1, NSTOKES
                     SUM = ZERO
                     DO O2 = 1, NSTOKES
                      HELP = ZMAT_DN(V,N,O1,O2) * LP_EMULT_DN(LUM,N,NV,V,Q) &
                  ...
                  DO IA = 1, N_USER_RELAZMS
                   ...
                   DO Q = 1, NV_PARAMETERS
                    DO O1 = 1, NSTOKES
                     ...
                     DO O2 = 1, NSTOKES
                      HELP = &
                          ZMAT_DN(V,N,O1,O2) * LP_UT_EMULT_DN(UM,UT,NV,IB,Q) &
                ...
                DO V = 1, N_GEOMETRIES
                 DO Q = 1, NV_PARAMETERS
                  DO O1 = 1, NSTOKES
                   ...
                   DO O2 = 1, NSTOKES
                    HELP = &
                        ZMAT_DN(V,N,O1,O2) * LP_UT_EMULT_DN(LUM,UT,NV,V,Q) &
...
!
      ...
      SUBROUTINE VLIDORT_LP_SSCORR_OUTGOING ( &
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
      ...
      DOUBLE PRECISION, INTENT (OUT) ::  LP_BOA_ATTN &
          ( MAXLAYERS, MAX_ATMOSWFS, MAX_GEOMETRIES )
...
!  MULTIPLIERS AND TRANSMITTANCES + LINEARIZATIONS
                ...
                CALL LP_OUTGOING_INTEGRATION_UP ( &
                  NLAYERS, NFINELAYERS, &
                  DO_PARTIALS, &
                  ...
                  LP_UP_MULTIPLIERS(1,1,1,V), &
                  L_UP_LOSTRANS(1,1,V), &
                  LP_BOA_ATTN(1,1,V), &
                  LP_UP_MULTIPLIERS_UT(1,1,1,V), &
...
!     &                UP_LOSTRANS(N,V),UP_MULTIPLIERS(N,V),
!     &                L_UP_LOSTRANS(N,1,V),(LP_UP_MULTIPLIERS(N,K,1,V),K=1,NLAYERS)
!                  ENDDO
!                ELSE
!                  DO N = 1, NLAYERS
...
!  GET THE SCATTERING MATRIX AND ITS LINEARIZATION
                  ...
                  CALL SSCORR_OUTGOING_L_ZMATRIX ( &
                    DO_SSCORR_TRUNCATION, SUNLIGHT, DO_PROFILE_LINEARIZATION, &
...
!  MULTIPLIERS, TRANSMITTANCES + LINEARIZATIONS
                ...
                CALL LP_OUTGOING_INTEGRATION_DN ( &
                  ...
                  LP_DN_MULTIPLIERS(1,1,1,V), &
                  L_DN_LOSTRANS(1,1,V), &
                  LP_DN_MULTIPLIERS_UT(1,1,1,V), &
...
!  IF N = K, THERE ARE EXTRA LINEARIZATIONS
              ...
              DO V = 1, N_GEOMETRIES
               DO Q = 1, K_PARAMETERS
                DO O1 = 1, NPOLAR
                 HELP = ZERO
                 DO O2 = 1, NSTOKES
                   LSS = ZMAT_UP(V,N,O1,O2) * LP_UP_MULTIPLIERS(N,KS,Q,V) &
...
!  IF N > K, N < K, TRANSMITTANCE + SOME MULTIPLICATION LINEARIZATIONS
            ...
            IF ( N.EQ.K ) THEN
              DO V = 1, N_GEOMETRIES
               DO Q = 1, K_PARAMETERS
                LMULT = LP_UP_MULTIPLIERS_UT(UT,KS,Q,V)
                DO O1 = 1, NPOLAR
                 HELP = ZERO
                 DO O2 = 1, NSTOKES
              ...
              DO V = 1, N_GEOMETRIES
               DO Q = 1, K_PARAMETERS
                DO O1 = 1, NPOLAR
                 HELP = ZERO
                 DO O2 = 1, NSTOKES
                   LSS = ZMAT_DN(V,N,O1,O2) * LP_DN_MULTIPLIERS(N,KS,Q,V) &
...
!  IF N > K, N < K, TRANSMITTANCE + SOME MULTIPLICATION LINEARIZATIONS
            ...
            IF ( N.EQ.K ) THEN
              DO V = 1, N_GEOMETRIES
               DO Q = 1, K_PARAMETERS
                LMULT = LP_DN_MULTIPLIERS_UT(UT,KS,Q,V)
                DO O1 = 1, NPOLAR
                 HELP = ZERO
                 DO O2 = 1, NSTOKES
...
!  FINISH
      ...
      RETURN
      END SUBROUTINE VLIDORT_LP_SSCORR_OUTGOING

!

      SUBROUTINE LP_OUTGOING_INTEGRATION_UP ( &
        NLAYERS, NFINELAYERS, &
        DO_PARTIALS, &
        ...
        LP_MULTIPLIERS,   L_LOSTRANS,   LP_BOA_ATTN, &
        LP_MULTIPLIERS_P, L_LOSTRANS_P )
...
!  -------
      ...
      DOUBLE PRECISION, INTENT (OUT) :: LP_MULTIPLIERS &
          (MAXLAYERS,MAXLAYERS,MAX_ATMOSWFS)
      ...
      DOUBLE PRECISION, INTENT (OUT) :: LP_MULTIPLIERS_P &
          (MAX_PARTLAYERS,MAXLAYERS,MAX_ATMOSWFS)
      DOUBLE PRECISION, INTENT (OUT) :: L_LOSTRANS_P &
          (MAX_PARTLAYERS,MAX_ATMOSWFS)

      DOUBLE PRECISION, INTENT (OUT) :: BOA_ATTN
      DOUBLE PRECISION, INTENT (OUT) :: LP_BOA_ATTN (MAXLAYERS,MAX_ATMOSWFS)
...
!  LOCAL ATTENUATION FACTORS, LINEARIZATIONS
      ...
      DOUBLE PRECISION :: L_ATTN &
          ( 0:MAXLAYERS, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION :: L_ATTN_FINE &
          ( MAXLAYERS, MAXFINELAYERS, MAXLAYERS, MAX_ATMOSWFS )
          ...
          DO K = 1, NTRAVERSE(N)
            IF ( LAYER_VARY_FLAG(K) ) THEN
              DO Q = 1, LAYER_VARY_NUMBER(K)
                ...
                IF (N.EQ.NLAYERS)LP_BOA_ATTN(K,Q)=L_ATTN(N,K,Q)
              ENDDO
            ENDIF
          ENDDO
        ENDDO
            ...
            IF ( LAYER_VARY_FLAG(K) ) THEN
             DO Q = 1, LAYER_VARY_NUMBER(K)
              IF ( K.EQ.N) THEN
               ...
               LP_MULTIPLIERS(N,N,Q) = STEP * ( L_KN * SUM + KN * L_SUM )
              ELSE
               ...
               DO J = 1, NFINELAYERS
                ...
                L_SUM = L_SUM + L_FUNC
               ENDDO
               LP_MULTIPLIERS(N,K,Q) = STEP * KN * L_SUM
              ENDIF
             ENDDO
            ENDIF
           ENDDO
...
!     &                              L_LOSTRANS(N,1)
!           DO K = 1, NLAYERS
!             WRITE(65,'(4X,I4,1PE19.9)')K,LP_MULTIPLIERS(N,K,1)
!           ENDDO
            ...
            IF ( LAYER_VARY_FLAG(K) ) THEN
             DO Q = 1, LAYER_VARY_NUMBER(K)
              IF ( K.EQ.NP) THEN
               ...
               LP_MULTIPLIERS_P(UT,NP,Q) =  ( TERM1 +   TERM2 ) * L_KN + &
                                         ( L_TERM1 + L_TERM2 ) * KN
              ELSE
               ...
               ELSE
                ...
                L_TERM2 = 0.5D0 * L_FUNC_2 * STEP_P
               ENDIF
               LP_MULTIPLIERS_P(UT,K,Q) =  ( L_TERM1 + L_TERM2 ) * KN
              ENDIF
             ENDDO
            ENDIF
           ENDDO
...
!        DO K = 1, NLAYERS
!           WRITE(78,*)K, LP_MULTIPLIERS_P(UT,K,1)
!        ENDDO
...
!                                    L_LOSTRANS(N,1)
!           DO K = 1, NLAYERS
!             WRITE(66,'(4X,I4,1PE19.9)')K,LP_MULTIPLIERS(N,K,1)
!           ENDDO
...
!                MULTIPLIERS_P(UT),LOSTRANS_P(UT),L_LOSTRANS_P(UT,1)
!        DO K = 1, NLAYERS
!           WRITE(79,*)K, LP_MULTIPLIERS_P(UT,K,1)
!        ENDDO
...
!  FINISH
      ...
      RETURN
      END SUBROUTINE LP_OUTGOING_INTEGRATION_UP

!

      SUBROUTINE LP_OUTGOING_INTEGRATION_DN ( &
        NLAYERS, NFINELAYERS, DO_PARTIALS, &
        DO_PROFILE_LINEARIZATION, LAYER_VARY_FLAG, &
        LAYER_VARY_NUMBER, &
        EXTINCTION, L_EXTINCTION, &
...
!  FINISH
      ...
      RETURN
      END SUBROUTINE LP_OUTGOING_INTEGRATION_DN
...
!   LINEARIZATION WITH RESPECT TO PROFILE ATMOSPHERIC VARIABLES
      ...
      DOUBLE PRECISION, INTENT (IN) :: L_UP_LOSTRANS_UT &
          ( MAX_PARTLAYERS, MAX_ATMOSWFS, MAX_GEOMETRIES )
      DOUBLE PRECISION, INTENT (IN) :: LP_BOA_ATTN &
         ( MAXLAYERS, MAX_ATMOSWFS, MAX_GEOMETRIES )
...
!mick fix 8/16/2013
                    ...
                    L_EXACTDB_SOURCE(Q,K,V,O1) = FACTOR * FLUXMULT * FLUXVEC(O1)
                  ELSE
...
!    (ALREADY FLUX-MULTIPLIED, BECAUSE EXACTBD IS A FINAL RESULT)
                  ...
                  ELSE
                   ...
                   ENDIF
                   L_ATTN = - L_DELTAU_SLANT(Q,NLAYERS,K,V) * &
                              TRANS_SOLAR_BEAM(V)
                  ENDIF
                  X0_FLUX = FOUR * X0_BOA
                  L_ATTN  = L_ATTN * X0_FLUX
...
!  Linearization flags
      ...
      LOGICAL   :: DO_PROFILEWFS, DO_SURFACEWFS
...
!  Optical inputs
      ...
      !fluxvec
      !fo_fluxvec = fluxvec
      !fo_fluxvec(1) = SS_FLUX_MULTIPLIER
...
!  Linearized control
      ...
      Lvaryflags(1:nlayers) = LAYER_VARY_FLAG(1:NLAYERS)
      Lvarynums(1:nlayers)  = LAYER_VARY_NUMBER(1:NLAYERS)
...
!  ===============
      ...
      END MODULE vlidort_lp_corrections
