...
!     (all 2-D SDS's have same dimensions, CM and QA SDS's are 3-D) GAT 8-6-08
	...
	if (dim_id == -1) then
       ...
       if (sdsname == 'Cloud_Mask') then
	   	  dim_id = sfdimid(var_id, 0)
	      err_code=sfsdmname(dim_id, 'NumberOfCMBytes')
	   endif
	   if (sdsname == 'Quality_Assurance') then
 	      dim_id = sfdimid(var_id, 0)
	      err_code=sfsdmname(dim_id, 'NumberOfQABytes')
	   endif
 ...
 subroutine write_float_array(filedata, sdsname, start, stride, edge, data_to_write, status) 
	...
	if (var_id == -1) then
		if (size(edge) == 2) then 
			...
			var_id = sfcreate(file_id, trim(sdsname), DFNT_FLOAT, 3, edge(1:3))
		endif
	endif
