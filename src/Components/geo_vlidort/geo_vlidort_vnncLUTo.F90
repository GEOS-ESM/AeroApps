...
! ---------------------------------------------
  ...
  integer, pointer                      :: indicesBIG(:) => null()  
...
!                                  -----------------------------
  ...
  real*8, allocatable                   :: brdf_cx_int(:,:,:)                     ! polarized surface reflectance
...
!                                  -------------------
  ...
  real*8, pointer                       :: brdf_cx(:,:,:,:,:,:)   => null()         ! polarized surface reflectance
...
!------------------------------
  ...
  integer, allocatable                  :: clmaski(:)                               ! cloud-land mask  
  integer, allocatable                  :: clmaskj(:)                               ! cloud-land mask  
  ...
  real*8 , allocatable                  :: U10M(:)                                   ! 10m wind speed m/s  
  real*8 , allocatable                  :: V10M(:)                                   ! 10m wind speed m/s 
  integer                               :: sza, vza, raa, uwind, vwind
...
! --------------------------
  if (MAPL_am_I_root()) then
    ...
    write(*,*) 'ANGLES & WIND SPEED dataset: ',trim(ANG_file)
...
!----------------------------------------------
  ...
  call readvar1D("u10m", ANG_file, U10M)
  call readvar1D("v10m", ANG_file, V10M)
...
!------------------------------------------
  ...
  allocate (clmaski(clrm))
  allocate (clmaskj(clrm))

  if (clrm == 0) then
    if (MAPL_am_I_root()) then
      write(*,*) 'No pixels over ocean, nothing to do'
      write(*,*) 'Exiting.....'
    end if
    GOTO 500
  end if
...
! Shuffle them and cut off to 50 profiles
  call MAPL_AllocNodeArray(indicesBIG,checkSizeR4((/clrm/)),rc=ierr)
  if (MAPL_am_I_root()) then
    indicesBIG = knuthshuffle(clrm)
  end if
  call MAPL_SyncSharedMemory(rc=ierr)
...
! --------------
  ...
  if (MAPL_am_I_root()) then
    write(*,*) '<> Created Ocean Mask'
    write(*,'(A,I3,A)') '       ',nint(100.*clrm/(im*jm)),'% of the domain ocean'
      ...
      do vwind = 1, nvwind
        do sza = 1, nsza
          ...
          !   ----------------------------------------------------    
              ...
              if (.not. scalar) then
                Q_(c,sza,vza,raa,uwind,vwind)      = Q(nobs,nch)
                U_(c,sza,vza,raa,uwind,vwind)      = U(nobs,nch)
              end if
            end do ! raa
          end do !vza
        end do !sza
...
! OUTFILES
  if (vlidort) then
    write(OUT_file,'(4A)') trim(outdir),'/',trim(instname),'-g5nr.outputs.vlidort.o.'
  else
    write(OUT_file,'(4A)') trim(outdir),'/',trim(instname),'-g5nr.outputs.lidort.o.'
  end if 
  call outfile_extname(OUT_file)

  if (additional_output) then
    if (vlidort) then
      write(ATMOS_file,'(4A)') trim(outdir),'/',trim(instname),'-g5nr.inputs.vlidort.o.'
    else
      write(ATMOS_file,'(4A)') trim(outdir),'/',trim(instname),'-g5nr.inputs.lidort.o.'
    end if
...
!;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;  
  ...
  ! -----------------------
    ...
    allocate (brdf_cx_int(nstokes,nobs,nch))   
...
!;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  subroutine create_outfile(date, time)
    ...
    integer                            :: ncid
    integer                            :: pixelDimID, ewDimID, nsDimID, levDimID, elevDimID, stokesDimID  
    integer                            :: szaDimID, vzaDimID, raaDimID, uwindDimID, vwindDimID     
    integer                            :: maskVarID
    integer                            :: levVarID
    integer                            :: szaVarID, vzaVarID, raaVarID, uwindVarID, vwindVarID, peVarID
    integer                            :: ch
...
!                                ----------------------
    ...
    write(comment,'(A)') 'Cox-munk surface reflectance'
...
!                                     ----------    
    ...
    call check(nf90_def_var(ncid,'uwind',nf90_float,(/uwindDimID/),uwindVarID),"create uwind var") 
    call check(nf90_def_var(ncid,'vwind',nf90_float,(/vwindDimID/),vwindVarID),"create vwind var")        
...
!                                     ----
    do ch=1,nch
      ...
      call check(nf90_def_var(ncid, 'ref_' // trim(adjustl(comment)) ,nf90_float,(/pixelDimID,szaDimID,vzaDimID,raaDimID,uwindDimID,vwindDimID/),refVarID(ch)),"create reflectance var")
      call check(nf90_def_var(ncid, 'rad_' // trim(adjustl(comment)) ,nf90_float,(/pixelDimID,szaDimID,vzaDimID,raaDimID,uwindDimID,vwindDimID/),radVarID(ch)),"create radiance var")
      call check(nf90_def_var(ncid, 'surf_ref_' // trim(adjustl(comment)) ,nf90_float,(/stokesDimID,szaDimID,vzaDimID,raaDimID,uwindDimID,vwindDimID/),refVarID(ch)),"create reflectance var")

      if (.not. scalar) then
        call check(nf90_def_var(ncid, 'q_' // trim(adjustl(comment)) ,nf90_float,(/pixelDimID,szaDimID,vzaDimID,raaDimID,uwindDimID,vwindDimID/),qVarID(ch)),"create Q var")
        call check(nf90_def_var(ncid, 'u_' // trim(adjustl(comment)) ,nf90_float,(/pixelDimID,szaDimID,vzaDimID,raaDimID,uwindDimID,vwindDimID/),uVarID(ch)),"create U var")
      end if        
    end do
...
!;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  subroutine get_config(rcfile, nodenumarg, ierr)
    ...
    call ESMF_ConfigGetAttribute(cf, ANG_file, label = 'ANGLE_FILE:',default='LUT_angles_wind.nc4')
...
!;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  subroutine deallocate_shared()         
    ...
    call MAPL_DeallocNodeArray(brdf_cx,rc=ierr) 

  end subroutine deallocate_shared


end program geo_vlidort_vnncLUTo
