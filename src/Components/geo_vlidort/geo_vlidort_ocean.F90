...
! ---------------------------------------------
  ...
  real, pointer                         :: V10M_(:,:,:) => null()  
  real, pointer                         :: U10M_(:,:,:) => null()  
...
!                                  -------------------
  ...
  real*8, pointer                       :: ALBEDO_I(:,:) => null()                ! bi-directional surface reflectance
  real*8, pointer                       :: ALBEDO_Q(:,:) => null()                ! bi-directional surface reflectance
  real*8, pointer                       :: ALBEDO_U(:,:) => null()                ! bi-directional surface reflectance
  real*8, pointer                       :: WIND_SPEED(:) => null()    
...
! --------------------------
  if (MAPL_am_I_root()) then
    ...
    write(*,*) 'BRDF dataset: ',trim(surfname)
...
!------------------------------------------
  ...
  if (clrm == 0) then
    if (MAPL_am_I_root()) then
      write(*,*) 'No clear pixels over water, nothing to do'
      write(*,*) 'Exiting.....'
    end if
    GOTO 500
  end if
...
! -------------------------------
  ...
  SSA_           = dble(MISSING)
  G_             = dble(MISSING)
  ALBEDO_I       = dble(MISSING)
  ALBEDO_Q       = dble(MISSING)
  ALBEDO_U       = dble(MISSING)
  WIND_SPEED     = dble(MISSING)
  ROT_           = dble(MISSING)
  PE_            = dble(MISSING)
...
!                             ----------------------
    ...
    do ch = 1, nch
      ...
      call check(nf90_put_var(ncid, varid, unpack(reshape(ALBEDO_I(:,ch),(/clrm_total/)),clmask,field), &
      ...
      if (.not. scalar) then 
        ...
        call check(nf90_put_var(ncid, varid, unpack(reshape(ALBEDO_Q(:,ch),(/clrm_total/)),clmask,field), &
                      start = (/1,1,1,nobs/), count = (/im,jm,1,nobs/)), "writing out albedo")

        call check(nf90_inq_varid(ncid, 'surf_ref_U_' // trim(adjustl(msg)), varid), "get ref vaird")
        call check(nf90_put_var(ncid, varid, unpack(reshape(ALBEDO_U(:,ch),(/clrm_total/)),clmask,field), &
                      start = (/1,1,1,nobs/), count = (/im,jm,1,nobs/)), "writing out albedo")
      end if
    ...
    call check(nf90_inq_varid(ncid, 'wind_speed', varid), "get wind vaird")
    call check(nf90_put_var(ncid, varid, unpack(WIND_SPEED,clmask,field), &
                  start = (/1,1,nobs/), count = (/im,jm,nobs/)), "writing out wind")
...
! OUTFILES
  if (vlidort) then
    write(OUT_file,'(4A)') trim(outdir),'/',trim(instname),'-g5nr.lc2.ocean.vlidort.'
  else
    write(OUT_file,'(4A)') trim(outdir),'/',trim(instname),'-g5nr.lc2.ocean.lidort.'
  end if 
  call outfile_extname(OUT_file)

  if (additional_output) then
    if (vlidort) then
      write(ATMOS_file,'(4A)') trim(outdir),'/',trim(instname),'-g5nr.add.ocean.vlidort.'
    else
      write(ATMOS_file,'(4A)') trim(outdir),'/',trim(instname),'-g5nr.add.ocean.lidort.'
    end if
...
!;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;  
  subroutine read_wind()
    ...
    call mp_readvar3Dchunk("U10M",   MET_file, (/im,jm,1/), 1, npet, myid, U10M_) 
    call mp_readvar3Dchunk("V10M",  MET_file, (/im,jm,1/), 1, npet, myid, V10M_) 
    call MAPL_SyncSharedMemory(rc=ierr)    

    if (MAPL_am_I_root()) then
      temp = U10M_(:,:,1)
      U10M = pack(temp,clmask)

      temp = V10M_(:,:,1)
      V10M = pack(temp,clmask)
...
!;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;  
  subroutine allocate_shared(clrm)
    ...
    call MAPL_AllocNodeArray(WIND_SPEED,(/clrm/),rc=ierr)
    ...
    call MAPL_AllocNodeArray(ALBEDO_I,(/clrm,nch/),rc=ierr)
    if (.not. scalar) then
      call MAPL_AllocNodeArray(ALBEDO_Q,(/clrm,nch/),rc=ierr)
      call MAPL_AllocNodeArray(ALBEDO_U,(/clrm,nch/),rc=ierr)
    end if
    ...
    call MAPL_AllocNodeArray(U10M_,(/im,jm,1/),rc=ierr) 
    call MAPL_AllocNodeArray(V10M_,(/im,jm,1/),rc=ierr) 
...
!;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  subroutine create_outfile(date, time)
    ...
    integer,dimension(nch)             :: qVarID, uVarID, albIVarID, albQVarID, albUVarID      
    integer,dimension(nch)             :: ssaVarID, tauVarID, gVarID, rotVarID
    integer                            :: peVarID, windVarID     
...
!                                ----------------------
    ...
    write(comment,'(2A)') lower_to_upper(trim(surfname)),' ocean surface BRDF'
...
!                                     ----
    do ch=1,nch
      ...
      call check(nf90_def_var(ncid, 'surf_ref_I_' // trim(adjustl(comment)) ,nf90_float,(/ewDimID,nsDimID,levDimID,timeDimID/),albIVarID(ch)),"create albedo var")
      if (.not. scalar) then
        call check(nf90_def_var(ncid, 'surf_ref_Q_' // trim(adjustl(comment)) ,nf90_float,(/ewDimID,nsDimID,levDimID,timeDimID/),albQVarID(ch)),"create albedo var")
        call check(nf90_def_var(ncid, 'surf_ref_U_' // trim(adjustl(comment)) ,nf90_float,(/ewDimID,nsDimID,levDimID,timeDimID/),albUVarID(ch)),"create albedo var")
      end if
...
!                                          ------------------------  
    do ch=1,size(channels)
      ...
      call check(nf90_put_att(ncid,albIVarID(ch),'standard_name',trim(adjustl(comment))),"standard_name attr")
      write(comment,'(F10.2,A)') channels(ch), ' nm Bi-Directional Surface Reflectance'
      call check(nf90_put_att(ncid,albIVarID(ch),'long_name',trim(adjustl(comment))),"long_name attr")
      call check(nf90_put_att(ncid,albIVarID(ch),'missing_value',real(MISSING)),"missing_value attr")
      call check(nf90_put_att(ncid,albIVarID(ch),'units','None'),"units attr")
      call check(nf90_put_att(ncid,albIVarID(ch),"_FillValue",real(MISSING)),"_Fillvalue attr")
...
!;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  subroutine get_config(rcfile, nodenumarg, ierr)
    ...
    call ESMF_ConfigGetAttribute(cf, mr, label = 'SURFMR:',__RC__) 
...
!;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  subroutine deallocate_shared()         
    ...
    call MAPL_DeallocNodeArray(ALBEDO_I,rc=ierr)
    if (.not. scalar) then
      call MAPL_DeallocNodeArray(ALBEDO_Q,rc=ierr)
      call MAPL_DeallocNodeArray(ALBEDO_U,rc=ierr)
    end if
