...
! Geostationary values, UTC
! -------------------------
  real(kind=8) :: tz = 0.0
  real(kind=8) :: hgtss = 35786.
  real(kind=8) :: latss = 0.0    ! latitude of satellite in degrees (-90 to 90)
...
end Module GeoAngles

! -------------------------- Python Interface -------------------

subroutine satAngles(n,yr,mo,day,hr,min,sec,latp,lonp,lonss, &
...
!                ---
  ...
  do i = 1, n
     ...
     sat_angles = satellite_angles(yr(i),mo(i),day(i),hr(i),min(i),sec(i),tz, &
                                   latp(i),lonp(i),latss,lonss,hgtss)
