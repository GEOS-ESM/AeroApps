...
!     The variables are needed as input to lidort
!     A shared memory array created with a call to MAPL_ShmemMod is used for the variables
!     Do some filtering and run the lidort code
! INPUT
...
! -----------------------------
  ...
  real*8                                :: clock_rate
  character(len=*), parameter           :: Iam = 'geo_lidort'
...
!                             ----------------------
    ...
    do ch = 1, nch
      call check(nf90_put_var(ncid, radVarID(ch), unpack(reshape(radiance_L(:,ch),(/clrm/)),clmask,field), &
...
! ------------------------------------
  call MAPL_SyncSharedMemory(rc=ierr)
  if (MAPL_am_I_root()) then
    write(*,*) '<> Finished LIDORT Simulation of '//trim(lower_to_upper(instname))//' domain'
    call sys_tracker()
    write(*,*) ' '
    call system_clock ( t2, clock_rate, clock_max )
    write ( *, * ) 'Elapsed real time = ', real ( t2 - t1 ) / real ( clock_rate )
...
function nn_interp(x,y,xint)
  ...
  below = minloc((xint - x), dim = 1, mask = (xint - x) .LE. 0)
  above = minloc((xint - x), dim = 1, mask = (xint - x) .GT. 0)
...
! OUTFILES
  write(OUT_file,'(4A)') trim(outdir),'/',trim(instname),'-g5nr.lb2.lidort.'
  write(ATMOS_file,'(4A)') trim(outdir),'/',trim(instname),'-g5nr.lb2.lidort_atmos.'
...
!     mp_check_lidort
! PURPOSE
!     exits if LIDORT does not give good output
! INPUT
!     none
! OUTPUT
!     none
...
!;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;  
  subroutine mp_check_lidort(radiance,reflectance,ierr)
    ...
    if (ANY(radiance == MISSING) .or. ANY(reflectance == MISSING)) then
      ...
      write(*,*) 'LIDORT returned rc code ', ierr
      write(*,*) 'Exiting......'
      write(*,*) 'Pixel is ',c
      call MPI_ABORT(MPI_COMM_WORLD,myid,ierr)
    end if 

  end subroutine mp_check_lidort
...
!;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;  
  ...
  ! -----------------------
    ...
    allocate (radiance_L_int(nobs,nch))
    allocate (reflectance_L_int(nobs, nch))    
...
!                                ----------
    ...
    ! Global Attributes
    write(comment,'(A)') 'LIDORT Simulation of GEOS-5 '//lower_to_upper(trim(instname))//' Sampler'
    ...
    write(comment,'(A)') 'This file contains LIDORT simulated surface reflectance (albedo) ' // &
...
!
    ...
    ! Global Attributes
    write(comment,'(A)') 'Atmospheric inputs for LIDORT Simulation of GEOS-5 '//lower_to_upper(trim(instname))//' Sampler'
    call check(nf90_put_att(ncid,NF90_GLOBAL,'title',trim(comment)),"title attr")
    ...
    write(comment,'(A)') 'This file contains intermediate input data for the LIDORT simulation'
    call check(nf90_put_att(ncid,NF90_GLOBAL,'comment',trim(comment)),"comment attr")   
...
!    cal_qm
! PURPOSE
!     calculates qm needed by LIDORT
! INPUT
