...
def unwrap_Vars_doMie(arg, **kwarg):
    return doMie(*arg, **kwarg)

def unwrap_Vars_doMieInt(arg, **kwarg):
    return doMieInt(*arg, **kwarg)    

def doMie(Vars,v,NAMES,channel,varnames,rcFile):
    VarsIn = MieCalc()
...
def doMieInt(Vars,v,NAMES,channel,varnames,rcFile):
    VarsIn = MieCalc()
    for n, name in enumerate(NAMES):
        Var = getattr(Vars,name)
        size = len(Var.shape)
...
#   """
    ...
    if options.station:
        ...
        for r in result:
            tau,ssa,g,ext,sca,backscat,aback_sfc,aback_toa,depol,ext2back = r
