...
#   --------------------------
    ...
    # ---------------------
    cmd = "geo_vlidort_ocean_lc2.py %s %s %s "%(Options,start_isotime,end_isotime)
    print(cmd)
    if not options.dryrun:
        if system(cmd):
            raise ValueError("geo_vlidort_ocean_lc2.py failed for %s to %s "%(start_isotime, end_isotime))
