...
subroutine DTE_Integral_I_UP &
   ( maxgeoms, maxlayers, maxpartials, maxfine, max_user_levels,        & ! Inputs (dimensioning)
     ...
     Do_Polarized_Emissivity, nstokes, User_QUVEmissivity,              & ! Optional Input.  12/11/17 Rob Add.
     StokesQUV_dts )                                                      ! Optional Output. 12/11/17 Rob Add.
...
!  Optional inputs for polarized emission. 12/11/17 Rob Add.
   ...
   logical  , intent(in), optional :: Do_Polarized_Emissivity
   integer  , intent(in), optional :: nstokes
   real(ffp), intent(in), optional :: USER_QUVEmissivity(3,MAXGEOMS)

!  outputs
!  -------

!  intensities and cumulative sourceterms
...
!  Help variables for Optional polarized emissivity. 12/11/17 Rob add.
   ...
   integer    :: ns
   logical    :: do_PolEmiss
   real(ffp)  :: QUV_dts(max_user_levels,3), PolEmiss(3,maxgeoms),CUMSOURCE_DSTEQUV(3)
...
!  Optional code for polarized emissivity, set Proxies and Initialize. 12/11/17 Rob add.
   ...
   do_PolEmiss = .false.
   if ( present(Do_Polarized_Emissivity).and. present(nstokes) ) then
     if ( nstokes.gt.1 .and. present(User_QUVEmissivity).and. present(StokesQUV_dts) ) then
        do_PolEmiss = .true. ; ns = nstokes - 1
        PolEmiss = User_QUVEmissivity ; StokesQUV_dts = zero
     endif
   endif
...
!  Zero the local sources (Lostrans formerly zeroed here)
...
!  5/5/20. Version 2.8.1 Upgrades. Must add geometry index to Lostrans
...
!  5/5/20. Version 2.8.1 Upgrades. Must add geometry index to Lostrans
      ...
      if ( do_PolEmiss ) then
         ...
         ENDDO
         if ( present(StokesQUV_dts)) StokesQUV_dts(1:N_USER_LEVELS,1:ns,v) = QUV_DTS(1:N_USER_LEVELS,1:ns)
      endif
