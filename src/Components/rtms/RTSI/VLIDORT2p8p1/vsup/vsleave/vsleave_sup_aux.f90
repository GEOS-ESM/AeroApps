...
! #                                                             #
! #      VSLEAVE_READ_ERROR          M. Christi, 2017           #
! #      GETQUAD2                    M. Christi, 2017           #
! #      HOMEGROWN_ERRFUNC           R. Spurr    2005           #
...
! ###############################################################
      ...
      USE VLIDORT_PARS_m, only : fpk, zero, one, two, quarter, half, pie

!  Everything public here

      PRIVATE
      PUBLIC :: VSLEAVE_ERRUNIT, VSLEAVE_READ_ERROR, &
                GETQUAD2, HOMEGROWN_ERRFUNC, &
                BSPLINE, SEVAL, DSEVAL

!  VSLEAVE error unit

      INTEGER, PARAMETER :: VSLEAVE_ERRUNIT = 41

      CONTAINS

      SUBROUTINE VSLEAVE_READ_ERROR ( ERRORFILE, VSLEAVE_Sup_InputStatus )
...
!  VSLEAVE file inputs status

      TYPE(VSLEAVE_Input_Exception_Handling), intent(in) :: VSLEAVE_Sup_InputStatus
...
!  Write VSLEAVE configuration file read errors to VSLEAVE error file 

      OPEN (UNIT = W, FILE = TRIM(ERRORFILE), STATUS = 'REPLACE')
      WRITE(W,*) ' FATAL:   Wrong input from VSLEAVE input file-read'
      WRITE(W,*) '  ------ Here are the messages and actions '
      WRITE(W,'(A,I3)') '    ** Number of messages = ', VSLEAVE_Sup_InputStatus%SL_NINPUTMESSAGES
      DO N = 1, VSLEAVE_Sup_InputStatus%SL_NINPUTMESSAGES
         WRITE(W,'(A,I3,A,A)') 'Message # ', N, ' : ',&
           ADJUSTL(TRIM(VSLEAVE_Sup_InputStatus%SL_INPUTMESSAGES(N)))
         WRITE(W,'(A,I3,A,A)') 'Action  # ', N, ' : ',&
           ADJUSTL(TRIM(VSLEAVE_Sup_InputStatus%SL_INPUTACTIONS(N)))
      ENDDO
      CLOSE(W)
      ...
      END SUBROUTINE VSLEAVE_READ_ERROR
