module VLIDORT_SurfaceMod
   ...
   PUBLIC  VLIDORT_OCIGissCoxMunk
   PUBLIC  VLIDORT_CoxMunk  
   PUBLIC  VLIDORT_OCICoxMunk   
   ...
   interface VLIDORT_SurfaceLamb
     module procedure VLIDORT_SurfaceLamb_SingleGeom
     module procedure VLIDORT_SurfaceLamb_MultiGeom
   end interface

   interface VLIDORT_CoxMunk
     module procedure VLIDORT_CoxMunk_SingleGeom
     module procedure VLIDORT_CoxMunk_MultiGeom
   end interface

   interface VLIDORT_LANDMODIS
     module procedure VLIDORT_LANDMODIS_SingleGeom
     module procedure VLIDORT_LANDMODIS_MultiGeom
   end interface
...
!     real*8,pointer,dimension(16)  :: B => NULL()  ! NSTOKES(4) * NSTOKES for GissCoxMunk
   ...
   TYPE VLIDORT_Surface_multigeom
...
!     real*8,pointer,dimension(16)  :: B => NULL()  ! NSTOKES(4) * NSTOKES for GissCoxMunk
      ...
      real*8,pointer                :: solar_zenith(:)
      real*8,pointer                :: relat_azimuth(:)
      real*8,pointer                :: sensor_zenith(:)


   END TYPE VLIDORT_Surface_multigeom


   contains

!.........................................................................
   Subroutine VLIDORT_OCICoxMunk(self,U10m,V10m,m, solar_zenith, &
...
!     ---------------
      ...
      WHICH_BRDF(1) =  9            ! specified in vlidort.pars.f90
      BRDF_FACTORS(1)      =  1.   
...
!      DO_DBONLY = .false.        ! only direct bounce BRDF (no Fourier terms calc)
   ...
   end subroutine VLIDORT_OCICoxMunk
!.........................................................................
   Subroutine VLIDORT_OCIGissCoxMunk(self,U10m,V10m,m, solar_zenith, &
                                  sensor_zenith, relative_azimuth, scalar,rc) 
...
!      DO_DBONLY = .false.        ! only direct bounce BRDF (no Fourier terms calc)
      ...
      call VBRDF_MAINMASTER (DO_DEBUG_RESTORATION, &         ! Inputs
                             ...
                             self%Base%VIO%VBRDF_Sup_Out, &  ! Outputs
                           self%Base%VIO%VBRDF_Sup_OutputStatus)          ! Outputs
   end subroutine VLIDORT_OCIGissCoxMunk

!.........................................................................

   Subroutine VLIDORT_SurfaceLamb_SingleGeom(self, albedo, solar_zenith, &
   ...
   end subroutine VLIDORT_SurfaceLamb_SingleGeom

!.........................................................................

   Subroutine VLIDORT_SurfaceLamb_MultiGeom(self, albedo, solar_zenith, &
                                  sensor_zenith, relative_azimuth, scalar)

         type(VLIDORT_Surface_multigeom),intent(inout)  :: self
         ...
         self%sfc_type      = LAMBERTIAN
         self%albedo        => albedo(1)
         self%solar_zenith  => solar_zenith
         self%sensor_zenith => sensor_zenith
         self%relat_azimuth => relative_azimuth
         self%scalar        = scalar

   end subroutine VLIDORT_SurfaceLamb_MultiGeom
...
!.........................................................................
   Subroutine VLIDORT_CoxMunk_SingleGeom(self,U10m,V10m,m, solar_zenith, &
                                  sensor_zenith, relative_azimuth, scalar,rc) 
...
!      DO_DBONLY = .false.        ! only direct bounce BRDF (no Fourier terms calc)
      ...
      call VBRDF_MAINMASTER (DO_DEBUG_RESTORATION, &         ! Inputs
                             ...
                             self%Base%VIO%VBRDF_Sup_Out, &  ! Outputs
                           self%Base%VIO%VBRDF_Sup_OutputStatus)          ! Outputs
   end subroutine VLIDORT_CoxMunk_SingleGeom

!.........................................................................
   Subroutine VLIDORT_CoxMunk_MultiGeom(self,U10m,V10m,m, solar_zenith, &
                                  sensor_zenith, relative_azimuth, scalar,rc) 
...
!      DO_DBONLY = .false.        ! only direct bounce BRDF (no Fourier terms calc)
      ...
      VBRDF%VBRDF_Sup_In%BS_NSTREAMS             = self%Base%NSTREAMS
      VBRDF%VBRDF_Sup_In%BS_NBEAMS               = self%Base%NBEAMS
      VBRDF%VBRDF_Sup_In%BS_BEAM_SZAS(1:self%Base%NBEAMS)         = solar_zenith
      VBRDF%VBRDF_Sup_In%BS_N_USER_RELAZMS       = self%Base%N_USER_RELAZMS
      VBRDF%VBRDF_Sup_In%BS_USER_RELAZMS(1:self%Base%N_USER_RELAZMS)      = relative_azimuth
      VBRDF%VBRDF_Sup_In%BS_N_USER_STREAMS       = self%Base%N_USER_STREAMS
      VBRDF%VBRDF_Sup_In%BS_USER_ANGLES_INPUT(1:self%Base%N_USER_STREAMS) = sensor_zenith
      VBRDF%VBRDF_Sup_In%BS_N_USER_OBSGEOMS      = self%Base%N_USER_OBSGEOMS 
      VBRDF%VBRDF_Sup_In%BS_USER_OBSGEOMS(1:self%Base%N_USER_OBSGEOMS,1)   = solar_zenith
      VBRDF%VBRDF_Sup_In%BS_USER_OBSGEOMS(1:self%Base%N_USER_OBSGEOMS,2)   = sensor_zenith
      VBRDF%VBRDF_Sup_In%BS_USER_OBSGEOMS(1:self%Base%N_USER_OBSGEOMS,3)   = relative_azimuth                       
...
!     ---------------
      ...
      self%sfc_type      = MODIS_ROSSTHICK_LISPARSE 
      self%solar_zenith  = solar_zenith
      self%sensor_zenith = sensor_zenith
      self%relat_azimuth = relative_azimuth  
      self%scalar = scalar     
...
!       print*, 'ok BRDF', self%Base%VIO%VBRDF_Sup_Out%BS_DBOUNCE_BRDFUNC(1,1,1,1)             
   end subroutine VLIDORT_LANDMODIS_SingleGeom

!.................................................................................

   Subroutine VLIDORT_LANDMODIS_MultiGeom(self, solar_zenith, sensor_zenith, relative_azimuth, &
                                fiso,fgeo,fvol, param, scalar,rc) 
...
!       BRDF = self%Base%VIO%VBRDF_Sup_Out%BS_DBOUNCE_BRDFUNC(1,1,1,1)     
!       print*, 'ok BRDF', self%Base%VIO%VBRDF_Sup_Out%BS_DBOUNCE_BRDFUNC(1,1,1,1)             
   end subroutine VLIDORT_LANDMODIS_MultiGeom
...
!     ---------------
      ...
      self%sfc_type      = MODIS_ROSSTHICK_LISPARSE_BPDF
      self%solar_zenith  = solar_zenith
      self%sensor_zenith = sensor_zenith
      self%relat_azimuth = relative_azimuth  
      self%scalar = scalar     
...
!.................................................................................
   ...
   Subroutine VLIDORT_LAMBERTIAN_BPDF(self, solar_zenith, sensor_zenith, relative_azimuth, &
                                albedo, BPDFparam, scalar,rc) 
...
!     ---------------
      ...
      N_BRDF_KERNELS = 2             ! Number of BRDF kernels (max 4)
      NSTREAMS_BRDF = 50             ! Number of azimuth quadrature streams for BRDF
                                     ! Not really needed for radiance calc if DO_USER_OBSGEOMS is TRUE 
      ...
      BRDF_NAMES(2) = 'BPDF-NDVI'    ! 3 free parameters
      WHICH_BRDF(2) =  14            ! specified in vlidort.pars.f90
      BRDF_FACTORS(2) =  1.0         ! No kernel factor
      N_BRDF_PARAMETERS(2) = 3  
      BRDF_PARAMETERS(2,1) = BPDFparam(1)    ! Refractive index of water = 1.5 
      BRDF_PARAMETERS(2,2) = BPDFparam(2)    ! NDVI, must be <=1 or >=1
      BRDF_PARAMETERS(2,3) = BPDFparam(3)    ! Scaling factor C
                                             ! From Table 1 Maignan 2009 RSE, differs by IGBP land use
      LAMBERTIAN_KERNEL_FLAG(2) = .false. 
...
!       BRDF = self%Base%VIO%VBRDF_Sup_Out%BS_DBOUNCE_BRDFUNC(1,1,1,1)     
!       print*, 'ok BRDF', self%Base%VIO%VBRDF_Sup_Out%BS_DBOUNCE_BRDFUNC(1,1,1,1)             
   end subroutine VLIDORT_LAMBERTIAN_BPDF

!.................................................................................

   Subroutine VLIDORT_BPDF(self, solar_zenith, sensor_zenith, relative_azimuth, &
                                BPDFparam, scalar,rc) 
...
!     ---------------
      ...
      N_BRDF_KERNELS = 1             ! Number of BRDF kernels (max 4)
      NSTREAMS_BRDF = 50             ! Number of azimuth quadrature streams for BRDF
                                     ! Not really needed for radiance calc if DO_USER_OBSGEOMS is TRUE 
      ...
      ! For each BRDF_KERNELS specify the name, factor and parameter  
      !-------------------------------------------------------------   
      BRDF_NAMES(1) = 'BPDF-NDVI'    ! 3 free parameters
      WHICH_BRDF(1) =  14            ! specified in vlidort.pars.f90
      BRDF_FACTORS(1) =  1.0         ! No kernel factor
      N_BRDF_PARAMETERS(1) = 3  
      BRDF_PARAMETERS(1,1) = BPDFparam(1)    ! Refractive index of water = 1.5 
      BRDF_PARAMETERS(1,2) = BPDFparam(2)    ! NDVI, must be <=1 or >=1
      BRDF_PARAMETERS(1,3) = BPDFparam(3)    ! Scaling factor C
                                             ! From Table 1 Maignan 2009 RSE, differs by IGBP land use
      LAMBERTIAN_KERNEL_FLAG(1) = .false. 
