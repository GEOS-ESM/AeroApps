...
!     * Generalized surface treatment with telescoped BVP
      ...
      SUBROUTINE LP_BVP_SOLUTION_MASTER ( &
         ...
         NSTOKES, NSTREAMS, NLAYERS, LAYER_TO_VARY, N_LAYERWFS, NSTREAMS_2, & ! Numbers (Basic)
         ...
         LP_TRANS_ATMOS, LP_INITIAL_TRANS, LP_T_DELT_MUBAR, LP_BVEC,        & ! Linearized Beam/PI solutions
...
!  Boundary condition flags for special cases
      ...
      CALL LP_BVP_COLUMN_SETUP ( &
        DO_INCLUDE_SURFACE, DO_LAMBERTIAN_SURFACE, DO_INCLUDE_DIRECTRF,             & ! Flags
        DO_WATER_LEAVING, DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_PLANE_PARALLEL, & ! Flags
        FOURIER, IBEAM, NSTOKES, NSTREAMS, NLAYERS, LAYER_TO_VARY, N_LAYERWFS,        & ! Basic Control number
        MODIFIED_BCL3, MODIFIED_BCL4, DO_LAYER_SCATTERING,                            & ! BVP control
        NSTREAMS_2, NTOTAL, NSTKS_NSTRMS, NSTKS_NSTRMS_2,                             & ! Other Numbers
        ...
        LP_TRANS_ATMOS, LP_INITIAL_TRANS, LP_T_DELT_MUBAR, LP_BVEC,                   & ! Linearized Beam/SolarPI  solutions
        L_WLOWER, L_WUPPER, COL2_WF, SCOL2_WF )                                         ! PI Solutions and Column vectors

!if (fourier.eq.0.and.LAYER_TO_VARY.eq.23)then
!   do n = 1, ntotal
!     WRITE(56,*)n,COL2_WF(n,1:2)
!   enddo
!endif
...
!  Enhancement # 2, 6/27/16
      ...
      IF ( DO_INCLUDE_THERMEMISS ) THEN
        O1 = 1
        L_WUPPER(1:NSTREAMS_2,O1,NV,1:N_LAYERWFS) = L_T_WUPPER(1:NSTREAMS_2,NV,1:N_LAYERWFS)
        L_WLOWER(1:NSTREAMS_2,O1,NV,1:N_LAYERWFS) = L_T_WLOWER(1:NSTREAMS_2,NV,1:N_LAYERWFS)
      ENDIF
...
!  Get the linearized beam solution for the varying layer
      ...
      IF ( DO_SOLAR_SOURCES ) THEN
        CALL LP_BEAMSOLUTION_NEQK ( &
          FOURIER, IBEAM, LAYER_TO_VARY, N_LAYERWFS, NSTOKES, & ! basic control
          NSTREAMS_2, DO_LAYER_SCATTERING, BEAM_CUTOFF,       & ! Bookkeeping
          T_DELT_MUBAR, INITIAL_TRANS, BVEC,                  & ! Beam/SolarPI nputs
          LP_T_DELT_MUBAR, LP_BVEC,                           & ! Linearized Beam/SolarPI 
...
!  Get the linearized beam solution for the next layer
        ...
        IF ( DO_SOLAR_SOURCES ) THEN
          CALL LP_BEAMSOLUTION_NNEK ( &
            DO_PLANE_PARALLEL, FOURIER, IBEAM, N1, LAYER_TO_VARY, N_LAYERWFS, & ! Basic control
            NSTOKES, NSTREAMS_2, DO_LAYER_SCATTERING, BEAM_CUTOFF,            & ! Bookkeeping
            T_DELT_MUBAR, INITIAL_TRANS, BVEC,                     & ! Beam/SolarPI nputs
            LP_INITIAL_TRANS, LP_T_DELT_MUBAR, LP_BVEC,            & ! Linearized Beam/SolarPI 
            L_WUPPER, L_WLOWER )                                     ! Output solutions
        ENDIF
...
!
      ...
      SUBROUTINE LP_BEAMSOLUTION_NEQK ( &
          FOURIER, IB, N, N_LAYERWFS, NSTOKES,                & ! basic control
          NSTREAMS_2, DO_LAYER_SCATTERING, BEAM_CUTOFF,       & ! Bookkeeping
          T_DELT_MUBAR, INITIAL_TRANS, BVEC,                  & ! Beam/SolarPI nputs
          LP_T_DELT_MUBAR, LP_BVEC,                           & ! Linearized Beam/SolarPI 
          L_WUPPER, L_WLOWER )                                  ! Output solutions
...
!  Enhancement # 13, 6/27/16
      DO Q = 1, N_LAYERWFS
        LBSOL(1:NSTREAMS_2,1:NSTOKES,Q,1) = CONST * LP_BVEC(1:NSTREAMS_2,1:NSTOKES,N,N,Q)
        LBSOL(1:NSTREAMS_2,1:NSTOKES,Q,2) = WDEL  * LBSOL(1:NSTREAMS_2,1:NSTOKES,Q,1) + & 
...
!  Enhancement # 14, 6/27/16
      L_WUPPER(1:NSTREAMS_2,1:NSTOKES,N,1:N_LAYERWFS) = &
        L_WUPPER(1:NSTREAMS_2,1:NSTOKES,N,1:N_LAYERWFS) + LBSOL(1:NSTREAMS_2,1:NSTOKES,1:N_LAYERWFS,1)
      L_WLOWER(1:NSTREAMS_2,1:NSTOKES,N,1:N_LAYERWFS) = &
        L_WLOWER(1:NSTREAMS_2,1:NSTOKES,N,1:N_LAYERWFS) + LBSOL(1:NSTREAMS_2,1:NSTOKES,1:N_LAYERWFS,2)
...
!
      ...
      SUBROUTINE LP_BEAMSOLUTION_NNEK ( &
          DO_PLANE_PARALLEL, FOURIER, IB, N, NV, NV_PARAMETERS,  & ! Basic control
          NSTOKES, NSTREAMS_2, DO_LAYER_SCATTERING, BEAM_CUTOFF, & ! Bookkeeping
          T_DELT_MUBAR, INITIAL_TRANS, BVEC,                     & ! Beam/SolarPI nputs
          LP_INITIAL_TRANS, LP_T_DELT_MUBAR, LP_BVEC,            & ! Linearized Beam/SolarPI 
          L_WUPPER, L_WLOWER )                                     ! Output solutions
...
!  ---------------
      ...
      INTEGER ::          Q
      DOUBLE PRECISION :: CONST, WDEL, TRANS2, VAR_U( MAXSTREAMS_2, MAXSTOKES)
...
!
      ...
      SUBROUTINE LP_BVPTEL_SOLUTION_MASTER ( &
        DO_INCLUDE_SURFACE, DO_LAMBERTIAN_SURFACE, DO_INCLUDE_DIRECTBEAM, DO_PLANE_PARALLEL, & ! Flags
        FOURIER, IBEAM, NSTOKES, NSTREAMS, NLAYERS, LAYER_TO_VARY, N_LAYERWFS, & ! Basic Control Numbers
...
!  Particular solution
            ...
            BEAM = ZERO
            DO J = 1, NSTREAMS
              H1 = DOT_PRODUCT(PS_W(J,1:NSTOKES),BRDF_F(M,MO2(1:NSTOKES),J,I))
              BEAM = BEAM + H1
            ENDDO
            DO Q = 1, N_LAYERWFS
              L_BEAM = ZERO
              DO J = 1, NSTREAMS
                L_H1 = DOT_PRODUCT(PV_W(J,1:NSTOKES,Q),BRDF_F(M,MO2(1:NSTOKES),J,I))
                L_BEAM = L_BEAM + L_H1
              ENDDO
...
!  Homogeneous real solutions
              ...
              DO K = 1, K_REAL(N)
                HOMP = ZERO ; HOMM = ZERO
                DO J = 1, NSTREAMS
                  HP = DOT_PRODUCT(HS_P(J,1:NSTOKES,K),BRDF_F(M,MO2(1:NSTOKES),J,I))
                  HM = DOT_PRODUCT(HS_M(J,1:NSTOKES,K),BRDF_F(M,MO2(1:NSTOKES),J,I))
                  HOMP = HOMP + HP ; HOMM = HOMM + HM
                ENDDO
                DO Q = 1, N_LAYERWFS
                  L_HOMP = ZERO ; L_HOMM = ZERO
                  DO J = 1, NSTREAMS
                    HP = DOT_PRODUCT(HV_P(J,1:NSTOKES,K,Q),BRDF_F(M,MO2(1:NSTOKES),J,I))
                    HM = DOT_PRODUCT(HV_M(J,1:NSTOKES,K,Q),BRDF_F(M,MO2(1:NSTOKES),J,I))
...
!  homogeneous complex solutions
              ...
              DO K = 1, K_COMPLEX(NLAYERS)
                ...
                DO J = 1, NSTREAMS
                  H1_S_CR = DOT_PRODUCT(HS_P(J,1:NSTOKES,K1),BRDF_F(M,MO2(1:NSTOKES),J,I))
                  H2_S_CR = DOT_PRODUCT(HS_M(J,1:NSTOKES,K1),BRDF_F(M,MO2(1:NSTOKES),J,I))
                  H1_S_CI = DOT_PRODUCT(HS_P(J,1:NSTOKES,K2),BRDF_F(M,MO2(1:NSTOKES),J,I))
                  H2_S_CI = DOT_PRODUCT(HS_M(J,1:NSTOKES,K2),BRDF_F(M,MO2(1:NSTOKES),J,I))
                  HOMPR = HOMPR + H1_S_CR
                  HOMMR = HOMMR + H2_S_CR
                  HOMPI = HOMPI + H1_S_CI
                  HOMMI = HOMMI + H2_S_CI
                ...
                DO Q = 1, N_LAYERWFS
                  ...
                  H1_S_CR = DOT_PRODUCT(HV_P(J,1:NSTOKES,K1,Q),BRDF_F(M,MO2(1:NSTOKES),J,I))
                  H2_S_CR = DOT_PRODUCT(HV_M(J,1:NSTOKES,K1,Q),BRDF_F(M,MO2(1:NSTOKES),J,I))
                  H1_S_CI = DOT_PRODUCT(HV_P(J,1:NSTOKES,K2,Q),BRDF_F(M,MO2(1:NSTOKES),J,I))
                  H2_S_CI = DOT_PRODUCT(HV_M(J,1:NSTOKES,K2,Q),BRDF_F(M,MO2(1:NSTOKES),J,I))
...
!  the appropriate solution module = L_BEAMSOLUTION_NNEK
        ...
        ELSE IF ( LAYER_TO_VARY .LT. N ) THEN
          ...
          CALL LP_BEAMSOLUTION_NNEK ( &
            DO_PLANE_PARALLEL, FOURIER, IBEAM, N, LAYER_TO_VARY, N_LAYERWFS, & ! Basic control
            NSTOKES, NSTREAMS_2, DO_LAYER_SCATTERING, BEAM_CUTOFF,           & ! Bookkeeping
            T_DELT_MUBAR, INITIAL_TRANS, BVEC,                     & ! Beam/SolarPI nputs
            LP_INITIAL_TRANS, LP_T_DELT_MUBAR, LP_BVEC,            & ! Linearized Beam/SolarPI 
            L_WUPPER, L_WLOWER )                                     ! Output solutions
