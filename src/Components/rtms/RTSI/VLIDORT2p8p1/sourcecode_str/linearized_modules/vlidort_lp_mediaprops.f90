...
! #                                                             #
! #            VLIDORT_LP_MediaProps                             #
! #                                                             #
! ###############################################################

MODULE VLIDORT_LP_MediaProps_m
...
subroutine VLIDORT_LP_MediaProps &
       ( DO_USER_STREAMS, DO_ALBTRN_MEDIA, NSTOKES, NLAYERS, NSTREAMS,       & ! Input Control
         ...
         DO_PROFILE_WFS, LAYER_VARY_FLAG, LAYER_VARY_NUMBER,                 & ! Input Lineariz control
...
!  Linearized special media-property output
      ...
      DOUBLE PRECISION, intent (inout) :: LP_ALBMED_USER   ( MAXSTOKES, MAX_USER_STREAMS, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, intent (inout) :: LP_TRNMED_USER   ( MAXSTOKES, MAX_USER_STREAMS, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, intent (inout) :: LP_ALBMED_FLUXES ( MAXSTOKES, 2, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, intent (inout) :: LP_TRNMED_FLUXES ( MAXSTOKES, 2, MAXLAYERS, MAX_ATMOSWFS )
...
!  other variables
      ...
      INTEGER          :: OFFSET, C0, CM, STATUS_SUB, KV, NVARY, KVARY

      DOUBLE PRECISION :: SHOM, H1, H2, H_R, H_CR, LUXR, MUXR, LUX_CR, LUX_CI, MUX_CR, MUX_CI
      DOUBLE PRECISION :: TOTAL_OD, DELTA, CNEG, CPOS, T1, T1R, T1I, T2, T2R, T2I, ISOFLUX
      
      DOUBLE PRECISION :: L_TOTAL_OD(MAXLAYERS,MAX_ATMOSWFS)
      DOUBLE PRECISION :: L_SOURCE(MAX_ATMOSWFS), L_SHOM_R(MAX_ATMOSWFS), L_SHOM_CR(MAX_ATMOSWFS)
      DOUBLE PRECISION :: NUXR, PUXR, LLUXR, MLUXR, L_HOM_R, L_HOM_CR
      ...
      DOUBLE PRECISION :: LP_ALBMED_QUAD ( MAXSTOKES, MAXSTREAMS, MAX_ATMOSWFS )
      DOUBLE PRECISION :: LP_TRNMED_QUAD ( MAXSTOKES, MAXSTREAMS, MAX_ATMOSWFS )
...
!  Start Profile WF loop
           ...
           DO KV = 1, NLAYERS
             KVARY = LAYER_VARY_NUMBER(KV)
             COL2_WF = zero

!   I. First condition at TOA, top layer is varying
!   -----------------------------------------------
...
!  II. Second Condition Intermediate Levels, KV greater than 1
!  -----------------------------------------------------------
...
!...Complex
                     ...
                     DO K = 1, K_COMPLEX(N)
                       ...
                       L_HOM_CR = L_HOM_CR + T1 + T2
                     ENDDO
!  ... Final contribution and end second condition loops
...
!  III. Third Condition. Intermediate Levels, KV < NLAYERS
!  -------------------------------------------------------

             IF ( KV.LT.NLAYERS ) THEN
               N = KV ; C0  = N*NSTKS_NSTRMS_2 - NSTKS_NSTRMS
               DO I = 1, NSTREAMS_2
                 IR = NSTOKES*(I-1)
                 DO O1 = 1, NSTOKES
                   IROW = IR + O1
...
!  IV. Fourth condition, Boundary layer
!  ------------------------------------

             IF ( KV.EQ.NLAYERS ) THEN
               N = KV ; C0 = (N-1)*NSTKS_NSTRMS_2 + NSTKS_NSTRMS
               DO I = 1, NSTREAMS
                 IR = NSTOKES*(I-1)
                 I1 = I + NSTREAMS
                 DO O1 = 1, NSTOKES
...
!mick mod 8/20/2019 - do vector assignment
            ...
            IF ( NLAYERS .EQ. 1 ) THEN
              ...
              !ENDDO
              SCOL2_WF(1:NSTKS_NSTRMS,1:KVARY) = COL2_WF(1:NSTKS_NSTRMS,1:KVARY)
            ENDIF
...
!  -- Upwelling output at User streams.
!  (a) If  KV = N, this is also the layer that is varying --> Extras!
!  (b) If  N > KV with variations in layer KV above N
...
!  ...Final contribution
                    DO Q = 1, KVARY
                      ...
                      LP_ALBMED_USER(O1,UM,KV,Q) =  L_SOURCE(Q) + T_DELT_USERM(N,UM) * LP_ALBMED_USER(O1,UM,KV,Q)
                      IF ( N.EQ.KV ) then
                        LP_ALBMED_USER(O1,UM,KV,Q) = LP_ALBMED_USER(O1,UM,KV,Q) + L_T_DELT_USERM(N,UM,Q)*CUMSOURCE_USER(O1,UM,NC-1)
                      ENDIF
                    ENDDO
!  ...Finish loops
                  ENDDO
...
!  ...Final contribution
                DO Q = 1, KVARY
                  L_SOURCE(Q) = L_SHOM_R(Q) + L_SHOM_CR(Q)
                  LP_ALBMED_QUAD(O1,I,Q) =  L_SOURCE(Q)
                ENDDO
...
!  ...FLux calculation
            DO Q = 1, KVARY
              DO O1 = 1, NSTOKES
                LP_ALBMED_FLUXES(O1,1,KV,Q) = TWO * DOT_PRODUCT ( LP_ALBMED_QUAD(O1,1:NSTREAMS,Q), QUAD_STRMWTS(1:NSTREAMS) )
              ENDDO
           ENDDO
...
!  ...FLux calculation
            DO Q = 1, KVARY
              DO O1 = 1, NSTOKES
                LP_ALBMED_FLUXES(O1,2,KV,Q) = TWO * DOT_PRODUCT ( LP_ALBMED_QUAD(O1,1:NSTREAMS,Q), QUAD_STRMWTS(1:NSTREAMS) )
              ENDDO
            ENDDO
...
!  First part is the BOA upwelling field linearization, always needed
...
!mick mod 8/20/2019 - do vector assignment
            ...
            IF ( NLAYERS .EQ. 1 ) THEN
              ...
              !   SCOL2_WF(I,1:KVARY) = COL2_WF(I,1:KVARY)
              !ENDDO
              SCOL2_WF(1:NSTKS_NSTRMS_2,1:KVARY) = COL2_WF(1:NSTKS_NSTRMS_2,1:KVARY)
            ENDIF

!  --Solve using backsubstitution.
!mick fix 8/20/2019 - trimmed 1st dim of SCOL2_WF --> SCOL2 and added IF block
...
!  ...Final contribution and end loops
                     DO Q = 1, KVARY
                       L_SOURCE(Q) = L_SHOM_R(Q) + L_SHOM_CR(Q)
                       LP_TRNMED_USER(O1,UM,KV,Q) =  L_SOURCE(Q) + T_DELT_USERM(N,UM) * LP_TRNMED_USER(O1,UM,KV,Q)
                       IF ( N.EQ.KV ) then
                         LP_TRNMED_USER(O1,UM,KV,Q) = LP_TRNMED_USER(O1,UM,KV,Q) + L_T_DELT_USERM(N,UM,Q)*CUMSOURCE_USER(O1,UM,NC-1)
                       ENDIF
                     ENDDO
                   ENDDO
                 ENDDO
...
!  ...Add direct tranmsittance of surface flux.
               ...
               DO UM = 1, N_USER_STREAMS
                 DO Q = 1, KVARY
                   LP_TRNMED_USER(O1,UM,KV,Q) = LP_TRNMED_USER(O1,UM,KV,Q) - TOTAL_USERTRN(UM) * L_TOTAL_OD(KV,Q) / USER_STREAMS(UM)
                 ENDDO
               ENDDO
             ENDIF
...
!  ...Final contribution and end loops
                 DO Q = 1, KVARY
                   L_SOURCE(Q) = L_SHOM_R(Q) + L_SHOM_CR(Q)
                   LP_TRNMED_QUAD(O1,I,Q) =  L_SOURCE(Q)
                 ENDDO
               ENDDO
             ENDDO
!  FLux calculation
             DO Q = 1, KVARY
               DO O1 = 1, NSTOKES
                 LP_TRNMED_FLUXES(O1,1,KV,Q) = TWO * DOT_PRODUCT ( LP_TRNMED_QUAD(O1,1:NSTREAMS,Q), QUAD_STRMWTS(1:NSTREAMS) )
               ENDDO
             ENDDO
...
!  ...FLux calculation
             DO Q = 1, KVARY
               DO O1 = 1, NSTOKES
                 LP_TRNMED_FLUXES(O1,2,KV,Q) = TWO * DOT_PRODUCT ( LP_TRNMED_QUAD(O1,1:NSTREAMS,Q), QUAD_STRMWTS(1:NSTREAMS) )
               ENDDO
             ENDDO
...
!  done
      ...
      return
end subroutine VLIDORT_LP_MediaProps

!  End module

END MODULE VLIDORT_LP_MediaProps_m
