      ...
      IF ( .NOT. DO_VARY .OR. .NOT. DO_FIRST ) THEN
        IF (.not. DO_OBSERVATION_GEOMETRY ) THEN
          IF ( DO_UPWELLING ) THEN
            ...
            LP_UPAR_UP_2(LOCAL_UM_START:N_USER_STREAMS, 1:NSTOKES, N, LVARY, 1:N_PARAMETERS) = ZERO
          ENDIF
          IF ( DO_DNWELLING ) THEN
            L_UPAR_DN_1 (LOCAL_UM_START:N_USER_STREAMS, 1:NSTOKES, N, 1:N_PARAMETERS)        = ZERO
            LP_UPAR_DN_2(LOCAL_UM_START:N_USER_STREAMS, 1:NSTOKES, N, LVARY, 1:N_PARAMETERS) = ZERO
          ENDIF
        ELSE
          IF ( DO_UPWELLING ) THEN
...
!  Enhancement # 18, 6/27/16
      ...
      IF ( .NOT. DO_FIRST ) THEN
        DO K = 1, N - 1
          ...
          IF (.not. DO_OBSERVATION_GEOMETRY ) THEN
            IF ( DO_UPWELLING ) THEN
              LP_UPAR_UP_2(LOCAL_UM_START:N_USER_STREAMS,1:NSTOKES,N,K,1:K_PARAMETERS) = ZERO
            ENDIF
            IF ( DO_DNWELLING ) THEN
              LP_UPAR_DN_2(LOCAL_UM_START:N_USER_STREAMS,1:NSTOKES,N,K,1:K_PARAMETERS) = ZERO
            ENDIF
          ELSE
            IF ( DO_UPWELLING ) THEN
