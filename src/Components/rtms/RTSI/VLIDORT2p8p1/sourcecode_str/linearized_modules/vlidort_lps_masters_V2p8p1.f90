...
!  Specific LP linearization modules
      ...
      USE VLIDORT_LP_WFATMOS_m   , Only : VLIDORT_LPS_CONVERGE, VLIDORT_LPS_CONVERGE_OBSGEO
...
!  from VFO_LPS_MASTER_INTERFACE or VFO_MASTER_INTERFACE
...
!  from VFO_LPS_MASTER_INTERFACE: Profile Weighting functions
...
!  from VFO_LPS_MASTER_INTERFACE: Surface Jacobians (only DB and DTS)
...
!  Local quantities
      ...
      DOUBLE PRECISION :: LP_SLTERM_LOCAL(MAX_ATMOSWFS), LS_SLTERM_LOCAL(MAX_SLEAVEWFS), TFACTOR
...
!  Local flags for media problem and Planetary problem control
      ...
      DOUBLE PRECISION :: TRANS, LP_TRANS
...
!  Misc. help
      ...
      INTEGER ::          M, OFF, O1, UA, UM, IB, G, G1, G2, K, L, Q, Q1, UTA, LUM, LUA, NMOMS, NWFS
...
!mick fix 8/20/2019 - changed sense of IF condition; added new IF section
      ...
      IF ( DO_FOCORR_EXTERNAL ) THEN
         ...
         PROFILEWF_DB(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,:,1:NSTOKES)   = &
...
!  ----------------
            ...
            IF ( .not. DO_OBSERVATION_GEOMETRY ) THEN
              ...
              CALL LP_EMULT_MASTER ( &
                ...
                NLAYERS, NBEAMS, N_USER_VZANGLES, N_PARTLAYERS, LAYER_VARY_FLAG, LAYER_VARY_NUMBER, & ! Input numbers
                PARTLAYERS_LAYERIDX, STERM_LAYERMASK_UP, STERM_LAYERMASK_DN,       & ! Input Level control
...
!  ----------------------
            ...
            ELSE
              ...
              CALL LP_EMULT_MASTER_OBSGEO ( &
                DO_UPWELLING, DO_DNWELLING, DO_PLANE_PARALLEL, TAYLOR_ORDER,       & ! Input flags + Taylor
                NLAYERS, NBEAMS, N_PARTLAYERS, LAYER_VARY_FLAG, LAYER_VARY_NUMBER, & ! Input numbers
                PARTLAYERS_LAYERIDX, STERM_LAYERMASK_UP, STERM_LAYERMASK_DN,       & ! Input Level control
                USER_SECANTS, DELTAU_VERT, PARTAU_VERT, L_DELTAU_VERT,             & ! Input optical, streams   
                BEAM_CUTOFF, T_DELT_MUBAR, EMULT_HOPRULE, SIGMA_M, SIGMA_P,        & ! Input solar beam + Multipliers
                T_DELT_USERM, T_UTUP_USERM, T_UTDN_USERM, ITRANS_USERM,            & ! Input User-stream Trans.
...
!               TRACE_3 = 'VLIDORT_TRANSFLUX_MASTER failed, VLIDORT_LPS_MASTER'
!               STATUS_CALCULATION = VLIDORT_SERIOUS
!               VLIDORT_Out%Status%TS_STATUS_CALCULATION = STATUS_CALCULATION
!               VLIDORT_Out%Status%TS_MESSAGE = MESSAGE
!               VLIDORT_Out%Status%TS_TRACE_1 = TRACE_1
...
!              Added input, water-leaving control
            ...
            CALL VFO_LPS_MASTER_INTERFACE ( &
                ...
                FO_CUMTRANS, FO_SLTERM, FO_LP_CUMTRANS, FO_LSSL_SLTERM,                             & ! Output Aux 4/9/19 New line
                FAIL, MESSAGE, TRACE_1, TRACE_2 )                                                     ! Output

!  Exception handling

            IF ( FAIL ) THEN
               TRACE_3 = 'VFO_LPS_MASTER_INTERFACE failed, VLIDORT_LPS_MASTER'
               STATUS_CALCULATION = VLIDORT_SERIOUS
               VLIDORT_Out%Status%TS_STATUS_CALCULATION = STATUS_CALCULATION
               VLIDORT_Out%Status%TS_MESSAGE = MESSAGE
               VLIDORT_Out%Status%TS_TRACE_1 = TRACE_1
...
!                      computing in the crossover region!
            ...
            IF ( DO_PROFILE_LINEARIZATION ) THEN
              PROFILEWF_SS(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES,:) = &
                FO_PROFILEWF_ATMOS(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES,:)
              PROFILEWF_DB(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES) = &
                FO_PROFILEWF_SURF(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES)
            ENDIF
                   ...
                   if ( do_profile_linearization ) then
                      DO K = 1, nlayers ; if ( layer_vary_flag(k) ) then
                         ...
                         do uta = 1, n_user_levels ; DO Q = 1, NWFS
                            ...
                            PROFILEWF_DB(Q,K,UTA,IB,O1) = PROFILEWF_DB(Q,K,UTA,IB,O1) + L_CUMSOURCE_DB
                         enddo ; enddo
                      endif ; enddo
                   endif
                   ...
                   if ( do_profile_linearization ) then
                      DO K = 1, nlayers ; if ( layer_vary_flag(k) ) then
                         ...
                         do uta = 1, n_user_levels ;  do g = g1, g2 ; DO Q = 1, NWFS
                            ...
                            PROFILEWF_DB(Q,K,UTA,G,O1) = PROFILEWF_DB(Q,K,UTA,G,O1) + L_CUMSOURCE_DB
                         enddo ; enddo ; enddo
                      endif ; enddo
                   endif
                   if ( do_water_leaving .and. DO_SLEAVE_WFS ) then
...
!mick fix 9/6/2012 - added N_SLEAVE_WFS to call
                 ...
                 IF ( DO_LINEARIZATION ) THEN
                   CALL VLIDORT_LPS_CONVERGE ( &
                     DO_FOCORR, DO_FOCORR_EXTERNAL, DO_FOCORR_ALONE, DO_DBCORRECTION,      & ! Input flags
                     DO_UPWELLING, DO_PROFILE_LINEARIZATION, DO_SURFACE_LINEARIZATION,     & ! Input flags
                     DO_LAMBERTIAN_SURFACE, DO_NO_AZIMUTH,                                 & ! Input flags
                     FOURIER, IBEAM, NSTOKES, NLAYERS, N_USER_LEVELS,                      & ! Input control numbers
                     ...
                     PROFILEWF_SS, PROFILEWF_DB, PROFILEWF_F, SURFACEWF_DB, SURFACEWF_F,   & ! Input RT fields
                     PROFILEWF, SURFACEWF )                                                  ! output
                 ENDIF
...
!    Output direct Mean/Flux added 17 May 2012
      ...
      VLIDORT_LinOut%Prof%TS_MEANST_DIFFUSE_PROFWF(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,:) = &
        MEANST_DIFFUSE_PROFWF(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,:)

      VLIDORT_LinOut%Prof%TS_FLUX_DIFFUSE_PROFWF(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,:) = &
        FLUX_DIFFUSE_PROFWF(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,:)

      VLIDORT_LinOut%Prof%TS_DNMEANST_DIRECT_PROFWF (1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES) = &
        DNMEANST_DIRECT_PROFWF(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES)

      VLIDORT_LinOut%Prof%TS_DNFLUX_DIRECT_PROFWF(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES) = &
        DNFLUX_DIRECT_PROFWF(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES)
...
!  SS weighting functions
      ...
      IF ( .NOT. DO_FOCORR_EXTERNAL ) THEN
         ...
         VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_SS(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES,:) = &
           PROFILEWF_SS(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES,:)

         VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_DB(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES) = &
           PROFILEWF_DB(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES)
...
!                     in VLIDORT_LPS_MASTER & passed down VIA the "Misc" type structure)
...
!  End Module
      ...
      END MODULE vlidort_lps_masters_m
