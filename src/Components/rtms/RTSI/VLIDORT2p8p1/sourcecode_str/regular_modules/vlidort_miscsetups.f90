...
!  4/9/19. renamed, distinguish between reflected beam and surface-leaving radiance output
      ...
      USE VLIDORT_PARS_m, Only : MAXLAYERS, MAXSTOKES, MAXMOMENTS, MAXSTREAMS, MAXBEAMS, &
                                 MAX_USER_STREAMS, MAX_SZANGLES, MAXSTOKES_SQ, ZERO, FOUR, PI4, DEG_TO_RAD
...
!  Solar beam reflected into quad directions
               ...
               DO I = 1, NSTREAMS
                  DO O1 = 1, NSTOKES
                     SUM = ZERO
                     DO O2 = 1, NSTOKES
                        OM = MUELLER_INDEX(O1,O2) ; SUM = SUM + FLUXVEC(O2)*BRDF_F_0(M,OM,I,IB)
                     ENDDO
            ...
            ELSE
               DO O1 = 1, NSTOKES
                  ...
                  IF ( DO_USER_STREAMS ) THEN
                     IF ( .not. DO_OBSERVATION_GEOMETRY ) THEN
                        DO UI = 1, N_USER_STREAMS
                           SL_USERTERM(UI,IB,O1) = USER_SLTERM_F_0(M,O1,UI,IB) * HELP
                        ENDDO
                     ELSE
                        SL_USERTERM(LUI,IB,O1) = USER_SLTERM_F_0(M,O1,LUI,IB)* HELP
                     ENDIF
                  ENDIF
               ENDDO
            ENDIF
