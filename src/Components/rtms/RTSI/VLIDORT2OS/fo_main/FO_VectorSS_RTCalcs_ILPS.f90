...
!  -------
   ...
   real(fpk), Intent(Out)  :: LP_Jacobians_up  ( max_user_levels, 4, maxgeoms, maxlayers, max_atmoswfs )
   real(fpk), Intent(Out)  :: LP_Jacobians_db  ( max_user_levels, 4, maxgeoms, maxlayers, max_atmoswfs )
...
!  Attenuations
   ...
   real(fpk)  :: LP_suntau            (0:maxlayers,maxlayers,max_atmoswfs)
   real(fpk)  :: LP_attenuations      (0:maxlayers,maxlayers,max_atmoswfs)
   real(fpk)  :: LP_attenuations_fine (maxlayers,maxfinelayers,maxlayers,max_atmoswfs)
...
!  Source function integration results
   ...
   real(fpk)  :: LP_sources_up      ( maxlayers,4,maxlayers,max_atmoswfs )
   real(fpk)  :: L_lostrans_up     ( maxlayers, max_atmoswfs )
   real(fpk)  :: L_multiplier_up   ( maxlayers,maxlayers,max_atmoswfs )
...
!  Help
   ...
   integer    :: n, ns, k,  j, q, L, o1, v, uta, nstart, nc, nut, nut_prev, Qnums(maxlayers)
...
!  Zero local sources
      ...
      L_lostrans_up = zero  ; LP_sources_up = zero ; L_exactscat_up = zero ; L_multiplier_up = zero
...
!  Initialize, only to layer Ncrit if applicable
      ...
      LP_Attenuations = zero ; LP_Attenuations_fine = zero
                     ...
                     do k = 1, nlayers
                        if ( Qvary(k) .and. k.le.ntraverse_fine(n,j,v) ) then
                           do q = 1, Qnums(k)
                               ...
                               LP_Attenuations_fine(n,j,k,q) = - Attenuations_fine(n,j) * L_sumd 
                           enddo
                        endif
                     enddo
                  endif
                ...
                if ( attenuations(n-1).ne.zero ) then
                  ...
                  if ( do_profilewfs ) then
                     do k = 1, nlayers
                        if ( Qvary(k).and.k.le.ntraverse(n,v) ) then
                           do q = 1, Qnums(k)
                              L_multiplier_up(n,k,q) = LP_Attenuations(n-1,k,q)
                           enddo
                        endif
                     enddo
                  endif
                        ...
                        do q = 1, Qnums(k)
                           if ( k.eq.n ) then
                              ...
                              do j = 1, nfinedivs(n,v)
                                 ...
                                 L_func = LP_attenuations_fine(n,j,k,q) * tran(j) + L_tran * func(j)
                              ...
                              L_sum = zero
                              do j = 1, nfinedivs(n,v)
                                 L_func = LP_attenuations_fine(n,j,k,q)  * tran(j)
                                 L_sum  = L_sum + L_func * wfine(n,j,v)
                              enddo
                     ...
                     if ( Qvary(k).and.k.le.ntraverse(n,v) ) then
                        do q = 1, Qnums(k)
                           if ( k.eq.n ) then
                              ...
                              do j = 1, nfinedivs(n,v)
                                 L_tran = - argum(j) * L_extinction(n,q)
                                 L_func = LP_attenuations_fine(n,j,k,q) * csqfine(n,j,v) * tran(j) + L_tran * func(j)
                                 L_sum  = L_sum + L_func * wfine(n,j,v)
                              enddo
                              L_multiplier_up(n,k,q)  = L_sum * ke + L_extinction(N,q) * Raycon(v) * sum
                           else
                              L_sum = zero
                              do j = 1, nfinedivs(n,v)
                                 L_func = LP_attenuations_fine(n,j,k,q) * csqfine(n,j,v) * tran(j)
                                 L_sum  = L_sum + L_func * wfine(n,j,v)
                              enddo
            ...
            if ( do_profilewfs ) then
               do k = 1, nlayers
                  if ( Qvary(k) ) then
                     do q = 1, Qnums(k)
                        do o1 = 1, nstokes
                           LP_sources_up(n,o1,k,q) = shelp(o1) * L_multiplier_up(n,k,q)
                           if ( k.eq.n ) then
                              L_Shelp = dot_product(L_exactscat_up(n,o1,1:ns,q),fluxvec(1:ns))
                              LP_sources_up(n,o1,k,q) =  LP_sources_up(n,o1,k,q) + L_Shelp * multiplier_up(n)
                           endif
                        enddo
                     enddo
                  endif
      ...
      if ( do_surfacewfs ) then
         ...
         do q = 1, n_surfacewfs
            if ( DO_LAMBERTIAN ) then
               ...
               do o1 = 1, nstokes
                  LS_cumsource(o1,q) = M4A * dot_product(LS_REFLEC(O1,1:ns,q,V),fluxvec(1:ns))
               enddo
            endif
         enddo
...
subroutine SSV_Integral_ILPS_DN &
   ( maxgeoms, maxlayers, maxfinelayers, maxmoments_input,                    & ! Inputs (dimensioning)
     ...
     Mu1, GenSpher, Rotations, radii, NCrit, RadCrit, CotCrit,                & ! Inputs (Geometry)
...
!  -------
   ...
   real(fpk), Intent(Out)  :: LP_Jacobians_dn  ( max_user_levels, 4, maxgeoms, maxlayers, max_atmoswfs )
...
!  Source function integration results
   ...
   real(fpk)  :: LP_sources_dn     ( maxlayers,4,maxlayers,max_atmoswfs )
   real(fpk)  :: L_lostrans_dn     ( maxlayers, max_atmoswfs )
   real(fpk)  :: L_multiplier_dn   ( maxlayers,maxlayers,max_atmoswfs )
...
!  Zero local sources
      ...
      L_lostrans_dn = zero  ; LP_sources_dn = zero ; L_exactscat_dn = zero ; L_multiplier_dn = zero
...
!mick test 8/12/2013 - added "taylor small" section
                                ...
                                L_multiplier_dn(n,k,q) = lospath * LP_Attenuations(n-1,k,q) * mult
                                if ( k.eq.n ) then
                                  L_multiplier_dn(n,k,q) = L_multiplier_dn(n,k,q) &
                                                         + lospath * Attenuations(n-1) * L_mult
                                endif
                              else
                ...
                if ( attenuations(n-1).ne.zero ) then
                  ...
                  if ( do_profilewfs ) then
                     do k = 1, nlayers
                        if ( Qvary(k).and.k.le.ntraverse(n,v) ) then
                           do q = 1, Qnums(k)
                              L_multiplier_dn(n,k,q) = LP_Attenuations(n-1,k,q)
                           enddo
                        endif
                     enddo
                  endif
                        ...
                        do q = 1, Qnums(k)
                           if ( k.eq.n ) then
                              ...
                              do j = 1, nfinedivs(n,v)
                                 ...
                                 L_sum  = L_sum + L_func * wfine(n,j,v)
                              enddo
                              L_multiplier_dn(n,k,q)  = L_sum * ke + L_extinction(N,q) * Raycon(v) * sum
                           else
                              L_sum = zero
                              do j = 1, nfinedivs(n,v)
                                 L_func = LP_attenuations_fine(n,j,k,q) * csqfine(n,j,v) * tran(j)
            ...
            if ( do_profilewfs ) then
               do k = 1, nlayers
                  if ( Qvary(k) ) then
                     do q = 1, Qnums(k)
                        do o1 = 1, nstokes
                           LP_sources_dn(n,o1,k,q) = shelp(o1) * L_multiplier_dn(n,k,q)
                           if ( k.eq.n ) then
                              L_Shelp = dot_product(L_exactscat_dn(n,o1,1:ns,q),fluxvec(1:ns))
                              LP_sources_dn(n,o1,k,q) =  LP_sources_dn(n,o1,k,q) + L_Shelp * multiplier_dn(n)
                           endif
                        enddo
                     enddo
                  endif
...
subroutine SSV_Integral_ILPS_UPDN &
   ( maxgeoms, maxlayers, maxfinelayers, maxmoments_input,                             & ! Inputs (dimensioning)
     max_user_levels, max_atmoswfs, max_surfacewfs,                                    & ! Inputs (dimensioning)
     do_upwelling, do_dnwelling, do_sunlight, do_deltam_scaling, do_lambertian,        & ! Inputs (Flags - General)
     do_PlanPar, do_regular_ps, do_enhanced_ps, doNadir, nstokes,                      & ! Inputs (Flags)
     do_profilewfs, do_surfacewfs, n_surfacewfs, Lvaryflags, Lvarynums, Lvarymoms,     & ! Inputs (control, Jacobian )
     ngeoms, nlayers, nfinedivs, nmoments_input, n_user_levels, user_levels,           & ! Inputs (control,  output)
     reflec, extinction, deltaus, omega, truncfac, greekmat, flux, fluxvec,            & ! Inputs (Optical - Regular)
     LS_reflec, L_extinction, L_deltaus, L_omega, L_truncfac, L_greekmat,              & ! Inputs (Optical - Linearized)
     ...
     stokes_dn, LP_Jacobians_dn )                                                        ! Output (downwelling)
