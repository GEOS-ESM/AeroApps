...
MODULE VLIDORT_LPS_SPHERCORR_ROUTINES_m
...
!  HERE IS THE VLIDORT IMPLEMENTATI=ON
!  =================================

!  This is the LPS linearization of the 2pt sphericity correction
...
public  :: MS2pt_LPS_SPHERCORR_V1, &
           MS3pt_LPS_SPHERCORR_V1, &
           MSMpt_LPS_SPHERCORR_V1


contains

subroutine MS2pt_LPS_SPHERCORR_V1 ( do_debug_input, & 
        ...
        FO_PROFILEWF_BOAGEOM, MS_PROFILEWF_BOAGEOM, MS_PROFILEWF_SPHERCORR, & ! LP Jacobian Component outputs (FO/MS)
        ...
        PROFILEWF_BOAGEOM, PROFILEWF_SPHERCORR, SURFACEWF_BOAGEOM, SURFACEWF_SPHERCORR, & ! Linearized SpherCorr output
...
!  Profile Jacobian output with explanations

      REAL(fpk), INTENT(OUT) :: FO_PROFILEWF_BOAGEOM   ( MAX_ATMOSWFS, MAXLAYERS, 4 ) ! 1. First-order (SS/DB) LP Jac for BOA geo
      REAL(fpk), INTENT(OUT) :: MS_PROFILEWF_BOAGEOM   ( MAX_ATMOSWFS, MAXLAYERS, 4 ) ! 2. MS LP Jac for BOA geo
      REAL(fpk), INTENT(OUT) :: MS_PROFILEWF_SPHERCORR ( MAX_ATMOSWFS, MAXLAYERS, 4 ) ! 3. MS LP Jac with 2-point corr
      REAL(fpk), INTENT(OUT) :: PROFILEWF_BOAGEOM      ( MAX_ATMOSWFS, MAXLAYERS, 4 ) ! 4. Full LP Jac for BOA geo.     4 = 1 + 2.
      REAL(fpk), INTENT(OUT) :: PROFILEWF_SPHERCORR    ( MAX_ATMOSWFS, MAXLAYERS, 4 ) ! 5. Full LP Jac with MS2pt corr. 5 = 1 + 3.
...
!  Variables for doing the sphericity calculation
      ...
      REAL(fpk) :: CUMSOURCE(0:MAXLAYERS,4), LP_CUMSOURCE(4), LS_CUMSOURCE(MAX_SURFACEWFS,4)
      REAL(fpk) :: LP_TRANS, LP_SMSST(MAX_ATMOSWFS,MAXLAYERS,4), LP_AMSST(MAX_ATMOSWFS,MAXLAYERS,MAXLAYERS,4)
...
!   -- DirIdx chooses TOA Upwelling (1) or BOA Downwelling (2)
      ...
      LOGICAL :: Verbose, DO_PROFILE_LINEARIZATION, DO_SURFACE_LINEARIZATION, LAYER_VARY_FLAG ( MAXLAYERS )
      INTEGER :: N, Q, NC, NS, O1, NLAYERS, K, NSWF, DIRIDX, LAYER_VARY_NUMBER ( MAXLAYERS )
...
!  Initialize
      ...
      FO_STOKES_BOAGEOM   = zero ; FO_PROFILEWF_BOAGEOM   = zero ; FO_SURFACEWF_BOAGEOM   = zero
      MS_STOKES_BOAGEOM   = zero ; MS_PROFILEWF_BOAGEOM   = zero ; MS_SURFACEWF_BOAGEOM   = zero
      MS_STOKES_SPHERCORR = zero ; MS_PROFILEWF_SPHERCORR = zero ; MS_SURFACEWF_SPHERCORR = zero
      STOKES_BOAGEOM      = zero ;    PROFILEWF_BOAGEOM   = zero ;    SURFACEWF_BOAGEOM   = zero
      STOKES_SPHERCORR    = zero ;    PROFILEWF_SPHERCORR = zero ;    SURFACEWF_SPHERCORR = zero
...
!  Exception handling (simply done here)
      ...
      if ( VLIDORT_Out%Status%TS_STATUS_INPUTCHECK  .eq. VLIDORT_SERIOUS .or. &
         ...
         Local_message = 'Some errors arising from VLIDORT CALL in MS2pt_LPS_SPHERCORR_V1'
      ...
      if ( DirIdx .eq. 1 ) then
           ...
           DO K = 1, NLAYERS
             IF ( LAYER_VARY_FLAG(K) ) THEN
               DO Q = 1, LAYER_VARY_NUMBER(K)
                 FO_PROFILEWF_BOAGEOM(Q,K,1:NS) = VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_SS(Q,K,1,1,1:NS,UPIDX) &
                                                + VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_DB(Q,K,1,1,1:NS) 
                 MS_PROFILEWF_BOAGEOM(Q,K,1:NS) = VLIDORT_LinOut%Prof%TS_PROFILEWF(Q,K,1,1,1:NS,UPIDX) &
                                                - FO_PROFILEWF_BOAGEOM(Q,K,1:NS)
                 PROFILEWF_BOAGEOM(Q,K,1:NS)    = VLIDORT_LinOut%Prof%TS_PROFILEWF(Q,K,1,1,1:NS,UPIDX)
               ENDDO
             ENDIF
           ENDDO
         ENDIF
         ...
         IF ( DO_PROFILE_LINEARIZATION ) THEN
           DO K = 1, NLAYERS
             IF ( LAYER_VARY_FLAG(K) ) THEN
               DO Q = 1, LAYER_VARY_NUMBER(K)
                 FO_PROFILEWF_BOAGEOM(Q,K,1:NS) = VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_SS(Q,K,1,1,1:NS,DNIDX)
                 MS_PROFILEWF_BOAGEOM(Q,K,1:NS) = VLIDORT_LinOut%Prof%TS_PROFILEWF(Q,K,1,1,1:NS,DNIDX) &
                                                                 - FO_PROFILEWF_BOAGEOM(Q,K,1:NS)
                 PROFILEWF_BOAGEOM(Q,K,1:NS)    = VLIDORT_LinOut%Prof%TS_PROFILEWF(Q,K,1,1,1:NS,DNIDX)
               ENDDO
             ENDIF
           ENDDO
         ENDIF
      ...
      if ( DirIdx .eq. 1 ) then
        ...
        IF ( DO_PROFILE_LINEARIZATION ) THEN
          DO K = 1, NLAYERS
            IF ( LAYER_VARY_FLAG(K) ) THEN
              DO Q = 1, LAYER_VARY_NUMBER(K)
                LP_SMSST(Q,K,1:NS)  = VLIDORT_LinOut%Prof%TS_LP_SURF_MSSTS(Q,K,1,1:NS)
              ENDDO
            ENDIF
          ENDDO
        ENDIF
        ...
        DO O1 = 1, NS
          ...
          IF ( DO_PROFILE_LINEARIZATION ) THEN
            DO K = 1, NLAYERS
              IF ( LAYER_VARY_FLAG(K) ) THEN
                DO Q = 1, LAYER_VARY_NUMBER(K)
                LP_AMSST(Q,K,N,O1)  =   F0 * VLIDORT_LinOut%Prof%TS_LP_LAYER_MSSTS(Q,K,1,O1,N) &
                                      + F1 * VLIDORT_LinOut%Prof%TS_LP_LAYER_MSSTS(Q,K,2,O1,N)  
                ENDDO
              ENDIF
            ENDDO
          ENDIF
          IF ( DO_SURFACE_LINEARIZATION ) THEN
            ...
            ENDDO
          ENDIF
!write(*,*)O1,N,AMSST(N,o1),LP_AMSST(1:2,23,N,o1),LS_AMSST(1,N,o1)
        ENDDO
      ENDDO
...
!   Upwelling case, ==> Start with the surface terms SMSST, LP_SMSST, LS_SMSST
      ...
      if ( DirIdx .eq. 1 ) then
        ...
        IF ( DO_PROFILE_LINEARIZATION ) THEN
          DO K = 1, NLAYERS
            IF ( LAYER_VARY_FLAG(K) ) THEN
              DO Q = 1, LAYER_VARY_NUMBER(K)
                LP_CUMSOURCE(1:NS) = LP_SMSST(Q,K,1:NS) ; NC = 0
                DO N = NLAYERS, 1, -1
                  NC = NC + 1
                  TRANS    = VLIDORT_Out%Main%TS_LOSTRANS(1,N)
                  LP_TRANS = zero ; if ( K.eq.N) LP_TRANS = VLIDORT_LinOut%Prof%TS_LP_LOSTRANS(Q,1,N)
                  LP_CUMSOURCE(1:NS) = TRANS * LP_CUMSOURCE(1:NS) + LP_TRANS * CUMSOURCE(NC-1,1:NS) + LP_AMSST(Q,K,N,1:NS)
                ENDDO
                MS_PROFILEWF_SPHERCORR(Q,K,1:NS) = LP_CUMSOURCE(1:NS)
              ENDDO
            ENDIF
          ENDDO
        ENDIF
      ...
      if ( dirIdx.eq. 2 ) then
        ...
        IF ( DO_PROFILE_LINEARIZATION ) THEN
          DO K = 1, NLAYERS
            IF ( LAYER_VARY_FLAG(K) ) THEN
              DO Q = 1, LAYER_VARY_NUMBER(K)
                LP_CUMSOURCE(1:NS) = zero
                DO N = 1, NLAYERS
                  TRANS    = VLIDORT_Out%Main%TS_LOSTRANS(1,N)
                  LP_TRANS = zero ; if ( K.eq.N) LP_TRANS = VLIDORT_LinOut%Prof%TS_LP_LOSTRANS(Q,1,N)
                  LP_CUMSOURCE(1:NS) = TRANS * LP_CUMSOURCE(1:NS) + LP_TRANS * CUMSOURCE(N-1,1:NS) + LP_AMSST(Q,K,N,1:NS)
                ENDDO
                MS_PROFILEWF_SPHERCORR(Q,K,1:NS) = LP_CUMSOURCE(1:NS)
              ENDDO
            ENDIF
...
!  Final answers: Add FO Outgoing results to MS fields
      ...
      IF ( DO_PROFILE_LINEARIZATION ) THEN
        DO K = 1, NLAYERS
          IF ( LAYER_VARY_FLAG(K) ) THEN
            DO Q = 1, LAYER_VARY_NUMBER(K)
               PROFILEWF_SPHERCORR(Q,K,1:NS) = MS_PROFILEWF_SPHERCORR(Q,K,1:NS) + FO_PROFILEWF_BOAGEOM(Q,K,1:NS)
            ENDDO
          ENDIF
        ENDDO
      ENDIF
...
!  normal Finish
      ...
      return
end subroutine MS2pt_LPS_SPHERCORR_V1

!

subroutine MS3pt_LPS_SPHERCORR_V1 ( do_debug_input, & 
        VLIDORT_FixIn, VLIDORT_ModIn, VLIDORT_Sup,          & ! VLIDORT Standard   Inputs
        VLIDORT_LinFixIn, VLIDORT_LinModIn, VLIDORT_LinSup, & ! VLIDORT Linearized Inputs
        FO_STOKES_BOAGEOM, MS_STOKES_BOAGEOM, MS_STOKES_SPHERCORR, & ! Standard    Component outputs (FO/MS)
        FO_PROFILEWF_BOAGEOM, MS_PROFILEWF_BOAGEOM, MS_PROFILEWF_SPHERCORR, & ! LP Jacobian Component outputs (FO/MS)
...
!   -- DirIdx chooses TOA Upwelling (1) or BOA Downwelling (2)
      ...
      LOGICAL :: Verbose, DO_PROFILE_LINEARIZATION, DO_SURFACE_LINEARIZATION, LAYER_VARY_FLAG ( MAXLAYERS )
      INTEGER :: N, Q, NC, NS, NLAYERS, K, NSWF, DIRIDX, LAYER_VARY_NUMBER ( MAXLAYERS )
...
!  Exception handling (simply done here)
      ...
      if ( VLIDORT_Out%Status%TS_STATUS_INPUTCHECK  .eq. VLIDORT_SERIOUS .or. &
           VLIDORT_Out%Status%TS_STATUS_CALCULATION .eq. VLIDORT_SERIOUS ) then
         Local_message = 'Some errors arising from VLIDORT CALL in MS3pt_LPS_SPHERCORR_V1'
         Local_Action  = 'In Driver, use call to VLIDORT_WRITE_STATUS to examine errors'
         FAIL = .true. ; return
      Endif
...
!write(*,*)N,F0,F1, VLIDORT_Out%Main%TS_LAYER_MSSTS(1:3,1,N),AMSST(N,1)
          IF ( DO_PROFILE_LINEARIZATION ) THEN
            DO K = 1, NLAYERS
              IF ( LAYER_VARY_FLAG(K) ) THEN
                DO Q = 1, LAYER_VARY_NUMBER(K)
                  LP_AMSST(Q,K,N,1:NS)  =  F0 * VLIDORT_LinOut%Prof%TS_LP_LAYER_MSSTS(Q,K,3,1:NS,N) &
                                         + F1 * VLIDORT_LinOut%Prof%TS_LP_LAYER_MSSTS(Q,K,2,1:NS,N)  
                ENDDO
              ENDIF
            ENDDO
          ENDIF
...
!write(*,*)N,F0,F1, VLIDORT_Out%Main%TS_LAYER_MSSTS(1:3,1,N),AMSST(N,1)
          IF ( DO_PROFILE_LINEARIZATION ) THEN
            DO K = 1, NLAYERS
              IF ( LAYER_VARY_FLAG(K) ) THEN
                DO Q = 1, LAYER_VARY_NUMBER(K)
                  LP_AMSST(Q,K,N,1:NS)  =  F0 * VLIDORT_LinOut%Prof%TS_LP_LAYER_MSSTS(Q,K,2,1:NS,N) &
                                         + F1 * VLIDORT_LinOut%Prof%TS_LP_LAYER_MSSTS(Q,K,1,1:NS,N)  
                ENDDO
              ENDIF
            ENDDO
          ENDIF
...
!  normal Finish
      ...
      return
end subroutine MS3pt_LPS_SPHERCORR_V1

!

subroutine MSMpt_LPS_SPHERCORR_V1 ( do_debug_input, & 
        VLIDORT_FixIn, VLIDORT_ModIn, VLIDORT_Sup,          & ! VLIDORT Standard   Inputs
        VLIDORT_LinFixIn, VLIDORT_LinModIn, VLIDORT_LinSup, & ! VLIDORT Linearized Inputs
        FO_STOKES_BOAGEOM, MS_STOKES_BOAGEOM, MS_STOKES_SPHERCORR, & ! Standard    Component outputs (FO/MS)
        FO_PROFILEWF_BOAGEOM, MS_PROFILEWF_BOAGEOM, MS_PROFILEWF_SPHERCORR, & ! LP Jacobian Component outputs (FO/MS)
...
!   -- DirIdx chooses TOA Upwelling (1) or BOA Downwelling (2)
      ...
      LOGICAL :: Verbose, DO_PROFILE_LINEARIZATION, DO_SURFACE_LINEARIZATION, LAYER_VARY_FLAG ( MAXLAYERS )
      INTEGER :: N, NMULT, NS, NG, NG1, Q, NC, NLAYERS, K, NSWF, DIRIDX, LAYER_VARY_NUMBER ( MAXLAYERS )
...
!  Exception handling (simply done here)
      ...
      if ( VLIDORT_Out%Status%TS_STATUS_INPUTCHECK  .eq. VLIDORT_SERIOUS .or. &
           VLIDORT_Out%Status%TS_STATUS_CALCULATION .eq. VLIDORT_SERIOUS ) then
         Local_message = 'Some errors arising from VLIDORT CALL in MSMpt_LPS_SPHERCORR_V1'
         Local_Action  = 'In Driver, use call to VLIDORT_WRITE_STATUS to examine errors'
         FAIL = .true. ; return
      Endif
      ...
      do N = 1, nlayers
        ...
        IF ( DO_PROFILE_LINEARIZATION ) THEN
          DO K = 1, NLAYERS
            IF ( LAYER_VARY_FLAG(K) ) THEN
              DO Q = 1, LAYER_VARY_NUMBER(K)
              LP_AMSST(Q,K,N,1:NS) =  HALF * ( VLIDORT_LinOut%Prof%TS_LP_LAYER_MSSTS(Q,K,NG,1:NS,N)  &
                                            +  VLIDORT_LinOut%Prof%TS_LP_LAYER_MSSTS(Q,K,NG1,1:NS,N) ) 
              ENDDO
            ENDIF
          ENDDO
        ENDIF
...
!  normal Finish
      ...
      return
end subroutine MSMpt_LPS_SPHERCORR_V1

!  End module

END MODULE VLIDORT_LPS_SPHERCORR_ROUTINES_m
