...
!  Help variables
      ...
      INTEGER            :: I, IA, IB, J, M, MQ(16), NS, NSSQ, NTASKS, &
                            Q, Q1, Q2, T, TEST, UM, W
      INTEGER            :: NBEAMS, NSTREAMS, N_USER_STREAMS, N_USER_RELAZMS, NUMGEO

      INTEGER, PARAMETER :: MAX_VEC_BRDFS = 7
      INTEGER            :: BRDF_IDX, IBRDF, VEC_BRDF(MAX_VEC_BRDFS)
...
!Note: Unlike the scalar BRDF self test driver (which tests all BRDFS),
!            this vector BRDF self test driver tests only the true vector BRDFS
!            (BRDF 10-14, 16, & 18)
!Note: MAX_VEC_BRDFS currently set to 7 above.
      ...
      !General set
      DO IBRDF = 1,MAX_VEC_BRDFS

      BRDF_IDX = VEC_BRDF(IBRDF)
...
!  Get the VBRDF inputs
      ...
      VBRDF_INPUT_FILE = 'vlidort_v_test/VBRDF_ReadInput_self_test.cfg'
...
!  ==============================
      ...
      IF (IBRDF == 1) THEN
        ...
        !open(36, file = 'vlidort_v_test/results_brdfplus_selftest.brdf' // cbrdf // &
        !                '_ref_ns' // cns // '_v3' , &
        !         status = 'unknown')
        open(36, file = 'vlidort_v_test/results_brdf_self_tester.res',&
                 status = 'unknown')
...
!  Close regular reflectance file when all VBRDFs finished
      ...
      IF (IBRDF == MAX_VEC_BRDFS) close(36)
...
!  Open linearized reflectance file for current VBRDF & write metadata
      ...
      IF (IBRDF == 1) THEN
        !open(37, file = 'vlidort_v_test/results_brdfplus_selftest.brdf' // cbrdf // &
        !                '_wfs_ns' // cns // '_v3' , &
        !         status = 'unknown')
        open(37, file = 'vlidort_v_test/results_brdf_self_tester.wfs',&
                 status = 'unknown')
...
!  Close linearized reflectance file when all VBRDFs finished
      ...
      IF (IBRDF == MAX_VEC_BRDFS) close(37)
