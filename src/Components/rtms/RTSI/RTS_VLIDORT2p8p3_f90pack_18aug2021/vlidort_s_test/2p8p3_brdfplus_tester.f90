...
!  Module files for VBRDF and VLIDORT. Strict usage, Version 2.8 upwards
...
!  Help variables
      ...
      INTEGER ::          N, N6, UTA, L, NDUM, LDUM, V, T, NTASKS, TASK, O1
...
!  Get the VBRDF inputs
      ...
      CALL VBRDF_LIN_INPUTMASTER ( &
        'vlidort_s_test/VBRDF_ReadInput.cfg', & ! Input
      ...
      IF ( VBRDF_Sup_InputStatus%BS_STATUS_INPUTREAD .ne. VLIDORT_SUCCESS ) &
        CALL VBRDF_READ_ERROR ( '2p8p3_VBRDF_ReadInput.log', VBRDF_Sup_InputStatus )
...
!  Write normal VBRDF supplement results (as a check)
      ...
      open(36, file='vlidort_s_test/results_brdf_supcheck.res', status = 'unknown')
        ...
        do um = 1, VBRDF_Sup_In%BS_n_user_streams
          do ia = 1, VBRDF_Sup_In%BS_n_user_relazms
            write(36,'(3i4,1p16e20.11)') ib, um, ia, &
             (VLIDORT_Sup%BRDF%TS_exactdb_brdfunc(q,um,ia,ib), q = 1, nstokessq)
          enddo
        enddo
      enddo
...
!mick mod 9/19/2017 - swapped index j for i in USER_BRDF_F write statement
        write(36,*)' '
        do um = 1, VBRDF_Sup_In%BS_n_user_streams
...
!  Write linearized VBRDF supplement results
      ...
      open(36, file = 'vlidort_s_test/results_brdf_supcheck.wfs', status = 'unknown')
      ...
      write(36,'(6i5,2x,L2)') &
        ...
        VBRDF_Sup_In%BS_n_user_relazms, NSTOKESSQ, GCM
...
!mick mod 9/19/2017 - swapped index j for i in LS_USER_BRDF_F write statement
         write(36,*) ' '
         do um = 1, VBRDF_Sup_In%BS_n_user_streams
          write(36,99) UM, &
...
!  Baseline calculation,
!      In/Outgoing  FO CORR correction, With delta-M scaling
...
!  Read the VBRDF information again
        ...
        CALL VBRDF_INPUTMASTER ( &
          'vlidort_s_test/VBRDF_ReadInput.cfg', &
          VBRDF_Sup_In, &
...
!  VBRDF call
        ...
        if ( VBRDF_Sup_OutputStatus%BS_STATUS_OUTPUT .ne. vlidort_success ) THEN
           write(c1,'(i1)')t ; TRACE = 'VBRDF_MAINMASTER failed, perturbation # '//c1 ; go to 678
        endif
...
!  Write files of Surface WF test results
!  ======================================

!  Title of output file

      OutUnit = 36
      OPEN(OutUnit,file='vlidort_s_test/results_brdfplus_tester.all',status='unknown')
