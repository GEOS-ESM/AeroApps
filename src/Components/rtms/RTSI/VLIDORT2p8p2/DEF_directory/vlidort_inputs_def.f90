...
!  Version 3.8, Inclusion of phase matrices, 03 March 2017
...
!  4/15/20. Version 2.8.2. Summary of Changes
!     1. MAXMOMENTS_INPUT ==> MAXMOMENTS
!     2. Removed several Modified Structures.
!     3. Added DOUBLET_GEOMETRY flag.
...
!  4/15/20. Version 2.8.2. The following Modified structures have been removed.
!          VLIDORT_Modified_Control, VLIDORT_Modified_Chapman, VLIDORT_Modified_Optical

!  4/15/20. Version 2.8.8.2. Only need MAXMOMENTS in this list, not MAXMOMENTS_INPUT

      USE VLIDORT_PARS_m, Only : fpk, MAXMOMENTS, MAXSTOKES, MAXLAYERS, MAXSTREAMS, MAX_GEOMETRIES,    &
...
!  4/15/20. Version 2.8.2. OMEGA_TOTAL_INPUT is now a fixed optical (formerly MOptical)

      REAL(fpk), dimension ( MAXLAYERS ) :: TS_DELTAU_VERT_INPUT
      REAL(fpk), dimension ( MAXLAYERS ) :: TS_OMEGA_TOTAL_INPUT

!  Phase function Generalized-spherical-function expansion coefficients
!            Version 2.8.1: Include all that you require for exact single scatter calculations
!   4/15/20. Version 2.8.2. Now require the maximum number of coefficients only for MS calculations

      REAL(fpk), dimension ( 0:MAXMOMENTS, MAXLAYERS, MAXSTOKES_SQ ) :: TS_GREEKMAT_TOTAL_INPUT
...
!  Outgoing sphericity - criticality flag
!  10/31/19. New for Version 3.8.2 

      LOGICAL    :: TS_DO_FOCORR_DOCRIT
...
!   4/15/20. Version 2.8.2. Variable is removed
...
!   4/15/20. Version 2.8.2. Variable is Retired. Now doing this automatically
...
!  4/15/20. Version 2.8.2. Doublet-Geometry input control.
...
!  Number of GSF Phase Matrix expansion coefficients.  May be re-set after Checking

      INTEGER   :: TS_NGREEK_MOMENTS_INPUT

!  Geometry specification height
!   4/15/20. Version 2.8.2. Moved here

      REAL(fpk) :: TS_GEOMETRY_SPECHEIGHT

!  Earth radius in [km] for Chapman function calculation
!  4/15/20. Version 2.8.2. Moved here

      REAL(fpk) :: TS_EARTH_RADIUS
      
!  Outgoing sphericity - criticality attenuation criterion
!  4/15/20. New for Version 2.8.2 

      REAL(fpk)  :: TS_DO_FOCORR_ACRIT
...
!  4/15/20. Version 2.8.2. Move Earth Radius to Modified_Sunrays
!  4/15/20. Version 2.8.2. This type structure no longer required here. Entries now in Geometry TS
...
!  4/15/20. Version 2.8.2. This type structure no longer required here. OMEGA_TOTAL_INPUT Moved to Fixed optical.
...
!  4/15/20. Version 2.8.2. MChapman and MOptical not required anymore.
...
!  4/15/20. Version 2.8.2. MChapman and MOptical removed from this list.
