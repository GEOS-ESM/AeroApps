...
! #                                                             #
! #            VLIDORT_RTCalc_LCSMaster (top-level master)      #
...
MODULE VLIDORT_RTCalc_LCSmaster_m

!  Module for calling VLIDORT, radiances and Column/Surface Jacobians
...
!  everything public, except the Fourier Routine
   ...
   PUBLIC  :: VLIDORT_RTCalc_LCSMaster
   PRIVATE :: VLIDORT_LCS_FOURIER

   CONTAINS

SUBROUTINE VLIDORT_RTCalc_LCSMaster ( &
...
!  Total number of column and profile Jacobians
...
!  Single scatter and Direct bounce Column and surface weighting functions
...
!  SS/DB Profile Jacobian Results at all angles and optical depths
...
!  Local quantities for the water-leaving configuration
      ...
      DOUBLE PRECISION :: LC_SLTERM_LOCAL(MAX_ATMOSWFS), LS_SLTERM_LOCAL(MAX_SLEAVEWFS), TFACTOR, TRANS
...
!  4/15/20. Version 2.8.2. No longer need NMOMENTS_INPUT, replace by NSTREAMS_2 = 2 * NSTREAMS
      ...
      L_GREEKMAT_TOTAL_INPUT(1:N_TOTALCOLUMN_WFS,0:NSTREAMS_2,1:NLAYERS,:) = &
        VLIDORT_LinFixIn%Optical%TS_L_GREEKMAT_TOTAL_INPUT(1:N_TOTALCOLUMN_WFS,0:NSTREAMS_2,1:NLAYERS,:)
...
!mick fix 9/19/2017 - swapped layer & geo indices. Local is same as input now,
      ...
      IF ( VLIDORT_ModIn%MBool%TS_DO_SOLAR_SOURCES ) THEN
        L_FMATRIX_UP (1:N_TOTALCOLUMN_WFS,1:NLAYERS,1:N_GEOMETRIES,:) = &
          VLIDORT_LinFixIn%Optical%TS_L_FMATRIX_UP(1:N_TOTALCOLUMN_WFS,1:NLAYERS,1:N_GEOMETRIES,:)
        L_FMATRIX_DN (1:N_TOTALCOLUMN_WFS,1:NLAYERS,1:N_GEOMETRIES,:) = &
          VLIDORT_LinFixIn%Optical%TS_L_FMATRIX_DN(1:N_TOTALCOLUMN_WFS,1:NLAYERS,1:N_GEOMETRIES,:)
      ENDIF
      IF ( .not. VLIDORT_Bookkeep%DO_FOCORR_ALONE ) THEN
        L_GREEKMAT_TOTAL_INPUT(1:N_TOTALCOLUMN_WFS,0:NSTREAMS_2,1:NLAYERS,:) = &
          VLIDORT_LinFixIn%Optical%TS_L_GREEKMAT_TOTAL_INPUT(1:N_TOTALCOLUMN_WFS,0:NSTREAMS_2,1:NLAYERS,:)
...
!mick fix 3/2/2020 - added DO_COLUMN_LINEARIZATION & DO_SURFACE_LINEARIZATION IF conditions

         IF (DO_COLUMN_LINEARIZATION .OR. DO_SURFACE_LINEARIZATION) THEN
           IF ( DO_FOCORR_EXTERNAL ) THEN
            COLUMNWF_SS  = VLIDORT_LinSup%SS%Col%TS_COLUMNWF_SS
            COLUMNWF_DB  = VLIDORT_LinSup%SS%Col%TS_COLUMNWF_DB
            SURFACEWF_DB = VLIDORT_LinSup%SS%Surf%TS_SURFACEWF_DB
           ELSE
             COLUMNWF_SS = ZERO ; COLUMNWF_DB = ZERO ; SURFACEWF_DB = ZERO
           ENDIF
...
!  4/15/20. Version 2.8.2. Now added exception handling (for performance output).
        ...
        IF ( STATUS_SUB .EQ. VLIDORT_SERIOUS) THEN
          ...
          TRACE_3 = 'VLIDORT_MISCSETUPS failed in Linearization option, called by VLIDORT_RTCALC_LCSMASTER'
...
!  ----------------
            ...
            IF ( .not. DO_OBSERVATION_GEOMETRY ) THEN
              ...
              CALL LC_EMULT_MASTER ( &
                ...
                NLAYERS, N_SZANGLES, N_USER_VZANGLES, N_PARTLAYERS, N_TOTALCOLUMN_WFS, & ! Input numbers
                PARTLAYERS_LAYERIDX, STERM_LAYERMASK_UP, STERM_LAYERMASK_DN,           & ! Input Level control
...
!  ----------------------
            ...
            ELSE
              ...
              CALL LC_EMULT_MASTER_OBSGEO ( &
                DO_UPWELLING, DO_DNWELLING, DO_PLANE_PARALLEL, TAYLOR_ORDER,      & ! Input flags + Taylor
                NLAYERS, N_SZANGLES, N_PARTLAYERS, N_TOTALCOLUMN_WFS,             & ! Input numbers
                PARTLAYERS_LAYERIDX, STERM_LAYERMASK_UP, STERM_LAYERMASK_DN,      & ! Input Level control
                USER_SECANTS, DELTAU_VERT, PARTAU_VERT, L_DELTAU_VERT,            & ! Input optical, streams   
                BEAM_CUTOFF, T_DELT_MUBAR, EMULT_HOPRULE, SIGMA_M, SIGMA_P,       & ! Input solar beam + Multipliers
                T_DELT_USERM, T_UTUP_USERM, T_UTDN_USERM, ITRANS_USERM,           & ! Input User-stream Trans.
...
!  4/15/20. Version 2.8.2. Now added exception handling (for performance output).
        ...
        IF ( STATUS_SUB .EQ. VLIDORT_SERIOUS) THEN
          TRACE_2 = 'PERFORMANCE SETUP failed, called by VLIDORT_MISCSETUPS'
          TRACE_3 = 'VLIDORT_MISCSETUPS failed in Regular option, called by VLIDORT_RTCALC_LCSMASTER'
          STATUS_CALCULATION = VLIDORT_SERIOUS
          VLIDORT_Out%Status%TS_STATUS_CALCULATION = STATUS_CALCULATION
          VLIDORT_Out%Status%TS_MESSAGE = MESSAGE
          VLIDORT_Out%Status%TS_TRACE_1 = TRACE_1
...
!      -- Add DO_DOUBLET_GEOMETRY flag and Offset array (SZD_OFFSET)
             ...
             CALL VFO_LCS_MASTER_INTERFACE ( &
                ...
                N_TOTALCOLUMN_WFS, LVARYFMAT, N_SLEAVE_WFS, N_SURFACE_WFS, N_TOTALSURFACE_WFS,    & ! Input numbers (Jacobians)
                ...
                FO_CUMTRANS, FO_SLTERM, FO_LC_CUMTRANS, FO_LSSL_SLTERM )                            ! Output - Auxiliary
...
!  column Jacobians
            ...
            IF ( DO_COLUMN_LINEARIZATION ) THEN
              if ( do_upwelling ) then
                do v = 1, N_GEOMETRIES
                  VLIDORT_LinSup%SS%Col%TS_COLUMNWF_SS(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,v,1:NSTOKES,UPIDX) = &
                     FO_COLUMNWF_ATMOS(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,v,1:NSTOKES,UPIDX)
                  VLIDORT_LinSup%SS%Col%TS_COLUMNWF_DB(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,v,1:NSTOKES) = &
                     FO_COLUMNWF_SURF(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,v,1:NSTOKES)
                enddo
              endif
              if ( do_dnwelling ) then
                do v = 1, N_GEOMETRIES
                  VLIDORT_LinSup%SS%Col%TS_COLUMNWF_SS(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,v,1:NSTOKES,DNIDX) = &
                     FO_COLUMNWF_ATMOS(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,v,1:NSTOKES,DNIDX)
                enddo
              endif
            ENDIF
...
!  4/15/20. Version 2.8.2. Fourier SLEAVE copying now moved into Fourier loop.
...
!  error handling
        ...
        IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
          TRACE_3 = ' Called by VLIDORT_RTCALC_LCSMASTER '
          STATUS_CALCULATION = VLIDORT_SERIOUS
          VLIDORT_Out%Status%TS_STATUS_CALCULATION = STATUS_CALCULATION
          VLIDORT_Out%Status%TS_MESSAGE = MESSAGE
          VLIDORT_Out%Status%TS_TRACE_1 = TRACE_1
...
!    ==> Any number of column wfs allowed for Fourier 0, 1, 2
...
!                                ==> Replace arrays COLUMNWF, SURFACEWF and associated FO inputs
...
!mick fix 9/6/2012 - added N_SLEAVE_WFS to call
                ...
                IF ( DO_LINEARIZATION ) THEN
                  CALL VLIDORT_LCS_CONVERGE_OBSGEO ( &
                    ...
                    NSTOKES, N_USER_LEVELS, N_TOTALCOLUMN_WFS, N_SURFACE_WFS, N_SLEAVE_WFS,        & ! Input lin control
                    IBEAM, FOURIER, N_DIRECTIONS, WHICH_DIRECTIONS,                                & ! Input bookkeeping
                    AZMFAC, COLUMNWF_F, SURFACEWF_F, VLIDORT_LinSup%SS, VLIDORT_LinOut )             ! Input Azm, fields
                ENDIF
...
!  Fourier summation of linearization quantities
                ...
                IF ( DO_LINEARIZATION ) THEN
                  CALL VLIDORT_LCS_CONVERGE_DOUBLET( &
                    DO_FOCORR, DO_FOCORR_ALONE, DO_UPWELLING, DO_NO_AZIMUTH, DO_DBCORRECTION,        & ! Input flags
                    DO_COLUMN_LINEARIZATION, DO_SURFACE_LINEARIZATION, DO_LAMBERTIAN_SURFACE,        & ! Input flags
                    NSTOKES, N_USER_VZANGLES, N_OUT_STREAMS, N_USER_LEVELS,                          & ! Input control numbers
...
!mick fix 9/6/2012 - added N_SLEAVE_WFS to call
                ...
                IF ( DO_LINEARIZATION ) THEN
                  CALL VLIDORT_LCS_CONVERGE ( &
                    DO_FOCORR, DO_FOCORR_ALONE, DO_UPWELLING, DO_NO_AZIMUTH, DO_DBCORRECTION,        & ! Input flags
                    DO_COLUMN_LINEARIZATION, DO_SURFACE_LINEARIZATION, DO_LAMBERTIAN_SURFACE,        & ! Input flags
                    NSTOKES, N_USER_VZANGLES, LOCAL_UM_START, N_OUT_STREAMS, N_USER_LEVELS,          & ! Input control numbers
                    N_TOTALCOLUMN_WFS, N_SURFACE_WFS, N_SLEAVE_WFS,                                  & ! Input linearization control
                    IBEAM, FOURIER, VZA_OFFSETS, N_DIRECTIONS, WHICH_DIRECTIONS,                     & ! Input bookkeeping
                    LOCAL_N_USERAZM, AZMFAC, COLUMNWF_F, SURFACEWF_F, VLIDORT_LinSup%SS, VLIDORT_LinOut )  ! Input Azm, fields
                ENDIF
...
!          IF ( DO_LINEARIZATION ) THEN
!            CALL VLIDORT_L_WRITEFOURIER ( DO_INCLUDE_SURFACE, FUNIT, FOURIER )
!          ENDIF
...
!  Column weighting functions for Stokes vectors and fluxes
!  --------------------------------------------------------
...
!  4/15/20. Version 2.8.2. Integrated values - Only  copy if necessary.
      ...
      IF ( DO_MVOUT_ONLY .or. DO_ADDITIONAL_MVOUT ) THEN
         IF ( DO_UPWELLING ) THEN
           VLIDORT_LinOut%Col%TS_MEANST_DIFFUSE_COLWF(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,UPIDX) = &
                                 MEANST_DIFFUSE_COLWF(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,UPIDX)
           VLIDORT_LinOut%Col%TS_FLUX_DIFFUSE_COLWF(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,UPIDX) = &
                                 FLUX_DIFFUSE_COLWF(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,UPIDX)
         ENDIF
         IF ( DO_DNWELLING ) THEN
           VLIDORT_LinOut%Col%TS_MEANST_DIFFUSE_COLWF (1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,DNIDX) = &
                                 MEANST_DIFFUSE_COLWF (1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,DNIDX)
           VLIDORT_LinOut%Col%TS_FLUX_DIFFUSE_COLWF   (1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,DNIDX) = &
                                 FLUX_DIFFUSE_COLWF   (1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,DNIDX)
...
!   4/15/20. Version 2.8.2. Only fill out what you need
      ...
      IF ( DO_ALBTRN_MEDIA(1) ) THEN
        VLIDORT_LinOut%Col%TS_ALBMED_USER_COLWF(1:NSTOKES,1:N_USER_VZANGLES,:) = LC_ALBMED_USER(1:NSTOKES,1:N_USER_VZANGLES,:)
        VLIDORT_LinOut%Col%TS_ALBMED_FLUXES_COLWF(1:NSTOKES,:,:)               = LC_ALBMED_FLUXES(1:NSTOKES,:,:)
      ENDIF
      IF ( DO_ALBTRN_MEDIA(2) ) THEN
        VLIDORT_LinOut%Col%TS_TRNMED_USER_COLWF(1:NSTOKES,1:N_USER_VZANGLES,:) = LC_TRNMED_USER(1:NSTOKES,1:N_USER_VZANGLES,:)
...
!  Surface weighting functions for Stokes vectors and fluxes
!  ---------------------------------------------------------
...
!  4/15/20. Version 2.8.2. Argument list rearranged/renamed, follows LIDORT Version 2.8.2.
...
!  4/15/20. Version 2.8.2. Remove NMOMS, Only printing First Fourier componnet.
      ...
      END SUBROUTINE VLIDORT_RTCALC_LCSMASTER
...
!                       db code, and fo code up to VLIDORT_RTCALC_LCSMASTER.  There, the needed
...
!  Linearized
      ...
      IF ( DO_ATMOS_LINEARIZATION ) THEN
        ...
        CALL VLIDORT_UNPACK_LAC_MISC ( LAC_Misc,                & ! Input
          NSTOKES, NSTREAMS, NLAYERS, N_PARTLAYERS,             & ! Input
          N_SZANGLES, N_USER_STREAMS, NMOMENTS, N_TOTALCOLUMN_WFS,  & ! Input
...
!                     in VLIDORT_RTCALC_LCSMASTER & passed down VIA the "Misc" type structure)
...
!  End Module
      ...
      END MODULE VLIDORT_RTCalc_LCSmaster_m
