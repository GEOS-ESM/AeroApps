...
! #                                                             #
! #            VLIDORT_RTCalc_LPSMaster (top-level master)      #
...
!       - Limitations on zeroing and internal copying.
!       - Use of dynamic memory allocation (started).
!       - Option to use doublet-geometry post-processing

MODULE VLIDORT_RTCalc_LPSmaster_m

!  Module for calling VLIDORT, radiances and Profile/Surface Jacobians
...
!  everything public, except the Fourier Routine
   ...
   PUBLIC  :: VLIDORT_RTCalc_LPSMaster
   PRIVATE :: VLIDORT_LPS_FOURIER

   CONTAINS

SUBROUTINE VLIDORT_RTCalc_LPSMaster ( &
      do_debug_input, nstreams_dm, nmoments_dm, nlayers_dm,  & ! debug write and Proxies for dynamic memory (DM)
      VLIDORT_FixIn,    VLIDORT_ModIn,    VLIDORT_Sup,       & ! INPUTS (possibly modified). Sup Inputs/Outputs
      VLIDORT_Bookkeep, VLIDORT_FOGeom,   VLIDORT_MSGeom,    & ! SETUPS (possibly modified)
      VLIDORT_LinFixIn, VLIDORT_LinModIn, VLIDORT_LinSup,    & ! INPUTS, Sup In/Out. Linearized
      VLIDORT_Out, VLIDORT_LinOut )                            ! OUTPUTS

!  parameter file

      USE VLIDORT_PARS_m

!  I/O Structures for VLIDORT

      USE VLIDORT_IO_DEFS_m
      USE VLIDORT_LIN_IO_DEFS_m

!  Setups Structures for LIDORT (Bookkeeping, Geometries)
...
!mick fix 3/2/2020 - re-added use of VLIDORT_CHECK_INPUT_DIMS and VLIDORT_L_CHECK_INPUT

      USE VLIDORT_INPUTS_m,     Only    : VLIDORT_CHECK_INPUT_DIMS, VLIDORT_CHECK_INPUT_OPTICAL
      USE VLIDORT_L_INPUTS_m,   Only    : VLIDORT_L_CHECK_INPUT, VLIDORT_L_CHECK_INPUT_DIMS
...
!  Misc setups and multipliers
      ...
      USE VLIDORT_LP_MISCSETUPS_m, Only : VLIDORT_LAP_MISCSETUPS, LP_EMULT_MASTER, LP_EMULT_MASTER_OBSGEO
...
!  4/15/20. Version 2.8.2. Converge modules now separated
...
!  LIDORT Setups, Type structures, Bookkeeping, FO and MS Geometry outputs
!  4/15/20. Version 2.8.2. New
...
!  4/15/20. Version 2.8.2. Return to Intent(InOut). Outputs must be pre-zeroed.
...
!  Total number of column/profile Jacobians

      INTEGER ::            N_TOTALCOLUMN_WFS
      INTEGER ::            N_TOTALPROFILE_WFS

!  Total number of Surface/Sleave Jacobians
!    Added TotalSurface 9/18/16, N_TOTALSURFACE = N_SURFACE + N_SLEAVE
...
!  4/15/20. Version 2.8.2. Replace 0:MAXMOMENTS_INPUT by 0:MAXMOMENTS. Only needed for MS calculations
      ...
      DOUBLE PRECISION ::   L_GREEKMAT_TOTAL_INPUT ( MAX_ATMOSWFS, 0:MAXMOMENTS, MAXLAYERS, MAXSTOKES_SQ )
...
!  Single scatter and Direct bounce Profile and surface weighting functions
!   4/15/20. Version 2.8.2. Not used except for debug.
...
!  SS/DB Column Jacobian Results at all angles and optical depths
...
!  Mean intensity (actinic flux), Regular Flux, Diffuse values, weighting functions

      DOUBLE PRECISION :: MEANST_DIFFUSE_PROFWF &
          ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_SZANGLES, MAXSTOKES, MAX_DIRECTIONS )
      DOUBLE PRECISION :: FLUX_DIFFUSE_PROFWF &
          ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_SZANGLES, MAXSTOKES, MAX_DIRECTIONS )

!  Mean intensity (actinic flux) and regular flux weighting functions. Direct beam only.

      DOUBLE PRECISION :: DNMEANST_DIRECT_PROFWF &
          ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_SZANGLES, MAXSTOKES )
      DOUBLE PRECISION :: DNFLUX_DIRECT_PROFWF &
          ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_SZANGLES, MAXSTOKES )

!  4/26-29/19. Special Media-property output. -- Introduced by R. Spurr.
!     ** Linearized Output developed for Profile Jacobians.

      DOUBLE PRECISION :: LP_ALBMED_USER  ( MAXSTOKES, MAX_USER_STREAMS, MAXLAYERS, MAX_ATMOSWFS ) !  TOA illumination
      DOUBLE PRECISION :: LP_TRNMED_USER  ( MAXSTOKES, MAX_USER_STREAMS, MAXLAYERS, MAX_ATMOSWFS ) !  BOA illumination
      
      DOUBLE PRECISION :: LP_ALBMED_FLUXES ( MAXSTOKES, 2, MAXLAYERS, MAX_ATMOSWFS )    !  TOA illumination
      DOUBLE PRECISION :: LP_TRNMED_FLUXES ( MAXSTOKES, 2, MAXLAYERS, MAX_ATMOSWFS )    !  BOA illumination
      
      DOUBLE PRECISION :: LP_TRANSBEAM ( MAXSTOKES, MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS ) !  Planetary problem
...
!  BLACKBODY Jacobians, New 18 March 2014. Version 2.7, Enabled in Version 2.8
!  ===========================================================================
...
!    SOLARBEAM_BOATRANS is computed with Unscaled optical depths       ==> derivative  LP_SOLARBEAM_BOATRANS
!    TRANS_SOLAR_BEAM with scaled ODs. [ Same if  no Deltam-scaling]   ==> derivative  LP_SOLARBEAM_ATRANS

      DOUBLE PRECISION :: LP_SOLARBEAM_BOATRANS  ( MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION :: LP_SOLARBEAM_ATRANS    ( MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS )
...
!  Fourier-component Profile weighting functions at user angles
      ...
      DOUBLE PRECISION :: PROFILEWF_F &
          ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_USER_VZANGLES, MAX_SZANGLES, MAXSTOKES, MAX_DIRECTIONS )
...
!  Solar beam attenuation to BOA (required for exact DB calculation)
!      DOUBLE PRECISION :: LP_BOA_ATTN(MAX_ATMOSWFS,MAX_GEOMETRIES)
...
!   4/15/20. Version 2.8.2. DO_SCATMAT_VARIATION renamed LVARYFMAT

      LOGICAL          :: LVARYFMAT      ( MAXLAYERS, MAX_ATMOSWFS )
...
!  Version 2.8, dimensioning changed USER_LEVELS --> PARTLAYERS
      ...
      DOUBLE PRECISION :: LP_T_UTDN_MUBAR ( MAX_PARTLAYERS, MAXLAYERS, MAXBEAMS, MAX_ATMOSWFS )

!mick fix 9/19/2017 - added LP_LEVELS_SOLARTRANS & LP_PARTIALS_SOLARTRANS
...
!  from VFO_RTCALC_LPSMASTER_INTERFACE or VFO_MASTER_INTERFACE
...
!  from VFO_RTCALC_LPSMASTER_INTERFACE: Profile Weighting functions
      ...
      DOUBLE PRECISION :: FO_PROFILEWF_ATMOS &
          ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_GEOMETRIES, MAXSTOKES, MAX_DIRECTIONS )
      DOUBLE PRECISION :: FO_PROFILEWF_SURF &
          ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_GEOMETRIES, MAXSTOKES )
      DOUBLE PRECISION :: FO_PROFILEWF &
          ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_GEOMETRIES, MAXSTOKES, MAX_DIRECTIONS )

!  from VFO_RTCALC_LPSMASTER_INTERFACE: Surface Jacobians (only DB and DTS)
...
!  4/9/19. Additional output from the SFO Interface, for the sleave correction
      ...
      DOUBLE PRECISION :: FO_LP_CUMTRANS ( max_user_levels, MAX_GEOMETRIES, MAXLAYERS, MAX_ATMOSWFS )
...
!    --  Also depends on surface/sleave quantities, but these linearizations (LS/LSSL) are neglected
      ...
      DOUBLE PRECISION :: LP_TRANS_ATMOS_FINAL   ( MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS )
...
!  Local quantities for the water-leaving configuration
      ...
      DOUBLE PRECISION :: LP_SLTERM_LOCAL(MAX_ATMOSWFS), LS_SLTERM_LOCAL(MAX_SLEAVEWFS), TFACTOR, TRANS
...
!  Misc. help (4/15/20, Version 2.8.2, some additions, removed LOCAL_N_USERAZM, IUNIT)

      INTEGER ::          OFF, O1, UA, UM, IB, G, G1, G2, K, L, V, N, Q, Q1, UT, UTA, LUM, LUA, NMOMS, NWFS
...
!  --------------------------------------------------------------------
!    - Introduced 1/12/16, Validated 2/3/16.
...
!  4/15/20. Version 2.8.2. No longer need NMOMENTS_INPUT, replace by NSTREAMS_2 = 2 * NSTREAMS
!mick fix 3/2/2020 - changed WF dimensions on these internal arrays from N_TOTALCOLUMN_WFS to
!                    N_TOTALPROFILE_WFS

     L_DELTAU_VERT_INPUT(1:N_TOTALPROFILE_WFS,1:NLAYERS) = &
         VLIDORT_LinFixIn%Optical%TS_L_DELTAU_VERT_INPUT(1:N_TOTALPROFILE_WFS,1:NLAYERS)
     L_OMEGA_TOTAL_INPUT(1:N_TOTALPROFILE_WFS,1:NLAYERS) = &
         VLIDORT_LinFixIn%Optical%TS_L_OMEGA_TOTAL_INPUT(1:N_TOTALPROFILE_WFS,1:NLAYERS)
     L_GREEKMAT_TOTAL_INPUT(1:N_TOTALPROFILE_WFS,0:NSTREAMS_2,1:NLAYERS,:) = &
         VLIDORT_LinFixIn%Optical%TS_L_GREEKMAT_TOTAL_INPUT(1:N_TOTALPROFILE_WFS,0:NSTREAMS_2,1:NLAYERS,:)
...
!                     N_TOTALPROFILE_WFS
      ...
      IF ( VLIDORT_ModIn%MBool%TS_DO_SOLAR_SOURCES ) THEN
        L_FMATRIX_UP (1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_GEOMETRIES,:) = &
          VLIDORT_LinFixIn%Optical%TS_L_FMATRIX_UP(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_GEOMETRIES,:)
        L_FMATRIX_DN (1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_GEOMETRIES,:) = &
          VLIDORT_LinFixIn%Optical%TS_L_FMATRIX_DN(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_GEOMETRIES,:)
      ENDIF
      IF ( .not. VLIDORT_Bookkeep%DO_FOCORR_ALONE ) THEN
        L_GREEKMAT_TOTAL_INPUT(1:N_TOTALPROFILE_WFS,0:NSTREAMS_2,1:NLAYERS,:) = &
          VLIDORT_LinFixIn%Optical%TS_L_GREEKMAT_TOTAL_INPUT(1:N_TOTALPROFILE_WFS,0:NSTREAMS_2,1:NLAYERS,:)
      ...
      IF ( .NOT.DO_LAMBERTIAN_SURFACE .AND. DO_SURFACE_LINEARIZATION ) THEN
        IF ( DO_USER_VZANGLES ) THEN
          LS_EXACTDB_BRDFUNC(1:N_SURFACE_WFS,:,1:N_USER_VZANGLES,1:N_USER_RELAZMS,1:N_SZANGLES) = &
            VLIDORT_LinSup%BRDF%TS_LS_EXACTDB_BRDFUNC(1:N_SURFACE_WFS,:,1:N_USER_VZANGLES,1:N_USER_RELAZMS,1:N_SZANGLES)
        ENDIF
      ENDIF
...
!                                Fourier SLEAVE copying now moved into Fourier loop.
      ...
      IF ( DO_SURFACE_LEAVING .AND. DO_SLEAVE_WFS ) THEN
        LSSL_SLTERM_ISOTROPIC(1:N_SLEAVE_WFS,1:NSTOKES,1:N_SZANGLES) = &
          VLIDORT_LinSup%SLEAVE%TS_LSSL_SLTERM_ISOTROPIC(1:N_SLEAVE_WFS,1:NSTOKES,1:N_SZANGLES)
        IF ( DO_USER_VZANGLES ) THEN
          LSSL_SLTERM_USERANGLES(1:N_SLEAVE_WFS,1:NSTOKES,1:N_USER_VZANGLES,1:N_USER_RELAZMS,1:N_SZANGLES) = &
     VLIDORT_LinSup%SLEAVE%TS_LSSL_SLTERM_USERANGLES(1:N_SLEAVE_WFS,1:NSTOKES,1:N_USER_VZANGLES,1:N_USER_RELAZMS,1:N_SZANGLES)
        ENDIF
      ENDIF

!  Linearized SS inputs
!  --------------------
!  New 12 March 2012 --> IF Linearized SS results already available copy them! Modified flagging, Version 2.8
!  4/15/20. Version 2.8.2. No Longer requires this code, now using type structure inputs directly.
!      IF ( DO_FOCORR_EXTERNAL ) THEN
!         PROFILEWF_SS(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,:,1:NSTOKES,:)  = &
!           VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_SS(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,:,1:NSTOKES,:)
!         PROFILEWF_DB(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,:,1:NSTOKES) = &
!           VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_DB(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,:,1:NSTOKES)
...
!  Chapman factors
!mick fix 3/2/2020 - modified defining of local Chapman factor arrays
...
!  Set local BRDF Fourier arrays for Fourier=0
         ...
         IF ( .NOT.DO_LAMBERTIAN_SURFACE ) THEN
           ...
           IF ( DO_SURFACE_LINEARIZATION ) THEN
             LS_BRDF_F_0(1:N_SURFACE_WFS,1:MAXSTOKES_SQ,1:NSTREAMS,1:N_SZANGLES) = &
                VLIDORT_LinSup%BRDF%TS_LS_BRDF_F_0(1:N_SURFACE_WFS,FOURIER,1:MAXSTOKES_SQ,1:NSTREAMS,1:N_SZANGLES)
             LS_BRDF_F(1:N_SURFACE_WFS,1:MAXSTOKES_SQ,1:NSTREAMS,1:NSTREAMS) = &
                VLIDORT_LinSup%BRDF%TS_LS_BRDF_F(1:N_SURFACE_WFS,FOURIER,1:MAXSTOKES_SQ,1:NSTREAMS,1:NSTREAMS)
             IF ( DO_USER_VZANGLES ) THEN
               LS_USER_BRDF_F_0(1:N_SURFACE_WFS,1:MAXSTOKES_SQ,1:N_USER_VZANGLES,1:N_SZANGLES) = &
                  VLIDORT_LinSup%BRDF%TS_LS_USER_BRDF_F_0(1:N_SURFACE_WFS,FOURIER,1:MAXSTOKES_SQ,1:N_USER_VZANGLES,1:N_SZANGLES)
               LS_USER_BRDF_F(1:N_SURFACE_WFS,1:MAXSTOKES_SQ,1:N_USER_VZANGLES,1:NSTREAMS) = &
                  VLIDORT_LinSup%BRDF%TS_LS_USER_BRDF_F(1:N_SURFACE_WFS,FOURIER,1:MAXSTOKES_SQ,1:N_USER_VZANGLES,1:NSTREAMS)
             ENDIF
           ENDIF
         ENDIF
...
!  Set local SLEAVE Fourier arrays for Fourier=0
         ...
         IF ( DO_SURFACE_LEAVING ) THEN
           ...
           IF ( DO_SURFACE_LINEARIZATION .AND. DO_SLEAVE_WFS ) THEN
             LSSL_SLTERM_F_0(1:N_SLEAVE_WFS,1:MAXSTOKES,1:NSTREAMS,1:N_SZANGLES) = &
                 VLIDORT_LinSup%SLEAVE%TS_LSSL_SLTERM_F_0(1:N_SLEAVE_WFS,FOURIER,1:MAXSTOKES,1:NSTREAMS,1:N_SZANGLES)
             IF ( DO_USER_VZANGLES ) THEN
               LSSL_USER_SLTERM_F_0(1:N_SLEAVE_WFS,1:MAXSTOKES,1:N_USER_VZANGLES,1:N_SZANGLES) = &
                   VLIDORT_LinSup%SLEAVE%TS_LSSL_USER_SLTERM_F_0(1:N_SLEAVE_WFS,FOURIER,1:MAXSTOKES,1:N_USER_VZANGLES,1:N_SZANGLES)
             ENDIF
           ENDIF
         ENDIF
...
!  New 12 March 2012 --> IF SS results already available, copy them otherwise initialize
!  Linearized Debug Call
!mick fix 3/2/2020 - added DO_PROFILE_LINEARIZATION & DO_SURFACE_LINEARIZATION IF conditions

         IF (DO_PROFILE_LINEARIZATION .OR. DO_SURFACE_LINEARIZATION) THEN
           IF ( DO_FOCORR_EXTERNAL ) THEN
            PROFILEWF_SS = VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_SS
            PROFILEWF_DB = VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_DB
            SURFACEWF_DB = VLIDORT_LinSup%SS%Surf%TS_SURFACEWF_DB
           ELSE
             PROFILEWF_SS = ZERO ; PROFILEWF_DB = ZERO ; SURFACEWF_DB = ZERO
           ENDIF
...
!  Main outputs (Stokes-vectors and fluxes)
!  ----------------------------------------
...
!  4/15/20. Version 2.8.2. No Longer required, now using type structure inputs directly.  
!      PROFILEWF(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,:,1:NSTOKES,:) = ZERO

!  Diffuse and direct-beam mean intensity and flux weighting functions

      MEANST_DIFFUSE_PROFWF(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,:) = ZERO
      FLUX_DIFFUSE_PROFWF  (1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,:) = ZERO
      DNMEANST_DIRECT_PROFWF(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES)  = ZERO
      DNFLUX_DIRECT_PROFWF  (1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES)  = ZERO
...
!  Special Media-property output
!  -----------------------------

!  Introduced 4/26/19 R. Spurr. Pre-initialized here.
...
!mick fix 8/20/2019 - initialize linearized media-property & planetary problem output
      ...
      LP_ALBMED_USER(1:NSTOKES,1:N_USER_VZANGLES,1:NLAYERS,1:MAX_ATMOSWFS) = ZERO
      LP_TRNMED_USER(1:NSTOKES,1:N_USER_VZANGLES,1:NLAYERS,1:MAX_ATMOSWFS) = ZERO
      
      LP_ALBMED_FLUXES(1:NSTOKES,1:2,1:NLAYERS,1:MAX_ATMOSWFS) = ZERO   
      LP_TRNMED_FLUXES(1:NSTOKES,1:2,1:NLAYERS,1:MAX_ATMOSWFS) = ZERO   
      
      LP_TRANSBEAM(1:NSTOKES,1:N_SZANGLES,1:NLAYERS,1:MAX_ATMOSWFS) = ZERO

!  Planetary problem output
!  4/15/20. Version 2.8.2. No Longer required, Should be pre-zeroed.
!      VLIDORT_LinOut%Prof%TS_PLANETARY_TRANSTERM_PROFWF (1:NSTOKES,:,1:NLAYERS,1:MAX_ATMOSWFS) = ZERO
!      VLIDORT_LinOut%Prof%TS_PLANETARY_SBTERM_PROFWF    (1:NLAYERS,1:MAX_ATMOSWFS) = ZERO

!  New 28 March 2014, Version 2.7. BLACKBODY Linearizations, initialized
!  ---------------------------------------------------------------------
...
!  Do need to check some linearized variables
...
!  1b.  VLIDORT_LAP_MISCSETUPS  : Linearizations of Item 1a
!  2a.  THERMAL_SETUP           : Coefficients, direct multipliers
!  2b.  THERMAL_SETUP_PLUS      : Coefficients, direct multipliers AND LINEARIZATIONS
!  3a. LP_EMULT_MASTER /        : Beam source function multipliers.  Not required for the
!  3b. LP_EMULT_MASTER_OBSGEO     Full SS calculation in outgoing mode
...
!  4/15/20. Version 2.8.2. Now added exception handling (for performance output).
        ...
        IF ( STATUS_SUB .EQ. VLIDORT_SERIOUS) THEN
          ...
          TRACE_3 = 'VLIDORT_MISCSETUPS failed in Linearization option, called by VLIDORT_RTCALC_LPSMASTER'
...
!mick fix 9/19/2017 - added DO_SOLAR_SOURCES to input
        ...
        CALL VLIDORT_LAP_MISCSETUPS ( &
          DO_SOLAR_SOURCES, DO_USER_VZANGLES, DO_DELTAM_SCALING,            & ! Input flags 
          DO_PLANE_PARALLEL, DO_SOLUTION_SAVING, DO_ATMOS_LINEARIZATION,    & ! Input flags 
          NSTOKES, NLAYERS, NSTREAMS, N_USER_VZANGLES, N_SZANGLES,          & ! Input numbers
          ...
          LAYER_VARY_FLAG, LAYER_VARY_NUMBER,                               & ! Input Lin control 
          ...
          LVARYFMAT, L_TRUNC_FACTOR,  L_OMEGA_GREEK,                             & ! Output Linearized scaled properties
...
!  Pack selected linearized MISCSETUPS results
        ...
        CALL VLIDORT_PACK_LAP_MISC ( &
          ...
          LP_AVERAGE_SECANT, LP_INITIAL_TRANS, LP_T_DELT_MUBAR,           & ! Input
          LP_T_UTDN_MUBAR, LP_LEVELS_SOLARTRANS, LP_PARTIALS_SOLARTRANS,  & ! Input
          LAP_Misc )                                                        ! Output
...
!  ----------------
            ...
            IF ( .not. DO_OBSERVATION_GEOMETRY ) THEN
              ...
              CALL LP_EMULT_MASTER ( &
                ...
                NLAYERS, N_SZANGLES, N_USER_VZANGLES, N_PARTLAYERS, LAYER_VARY_FLAG, LAYER_VARY_NUMBER, & ! Input numbers
                PARTLAYERS_LAYERIDX, STERM_LAYERMASK_UP, STERM_LAYERMASK_DN,       & ! Input Level control
...
!  ----------------------
            ...
            ELSE
              ...
              CALL LP_EMULT_MASTER_OBSGEO ( &
                DO_UPWELLING, DO_DNWELLING, DO_PLANE_PARALLEL, TAYLOR_ORDER,           & ! Input flags + Taylor
                NLAYERS, N_SZANGLES, N_PARTLAYERS, LAYER_VARY_FLAG, LAYER_VARY_NUMBER, & ! Input numbers
                PARTLAYERS_LAYERIDX, STERM_LAYERMASK_UP, STERM_LAYERMASK_DN,           & ! Input Level control
                USER_SECANTS, DELTAU_VERT, PARTAU_VERT, L_DELTAU_VERT,                 & ! Input optical, streams   
                BEAM_CUTOFF, T_DELT_MUBAR, EMULT_HOPRULE, SIGMA_M, SIGMA_P,            & ! Input solar beam + Multipliers
                T_DELT_USERM, T_UTUP_USERM, T_UTDN_USERM, ITRANS_USERM,                & ! Input User-stream Trans.
...
!  4/15/20. Version 2.8.2. Now added exception handling (for performance output).
        ...
        IF ( STATUS_SUB .EQ. VLIDORT_SERIOUS) THEN
          TRACE_2 = 'PERFORMANCE SETUP failed, called by VLIDORT_MISCSETUPS'
          TRACE_3 = 'VLIDORT_MISCSETUPS failed in Regular option, called by VLIDORT_RTCALC_LPSMASTER'
          STATUS_CALCULATION = VLIDORT_SERIOUS
          VLIDORT_Out%Status%TS_STATUS_CALCULATION = STATUS_CALCULATION
          VLIDORT_Out%Status%TS_MESSAGE = MESSAGE
          VLIDORT_Out%Status%TS_TRACE_1 = TRACE_1
...
!               TRACE_3 = 'VLIDORT_TRANSFLUX_MASTER failed, VLIDORT_RTCALC_LPSMASTER'
!               STATUS_CALCULATION = VLIDORT_SERIOUS
!               VLIDORT_Out%Status%TS_STATUS_CALCULATION = STATUS_CALCULATION
!               VLIDORT_Out%Status%TS_MESSAGE = MESSAGE
!               VLIDORT_Out%Status%TS_TRACE_1 = TRACE_1
...
!           endif
!         enddo
!  Scale the Non-isotropic terms, 12/24/15, 2/3/16. if flagged
!  Add the same factor for the linearized terms (if present). 12/18/15, 2/3/16
...
!mick mod 9/19/2017 - added FO_STOKES_ATMOS, FO_STOKES_SURF, FO_PROFILEWF_ATMOS,
!                     FO_PROFILEWF_SURF (new output from vector FO code)
...
!      -- Add DO_DOUBLET_GEOMETRY flag and Offset array (SZD_OFFSET)
            ...
            CALL VFO_LPS_MASTER_INTERFACE ( &
                ...
                DO_PLANE_PARALLEL, DO_FOCORR_OUTGOING, DO_DELTAM_SCALING,                         & ! Input SS control flags
                DO_UPWELLING, DO_DNWELLING, DO_OBSERVATION_GEOMETRY,                              & ! input Model control flags
                DO_DOUBLET_GEOMETRY, DO_PARTLAYERS, DO_LAMBERTIAN_SURFACE,                        & ! input Model control flags
                ...
                LAYER_VARY_FLAG, LAYER_VARY_NUMBER, LVARYFMAT, N_SLEAVE_WFS, N_SURFACE_WFS,       & ! Input numbers(J)
                N_TOTALSURFACE_WFS, VLIDORT_FOGeom, HEIGHT_GRID, SS_FLUX_MULTIPLIER, FLUXVEC,     & ! Input Geometry/Solar
                ...
                FO_CUMTRANS, FO_SLTERM, FO_LP_CUMTRANS, FO_LSSL_SLTERM )                            ! Output - Auxiliary
...
!mick fix 9/19/2017  - modified argument passing for SURFACEWF_DB from FO_SURFACEWF_DB to FO_SURFACEWF
!                    - changed 1st dim of SURFACEWF_DB & FO_SURFACEWF from N_SURFACE_WFS to N_TOTALSURFACE_WFS
!mick mod 9/19/2017  - added IF conditions
!mick note 3/22/2017 - Important! STOKES_SS, STOKES_DB, PROFILEWF_SS, PROFILEWF_DB, &
!                      SURFACEWF_DB contain BOTH solar AND thermal direct terms when computing in crossover region!
...
!  regular
            ...
            IF ( DO_PROFILE_LINEARIZATION ) THEN
              if ( do_upwelling ) then
                do v = 1, N_GEOMETRIES
                  do n = 1, nlayers
                    VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_SS(1:N_TOTALPROFILE_WFS,n,1:N_USER_LEVELS,v,1:NSTOKES,UPIDX) = &
                        FO_PROFILEWF_ATMOS(1:N_TOTALPROFILE_WFS,n,1:N_USER_LEVELS,v,1:NSTOKES,UPIDX)
                    VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_DB(1:N_TOTALPROFILE_WFS,n,1:N_USER_LEVELS,v,1:NSTOKES) = &
                        FO_PROFILEWF_SURF(1:N_TOTALPROFILE_WFS,n,1:N_USER_LEVELS,v,1:NSTOKES)
                  enddo
                enddo
              endif
              if ( do_dnwelling ) then
                do v = 1, N_GEOMETRIES
                  do n = 1, nlayers
                    VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_SS(1:N_TOTALPROFILE_WFS,n,1:N_USER_LEVELS,v,1:NSTOKES,DNIDX) = &
                        FO_PROFILEWF_ATMOS(1:N_TOTALPROFILE_WFS,n,1:N_USER_LEVELS,v,1:NSTOKES,DNIDX)
                  enddo
                enddo
              endif
            ENDIF
...
!  surface Jacobians
            ...
            IF ( DO_SURFACE_LINEARIZATION ) THEN
              if ( do_upwelling ) then
                do v = 1, N_GEOMETRIES
                  VLIDORT_LinSup%SS%Surf%TS_SURFACEWF_DB(1:N_TOTALSURFACE_WFS,1:N_USER_LEVELS,v,1:NSTOKES) = &
                      FO_SURFACEWF(1:N_TOTALSURFACE_WFS,1:N_USER_LEVELS,v,1:NSTOKES)
                enddo
              endif
            ENDIF
...
!                NSTOKES, NLAYERS, N_SZANGLES, N_SLEAVE_WFS, N_SURFACE_WFS,   &
...
!  4/15/20. Version 2.8.2. Copy Linearized Local BRDF Fourier-component Input (only what you need)
...
!  Call to Fourier routine
        ...
        CALL VLIDORT_LPS_FOURIER ( FOURIER,  &
            ...
            DO_ATMOS_LINEARIZATION, DO_PROFILE_LINEARIZATION, DO_SURFACE_LINEARIZATION,             & !Input Linearization flags
            DO_SIMULATION_ONLY, DO_SLEAVE_WFS, DO_ATMOS_LBBF, DO_SURFACE_LBBF,                      & !Input Linearization flags
            NSTOKES, NSTREAMS, NLAYERS, N_SZANGLES, N_USER_VZANGLES, N_USER_LEVELS,                 & !In Num(Basic)
            N_THERMAL_COEFFS, NMOMENTS, NSTREAMS_2, NTOTAL, N_SUBDIAG, N_SUPDIAG,                   & !In Num(Deriv)
            NSTKS_NSTRMS, NSTKS_NSTRMS_2, N_PARTLAYERS, N_DIRECTIONS, TAYLOR_ORDER,                 & !In Num(Deriv)
            N_TOTALPROFILE_WFS, LAYER_VARY_FLAG, LAYER_VARY_NUMBER, N_SURFACE_WFS, N_SLEAVE_WFS,                    & !In Num(Lin)
            N_ALLLAYERS_UP, N_ALLLAYERS_DN, FLUX_FACTOR, FLUXVEC, COS_SZANGLES, SZA_LOCAL_INPUT, SUN_SZA_COSINES,   & !In SZAs/FLUX
            USER_STREAMS, USER_SECANTS, QUAD_STREAMS, QUAD_WEIGHTS, QUAD_STRMWTS, QUAD_HALFWTS,                     & !In Strms
            ...
            STERM_LAYERMASK_UP, STERM_LAYERMASK_DN, DFLUX, LP_SOLARBEAM_BOATRANS, LP_SOLARBEAM_ATRANS,              & !In Bkkeep
            Misc, Therm, Mult, LAP_Misc, L_Therm, LP_Mult,                                                          & !In
            ALBEDO, BRDF_F, BRDF_F_0, USER_BRDF_F, USER_BRDF_F_0,                                                   & !In ALB/BRDF
            SLTERM_ISOTROPIC, SLTERM_F_0, USER_SLTERM_F_0, SURFACE_BB_INPUT, EMISSIVITY, USER_EMISSIVITY,           & !In SL/EMISS
            ...
            PROFILEWF_F, MEANST_DIFFUSE_PROFWF, FLUX_DIFFUSE_PROFWF, DNMEANST_DIRECT_PROFWF, DNFLUX_DIRECT_PROFWF,  & !Out COL JACS
            ...
            LP_TRANS_ATMOS_FINAL, LP_TRANSBEAM, LP_ALBMED_USER, LP_ALBMED_FLUXES, LP_TRNMED_USER, LP_TRNMED_FLUXES, & !Out 4/26/19 
            STATUS_SUB, MESSAGE, TRACE_1, TRACE_2, TRACE_3 )                                                          !Out Status

!do n = 1, nlayers
!  if ( FOURIER.eq.0)write(*,*)n,profilewf_F(1:2,n,1,1,1,1,1)
!enddo

!  error handling

        IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
          TRACE_3 = ' Called by VLIDORT_RTCALC_LPSMASTER '
          STATUS_CALCULATION = VLIDORT_SERIOUS
          VLIDORT_Out%Status%TS_STATUS_CALCULATION = STATUS_CALCULATION
          VLIDORT_Out%Status%TS_MESSAGE = MESSAGE
          VLIDORT_Out%Status%TS_TRACE_1 = TRACE_1
           ...
           if ( do_OBSERVATION_GEOMETRY ) THEN
              DO IB = 1, N_SZANGLES
                 SLTERM_LOCAL = FO_SLTERM(O1,ib) * TRANS_ATMOS_FINAL(ib)
                   ...
                   enddo
                   if ( do_profile_linearization ) then
                      DO K = 1, nlayers ; if ( layer_vary_flag(k) ) then
                         NWFS = Layer_vary_number(k)
                         LP_SLTERM_LOCAL(1:NWFS) = FO_SLTERM(O1,ib) * LP_TRANS_ATMOS_FINAL(ib,K,1:NWFS)
                         do uta = 1, n_user_levels ; DO Q = 1, NWFS
                            L_CUMSOURCE_DB =    FO_CUMTRANS(UTA,IB)     * LP_SLTERM_LOCAL(Q) &
                                           + FO_LP_CUMTRANS(UTA,IB,K,Q) *    SLTERM_LOCAL
                             VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_DB(Q,K,UTA,IB,O1) = &
                                     VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_DB(Q,K,UTA,IB,O1) + L_CUMSOURCE_DB
                         enddo ; enddo
                      endif ; enddo
                   endif
                   if ( do_water_leaving .and. DO_SLEAVE_WFS ) then
                      ...
                      do uta = 1, n_user_levels
                         DO Q = 1, NWFS
                            ...
                            VLIDORT_LinSup%SS%Surf%TS_SURFACEWF_DB(Q,UTA,IB,O1) = &
                                    VLIDORT_LinSup%SS%Surf%TS_SURFACEWF_DB(Q,UTA,IB,O1) + LS_CUMSOURCE_DB
                         enddo
                      enddo
                   endif
                enddo
                ...
                DO IB = 1, N_SZANGLES
                   ...
                   if ( do_profile_linearization ) then
                      DO K = 1, nlayers ; if ( layer_vary_flag(k) ) then
                         NWFS = Layer_vary_number(k)
                         LP_SLTERM_LOCAL(1:NWFS) = FO_SLTERM(O1,g1) * LP_TRANS_ATMOS_FINAL(ib,K,1:NWFS)
                         do uta = 1, n_user_levels ;  do g = g1, g2 ; DO Q = 1, NWFS
                            L_CUMSOURCE_DB =    FO_CUMTRANS(UTA,G)     * LP_SLTERM_LOCAL(Q) &
                                           + FO_LP_CUMTRANS(UTA,G,K,Q) *    SLTERM_LOCAL
                            VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_DB(Q,K,UTA,G,O1) = &
                                    VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_DB(Q,K,UTA,G,O1) + L_CUMSOURCE_DB
                         enddo ; enddo ; enddo
                      endif ; enddo
                   endif
                   if ( do_water_leaving .and. DO_SLEAVE_WFS ) then
                      ...
                      do uta = 1, n_user_levels
                         DO Q = 1, NWFS
                            LS_CUMSOURCE_DB =  LS_SLTERM_LOCAL(Q) ; Q1 = Q + N_SURFACE_WFS
                            VLIDORT_LinSup%SS%Surf%TS_SURFACEWF_DB(Q,UTA,G,O1) = &
                                    VLIDORT_LinSup%SS%Surf%TS_SURFACEWF_DB(Q,UTA,G,O1) + LS_CUMSOURCE_DB
                         enddo
                      enddo
                   endif
                enddo
...
!                 NWFS = Layer_vary_number(k)
!                 DO IB =1, NBEAMS ; DO O1 = 1, NSTOKES ; LUM = 1
!                   VLIDORT_LinOut%Prof%TS_TOAUP_RAYPROFWF_FOURIER(1:NWFS,K,LUM,IB,O1,M) = PROFILEWF_F(1:NWFS,K,UTA,LUM,IB,O1,UPIDX)
!                 ENDDO ; ENDDO
!               ENDIF ; ENDDO
!             ENDIF
...
!                 NWFS = Layer_vary_number(k)
!                 DO IB =1, NBEAMS ; DO UM = 1, N_USER_VZANGLES ; DO O1 = 1, NSTOKES
!                   VLIDORT_LinOut%Prof%TS_TOAUP_RAYPROFWF_FOURIER(1:NWFS,K,UM,IB,O1,M) = PROFILEWF_F(1:NWFS,K,UTA,UM,IB,O1,UPIDX)
!                 ENDDO ; ENDDO ; ENDDO
!               ENDIF ; ENDDO
!             ENDIF
!             if ( do_surface_linearization .and. Fourier.eq.0  ) then
...
!                     before call to VLIDORT_LPS_FOURIER to here
...
!  Convergence and Fourier-azimuth summation. Version 2.8, Clean up I/O listings, 7/8/16
!  4/15/20. Version 2.8.2. Use Type structure variables directly. 
!                                ==> Replace arrays STOKES, STOKES_SS, STOKES_DB
!                                ==> Replace arrays PROFILEWF, SURFACEWF and associated FO inputs
!                                ==> Add Doublet geometry option (new convergence routines)
...
!mick fix 9/6/2012 - added N_SLEAVE_WFS to call
                ...
                IF ( DO_LINEARIZATION ) THEN
                  CALL VLIDORT_LPS_CONVERGE_OBSGEO ( &
                   ...
                   NSTOKES, NLAYERS, N_USER_LEVELS, LAYER_VARY_FLAG, LAYER_VARY_NUMBER, N_SURFACE_WFS, & ! Input lin control
                   N_SLEAVE_WFS, IBEAM, FOURIER, N_DIRECTIONS, WHICH_DIRECTIONS,                       & ! Input bookkeeping
                   AZMFAC, PROFILEWF_F, SURFACEWF_F, VLIDORT_LinSup%SS, VLIDORT_LinOut )                 ! Input Azm, fields
                ENDIF
...
!  Fourier summation of linearization quantities
                ...
                IF ( DO_LINEARIZATION ) THEN
                  CALL VLIDORT_LPS_CONVERGE_DOUBLET( &
                    DO_FOCORR, DO_FOCORR_ALONE, DO_UPWELLING, DO_NO_AZIMUTH, DO_DBCORRECTION,        & ! Input flags
                    DO_PROFILE_LINEARIZATION, DO_SURFACE_LINEARIZATION, DO_LAMBERTIAN_SURFACE,       & ! Input flags
                    NSTOKES, NLAYERS, N_USER_VZANGLES, N_OUT_STREAMS, N_USER_LEVELS,                 & ! Input control numbers
...
!mick fix 9/6/2012 - added N_SLEAVE_WFS to call
                ...
                IF ( DO_LINEARIZATION ) THEN
                  CALL VLIDORT_LPS_CONVERGE ( &
                   DO_FOCORR, DO_FOCORR_ALONE, DO_UPWELLING, DO_NO_AZIMUTH, DO_DBCORRECTION,        & ! Input flags
                   DO_PROFILE_LINEARIZATION, DO_SURFACE_LINEARIZATION, DO_LAMBERTIAN_SURFACE,       & ! Input flags
                   NSTOKES, NLAYERS, N_USER_VZANGLES, LOCAL_UM_START, N_OUT_STREAMS, N_USER_LEVELS, & ! Input control numbers
                   LAYER_VARY_FLAG, LAYER_VARY_NUMBER, N_SURFACE_WFS, N_SLEAVE_WFS,                 & ! Input linearization control
                   IBEAM, FOURIER, VZA_OFFSETS, N_DIRECTIONS, WHICH_DIRECTIONS, LOCAL_N_USERAZM,    & ! Input bookkeeping
                   AZMFAC, PROFILEWF_F, SURFACEWF_F, VLIDORT_LinSup%SS, VLIDORT_LinOut )              ! Input Azm, fields
                ENDIF
...
!  ----------------------------
      ...
      IF ( DO_WRITE_RESULTS ) THEN
        ...
        !    MEANST_DIFFUSE_PROFWF, FLUX_DIFFUSE_SURFWF, &
        !    FLUX_DIFFUSE_PROFWF )
        !ENDIF
...
!  Additional Control for Externalized input (WLEAVE). Introduced 3/18/19 for Version 2.8.1
...
!  Profile weighting functions for Stokes vectors and fluxes
!  ---------------------------------------------------------

!    Output direct Mean/Flux added 17 May 2012

!  4/15/20. Version 2.8.2. Stokes Jacobians - Type structure filled directly
!      VLIDORT_LinOut%Prof%TS_PROFILEWF(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES,:) = &
!        PROFILEWF(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES,:)
      ...
      IF ( DO_MVOUT_ONLY .or. DO_ADDITIONAL_MVOUT ) THEN
        IF ( DO_UPWELLING ) THEN
          do q = 1, N_TOTALPROFILE_WFS
            VLIDORT_LinOut%Prof%TS_MEANST_DIFFUSE_PROFWF(q,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,UPIDX) = &
                   MEANST_DIFFUSE_PROFWF(q,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,UPIDX)
            VLIDORT_LinOut%Prof%TS_FLUX_DIFFUSE_PROFWF(q,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,UPIDX) = &
                   FLUX_DIFFUSE_PROFWF(q,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,UPIDX)
          enddo
        ENDIF
        IF ( DO_DNWELLING ) THEN
          do q = 1, N_TOTALPROFILE_WFS
            VLIDORT_LinOut%Prof%TS_MEANST_DIFFUSE_PROFWF(q,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,DNIDX) = &
                   MEANST_DIFFUSE_PROFWF(q,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,DNIDX)
            VLIDORT_LinOut%Prof%TS_FLUX_DIFFUSE_PROFWF(q,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,DNIDX) = &
                   FLUX_DIFFUSE_PROFWF(q,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,DNIDX)
            VLIDORT_LinOut%Prof%TS_DNMEANST_DIRECT_PROFWF (q,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES) = &
                   DNMEANST_DIRECT_PROFWF(q,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES)
            VLIDORT_LinOut%Prof%TS_DNFLUX_DIRECT_PROFWF(q,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES) = &
                   DNFLUX_DIRECT_PROFWF(q,1:NLAYERS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES)
          enddo
        ENDIF
      ENDIF
...
!   4/15/20. Version 2.8.2. Only fill out what you need
      ...
      IF ( DO_ALBTRN_MEDIA(1) ) THEN
        VLIDORT_LinOut%Prof%TS_ALBMED_USER_PROFWF(1:NSTOKES,1:N_USER_VZANGLES,1:NLAYERS,:) = &
            LP_ALBMED_USER(1:NSTOKES,1:N_USER_VZANGLES,1:NLAYERS,:)
        VLIDORT_LinOut%Prof%TS_ALBMED_FLUXES_PROFWF(1:NSTOKES,:,1:NLAYERS,:) = LP_ALBMED_FLUXES(1:NSTOKES,:,1:NLAYERS,:)
      ENDIF
      IF ( DO_ALBTRN_MEDIA(2) ) THEN
        VLIDORT_LinOut%Prof%TS_TRNMED_USER_PROFWF(1:NSTOKES,1:N_USER_VZANGLES,1:NLAYERS,:) = &
            LP_TRNMED_USER(1:NSTOKES,1:N_USER_VZANGLES,1:NLAYERS,:)
        VLIDORT_LinOut%Prof%TS_TRNMED_FLUXES_PROFWF(1:NSTOKES,:,1:NLAYERS,:) = LP_TRNMED_FLUXES(1:NSTOKES,:,1:NLAYERS,:)
      ENDIF
...
!    ==> Revised code to use only the first index of TRANSBEAM ==> makes the Q-problem valid
      ...
      IF ( DO_PLANETARY_PROBLEM .and. DO_PROFILE_LINEARIZATION ) THEN
         VLIDORT_LinOut%Prof%TS_PLANETARY_SBTERM_PROFWF(1:NLAYERS,:) = LP_TRNMED_FLUXES(1,2,1:NLAYERS,:)
         IF ( DO_OBSERVATION_GEOMETRY ) THEN
            DO IB = 1, N_GEOMETRIES ; DO O1 = 1, NSTOKES
               DO K = 1, nlayers ; if ( layer_Vary_Flag(k) ) then ; DO Q = 1, layer_Vary_Number(k)
                  VLIDORT_LinOut%Prof%TS_PLANETARY_TRANSTERM_PROFWF(O1,IB,K,Q) = &
                          ( LP_TRANSBEAM(1,IB,K,Q) * TRNMED_USER(O1,IB) + TRANSBEAM(1,IB) * LP_TRNMED_USER(O1,IB,K,Q) ) / PIE
               ENDDO ; ENDIF ; ENDDO
            ENDDO ; ENDDO
         ELSE
            DO IB =1, N_SZANGLES ; DO UM = 1, N_USER_VZANGLES ; DO O1 = 1, NSTOKES
               OFF = VZA_OFFSETS(IB,UM) ; TRANS = TRANSBEAM(1,IB) * TRNMED_USER(O1,UM) / PIE
               DO K = 1, nlayers ; if ( layer_Vary_Flag(k) ) then ; DO Q = 1, layer_Vary_Number(k)
                  L_TRANS(Q) = ( LP_TRANSBEAM(1,IB,K,Q)*TRNMED_USER(O1,UM) + TRANSBEAM(1,IB)*LP_TRNMED_USER(O1,UM,K,Q) ) / PIE
                  VLIDORT_LinOut%Prof%TS_PLANETARY_TRANSTERM_PROFWF(O1,OFF+1:OFF+N_USER_RELAZMS,K,Q) = L_TRANS(Q)
               ENDDO ; ENDIF ; ENDDO
            ENDDO ; ENDDO ; ENDDO
         ENDIF   
      ENDIF
...
!  4/15/20. Version 2.8.2. Integrated values - Only  copy if necessary.
      ...
      IF ( DO_MVOUT_ONLY .or. DO_ADDITIONAL_MVOUT ) THEN
         IF ( DO_UPWELLING ) THEN
            VLIDORT_LinOut%Surf%TS_MEANST_DIFFUSE_SURFWF(1:N_TOTALSURFACE_WFS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,UPIDX) = &
                                   MEANST_DIFFUSE_SURFWF(1:N_TOTALSURFACE_WFS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,UPIDX)
            VLIDORT_LinOut%Surf%TS_FLUX_DIFFUSE_SURFWF(1:N_TOTALSURFACE_WFS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,UPIDX) = &
                                   FLUX_DIFFUSE_SURFWF(1:N_TOTALSURFACE_WFS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,UPIDX)
         ENDIF
         IF ( DO_DNWELLING ) THEN
            VLIDORT_LinOut%Surf%TS_MEANST_DIFFUSE_SURFWF(1:N_TOTALSURFACE_WFS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,DNIDX) = &
                                   MEANST_DIFFUSE_SURFWF(1:N_TOTALSURFACE_WFS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,DNIDX)
            VLIDORT_LinOut%Surf%TS_FLUX_DIFFUSE_SURFWF(1:N_TOTALSURFACE_WFS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,DNIDX) = &
                                   FLUX_DIFFUSE_SURFWF(1:N_TOTALSURFACE_WFS,1:N_USER_LEVELS,1:N_SZANGLES,1:NSTOKES,DNIDX)
         ENDIF
      ENDIF

!  BLACKBODY JACOBIANS, New 28 March 2014. 
!  4/15/20. Version 2.8.2. Only  copy if necessary.
...
!  4/15/20. Version 2.8.2. Remove NMOMENTS, Only printing Fourier-zero component.
...
!mick mod 9/19/2017 - added DO_USER_VZANGLES & DO_SURFACE_EMISSION for better output control
...
!  4/15/20. Version 2.8.2. Remove NMOMS, Only printing First Fourier componnet.
      ...
      END SUBROUTINE VLIDORT_RTCALC_LPSMASTER

!

      SUBROUTINE VLIDORT_LPS_FOURIER ( FOURIER,  &
            ...
            DO_ATMOS_LINEARIZATION, DO_PROFILE_LINEARIZATION, DO_SURFACE_LINEARIZATION,             & !Input Linearization flags
            DO_SIMULATION_ONLY, DO_SLEAVE_WFS, DO_ATMOS_LBBF, DO_SURFACE_LBBF,                      & !Input Linearization flags
            NSTOKES,NSTREAMS,NLAYERS,N_SZANGLES,N_USER_STREAMS,N_USER_LEVELS,N_THERMAL_COEFFS,NMOMENTS,NSTREAMS_2, & !In Num(Basic)
            ...
            UTAU_LEVEL_MASK_UP, UTAU_LEVEL_MASK_DN, PARTLAYERS_OUTFLAG, PARTLAYERS_OUTINDEX, PARTLAYERS_LAYERIDX,  & !In Bkkeep
            STERM_LAYERMASK_UP, STERM_LAYERMASK_DN, DFLUX, LP_SOLARBEAM_BOATRANS, LP_SOLARBEAM_ATRANS,             & !In Bkkeep
            Misc, Therm, Mult, LAP_Misc, L_Therm, LP_Mult,                                                         & !In Packing
            ALBEDO, BRDF_F, BRDF_F_0, USER_BRDF_F, USER_BRDF_F_0,                                                  & !In ALB/BRDF
            ...
            STOKES_F, MEANST_DIFFUSE, FLUX_DIFFUSE, DNMEANST_DIRECT, DNFLUX_DIRECT, MS_CONTRIBS_F,                 & !Out STOKES
            PROFILEWF_F, MEANST_DIFFUSE_PROFWF, FLUX_DIFFUSE_PROFWF, DNMEANST_DIRECT_PROFWF, DNFLUX_DIRECT_PROFWF, & !Out COL JACS
            SURFACEWF_F, MEANST_DIFFUSE_SURFWF, FLUX_DIFFUSE_SURFWF,                                               & !Out SURF JAS
            ABBWFS_JACOBIANS, ABBWFS_FLUXES, SBBWFS_JACOBIANS, SBBWFS_FLUXES,                                      & !Out LBBF JAS
            TRANS_ATMOS_FINAL, TRANSBEAM, ALBMED_USER, ALBMED_FLUXES, TRNMED_USER, TRNMED_FLUXES,                  & !Out 4/26/19
            LP_TRANS_ATMOS_FINAL, LP_TRANSBEAM, LP_ALBMED_USER, LP_ALBMED_FLUXES, LP_TRNMED_USER, LP_TRNMED_FLUXES,& !Out 4/26/19 
...
!  valid only for LP linearization (this subroutine)

      USE VLIDORT_LP_SOLUTIONS_m
      USE VLIDORT_LP_BVPROBLEM_m , Only : LP_BVP_SOLUTION_MASTER, LP_BVPTEL_SOLUTION_MASTER
      USE VLIDORT_LP_WFATMOS_m   , Only : UPUSER_PROFILEWF, DNUSER_PROFILEWF, MIFLUX_PROFILEWF
...
!@@@ Addition of SLEAVE WF stuff, R. Spurr, 22 August 2012 @@@@@@@@@
      ...
      USE VLIDORT_LS_WFSLEAVE_m , Only : VLIDORT_LSSL_WFS

!  Unpack

      USE VLIDORT_UNPACK_m
      USE VLIDORT_L_UNPACK_m
      USE VLIDORT_LP_UNPACK_m
...
!Rob  fix 4/9/19    - added LP_SOLARBEAM_BOATRANS, LP_SOLARBEAM_ATRANS

      DOUBLE PRECISION, intent(in)  :: LP_SOLARBEAM_BOATRANS  ( MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, intent(in)  :: LP_SOLARBEAM_ATRANS    ( MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS )
...
!    --  Also depends on surface/sleave quantities, but these linearizations (LS/LSSL) are neglected
      ...
      DOUBLE PRECISION, INTENT (INOUT) :: LP_TRANS_ATMOS_FINAL ( MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS )

!  4/26-29/19. Special Media-property output. -- Introduced by R. Spurr.
!     ** Linearized Output developed for Profile Jacobians.

      DOUBLE PRECISION, INTENT (INOUT) :: LP_ALBMED_USER  ( MAXSTOKES, MAX_USER_STREAMS, MAXLAYERS, MAX_ATMOSWFS ) !TOA illumination
      DOUBLE PRECISION, INTENT (INOUT) :: LP_TRNMED_USER  ( MAXSTOKES, MAX_USER_STREAMS, MAXLAYERS, MAX_ATMOSWFS ) !BOA illumination
      
      DOUBLE PRECISION, INTENT (INOUT) :: LP_ALBMED_FLUXES ( MAXSTOKES, 2, MAXLAYERS, MAX_ATMOSWFS )    !TOA illumination
      DOUBLE PRECISION, INTENT (INOUT) :: LP_TRNMED_FLUXES ( MAXSTOKES, 2, MAXLAYERS, MAX_ATMOSWFS )    !BOA illumination
      
      DOUBLE PRECISION, INTENT (INOUT) :: LP_TRANSBEAM ( MAXSTOKES, MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS ) !Planetary problem
...
!  Disabled
      ...
      !DOUBLE PRECISION, INTENT (INOUT) :: PROFILEWF_SS ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_GEOMETRIES, &
      !                                                   MAXSTOKES, MAX_DIRECTIONS )
      !DOUBLE PRECISION, INTENT (INOUT) :: PROFILEWF_DB ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_GEOMETRIES, MAXSTOKES )
...
!  -------------------------
!mick fix 9/19/2017 - added LP_LEVELS_SOLARTRANS & LP_PARTIALS_SOLARTRANS to facilitate
...
!  Linearized Particular integral
      ...
      DOUBLE PRECISION :: LP_BVEC ( MAXSTREAMS_2, MAXSTOKES, MAXLAYERS, MAXLAYERS, MAX_ATMOSWFS )
      ...
      DOUBLE PRECISION :: LP_UPAR_DN_2 ( MAX_USER_STREAMS, MAXSTOKES, MAXLAYERS, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION :: LP_UPAR_UP_2 ( MAX_USER_STREAMS, MAXSTOKES, MAXLAYERS, MAXLAYERS, MAX_ATMOSWFS )
...
!  Indices and help for the Planetary problem
      ...
      INTEGER ::          I, IBEAM, IPARTIC, LAYER, N, NV, O1, K, K0, K1, K2, KO1, LUI, Q
...
!  Weighting function indices
      ...
      INTEGER ::          N_LAYER_WFS, LAYER_TO_VARY, NPARAMS_VARY
...
!   Definition extended, 28 March 2014
      ...
      DO_SIMULATION_ONLY = ( .NOT. DO_PROFILE_LINEARIZATION .AND. .NOT. DO_SURFACE_LINEARIZATION .and. &
...
!                       db code, and fo code up to VLIDORT_RTCALC_LPSMASTER.  There, the needed
...
!  Linearized
      ...
      IF ( DO_ATMOS_LINEARIZATION ) THEN
        ...
        CALL VLIDORT_UNPACK_LAP_MISC ( LAP_Misc,                & ! Input
          NSTOKES, NSTREAMS, NLAYERS, N_PARTLAYERS,             & ! Input
          N_SZANGLES, N_USER_STREAMS, NMOMENTS, N_TOTALPROFILE_WFS, & ! Input
        ...
        IF ( DO_SOLAR_SOURCES .AND. DO_USER_STREAMS ) THEN
          IF ( .NOT. DO_FOCORR_ALONE ) THEN
            CALL VLIDORT_UNPACK_LP_MULT ( LP_Mult,                     & ! Input
              NLAYERS, N_PARTLAYERS, N_SZANGLES, N_USER_STREAMS,           & ! Input
...
!mick fix 7/17/2014 - this IF block moved from above and modified to accomodate move of
!                     "Fourier = 0" subroutines
!mick mod 9/19/2017 - commented out this IF block (DO_REFLECTED_DIRECTBEAM already defined
!                     in VLIDORT_RTCALC_LPSMASTER & passed down VIA the "Misc" type structure)
...
!           NSTOKES, NSTREAMS, N_SZANGLES, N_USER_STREAMS, N_SLEAVE_WFS,      &
...
!  .. error tracing
          ...
          IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
            write(CF,'(I2)')FOURIER
            TRACE_2 = 'Called in VLIDORT_LPS_FOURIER, Fourier component '//CF
...
!         if ( do_write_screen) write(*,*)'l_homsolution',layer
            CALL VLIDORT_L_QHOM_SOLUTION ( &
              DO_SOLUTION_SAVING, DO_DEBUG_WRITE, LAYER, FOURIER,                     & ! Input Flags and Indices
              NSTOKES, NSTREAMS, N_PARTLAYERS, DO_RTSOL_VARY, NPARAMS_VARY, NMOMENTS, & ! Input Numbers+Lin.Control
...
!  Telescoped case
        ...
        ELSE
          ...
          IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
            write(CF,'(I2)')FOURIER
            TRACE_2 = 'Error from BVPTEL_MATRIXSETUP_MASTER, '//'Called in VLIDORT_LPS_FOURIER, Fourier # '//CF
            STATUS = VLIDORT_SERIOUS
            RETURN
          ENDIF
...
!  4/26/19. Add Call to Media properties subroutine (regular or LP_Jacobian)
!   -- Stand-alone output, but this is a necessary call for the planetary problem
        ...
        IF ( DO_ALBTRN_MEDIA(1) .or. DO_ALBTRN_MEDIA(2) .or. DO_PLANETARY_PROBLEM ) THEN
           IF ( DO_PROFILE_LINEARIZATION ) THEN
              CALL VLIDORT_LP_MediaProps &
               ( DO_USER_STREAMS, DO_ALBTRN_MEDIA, NSTOKES, NLAYERS, NSTREAMS,       & ! Input Control
                 N_USER_STREAMS, NSTREAMS_2, NSTKS_NSTRMS, NSTKS_NSTRMS_2,           & ! Input control numbers
                 NTOTAL, N_SUBDIAG, N_SUPDIAG, QUAD_STRMWTS, DELTAU_VERT,            & ! Input quad/deltaus
                 DO_PROFILE_LINEARIZATION, LAYER_VARY_FLAG, LAYER_VARY_NUMBER,       & ! Input Lineariz control
                 ...
                 LP_ALBMED_USER, LP_ALBMED_FLUXES, LP_TRNMED_USER, LP_TRNMED_FLUXES, & ! output Linearized
                 STATUS_SUB, MESSAGE, TRACE_1, TRACE_2 )                               ! Output
              IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
                 TRACE_3 = 'Error from VLIDORT_LP_MediaProps, '//'Called in VLIDORT_LPS_FOURIER'
...
!   Version 2.8. remove GOTO 788 statement
!        IF ( DO_THERMAL_TRANSONLY ) GO TO 788

                IF ( .not. DO_THERMAL_TRANSONLY ) THEN

!  4/9/19. Additional code to set the LP derivative of TRANS_ATMOS_FINAL
...
!  4/9/19. Water-leaving control. Also need LP_TRANS_ATMOS_FINAL
                    ...
                    CALL LP_BVP_SOLUTION_MASTER ( &
                      ...
                      DO_PLANE_PARALLEL, DO_LAYER_SCATTERING, FOURIER, IBEAM,             & ! Flags, FOURIER/IBEAM
                      NSTOKES, NSTREAMS, NLAYERS, LAYER_TO_VARY, N_LAYER_WFS, NSTREAMS_2, & ! Numbers (Basic)
                      NSTKS_NSTRMS, NSTKS_NSTRMS_2, NTOTAL, N_SUBDIAG, N_SUPDIAG,         & ! Numbers (derived)
                      ...
                      LP_TRANS_ATMOS_FINAL, LP_INITIAL_TRANS, LP_T_DELT_MUBAR, LP_BVEC, & ! Linearized Beam/PI solutions
...
!             -- Note use of LP_TRANS_ATMOS_FINAL to go with adjusted water-leaving
                ...
                IF ( DO_UPWELLING ) THEN
                  CALL UPUSER_PROFILEWF ( &
                    ...
                    N_USER_STREAMS, N_USER_LEVELS, LAYER_TO_VARY, N_LAYER_WFS, LOCAL_UM_START, & ! Input numbers (basic)
                    ...
                    L_UHOM_UPDN, L_UHOM_UPUP, L_UPAR_UP_1, LP_UPAR_UP_2,                    & ! Linearized homog. and Beam sol
                    L_HMULT_1, L_HMULT_2, LP_EMULT_UP,                                      & ! Linearized Homog. and Beam mult
                    L_UT_HMULT_UU, L_UT_HMULT_UD, LP_UT_EMULT_UP,                           & ! Linearized Homog. and Beam mult  
...
!  Streamlined for  Version 2.8. 7/22/16
                ...
                IF ( DO_DNWELLING ) THEN
                  CALL DNUSER_PROFILEWF ( &
                    ...
                    DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS,   DO_THERMAL_TRANSONLY,            & ! Input flags (sources)
                    DO_LAYER_SCATTERING, FOURIER, IPARTIC, LAYER_TO_VARY, N_LAYER_WFS, NSTOKES, & ! Input numbers (basic)
                    ...
                    NCON, PCON, L_UHOM_DNDN, L_UHOM_DNUP, L_UPAR_DN_1, LP_UPAR_DN_2,            & ! Linearized homog. and Beam sol
                    L_HMULT_1, L_HMULT_2, LP_EMULT_DN,                                          & ! Linearized Homog. and Beam mult
                    L_UT_HMULT_DD, L_UT_HMULT_DU, LP_UT_EMULT_DN,                               & ! Linearized Homog. and Beam mult
                    PROFILEWF_F )                                                                 ! OUTPUT
                ENDIF 

! @@@ Rob fix 1/31/11, - added FLUX_FACTOR argument
!    - Version 2.8, Cleaned-up I/O presentation. 7/22/16. Removed QUAD_OUTPUT flag.
!mick mod 9/19/2017 - added LP_LEVELS_SOLARTRANS & LP_PARTIALS_SOLARTRANS to input arguments

                IF ( DO_INCLUDE_MVOUTPUT ) THEN
                  CALL MIFLUX_PROFILEWF ( &
                    DO_INCLUDE_MVOUTPUT, DO_INCLUDE_DIRECTRF,                        & ! Input flags
                    DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY,   & ! Input flags
                    IPARTIC, LAYER_TO_VARY, N_LAYER_WFS, NSTOKES, NSTREAMS, NLAYERS, & ! Input numbers
...
!  Exception handling
            ...
            IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
              write(CF,'(I2)')FOURIER
              TRACE_2 = 'Error return from SURFACEWF_BVP_SOLUTION, '//'Called in VLIDORT_LPS_FOURIER, Fourier # '//CF
              STATUS = VLIDORT_SERIOUS ; RETURN
            ENDIF
...
!  error flag
            ...
            IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
              write(CF,'(I2)')FOURIER
              TRACE_2 = 'Error return from VLIDORT_QBEAM_SOLUTION, '//'Called in VLIDORT_LPS_FOURIER, Fourier # '//CF
              STATUS = VLIDORT_SERIOUS ; RETURN
            ENDIF
...
!  For the linearizations of the classical solution
            ...
            IF ( DO_ATMOS_LINEARIZATION ) THEN
              ...
              CALL VLIDORT_LP_QBEAM_SOLUTION ( &
                DO_PLANE_PARALLEL, DO_RTSOL_VARY, LAYER, FOURIER, IBEAM, NSTOKES, & ! Input flags/numbers
                NSTREAMS, NMOMENTS, NPARAMS_VARY, NSTREAMS_2, NSTKS_NSTRMS,       & ! Input numbers
                DO_LAYER_SCATTERING, LAYER_VARY_FLAG, LAYER_VARY_NUMBER,          & ! Input linearization control
                ...
                L_OMEGA_GREEK, LP_AVERAGE_SECANT,                        & ! Input optical + beam
                LP_BVEC, STATUS_SUB, MESSAGE, TRACE_1 )                    ! Output + status

              IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
                write(CF,'(I2)')FOURIER
                TRACE_2 = 'Error from VLIDORT_LP_QBEAM_SOLUTION, '//'Called in VLIDORT_LPS_FOURIER, Fourier # '//CF
                STATUS = VLIDORT_SERIOUS ; RETURN
              ENDIF
              ...
              IF ( DO_USER_STREAMS ) THEN
                ...
                IF ( DO_ATMOS_LINEARIZATION ) THEN
                  CALL VLIDORT_LP_UBEAM_SOLUTION ( &
                    ...
                    DO_PLANE_PARALLEL, DO_RTSOL_VARY, LAYER, FOURIER, IBEAM,     & ! Input flags/indices
...
!    - Version 2.8, Cleaned-up I/O presentation. 7/8/16.
          ...
          ELSE
            ...
            IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
              write(CF,'(I2)')FOURIER
              TRACE_2 = 'Error return from BVPTEL_SOLUTION_MASTER, '//'Called in VLIDORT_LPS_FOURIER, Fourier # '//CF
              STATUS = VLIDORT_SERIOUS
              RETURN
            ENDIF
...
!mick mod 9/19/2017 - replaced DO_REFLECTED_DIRECTBEAM(IBEAM) with DO_LOCALBEAM(IBEAM) in
!                     both LP_BVP_SOLUTION_MASTER & LP_BVPTEL_SOLUTION_MASTER

!  4/9/19. Version 2.8.1, Water-leaving control. Also need LP_TRANS_ATMOS_FINAL
                     ...
                     IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
                        write(CF,'(I2)')FOURIER
                        TRACE_2 = 'Error return from LP_BVP_SOLUTION_MASTER, Beam solutions '
                        TRACE_3 =  'Called in VLIDORT_LPS_FOURIER, Fourier # '//CF
                        STATUS = VLIDORT_SERIOUS ; RETURN
                     ENDIF
...
!    - Version 2.8, Cleaned-up I/O presentation. 7/23/16.
                  ...
                  ELSE
                    ...
                    CALL LP_BVPTEL_SOLUTION_MASTER ( &
                      DO_INCLUDE_SURFACE, DO_LAMBERTIAN_SURFACE, DO_LOCALBEAM(IBEAM), DO_PLANE_PARALLEL, & ! Flags
                      FOURIER, IBEAM, NSTOKES, NSTREAMS, NLAYERS, LAYER_TO_VARY, N_LAYER_WFS, & ! Basic Control Numbers
                      ...
                      LP_INITIAL_TRANS, LP_T_DELT_MUBAR, LP_BVEC,             & ! Linearized Beam/PI solutions
...
!  here is where you save the solutions you need for the planetary problem
!     SBTERM is already available, TRANSTERM(IB,UM) = TRANSBEAM(IB) * TRNMED_USER(UM) / PIE
!     -- Linearization similar to that in (QUADINTENS_LEVEL_DN), lp_wfatmos routine
             
!  BugFix 8/12/19. First 2 indices of LP_TRANSBEAM needed to be reversed.

                  IF ( DO_PLANETARY_PROBLEM ) THEN
                    N = NLAYERS ; NV = LAYER_TO_VARY ; KO1 = K_REAL(N) + 1
                    IF ( NV.eq.N) then
                      DO I = 1, NSTREAMS
                        DO O1 = 1, NSTOKES
                          DO Q = 1, N_LAYER_WFS
                        ...
                        DO O1 = 1, NSTOKES
                          DO Q = 1, N_LAYER_WFS
                            ...
                            DO K = 1, K_COMPLEX(N)
                              ...
                              NXR2  =   NCON(K1,N,Q) *   SOLA_XPOS(I,O1,K2,N) + NCON(K2,N,Q) *   SOLA_XPOS(I,O1,K1,N)
                              PXR1  =   PCON(K1,N,Q) *   SOLB_XNEG(I,O1,K1,N) - PCON(K2,N,Q) *   SOLB_XNEG(I,O1,K2,N)
                              SHOM_CR = SHOM_CR + NXR1 * T_DELT_EIGEN(K1,N) - NXR2 * T_DELT_EIGEN(K2,N) + PXR1
                            ENDDO
                            SHOM = SHOM_R + SHOM_CR
                            L_TRANSQUAD(I,O1,Q) = L_WLOWER(I,O1,N,Q) + SHOM
                          ENDDO
                    ...
                    ENDIF
                    DO Q = 1, N_LAYER_WFS
                      L_TRANS_DIRC = LOCAL_CSZA(NLAYERS,IBEAM) * FLUX_FACTOR * LP_SOLARBEAM_ATRANS(IBEAM,NV,Q)   ! Direct
                      DO O1 = 1, NSTOKES
                         L_TRANS_DIFF = PI2 * DOT_PRODUCT(L_TRANSQUAD(1:NSTREAMS,O1,Q),QUAD_STRMWTS(1:NSTREAMS))    ! Diffuse
!                         LP_TRANSBEAM(IBEAM,O1,NV,Q) = L_TRANS_DIFF + L_TRANS_DIRC * FLUXVEC(O1) ! Bug 8/12/19
                         LP_TRANSBEAM(O1,IBEAM,NV,Q) = L_TRANS_DIFF + L_TRANS_DIRC * FLUXVEC(O1)
                      ENDDO
                    ENDDO   
                  ENDIF
...
!  Streamlined for  Version 2.8. 7/22/16
!mick fix 9/19/2017 - replaced IPARTIC with IBEAM in UPUSER_PROFILEWF, DNUSER_PROFILEWF,
!                     & MIFLUX_PROFILEWF in this section
...
!             -- Note use of LP_TRANS_ATMOS_FINAL to go with adjusted water-leaving
                  ...
                  IF ( DO_UPWELLING ) THEN
                    CALL UPUSER_PROFILEWF ( &
                      ...
                      T_DELT_DISORDS,  T_DELT_USERM, T_UTUP_USERM, CUMSOURCE_UP,              & ! Input Transmittances
                      ALBEDO, BRDF_F, USER_BRDF_F, DELTAU_SLANT,                              & ! Input Surface BRDF.
                      RF_USER_DIRECT_BEAM, SL_USERTERM, LP_TRANS_ATMOS_FINAL,                 & ! Input surf rads (beam/sleave)
                      K_REAL, K_COMPLEX, SOLA_XPOS, SOLB_XNEG, T_DELT_EIGEN, LCON, MCON,      & ! Input Homog. RTE Soln.
                      T_WLOWER, UHOM_UPDN, UHOM_UPUP, UPAR_UP_1, UPAR_UP_2,                   & ! Input thermal and User solutions
                      HMULT_1, HMULT_2, EMULT_UP, UT_HMULT_UU, UT_HMULT_UD, UT_EMULT_UP,      & ! Input multiplier  
                      L_DELTAU_VERT, L_T_DELT_USERM, L_T_UTUP_USERM,  L_T_DELT_DISORDS,       & ! Lin delta and transmittances
                      L_T_DELT_EIGEN, L_SOLA_XPOS, L_SOLB_XNEG, L_WLOWER, NCON, PCON,         & ! Lin Discr.Ord. solutions.
                      L_T_WLOWER, L_LAYER_TSUP_UP, L_LAYER_TSUP_UTUP,                         & ! Lin thermal solutions
                      L_UHOM_UPDN, L_UHOM_UPUP, L_UPAR_UP_1, LP_UPAR_UP_2,                    & ! Lin homog. and Beam sol
                      L_HMULT_1, L_HMULT_2, LP_EMULT_UP,                                      & ! Lin Homog. and Beam mult
                      L_UT_HMULT_UU, L_UT_HMULT_UD, LP_UT_EMULT_UP,                           & ! Lin Homog. and Beam mult  
                      L_BOA_THTONLY_SOURCE, PROFILEWF_F )                                       ! OUTPUT
                  ENDIF
...
!  Streamlined for  Version 2.8. 7/22/16
                  ...
                  IF ( DO_DNWELLING ) THEN
                    CALL DNUSER_PROFILEWF ( &
                      DO_USER_STREAMS,  DO_OBSERVATION_GEOMETRY, DO_MSMODE_VLIDORT,               & ! Input flags (RT mode)
                      DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS,   DO_THERMAL_TRANSONLY,            & ! Input flags (sources)
                      DO_LAYER_SCATTERING, FOURIER, IBEAM, LAYER_TO_VARY, N_LAYER_WFS, NSTOKES,   & ! Input numbers (basic)
                      N_USER_STREAMS, LOCAL_UM_START, N_USER_LEVELS, UTAU_LEVEL_MASK_DN,          & ! Input bookkeeping + levels
                      PARTLAYERS_OUTFLAG, PARTLAYERS_OUTINDEX, PARTLAYERS_LAYERIDX,               & ! Input partial-layer control
                      FLUX_MULTIPLIER, T_DELT_USERM, T_UTDN_USERM, CUMSOURCE_DN,                  & ! Input Transmittances, Flux
                      K_REAL, K_COMPLEX, LCON, MCON, UHOM_DNDN, UHOM_DNUP, UPAR_DN_1, UPAR_DN_2,  & ! Input RT and User solutions
...
!    - Version 2.8, Cleaned-up I/O presentation. 7/22/16. Removed QUAD_OUTPUT flag.
!mick mod 9/19/2017 - replaced DO_INCLUDE_DIRECTBEAM with DO_LOCALBEAM(IBEAM)
!                   - added LP_LEVELS_SOLARTRANS & LP_PARTIALS_SOLARTRANS to input arguments

                  IF ( DO_INCLUDE_MVOUTPUT ) THEN
                    CALL MIFLUX_PROFILEWF ( &
                      DO_INCLUDE_MVOUTPUT, DO_LOCALBEAM(IBEAM),                        & ! Input flags
                      DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY,   & ! Input flags
                      IBEAM, LAYER_TO_VARY, N_LAYER_WFS, NSTOKES, NSTREAMS, NLAYERS,   & ! Input numbers
                      N_DIRECTIONS, WHICH_DIRECTIONS,                                  & ! Input directions
                      N_USER_LEVELS, UTAU_LEVEL_MASK_UP, UTAU_LEVEL_MASK_DN,           & ! Level output control
                      PARTLAYERS_OUTFLAG, PARTLAYERS_OUTINDEX, PARTLAYERS_LAYERIDX,    & ! Partial layer output control
                      FLUX_MULTIPLIER, FLUX_FACTOR, FLUXVEC, LOCAL_CSZA,               & ! Input Flux
...
!  Error handling
              ...
              IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
                write(CF,'(I2)')FOURIER
                 TRACE_2 = 'Error return from vlidort_lbbf_jacobians, thermal+solar'// &
                           'Called in VLIDORT_LPS_FOURIER, Fourier # '//CF
                STATUS = VLIDORT_SERIOUS ; RETURN
              ENDIF
...
!  End Module
      ...
      END MODULE VLIDORT_RTCalc_LPSmaster_m
