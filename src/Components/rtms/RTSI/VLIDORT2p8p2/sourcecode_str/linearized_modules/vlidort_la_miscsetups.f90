...
!  4/15/20. Version 2.8.2. Replace MAXMOMENTS_INPUT dimension ==> MAXMOMENTS. Otherwise no change.
...
!   4/15/20. Version 2.8.2. Remove MAXMOMENTS_INPUT dimension
      ...
      USE VLIDORT_PARS_m, Only : MAX_ATMOSWFS, MAXMOMENTS, MAXLAYERS, &
                                 MAXBEAMS, MAXSTOKES_SQ, zero, one, SMALLNUM

      IMPLICIT NONE

!  deltam scaling and overall linearization flag

      LOGICAL, INTENT (IN) ::          DO_DELTAM_SCALING
      LOGICAL, INTENT (IN) ::          DO_ATMOS_LINEARIZATION

!  Number of layers, moments and beams, Stokes

      INTEGER, intent(in) ::           NLAYERS, NBEAMS, NMOMENTS, NSTOKES, NSTOKES_SQ
...
!  Input Linearized Optical (original)
!   4/15/20. Version 2.8.2. No longer need MAXMOMENTS_INPUT ==> use MAXMOMENTS instead
...
!            IF ( LAYER_VARY_FLAG(K) ) THEN
!              DO Q = 1, LAYER_VARY_NUMBER(K)
!                L_FAC1 = L_TRUNC_FACTOR(Q,K) * OMEGA_TOTAL_INPUT(K) + TRUNC_FACTOR(K) * L_OMEGA_TOTAL_INPUT(Q,K)
!                DO IB = 1, NBEAMS
!                  DELS = DELTAU_SLANT(N,K,IB)/FAC1(K)
