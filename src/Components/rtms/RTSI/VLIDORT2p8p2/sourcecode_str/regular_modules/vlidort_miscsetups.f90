...
!    -- VLIDORT_PERFORMANCE_SETUP added to the list of routines (code foremrly in derive_inputs)
!    -- MAXMOMENTS is the dimension now instead of MAXMOMENTS_INPUT
!    -- Main routine has additional inputs and exception handling

      MODULE vlidort_miscsetups_m

      PRIVATE :: VLIDORT_PERFORMANCE_SETUP, &
                 VLIDORT_DELTAMSCALE, &
                 VLIDORT_SSALBINIT,   &
                 VLIDORT_QSPREP,      &
                 VLIDORT_PREPTRANS
      ...
      SUBROUTINE VLIDORT_MISCSETUPS ( &
        ...
        NSTOKES, NLAYERS, NSTREAMS, N_USER_STREAMS,                    & ! Input Numbers
        NBEAMS, NMOMENTS, NLAYERS_CUTOFF, NLAYERS_NOMS,                & ! Input Numbers
        ...
        STATUS, MESSAGE, TRACE )                                         ! Output PERFORMANCE

!  Miscellaneous set-ups
!     Performance, Delta-M scaling, average secant parameterization, various transmittances
...
!   4/15/20. Version 2.8.2. Remove MAXMOMENTS_INPUT
      ...
      USE VLIDORT_PARS_m, Only : MAXSTOKES, MAXSTREAMS, MAXLAYERS, MAX_PARTLAYERS,          &
                                 ...
                                 MAXMOMENTS, MAXSTOKES_SQ, VLIDORT_WARNING
...
!  input optical
!   4/15/20. Version 2.8.2. MAXMOMENTS is the dimension now instead of MAXMOMENTS_INPUT
...
!  4/15/20. Version 2.8.2. Introduced performance setup here.
!      Performance set-up WAS ORIGINALLY in VLIDORT_DERIVE_INPUTS (vlidort_inputs.f)

      CALL VLIDORT_PERFORMANCE_SETUP & 
        ( DO_SOLAR_SOURCES, DO_SOLUTION_SAVING,   DO_BVP_TELESCOPING,     & ! Input
          DO_RAYLEIGH_ONLY, DO_THERMAL_TRANSONLY, DO_SPECIALIST_OPTION_2, & ! Input
          NSTOKES, NLAYERS, NMOMENTS, NLAYERS_NOMS,                       & ! Input
          GREEKMAT_TOTAL_INPUT, OMEGA_TOTAL_INPUT,                        & ! Input
          DO_LAYER_SCATTERING, BVP_REGULAR_FLAG, DO_REAL_EIGENSOLVER,     & ! Output
...
!     Even though this is a warning, must exit with Serious condition
      ...
      IF ( STATUS .EQ. VLIDORT_WARNING ) RETURN
...
!                         LEVELS_SOLARTRANS, & PARTIALS_SOLARTRANS as outputs
      ...
      CALL VLIDORT_DELTAMSCALE ( &
        ...
        NSTOKES, NLAYERS, NMOMENTS, NBEAMS,                              & ! Input Numbers
        N_PARTLAYERS, PARTLAYERS_LAYERIDX, PARTLAYERS_VALUES,            & ! Input Partlayers
        OMEGA_TOTAL_INPUT, DELTAU_VERT_INPUT, GREEKMAT_TOTAL_INPUT,      & ! Input Optical
        TAUGRID_INPUT, CHAPMAN_FACTORS, PARTIAL_CHAPFACS,                & ! Input Chapman
...
!
      ...
      SUBROUTINE VLIDORT_PERFORMANCE_SETUP & 
        ( DO_SOLAR_SOURCES, DO_SOLUTION_SAVING,   DO_BVP_TELESCOPING,     & ! Input
          DO_RAYLEIGH_ONLY, DO_THERMAL_TRANSONLY, DO_SPECIALIST_OPTION_2, & ! Input
          NSTOKES, NLAYERS, NMOMENTS, NLAYERS_NOMS,                       & ! Input
          GREEKMAT_TOTAL_INPUT, OMEGA_TOTAL_INPUT,                        & ! Input
...
!  New routine in Version 2.8.2. 4/15/20.
!    - Code was formerly found in VLIDORT_DERIVE_INPUTS, moved here
!    - Logic and patterning follows closely the LIDORT way of doing things
...
!  module, dimensions and numbers
      ...
      USE VLIDORT_pars_m, only : MAXLAYERS, MAXMOMENTS, MAXSTOKES_SQ, &
                                 ZERO, VLIDORT_SUCCESS, VLIDORT_WARNING
...
!  Number of layers, stokes

      INTEGER  , intent(in)  :: NSTOKES, NLAYERS

!  Number of moments
!   4/15/20. Version 2.8.2. NMOMENTS is the input now instead of NGREEK_MOMENTS_INPUT

      INTEGER  , intent(in)  :: NMOMENTS, NLAYERS_NOMS
...
!  Local flags,  Eigensolver choise
...
!  ---------------
      ...
      INTEGER      :: M, N, Q, QC, L, NS, NA, NAP, EC
      LOGICAL      :: NOWARNING
...
!   - initialise (M = Fourier index) to normal  mode.
      ...
      BVP_REGULAR_FLAG(0:NMOMENTS) = .TRUE.
      DO N = 1, NLAYERS
         DO_LAYER_SCATTERING(0:NMOMENTS,N) = .TRUE.
      ENDDO
...
!  Collect warning and re-set default option
        ...
        IF ( .not. NOWARNING ) then
          ...
          TRACE   = 'Warning generated in VLIDORT_PERFORMANCE SETUP'
          DO_BVP_TELESCOPING = .FALSE.
          BVP_REGULAR_FLAG(3:NMOMENTS) = .TRUE.
        ENDIF
...
!      IF ( DO_SOLAR_SOURCES .and. ( DO_FOCORR_NADIR .OR. DO_FOCORR_OUTGOING ) ) THEN
!         IF ( DO_RAYLEIGH_ONLY ) THEN
!            LAYER_MAXMOMENTS(1:NLAYERS) = 2
!         ELSE
!            DO N = 1, NLAYERS
!              L = 2 ; LOOP = .TRUE.
!              DO WHILE (LOOP.AND.L.LT.NGREEK_MOMENTS_INPUT)
!                L = L + 1 ; LOOP = (GREEKMAT_TOTAL_INPUT(L,N).NE.ZERO)
!              ENDDO
!              LAYER_MAXMOMENTS(N) = L ; !  LAYER_MAXMOMENTS(N) = L - 1
!            ENDDO
!         ENDIF
!      ENDIF
...
!   2. Otherwise, examine the occurrence of the "epsilon" Greek constant
      ...
      IF ( DO_RAYLEIGH_ONLY .OR. NSTOKES.EQ.1 ) THEN
        DO M = 0, NMOMENTS
          DO_REAL_EIGENSOLVER(M,1:NLAYERS) = .TRUE.
        ENDDO
      ELSE
        DO M = 0, NMOMENTS
          DO N = 1, NLAYERS
...
!  Finish
      ...
      RETURN
END SUBROUTINE VLIDORT_PERFORMANCE_SETUP
...
!  module, dimensions and numbers
!   4/15/20. Version 2.8.2. Remove MAXMOMENTS_INPUT dimension

      USE VLIDORT_PARS_m, Only : MAXLAYERS, MAX_USER_LEVELS, MAX_PARTLAYERS, MAXBEAMS, &
...
!  Outputs (optical)
!   4/15/20. Version 2.8.2. No longer need MAXMOMENTS_INPUT ==> use MAXMOMENTS instead
...
!  local variables
      ...
      INTEGER          :: K, K1, K2, N, N1, L, UT, NM1, IB
...
!
      ...
      SUBROUTINE VLIDORT_DIRECTRADIANCE ( &
        ...
        LAMBERTIAN_ALBEDO, BRDF_F_0,  USER_BRDF_F_0,                        & ! Input surface reflectance
        SLTERM_ISOTROPIC, SLTERM_F_0, USER_SLTERM_F_0,                      & ! Input surface leaving
        ATMOS_ATTN, DIRECT_BEAM, USER_DIRECT_BEAM, SL_QUADTERM, SL_USERTERM ) ! Output reflected direct beams.
...
!  4/15/20. Version 2.8.2. Remove MAXMOMENTS dimension.
      ...
      USE VLIDORT_PARS_m, Only : MAXLAYERS, MAXSTOKES, MAXSTREAMS, MAXBEAMS, MAX_SZANGLES, &
                                 MAX_USER_STREAMS, MAXSTOKES_SQ, ZERO, FOUR, PI4, DEG_TO_RAD
...
!  4/15/20. Version 2.8.2. SLEAVE arrays defined locally, each Fourier, remove MAXMOMENTS dimension
...
!  Natural light only, so only the first Stokes component is nonzero
            ...
            IF ( DO_LAMBERTIAN_SURFACE ) THEN
               ...
               IF ( FOURIER .EQ. 0 ) THEN
                  ...
                  IF ( DO_USER_STREAMS ) THEN
                     IF ( .not. DO_OBSERVATION_GEOMETRY ) THEN
                        USER_DIRECT_BEAM(1:N_USER_STREAMS,IB,1) = REFLEC
                     ELSE
               ...
               IF ( DO_USER_STREAMS ) THEN
                  ...
                  ELSE
                     DO O1 = 1, NSTOKES
                        ...
                        DO O2 = 1, NSTOKES
                           OM = MUELLER_INDEX(O1,O2)
                           SUM = SUM + FLUXVEC(O2)*USER_BRDF_F_0(OM,LUI,IB)
                        ENDDO
...
!  4/15/20. Version 2.8.2. SLEAVE arrays defined locally, remove M=Fourier index
         ...
         IF ( DO_CALC_SLTERMS ) THEN
            ...
            IF ( DO_SL_ISOTROPIC .and. M.EQ.0 ) THEN
               DO O1 = 1, NSTOKES
                  ...
                  IF ( DO_USER_STREAMS ) THEN
                     IF ( .not. DO_OBSERVATION_GEOMETRY ) THEN
                        SL_USERTERM(1:N_USER_STREAMS,IB,O1) =  SL
                     ELSE
                        SL_USERTERM(LUI,IB,O1) = SL
                     ENDIF
                  ENDIF
               ENDDO
            ELSE
               DO O1 = 1, NSTOKES
                  DO I = 1, NSTREAMS
                     SL = SLTERM_F_0(O1,I,IB) * HELP
                     SL_QUADTERM(I,IB,O1) = SL
                  ENDDO
                  IF ( DO_USER_STREAMS ) THEN
                     IF ( .not. DO_OBSERVATION_GEOMETRY ) THEN
                        DO UI = 1, N_USER_STREAMS
                        ...
                        ENDDO
                     ELSE
                        SL_USERTERM(LUI,IB,O1) = USER_SLTERM_F_0(O1,LUI,IB)* HELP
                     ENDIF
                  ENDIF
               ENDDO
            ENDIF
...
!    -- 4/15/20. Version 2.8.2. removed MUELLER_INDEX, SUN_SZA_COSINES
...
!mick fix 3/2/2020 - added DO_SOLAR_SOURCES to inputs

!  Flags and Fourier

      LOGICAL, INTENT (IN) ::           DO_SOLAR_SOURCES, DO_REFRACTIVE_GEOMETRY
      LOGICAL, INTENT (IN) ::           DO_USER_STREAMS
      INTEGER, INTENT (IN) ::           FOURIER
...
!                DO SK1 = 1, NSTOKES
!                  DO SK2 = 1, NSTOKES
!                    PI_X0P(L,IB,N,SK1,SK2) = PI(L,IP,SK1,SK2) * PINORM(L,M)
!                  ENDDO
!                ENDDO
!              ENDDO
!            ENDDO
