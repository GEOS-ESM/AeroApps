      ...
      USE VLIDORT_SETUP_MASTER_m
      USE VLIDORT_RTCALC_MASTER_m
      ...
      PUBLIC  VLIDORT_Run_Scalar      ! Run for each profile (pixel) 
      PUBLIC  VLIDORT_Run_Vector_cloud      ! Run for each profile (pixel) 
      PUBLIC  VLIDORT_Run_Scalar_cloud      ! Run for each profile (pixel)       
      ...
      interface VLIDORT_Run_Vector_cloud
         module procedure VLIDORT_Run_Vector_cloud_SingleGeom
         module procedure VLIDORT_Run_Vector_cloud_MultiGeom
      end interface VLIDORT_Run_Vector_cloud
      ...
      type VLIDORT_output_vector
         ...
         real*8     :: ADJUSTED_SLEAVE ! Adjusted water leaving radiance 
...
!
      ...
      USE VLIDORT_SETUP_MASTER_m
      USE VLIDORT_RTCALC_MASTER_m
      USE vfzmat_ExpandCoeffs_m, Only : vfzmat_ExpandCoeffs2
...
!     ---------------
      ...
      self%Surface%Base%VIO%VLIDORT_FixIn%Optical%TS_OMEGA_TOTAL_INPUT = omega_total_input
      self%Surface%Base%VIO%VLIDORT_FixIn%Optical%TS_GREEKMAT_TOTAL_INPUT = greekmat_total_input(0:MAXMOMENTS,:,:)

!     Geometry setup
      call VLIDORT_Setup_Master (self%Surface%Base%VIO%VLIDORT_FixIn, &
        self%Surface%Base%VIO%VLIDORT_ModIn, & ! INPUTS (possibly modified)
        self%Surface%Base%VIO%VLIDORT_Bookkeep, &
        self%Surface%Base%VIO%VLIDORT_FOGeom, &
        self%Surface%Base%VIO%VLIDORT_MSGeom, &
        self%Surface%Base%VIO%VLIDORT_Out )


!     Get FMATRIX needed for FO_CALC
      do_sunlight = .true.
      call vfzmat_ExpandCoeffs2 (MAXMOMENTS_INPUT, MAXLAYERS, &
           MAX_GEOMETRIES, self%NSTOKES, self%nmom, NLAYERS, &  ! Inputs
           self%Surface%Base%VIO%VLIDORT_Bookkeep%N_GEOMETRIES, & !Inputs
           do_sunlight, greekmat_total_input, & !Inputs
           self%Surface%Base%VIO%VLIDORT_FOGeom%GENSPHER_UP(0,1,1), & ! Inputs
           self%Surface%Base%VIO%VLIDORT_FixIn%Optical%TS_FMATRIX_UP)

      call vfzmat_ExpandCoeffs2 (MAXMOMENTS_INPUT, MAXLAYERS, &
           MAX_GEOMETRIES, self%NSTOKES, self%nmom, NLAYERS, &  ! Inputs
           self%Surface%Base%VIO%VLIDORT_Bookkeep%N_GEOMETRIES, & !Inputs
           do_sunlight, greekmat_total_input, & !Inputs
           self%Surface%Base%VIO%VLIDORT_FOGeom%GENSPHER_DN(0,1,1), & ! Inputs
           self%Surface%Base%VIO%VLIDORT_FixIn%Optical%TS_FMATRIX_DN)
       
!     Call the MASTER driver for doing the actual calculation
!     -----------------------------------------------------------
      do_debug_input = .false.
      call VLIDORT_RTCalc_MASTER (do_debug_input, &
           self%Surface%Base%VIO%VLIDORT_FixIn%Cont%TS_nstreams, &
           self%Surface%Base%VIO%VLIDORT_Bookkeep%NMOMENTS, &
           NLAYERS, &
...
!     -------------------
      ...
      if (self%Surface%sleave_adjust) then
          output%ADJUSTED_SLEAVE = self%Surface%Base%VIO%VLIDORT_Out%WLOut%TS_WLADJUSTED_ISOTROPIC(1,1)
      end if
...
!.............................................................................
      subroutine VLIDORT_Run (self,radiance, reflectance, AOT, SSA, rc,Q,U, &
                                 scalar,aerosol,ROT)
!
!     Computes radiances for a single wavelength, pixel. Optical properties
!     and met fields in self are assumed to have been updated with the
...
!
      ...
      real*8,dimension(:),optional,intent(out)   :: ROT         ! Rayleigh Optical Thickness
...
!     ---------------
      ...
      ! normalize scattering optical depth by total to get SSA
      SSA = SSA/AOT
...
!     Get FMATRIX needed for FO_CALC
      do_sunlight = .true.
      call vfzmat_ExpandCoeffs2 (MAXMOMENTS_INPUT, MAXLAYERS, &
           MAX_GEOMETRIES, NSTOKES, self%nmom, NLAYERS, &  ! Inputs
           self%Surface%Base%VIO%VLIDORT_Bookkeep%N_GEOMETRIES, & !Inputs
           do_sunlight, greekmat_total_input, & !Inputs
           self%Surface%Base%VIO%VLIDORT_FOGeom%GENSPHER_UP(0,1,1), & ! Inputs
           self%Surface%Base%VIO%VLIDORT_FixIn%Optical%TS_FMATRIX_UP)
...
!.............................................................................
      subroutine VLIDORT_Run_Vector_Cloud_SingleGeom (self, output, rc)
!
!     Computes radiances for a single wavelength, pixel. Optical properties
!     and met fields in self are assumed to have been updated with the
!     apropriate values.
...
!     -------------------
      ...
      end subroutine VLIDORT_Run_Vector_Cloud_SingleGeom


!.............................................................................
      subroutine VLIDORT_Run_Vector_Cloud_MultiGeom (self, output, rc)
!
!     Computes radiances for a single wavelength, pixel. Optical properties
!     and met fields in self are assumed to have been updated with the
!     apropriate values.
...
!     -------------------
      ...
      end subroutine VLIDORT_Run_Vector_Cloud_MultiGeom

!.............................................................................
      subroutine VLIDORT_Run_Scalar_Cloud (self, output, rc)
!
!     Computes radiances for a single wavelength, pixel. Optical properties
!     and met fields in self are assumed to have been updated with the
!     apropriate values.
...
!        ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~   
         ...
         greekmat_total_input(0,i,1) = 1.0
         greekmat_total_input(1,i,1) = aersmom(1) * aerswt + clLsmom(1) * clLswt + clIsmom(1) * clIswt
         greekmat_total_input(2,i,1) = raysmom2 * rayswt + aersmom(2) * aerswt + clLsmom(2) * clLswt + clIsmom(2) * clIswt

         do l = 3, self%nmom
            greekmat_total_input(l,i,1) = aersmom(l) * aerswt + clLsmom(l) * clLswt + clIsmom(l) * clIswt
         end do
...
!     -------------------
      ...
      end subroutine VLIDORT_Run_Scalar_cloud
