...
!  4/15/20. Version 1.5.2. Introduce doublet geometry option
...
subroutine FO_SSWPGeometry_Master &
       ( maxgeoms, maxszas, maxvzas, maxazms, maxlayers, maxpartials, maxfine, dtr, Pie, vsign,  & ! Input dimensions/constants
         eradius, do_obsgeom, do_doublet, do_Chapman, do_planpar, do_enhanced_ps, do_Partials,   & ! Input flags
...
!  4/15/20. Version 1.5.2, DOUBLET GEOMETRY option (flag do_doublet added)
...
!   4/15/20. Version 1.5.2. Doublet flag added
...
!   real(ffp)              :: cutoff
   ...
   character*2            :: c2
   character*3            :: c3, c33
   integer                :: v, n1, ns, nv, nd_offset(maxszas), nv_offset(maxszas), na_offset(maxszas,maxvzas)
...
!  Mode bookkeeping
!   4/15/20. Version 1.5.2. Doublet mode added
...
!  2. DOUBLET GEOMETRIES (4/15/20. Version 1.5.2. Section added for Doublet option)
...
!  4/15/20. Version 1.5.2. Add call to Doublet-geometry calculation
...
!  4/15/20. Version 1.5.2. Add calls to Doublet-geometry calculation
...
!mick fix 3/25/2015 - adjusted extent of 1st dim
      ...
      do v = 1, nvzas
         ...
         if (n1 .gt. maxfine ) then
            ...
            trace   = 'Dimensioning check AFTER call to LosOut_EnhancedPS_FindCrit, SSWPgeometry_master (LATTICE/DOUBLET mode)'
            fail = .true. ;  return
          endif
      enddo
