...
!  (1) For Profile WF tests

!     PART1 = .FALSE.
      PART1 = .TRUE.

!  (2) For Column WF tests
...
!   Updated, 2.8, 3/1/17.
!mick note 9/19/2017 - inputs in this section NOT currently active
!                      (not passed to VLIDORT)
...
!  Task
      ...
      k5 = 23
      task = 4; t = task; t4 = t
      write(*,*)'Doing task 4: '// 'Finite Difference, perturb aerosol optical depth in layer 23'
...
!        OPEN(36,file = 'vlidort_s_test/results_solar_lps_tester.all', status = 'unknown')
!      else
!        OPEN(36,file = 'vlidort_s_test/results_solar_lps_tester' // '_' // c3 // '.all', status = 'unknown')
!      end if

!  Version 2.8 only the FO calculation

      OutUnit = 36
      OPEN(OutUnit,file = 'vlidort_s_test/results_solar_lps_tester.all',status='unknown')
...
!  Task
      ...
      task = 5; t = task; t5 = t
      write(*,*)'Doing task 5: '// &
               'Finite Difference, perturb Lambertian albedo'
...
!  Task
      ...
      task = 6; t = task; t6 = t
      write(*,*)'Doing task 6: '// &
              'Finite Difference, perturb total molecular absoprtion'
...
!      c3 = 'FO'
!      if ( .not. VLIDORT_ModIn%MBool%TS_DO_FO_CALC ) then
!        OPEN(36,file = 'vlidort_s_test/results_solar_lcs_tester.all', status = 'unknown')
!      else
!        OPEN(36,file = 'vlidort_s_test/results_solar_lcs_tester' // '_' // c3 // '.all', status = 'unknown')
!      end if
