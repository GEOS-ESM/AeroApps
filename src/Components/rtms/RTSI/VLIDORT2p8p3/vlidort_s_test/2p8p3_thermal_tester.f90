...
!   Rob 3/22/15. renamed thread --> task
      ...
      INTEGER ::          M, N,N6,UTA,UTAM,L,NDUM,LDUM,V,T,NTASKS,TASK
      INTEGER ::          O1,I,J,IB,UM,IA
...
!   Array of temperatures should be 24 levels
      ...
      if ( NTEMPS .ne. NLAYERS ) then
         ...
         stop 'Test program 2p8p3_thermal_tester stopped'
      endif
      do n = 0, NTEMPS
...
!  Do the BB inputs
      ...
      if ( THERMFAIL ) THEN
         ...
         stop 'Failure from call to get_planckfunction in program 2p8p3_thermal_tester'
      endif
...
!  Fix layers with aerosol
        ...
        do n = n6 + 1, nlayers
          ...
          do L = 0, 2
            greekmat_total_input(L,n,1) = raywt * raymoms(L,n) + &
                                          aerwt * aermoms(L)
          enddo
...
!  For special telescoping check w/BRDF
...
!  Set sup inputs (hard-wired, copy VLIDORT Main inputs)
...
!  Rob Fix 9/29/14 Change names DBONLY, remove DO_EXACT flag
!          VBRDF_Sup_In%BS_DO_EXACT               = .false.
...
!  Perform supplement calculation for Tasks 5 through 6
...
!  This is the checking routine
          ...
          IF ( VLIDORT_VBRDFCheck_Status%TS_STATUS_INPUTCHECK .ne. VLIDORT_SUCCESS ) &
            CALL VLIDORT_VBRDF_INPUT_CHECK_ERROR ( &
              '2p8p3_VLIDORT_VBRDFcheck.log', VLIDORT_VBRDFCheck_Status )
...
!    The output will now be used for the Main calculation
          ...
          IF ( VBRDF_Sup_OutputStatus%BS_STATUS_OUTPUT .ne. VLIDORT_SUCCESS ) then
            ...
            enddo
            Stop 'VBRDF supplement master failed, in program 2p8p3_thermal_tester'
          ENDIF     

!  Output BRDF Supplement results (as check, Only Task 6)

          IF ( TASK .EQ. 6 ) THEN
            OutUnit = 36
            open(OutUnit, file='vlidort_s_test/results_brdf_thermcheck.res', status = 'unknown')
            write(OutUnit,'(/a/)') ' Number of Beams/Quad-streams/User-streams/User-relazms'
            write(OutUnit,'(4i5)') &
              VBRDF_Sup_In%bs_nbeams, &
              VBRDF_Sup_In%bs_nstreams, &
              VBRDF_Sup_In%bs_n_user_streams, &
              VBRDF_Sup_In%bs_n_user_relazms

            write(OutUnit,'(/T20,a/)') 'Exact BRDF, all geometries'
            do ib = 1, VBRDF_Sup_In%bs_nbeams
              do um = 1, VBRDF_Sup_In%bs_n_user_streams
                do ia = 1, VBRDF_Sup_In%bs_n_user_relazms
                  write(OutUnit,'(3i4,1p16e20.11)') ib, um, ia,   VLIDORT_Sup%BRDF%TS_exactdb_brdfunc(1,um,ia,ib)
                enddo
              enddo
            enddo

            do M = 0, BS_NMOMENTS_INPUT
              write(OutUnit,'(/T20,a,i3/)') 'Fourier component # ', M
              do i = 1, VBRDF_Sup_In%bs_nstreams
                write(OutUnit,99) I, (VLIDORT_Sup%BRDF%TS_BRDF_F(M,1,I,J),    J  = 1, VBRDF_Sup_In%bs_nstreams)
                write(OutUnit,99) I, (VLIDORT_Sup%BRDF%TS_BRDF_F_0(M,1,I,IB), IB = 1, VBRDF_Sup_In%bs_nbeams)
              enddo
              write(OutUnit,*) ' '
              do um= 1, VBRDF_Sup_In%bs_n_user_streams
                write(OutUnit,99) UM, (VLIDORT_Sup%BRDF%TS_USER_BRDF_F(M,1,um,J),    J  = 1, VBRDF_Sup_In%bs_nstreams)
                write(OutUnit,99) UM, (VLIDORT_Sup%BRDF%TS_USER_BRDF_F_0(M,1,um,IB), IB = 1, VBRDF_Sup_In%bs_nbeams)
              enddo
            enddo

            write(OutUnit,'(/T20,a/)') 'Emissivities'
            write(OutUnit,98) (VLIDORT_Sup%BRDF%TS_EMISSIVITY(1,J),      J = 1, VBRDF_Sup_In%bs_nstreams)
            write(OutUnit,98) (VLIDORT_Sup%BRDF%TS_USER_EMISSIVITY(1,J), J = 1, VBRDF_Sup_In%bs_n_user_streams)
...
!  Title of output file
      ...
      OutUnit = 36
      OPEN(OutUnit,file = 'vlidort_s_test/results_thermal_tester.all', status = 'unknown')
