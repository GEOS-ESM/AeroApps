...
! ###################################################################
      ...
      PROGRAM VLIDORT_SPHERCORR_TESTER_V2
...
!  MS 2-, 3- and Multi-point sphericity correction routines

      USE VLIDORT_SPHERCORR_ROUTINES_m
...
!  Help variables
      ...
      REAL(FPK) :: CUTOFF, MOM, EPS, EPSFAC, SK
...
!  VLIDORT control Read input, abort if failed
      ...
      CALL VLIDORT_INPUT_MASTER ( Trim(Input_Filename), & ! Input
      ...
      IF ( VLIDORT_InputStatus%TS_STATUS_INPUTREAD .ne. VLIDORT_SUCCESS ) &
        CALL VLIDORT_READ_ERROR ( 'VLIDORT_SpherCorr_ReadInput.log', VLIDORT_InputStatus )
...
!  Set Proxies (saved values)
      ...
      nstokes = VLIDORT_FixIn%Cont%TS_nstokes  ; ns = nstokes
...
!  Write to screen
      ...
      if ( MSType.eq.'2' .or. MSType.eq.'3' ) then
        write(*,'(/a/)')'This is a ' // MSType // '-pt sphericity correction test for the ' // &
...
!  Fix optical properties for the non-aerosol layers
      ...
      do n = 1, n6
        molabs = (one - molomg(n) ) * molext(n)
        molsca = molomg(n) * molext(n)
        totext = molsca + molabs
        deltau_vert_input(n) = totext
        omega_total_input(n) = molsca / totext
...
!  Call to dedicated sphericity correction routine (either 2-point, 3-point or multi-point)
      ...
      if ( MSType .eq. '2' ) then
        CALL MS2pt_SPHERICITY_CORRECTION_V3 ( do_debug_input, &
             ...
             FO_STOKES_BOAGEOM  (:,igeo), MS_STOKES_BOAGEOM(:,igeo),        & ! Component outputs (FO/MS)
             MS_STOKES_SPHERCORR(:,igeo),                                   & ! Component outputs (FO/MS)
             VLIDORT_Out, STOKES_BOAGEOM(:,igeo), STOKES_SPHERCORR(:,igeo), & ! VLIDORT and Complete outputs
...
!  Exception handling, write-up (optional)
      ...
      IF ( Fail ) THEN
        write(*,'(A)')'Failure from Call to MS'//MStype//'pt_SPHERICITY_CORRECTION, here are 2 local messages - '
        write(*,'(A)')'  --'//Trim(Local_Message)
        write(*,'(A)')'  --'//Trim(Local_Action)
        write(*,'(A)')'Now look at "VLIDORT_SpherCorr_tester_V3.log" file for error details'
        CALL VLIDORT_WRITE_STATUS ('VLIDORT_SpherCorr_tester_V3.log', VLIDORT_ERRUNIT, OPENFILEFLAG, VLIDORT_Out%Status )
...
!mick fix 1/5/2021 - added IF condition for case with zero U-component
      ...
      if ( .not. Twilight_Flag ) then
        ...
        DO iv = 1,NVZA
          ...
          write(701,155) igeo,sza_set(is),vza_set(iv),raa_set(ia),'  I  ',&
              FO_STOKES_BOAGEOM(1,igeo), MS_STOKES_BOAGEOM(1,igeo), STOKES_BOAGEOM(1,igeo),&
              MS_STOKES_SPHERCORR(1,igeo), STOKES_SPHERCORR(1,igeo),&
              100.0d0*(1.0-(MS_STOKES_SPHERCORR(1,igeo)/MS_STOKES_BOAGEOM(1,igeo))),&
              100.0d0*(1.0-(STOKES_SPHERCORR(1,igeo)/STOKES_BOAGEOM(1,igeo)))
        ENDDO ; ENDDO ; ENDDO
        ...
        if (nstokes.gt.1) then
          ...
          DO iv = 1,NVZA
            ...
            write(701,155) igeo,sza_set(is),vza_set(iv),raa_set(ia),'  Q  ',&
                FO_STOKES_BOAGEOM(2,igeo), MS_STOKES_BOAGEOM(2,igeo), STOKES_BOAGEOM(2,igeo),&
                MS_STOKES_SPHERCORR(2,igeo), STOKES_SPHERCORR(2,igeo),&
                100.0d0*(1.0-(MS_STOKES_SPHERCORR(2,igeo)/MS_STOKES_BOAGEOM(2,igeo))),&
                100.0d0*(1.0-(STOKES_SPHERCORR(2,igeo)/STOKES_BOAGEOM(2,igeo)))
          ENDDO ; ENDDO ; ENDDO
        endif
        ...
        if (nstokes.eq.3) then
          ...
          DO iv = 1,NVZA
            if (iv .eq. 1) write(701,*)
            igeo = igeo + 1
            if (abs(STOKES_BOAGEOM(3,igeo)) .gt. ZERO) then
              write(701,155) igeo,sza_set(is),vza_set(iv),raa_set(ia),'  U  ',&
                  FO_STOKES_BOAGEOM(3,igeo), MS_STOKES_BOAGEOM(3,igeo), STOKES_BOAGEOM(3,igeo),&
                  MS_STOKES_SPHERCORR(3,igeo), STOKES_SPHERCORR(3,igeo),&
                  100.0d0*(1.0-(MS_STOKES_SPHERCORR(3,igeo)/MS_STOKES_BOAGEOM(3,igeo))),&
                  100.0d0*(1.0-(STOKES_SPHERCORR(3,igeo)/STOKES_BOAGEOM(3,igeo)))
            else
              write(701,155) igeo,sza_set(is),vza_set(iv),raa_set(ia),'  U  ', &
                  FO_STOKES_BOAGEOM(3,igeo), MS_STOKES_BOAGEOM(3,igeo), STOKES_BOAGEOM(3,igeo),&
                  MS_STOKES_SPHERCORR(3,igeo), STOKES_SPHERCORR(3,igeo),&
...
!  Finish
      ...
      write(*,'(/a/)') 'Main program VLIDORT_SPHERCORR_TESTER_V2 finished successfully'

      END PROGRAM VLIDORT_SPHERCORR_TESTER_V2
