...
! ###################################################################
      ...
      PROGRAM VLIDORT_LPS_SPHERCORR_TESTER_V1
...
!  MS 2-, 3- and Multi-point sphericity correction modules
      ...
      USE VLIDORT_LPS_SPHERCORR_ROUTINES_m
...
!  Profile Jacobian output with explanations
      ...
      REAL(fpk) :: FO_PROFILEWF_BOAGEOM   ( MAX_ATMOSWFS, MAXLAYERS, MAXSTOKES, MAXGEO ) ! 1. FO (SS/DB) LP Jac for BOA geometry
      REAL(fpk) :: MS_PROFILEWF_BOAGEOM   ( MAX_ATMOSWFS, MAXLAYERS, MAXSTOKES, MAXGEO ) ! 2. MS LP Jac for BOA geometry
      REAL(fpk) :: MS_PROFILEWF_SPHERCORR ( MAX_ATMOSWFS, MAXLAYERS, MAXSTOKES, MAXGEO ) ! 3. MS LP Jac with 2-point corr
      REAL(fpk) :: PROFILEWF_BOAGEOM      ( MAX_ATMOSWFS, MAXLAYERS, MAXSTOKES, MAXGEO ) ! 4. Full LP Jac for BOA geometry      4 = 1 + 2
      REAL(fpk) :: PROFILEWF_SPHERCORR    ( MAX_ATMOSWFS, MAXLAYERS, MAXSTOKES, MAXGEO ) ! 5. Full LP Jac with Sphericity corr  5 = 1 + 3

!  Surface Jacobian output with explanations

      REAL(fpk) :: FO_SURFACEWF_BOAGEOM   ( MAX_SURFACEWFS, MAXSTOKES, MAXGEO ) ! 1. FO (SS/DB) LP Jac for BOA geometry
      REAL(fpk) :: MS_SURFACEWF_BOAGEOM   ( MAX_SURFACEWFS, MAXSTOKES, MAXGEO ) ! 2. MS LP Jac for BOA geometry
      REAL(fpk) :: MS_SURFACEWF_SPHERCORR ( MAX_SURFACEWFS, MAXSTOKES, MAXGEO ) ! 3. MS LP Jac with 2-point corr
      REAL(fpk) :: SURFACEWF_BOAGEOM      ( MAX_SURFACEWFS, MAXSTOKES, MAXGEO ) ! 4. Full LP Jac for BOA geometry      4 = 1 + 2
      REAL(fpk) :: SURFACEWF_SPHERCORR    ( MAX_SURFACEWFS, MAXSTOKES, MAXGEO ) ! 5. Full LP Jac with Sphericity corr  5 = 1 + 3
...
!  Other help variables
      ...
      INTEGER          :: I, IA, IS, IV, ICASE, IGEO, LAY, NGEO, CorrType, &
                          NSZA, NVZA, NRAA
...
!  VLIDORT control Read input, abort if failed
      ...
      IF ( VLIDORT_InputStatus%TS_STATUS_INPUTREAD .ne. VLIDORT_SUCCESS ) &
        CALL VLIDORT_READ_ERROR ( 'VLIDORT_LPS_SpherCorr_ReadInput.log', VLIDORT_InputStatus )
...
!  Write to screen
      ...
      if ( MSType.eq.'2' .or. MSType.eq.'3' ) then
        ...
        write(*,'(/A/)')'This is a MULTI-pt sphericity correction test for a ' // &
                        ' Scenario, Albedo '//CALB//'%'
      endif
...
!mick fix 1/5/2021 - added IF conditions for Q & U (as in "VLIDORT_SpherCorr_tester_V2.f90")
      ...
      if ( ICASE .eq. 1 ) then
        Output_FileName = 'vlidort_sc_test/results_LPS_SpherCorr_test_' // &
...
!  Linearization proxies
      ...
      DO_PROFILE_LINEARIZATION = .TRUE. ; layer_vary_number = 0
...
!  Call to dedicated sphericity correction routine (either 2-point, 3-point or multi-point)
      ...
      if ( MSType .eq. '2' ) then
        CALL MS2pt_LPS_SPHERCORR_V1 ( do_debug_input, & 
            ...
            FO_PROFILEWF_BOAGEOM  (:,:,:,igeo), MS_PROFILEWF_BOAGEOM(:,:,:,igeo),& ! LP Jacobian Component outputs (FO/MS)
            MS_PROFILEWF_SPHERCORR(:,:,:,igeo),                                  & ! LP Jacobian Component outputs (FO/MS)
            ...
            PROFILEWF_BOAGEOM     (:,:,:,igeo), PROFILEWF_SPHERCORR (:,:,:,igeo),& ! Linearized SpherCorr output
        ...
        CALL MS3pt_LPS_SPHERCORR_V1 ( do_debug_input, & 
            VLIDORT_FixIn, VLIDORT_ModIn, VLIDORT_Sup,                           & ! VLIDORT Standard   Inputs
            VLIDORT_LinFixIn, VLIDORT_LinModIn, VLIDORT_LinSup,                  & ! VLIDORT Linearized Inputs
            FO_STOKES_BOAGEOM     (:,igeo),     MS_STOKES_BOAGEOM(:,igeo),       & ! Standard Component outputs (FO/MS)
            MS_STOKES_SPHERCORR   (:,igeo),                                      & ! Standard Component outputs (FO/MS)
        ...
        CALL MSMpt_LPS_SPHERCORR_V1 ( do_debug_input, & 
            VLIDORT_FixIn, VLIDORT_ModIn, VLIDORT_Sup,                           & ! VLIDORT Standard   Inputs
            VLIDORT_LinFixIn, VLIDORT_LinModIn, VLIDORT_LinSup,                  & ! VLIDORT Linearized Inputs
            FO_STOKES_BOAGEOM     (:,igeo),     MS_STOKES_BOAGEOM(:,igeo),       & ! Standard Component outputs (FO/MS)
            MS_STOKES_SPHERCORR   (:,igeo),                                      & ! Standard Component outputs (FO/MS)
...
!  Exception handling, write-up (optional)
      ...
      IF ( Fail ) THEN
        write(*,'(A)')'Failure from Call to MS'//MStype//'pt_LPS_SPHERCORR_V1, here are 2 local messages - '
        write(*,'(A)')'  --'//Trim(Local_Message)
        write(*,'(A)')'  --'//Trim(Local_Action)
        write(*,'(A)')'Now look at "VLIDORT_LPS_SpherCorr_tester_V1.log" file for error details'
        CALL VLIDORT_WRITE_STATUS ('VLIDORT_LPS_SpherCorr_tester_V1.log', VLIDORT_ERRUNIT, OPENFILEFLAG, VLIDORT_Out%Status )
...
!  One line for each SZA and VZA BOA-geometry calculation
...
!    -- 6. relative difference (%) between SpherCorr and Standard VLIDORT results (MS FIELD ONLY)
!    -- 7. relative difference (%) between SpherCorr and Standard VLIDORT results (TOTAL FIELD)
!    -- No result for Twlilight cases (SZA > 90 at TOA or BOA destination). Set default value 99
      ...
      if ( .not. Twilight_Flag ) then
        ...
        DO iv = 1,NVZA
          ...
          write(701,155) igeo,sza_set(is),vza_set(iv),raa_set(ia),'  I  ',&
              FO_PROFILEWF_BOAGEOM(1,lay,1,igeo), MS_PROFILEWF_BOAGEOM(1,lay,1,igeo), PROFILEWF_BOAGEOM(1,lay,1,igeo), &
              MS_PROFILEWF_SPHERCORR(1,lay,1,igeo), PROFILEWF_SPHERCORR(1,lay,1,igeo),&
              100.0d0*(1.0-(MS_PROFILEWF_SPHERCORR(1,lay,1,igeo)/MS_PROFILEWF_BOAGEOM(1,lay,1,igeo))),&
              100.0d0*(1.0-(PROFILEWF_SPHERCORR(1,lay,1,igeo)/PROFILEWF_BOAGEOM(1,lay,1,igeo)))
        ENDDO ; ENDDO ; ENDDO
        ...
        if (nstokes.gt.1) then
          ...
          DO iv = 1,NVZA
            ...
            write(701,155) igeo,sza_set(is),vza_set(iv),raa_set(ia),'  Q  ',&
                FO_PROFILEWF_BOAGEOM(1,lay,2,igeo), MS_PROFILEWF_BOAGEOM(1,lay,2,igeo), PROFILEWF_BOAGEOM(1,lay,2,igeo), &
                MS_PROFILEWF_SPHERCORR(1,lay,2,igeo), PROFILEWF_SPHERCORR(1,lay,2,igeo),&
                100.0d0*(1.0-(MS_PROFILEWF_SPHERCORR(1,lay,2,igeo)/MS_PROFILEWF_BOAGEOM(1,lay,2,igeo))),&
                100.0d0*(1.0-(PROFILEWF_SPHERCORR(1,lay,2,igeo)/PROFILEWF_BOAGEOM(1,lay,2,igeo)))
          ENDDO ; ENDDO ; ENDDO
        endif
        ...
        if (nstokes.eq.3) then
          ...
          DO iv = 1,NVZA
            if (iv .eq. 1) write(701,*)
            igeo = igeo + 1
            if (abs(PROFILEWF_BOAGEOM(1,lay,3,igeo)) .gt. ZERO) then
              write(701,155) igeo,sza_set(is),vza_set(iv),raa_set(ia),'  U  ',&
                  FO_PROFILEWF_BOAGEOM(1,lay,3,igeo), MS_PROFILEWF_BOAGEOM(1,lay,3,igeo), PROFILEWF_BOAGEOM(1,lay,3,igeo), &
                  MS_PROFILEWF_SPHERCORR(1,lay,3,igeo), PROFILEWF_SPHERCORR(1,lay,3,igeo),&
                  100.0d0*(1.0-(MS_PROFILEWF_SPHERCORR(1,lay,3,igeo)/MS_PROFILEWF_BOAGEOM(1,lay,3,igeo))),&
                  100.0d0*(1.0-(PROFILEWF_SPHERCORR(1,lay,3,igeo)/PROFILEWF_BOAGEOM(1,lay,3,igeo)))
            else
              write(701,155) igeo,sza_set(is),vza_set(iv),raa_set(ia),'  U  ', &
                  FO_PROFILEWF_BOAGEOM(1,lay,3,igeo), MS_PROFILEWF_BOAGEOM(1,lay,3,igeo), PROFILEWF_BOAGEOM(1,lay,3,igeo), &
                  MS_PROFILEWF_SPHERCORR(1,lay,3,igeo), PROFILEWF_SPHERCORR(1,lay,3,igeo),&
...
156   format(1x,i3,3f8.3,a5,7es16.8,i2)
...
!  Finish
      ...
      write(*,'(/a/)')'Main program VLIDORT_LPS_SPHERCORR_TESTER_V1 finished successfully'

      END PROGRAM VLIDORT_LPS_SPHERCORR_TESTER_V1
