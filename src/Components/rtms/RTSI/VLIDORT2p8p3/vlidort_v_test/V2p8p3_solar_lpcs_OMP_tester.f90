...
!  mick chg 4/3/2015 - renamed thread --> task
      ...
      INTEGER ::          O1,Q,TASK,NTASKS, UTA,K3,K4,K5, KN
      INTEGER ::          NBEAMS
...
!  Saved baseline results
      ...
      DOUBLE PRECISION, DIMENSION(:,:,:,:,:,:,:), ALLOCATABLE :: &
        PROFWF_SAVE, MEANST_PROFWF_SAVE, FLUX_PROFWF_SAVE

      DOUBLE PRECISION, DIMENSION(:,:,:,:,:,:), ALLOCATABLE :: &
        COLWF_SAVE,  MEANST_COLWF_SAVE,  FLUX_COLWF_SAVE, &
        SURFWF_SAVE, MEANST_SURFWF_SAVE, FLUX_SURFWF_SAVE
...
!  VLIDORT control Read input, abort if failed
      ...
      CALL VLIDORT_L_INPUT_MASTER ( &
        ...
        VLIDORT_LinFixIn_SAVE,   & ! Outputs
        VLIDORT_LinModIn_SAVE,   & ! Outputs
...
!  Initialize some input variables not handled by VLIDORT_L_INPUT_MASTER
      ...
      CALL VLIDORT_LinSup_Init ( VLIDORT_LinSup_SAVE )
...
!  Set output file designation for later use
      ...
      do_observation_geometry = VLIDORT_ModIn_SAVE%MBool%TS_DO_OBSERVATION_GEOMETRY
...
!mick add 3/25/2015 - added FO control (by hand)
!  Re-define number of output levels for testing FO code
!    since its output is presently restricted to LAYER BOUNDARIES
!      VLIDORT_ModIn_SAVE%MBool%TS_DO_FO_CALC = .false.
!      !VLIDORT_ModIn_SAVE%MBool%TS_DO_FO_CALC = .true.
!      IF ( VLIDORT_ModIn_SAVE%MBool%TS_DO_FO_CALC ) then
!         n_user_levels = 3
!         user_levels(1:2) = VLIDORT_ModIn_SAVE%MUserVal%TS_USER_LEVELS(1:2)
!         user_levels(3)   = VLIDORT_ModIn_SAVE%MUserVal%TS_USER_LEVELS(5)
!         VLIDORT_ModIn_SAVE%MUserVal%TS_USER_LEVELS  = zero
!         VLIDORT_FixIn_SAVE%UserVal%TS_N_USER_LEVELS = n_user_levels
!         VLIDORT_ModIn_SAVE%MUserVal%TS_USER_LEVELS(1:3) = user_levels(1:3)
!      endif
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
      ...
      write(*,*)'Doing PROFILEWF part: '// &
        'Intensities + 3 profile Jacobians + 1 Surface Jacobian'
...
!     Allocate output memory
      ...
      ALLOCATE ( & 
        ...
        STOKES_BAS          ( MAX_USER_LEVELS, MAX_GEOMETRIES, MAXSTOKES, &
                              MAX_DIRECTIONS, MAXWVN ), &
        MEANST_BAS          ( MAX_USER_LEVELS, MAX_SZANGLES,   MAXSTOKES, &
                              MAX_DIRECTIONS, MAXWVN ), &
        FLUX_BAS            ( MAX_USER_LEVELS, MAX_SZANGLES,   MAXSTOKES, &
                              MAX_DIRECTIONS, MAXWVN ), &

        PROFWF_SAVE         ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_GEOMETRIES, &
                              MAXSTOKES, MAX_DIRECTIONS, MAXWVN ), &
        SURFWF_SAVE         ( MAX_SURFACEWFS, MAX_USER_LEVELS, MAX_GEOMETRIES, &
                              MAXSTOKES, MAX_DIRECTIONS, MAXWVN ), &

        MEANST_PROFWF_SAVE  ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_SZANGLES, &
                              MAXSTOKES, MAX_DIRECTIONS, MAXWVN ), &
        MEANST_SURFWF_SAVE  ( MAX_SURFACEWFS, MAX_USER_LEVELS, MAX_SZANGLES, &
                              MAXSTOKES, MAX_DIRECTIONS, MAXWVN ), &

        FLUX_PROFWF_SAVE    ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_SZANGLES, &
                              MAXSTOKES, MAX_DIRECTIONS, MAXWVN ), &
        FLUX_SURFWF_SAVE    ( MAX_SURFACEWFS, MAX_USER_LEVELS, MAX_SZANGLES, &
                              MAXSTOKES, MAX_DIRECTIONS, MAXWVN ), &
...
!     Initialize output
        ...
        MEANST_PROFWF_SAVE = ZERO
        MEANST_SURFWF_SAVE = ZERO

        FLUX_PROFWF_SAVE = ZERO
        FLUX_SURFWF_SAVE = ZERO
...
!$OMP     NSTOKES, N_SZANGLES, NUMWVN, NLAYERS, NGREEKMAT_ENTRIES, &
!$OMP     HEIGHT_GRID, MOLEXT, MOLOMG, PROBLEM_RAY, &
!$OMP     AERMOMS, NGREEK_MOMENTS_INPUT, LAMBERTIAN_ALBEDO_SAVE, EPS, &
...
!$OMP     OMP_N_GEOMETRIES, STOKES_BAS, MEANST_BAS, FLUX_BAS, &
!$OMP     PROFWF_SAVE, SURFWF_SAVE, &
!$OMP     MEANST_PROFWF_SAVE, MEANST_SURFWF_SAVE, &
!$OMP     FLUX_PROFWF_SAVE, FLUX_SURFWF_SAVE, &
!$OMP     STOKES_PT, MEANST_PT, FLUX_PT) &
...
!  Copy local standard control. Upgraded for Version 2.8. 3/1/17
...
!  Save baseline results for the current wavenumber

      do uta = 1, n_user_levels
        do o1 = 1, nstokes

          do v = 1, n_geometries
            stokes_BAS(uta,v,o1,upidx,wvn) = VLIDORT_Out%Main%TS_stokes(uta,v,o1,upidx)
            stokes_BAS(uta,v,o1,dnidx,wvn) = VLIDORT_Out%Main%TS_stokes(uta,v,o1,dnidx)
            do n = 1, nlayers
              do q = 1, n_totalprofile_wfs
                PROFWF_SAVE(q,n,uta,v,o1,upidx,wvn) = VLIDORT_LinOut%Prof%TS_profilewf(q,n,uta,v,o1,upidx)
                PROFWF_SAVE(q,n,uta,v,o1,dnidx,wvn) = VLIDORT_LinOut%Prof%TS_profilewf(q,n,uta,v,o1,dnidx)
              enddo
            enddo
            do q = 1, n_surface_wfs
              SURFWF_SAVE(q,uta,v,o1,upidx,wvn) = VLIDORT_LinOut%Surf%TS_surfacewf(q,uta,v,o1,upidx)
              SURFWF_SAVE(q,uta,v,o1,dnidx,wvn) = VLIDORT_LinOut%Surf%TS_surfacewf(q,uta,v,o1,dnidx)
            enddo
          enddo

          do v = 1, n_szangles
            MEANST_BAS(uta,v,o1,upidx,wvn) = VLIDORT_Out%Main%TS_MEANST_DIFFUSE(uta,v,o1,upidx)
            MEANST_BAS(uta,v,o1,dnidx,wvn) = VLIDORT_Out%Main%TS_MEANST_DIFFUSE(uta,v,o1,dnidx) &
                                           + VLIDORT_Out%Main%TS_DNMEANST_DIRECT(uta,v,o1)
            FLUX_BAS(uta,v,o1,upidx,wvn)   = VLIDORT_Out%Main%TS_FLUX_DIFFUSE(uta,v,o1,upidx)
            FLUX_BAS(uta,v,o1,dnidx,wvn)   = VLIDORT_Out%Main%TS_FLUX_DIFFUSE(uta,v,o1,dnidx) &
                                           + VLIDORT_Out%Main%TS_DNFLUX_DIRECT(uta,v,o1)

            do n = 1, nlayers
              do q = 1, n_totalprofile_wfs
                MEANST_PROFWF_save(q,n,uta,v,o1,upidx,wvn) = VLIDORT_LinOut%Prof%TS_MEANST_DIFFUSE_PROFWF(q,n,uta,v,o1,upidx)
                MEANST_PROFWF_save(q,n,uta,v,o1,dnidx,wvn) = VLIDORT_LinOut%Prof%TS_MEANST_DIFFUSE_PROFWF(q,n,uta,v,o1,dnidx) &
                                                           + VLIDORT_LinOut%Prof%TS_DNMEANST_DIRECT_PROFWF(q,n,uta,v,o1)
                FLUX_PROFWF_SAVE(q,n,uta,v,o1,upidx,wvn)   = VLIDORT_LinOut%Prof%TS_FLUX_DIFFUSE_PROFWF(q,n,uta,v,o1,upidx)
                FLUX_PROFWF_SAVE(q,n,uta,v,o1,dnidx,wvn)   = VLIDORT_LinOut%Prof%TS_FLUX_DIFFUSE_PROFWF(q,n,uta,v,o1,dnidx) &
                                                           + VLIDORT_LinOut%Prof%TS_DNFLUX_DIRECT_PROFWF(q,n,uta,v,o1)
              enddo
            enddo
            do q = 1, n_surface_wfs
              MEANST_SURFWF_SAVE(q,uta,v,o1,upidx,wvn) = VLIDORT_LinOut%Surf%TS_MEANST_DIFFUSE_SURFWF(q,uta,v,o1,upidx)
              MEANST_SURFWF_SAVE(q,uta,v,o1,dnidx,wvn) = VLIDORT_LinOut%Surf%TS_MEANST_DIFFUSE_SURFWF(q,uta,v,o1,dnidx)
              FLUX_SURFWF_SAVE(q,uta,v,o1,upidx,wvn)   = VLIDORT_LinOut%Surf%TS_FLUX_DIFFUSE_SURFWF(q,uta,v,o1,upidx)
              FLUX_SURFWF_SAVE(q,uta,v,o1,dnidx,wvn)   = VLIDORT_LinOut%Surf%TS_FLUX_DIFFUSE_SURFWF(q,uta,v,o1,dnidx)
            enddo
          enddo
...
!  Task 2 or 3
        ...
        if ( task.eq.2 .or. task.eq.3  ) then
          ...
          !  ': Finite Difference, perturb molecular absorption Layer ',kn
            ...
            do L = 0, ngreek_moments_input
              do k = 1, 16
                greekmat_total_input(L,n,k) = &
                  greekmat_total_input_save(L,n,k)
              enddo
            enddo
          enddo
...
!  Save perturbed results for the current wavenumber
...
!  Write Final file of Profile WF test results for all threads
!  ===========================================================
...
!  Shorthand
      ...
      n_geometries = OMP_N_GEOMETRIES(t1,1)
...
!  Prepare output file
      ...
      if ( do_std_output_file ) then
        ...
        OPEN(OutUnit,file='vlidort_v_test/results_solar_lps_tester_' &
             // trim(c1) // '_' // c2 // '.all' // trim(tail),status='replace')
      endif
...
!  Write Stokes Intensity and Mean values
      ...
      call write_stokes_lps ( &
        OutUnit, 1, 'I', wvn, VLIDORT_TID_SAVE, k3, k4, k5, t1, t2, t3, t4, &
        maxtasks, n_geometries, n_szangles, n_user_levels, maxwvn, &
        user_levels, lambertian_albedo_save, eps, &
        Stokes_BAS(:,:,:,:,wvn),  MEANST_BAS(:,:,:,:,wvn), &
        FLUX_BAS(:,:,:,:,wvn), &
        Stokes_PT(:,:,:,:,:,wvn), MEANST_PT(:,:,:,:,:,wvn), &
        FLUX_PT(:,:,:,:,:,wvn), &
        PROFWF_SAVE(:,:,:,:,:,:,wvn),        SURFWF_SAVE(:,:,:,:,:,wvn), &
        MEANST_PROFWF_SAVE(:,:,:,:,:,:,wvn), MEANST_SURFWF_SAVE(:,:,:,:,:,wvn), &
        FLUX_PROFWF_SAVE(:,:,:,:,:,:,wvn),   FLUX_SURFWF_SAVE(:,:,:,:,:,wvn) )

!  Write Stokes Q and U and Mean values

      if ( nstokes .ge. 3 ) then
        call write_stokes_lps ( &
          OutUnit, 2, 'Q', wvn, VLIDORT_TID_SAVE, k3, k4, k5, t1, t2, t3, t4, &
          maxtasks, n_geometries, n_szangles,  n_user_levels, maxwvn, &
          user_levels, lambertian_albedo_save, eps, &
          Stokes_BAS(:,:,:,:,wvn),  MEANST_BAS(:,:,:,:,wvn), &
          FLUX_BAS(:,:,:,:,wvn), &
        ...
        call write_stokes_lps ( &
          OutUnit, 3, 'U', wvn, VLIDORT_TID_SAVE, k3, k4, k5, t1, t2, t3, t4, &
          maxtasks, n_geometries, n_szangles,  n_user_levels, maxwvn, &
          user_levels, lambertian_albedo_save, eps, &
          Stokes_BAS(:,:,:,:,wvn),  MEANST_BAS(:,:,:,:,wvn), &
          FLUX_BAS(:,:,:,:,wvn), &
...
!  Write Stokes V and Mean values
      ...
      if ( nstokes .eq. 4 ) then
        call write_stokes_lps ( &
          OutUnit, 4, 'V', wvn, VLIDORT_TID_SAVE, k3, k4, k5, t1, t2, t3, t4, &
          maxtasks, n_geometries, n_szangles,  n_user_levels, maxwvn, &
          user_levels, lambertian_albedo_save, eps, &
          Stokes_BAS(:,:,:,:,wvn),  MEANST_BAS(:,:,:,:,wvn), &
          FLUX_BAS(:,:,:,:,wvn), &
...
!     Deallocate output memory
      ...
      DEALLOCATE ( OMP_N_GEOMETRIES, &
                   STOKES_BAS, MEANST_BAS, FLUX_BAS, &
                   PROFWF_SAVE, SURFWF_SAVE, &
                   MEANST_PROFWF_SAVE, MEANST_SURFWF_SAVE, &
                   FLUX_PROFWF_SAVE, FLUX_SURFWF_SAVE, &
                   STOKES_PT,  MEANST_PT,  FLUX_PT )
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
      ...
      write(*,*)'Doing COLUMNWF part: '// &
       'Intensities + 2 Column Jacobians + 1 Surface Jacobian'
...
!     Allocate output memory
      ...
      ALLOCATE ( &
        ...
        COLWF_SAVE          ( MAX_ATMOSWFS, MAX_USER_LEVELS, MAX_GEOMETRIES, &
                              MAXSTOKES, MAX_DIRECTIONS, MAXWVN ), &
        SURFWF_SAVE         ( MAX_SURFACEWFS, MAX_USER_LEVELS, MAX_GEOMETRIES, &
                              MAXSTOKES, MAX_DIRECTIONS, MAXWVN ), &

        MEANST_COLWF_SAVE   ( MAX_ATMOSWFS, MAX_USER_LEVELS, MAX_SZANGLES, &
                              MAXSTOKES, MAX_DIRECTIONS, MAXWVN ), &
        MEANST_SURFWF_SAVE  ( MAX_SURFACEWFS, MAX_USER_LEVELS, MAX_SZANGLES, &
                              MAXSTOKES, MAX_DIRECTIONS, MAXWVN ), &

        FLUX_COLWF_SAVE     ( MAX_ATMOSWFS, MAX_USER_LEVELS, MAX_SZANGLES, &
                              MAXSTOKES, MAX_DIRECTIONS, MAXWVN ), &
        FLUX_SURFWF_SAVE    ( MAX_SURFACEWFS, MAX_USER_LEVELS, MAX_SZANGLES, &
                              MAXSTOKES, MAX_DIRECTIONS, MAXWVN ), &
...
!     Initialize output
        ...
        MEANST_COLWF_SAVE  = ZERO
        MEANST_SURFWF_SAVE = ZERO

        FLUX_COLWF_SAVE  = ZERO
        FLUX_SURFWF_SAVE = ZERO
...
!$OMP     VLIDORT_LinFixIn_SAVE, VLIDORT_LinModIn_SAVE, VLIDORT_LinSup_SAVE, &
!$OMP     OMP_N_GEOMETRIES, STOKES_BAS, MEANST_BAS, FLUX_BAS, &
!$OMP     COLWF_SAVE, SURFWF_SAVE, &
!$OMP     MEANST_COLWF_SAVE, MEANST_SURFWF_SAVE, &
!$OMP     FLUX_COLWF_SAVE, FLUX_SURFWF_SAVE, &
!$OMP     STOKES_PT, MEANST_PT, FLUX_PT) &
!$OMP   FIRSTPRIVATE (OPENFILEFLAG)
        ...
        do o1 = 1, nstokes
          ...
          do v = 1, n_geometries
            ...
            do q = 1, n_totalcolumn_wfs
              COLWF_SAVE(q,uta,v,o1,upidx,wvn) = VLIDORT_LinOut%Col%TS_columnwf(q,uta,v,o1,upidx)
              COLWF_SAVE(q,uta,v,o1,dnidx,wvn) = VLIDORT_LinOut%Col%TS_columnwf(q,uta,v,o1,dnidx)
            enddo
            do q = 1, n_surface_wfs
              SURFWF_SAVE(q,uta,v,o1,upidx,wvn) = VLIDORT_LinOut%Surf%TS_surfacewf(q,uta,v,o1,upidx)
              SURFWF_SAVE(q,uta,v,o1,dnidx,wvn) = VLIDORT_LinOut%Surf%TS_surfacewf(q,uta,v,o1,dnidx)
          ...
          do v = 1, n_szangles
            ...
            do q = 1, n_totalcolumn_wfs
              MEANST_COLWF_save(q,uta,v,o1,upidx,wvn) = VLIDORT_LinOut%Col%TS_MEANST_DIFFUSE_COLWF(q,uta,v,o1,upidx)
              MEANST_COLWF_save(q,uta,v,o1,dnidx,wvn) = VLIDORT_LinOut%Col%TS_MEANST_DIFFUSE_COLWF(q,uta,v,o1,dnidx) &
                                                      + VLIDORT_LinOut%Col%TS_DNMEANST_DIRECT_COLWF(q,uta,v,o1)
              FLUX_COLWF_SAVE(q,uta,v,o1,upidx,wvn)   = VLIDORT_LinOut%Col%TS_FLUX_DIFFUSE_COLWF(q,uta,v,o1,upidx)
              FLUX_COLWF_SAVE(q,uta,v,o1,dnidx,wvn)   = VLIDORT_LinOut%Col%TS_FLUX_DIFFUSE_COLWF(q,uta,v,o1,dnidx) &
...
!  Write Final file of Column WF test results for all threads
!  ==========================================================
...
!  Shorthand
      ...
      n_geometries = OMP_N_GEOMETRIES(t5,1)
...
!  write Stokes Intensity and Mean values
        ...
        call write_stokes_lcs ( &
          OutUnit, 1, 'I', wvn, VLIDORT_TID_SAVE, t5, t6, t7, &
          maxtasks, n_geometries, n_szangles,  n_user_levels, maxwvn, &
          user_levels, lambertian_albedo_save, eps, &
          Stokes_BAS(:,:,:,:,wvn),  MEANST_BAS(:,:,:,:,wvn),  FLUX_BAS(:,:,:,:,wvn), &
          Stokes_PT(:,:,:,:,:,wvn), MEANST_PT(:,:,:,:,:,wvn), FLUX_PT(:,:,:,:,:,wvn), &
          COLWF_SAVE(:,:,:,:,:,wvn),        SURFWF_SAVE(:,:,:,:,:,wvn), &
          MEANST_COLWF_SAVE(:,:,:,:,:,wvn), MEANST_SURFWF_SAVE(:,:,:,:,:,wvn), &
          FLUX_COLWF_SAVE(:,:,:,:,:,wvn),   FLUX_SURFWF_SAVE(:,:,:,:,:,wvn) )

!  write Stokes Q and U and Mean values

        if ( NSTOKES .ge. 3 ) then
          call write_stokes_lcs ( &
            OutUnit, 2, 'Q', wvn, VLIDORT_TID_SAVE, t5, t6, t7, &
            maxtasks, n_geometries, n_szangles,  n_user_levels, maxwvn, &
            user_levels, lambertian_albedo_save, eps, &
            Stokes_BAS(:,:,:,:,wvn),  MEANST_BAS(:,:,:,:,wvn),  FLUX_BAS(:,:,:,:,wvn), &
            Stokes_PT(:,:,:,:,:,wvn), MEANST_PT(:,:,:,:,:,wvn), FLUX_PT(:,:,:,:,:,wvn),   &
          ...
          call write_stokes_lcs ( &
            OutUnit, 3, 'U', wvn, VLIDORT_TID_SAVE, t5, t6, t7, &
            maxtasks, n_geometries, n_szangles,  n_user_levels, maxwvn, &
            user_levels, lambertian_albedo_save, eps, &
            Stokes_BAS(:,:,:,:,wvn),  MEANST_BAS(:,:,:,:,wvn),  FLUX_BAS(:,:,:,:,wvn), &
            Stokes_PT(:,:,:,:,:,wvn), MEANST_PT(:,:,:,:,:,wvn), FLUX_PT(:,:,:,:,:,wvn), &
...
!  write Stokes V and Mean values
        ...
        if ( NSTOKES .eq. 4 ) then
          call write_stokes_lcs ( &
            OutUnit, 4, 'V', wvn, VLIDORT_TID_SAVE, t5, t6, t7, &
            maxtasks, n_geometries, n_szangles,  n_user_levels, maxwvn, &
            user_levels, lambertian_albedo_save, eps, &
            Stokes_BAS(:,:,:,:,wvn),  MEANST_BAS(:,:,:,:,wvn),  FLUX_BAS(:,:,:,:,wvn), &
            Stokes_PT(:,:,:,:,:,wvn), MEANST_PT(:,:,:,:,:,wvn), FLUX_PT(:,:,:,:,:,wvn), &
...
!     Deallocate output memory
      ...
      DEALLOCATE ( OMP_N_GEOMETRIES, &
                   ...
                   MEANST_COLWF_SAVE, MEANST_SURFWF_SAVE, &
                   FLUX_COLWF_SAVE,   FLUX_SURFWF_SAVE, &
                   STOKES_PT,  MEANST_PT,  FLUX_PT )
...
!  ****************************************************************************
      subroutine write_stokes_lps ( &
        un, o1, a1, wvn, vlidort_tid_save, k3, k4, k5, t1, t2, t3, t4, &
        maxtasks, n_geometries, nbeams, n_user_levels, maxwvn, &
        user_levels, lambertian_albedo_save, eps, &
        Stokes_BAS, MEANST_BAS, FLUX_BAS, &
        Stokes_PT,  MEANST_PT,  FLUX_PT, &
        PROFWF_SAVE,        SURFWF_SAVE, &
...
!  input
      ...
      integer, intent(in) ::           maxtasks, n_geometries, nbeams, n_user_levels, maxwvn
...
!
      ...
      subroutine write_stokes_lcs ( &
        un, o1, a1, wvn, vlidort_tid_save, t5, t6, t7, &
        maxtasks, n_geometries, nbeams,  n_user_levels, maxwvn, &
        user_levels, lambertian_albedo_save, eps, &
        Stokes_BAS, MEANST_BAS, FLUX_BAS, &
        Stokes_PT,  MEANST_PT,  FLUX_PT, &
