...
!   Array of temperatures should be 24 levels
      ...
      if ( NTEMPS .ne. NLAYERS ) then
         ...
         stop 'Test program V2p8p3_lpcs_thermal_tester stopped'
      endif
      do n = 0, NTEMPS
...
!  Do the BB inputs
      ...
      call get_planckfunction &
         ( wnumlo,wnumhi, SURFTEMP, SURFBB, SMALLV, THERMFAIL, THERMALMESSAGE )
      do n = 0, nlayers
         call get_planckfunction &
         ( wnumlo,wnumhi, AIRTEMPS(N), THERMAL_BB_INPUT(N), SMALLV, THERMFAIL, THERMALMESSAGE )
      enddo
      if ( THERMFAIL ) THEN
         write(*,'(A)')Trim(THERMALMESSAGE)
         stop 'Failure from call to get_planckfunction in program V2p8p3_lpcs_thermal_tester'
      endif
...
!        OPEN(OutUnit,file='vlidort_v_test/results_thermal_lps_tester_' &
!             // c2 // '.all',status='unknown')
!      else
!        OPEN(OutUnit,file='vlidort_v_test/results_thermal_lps_tester_' &
!             // c2 // '_' // c3 //'.all',status='unknown')
!      end if
...
!      c3 = 'FO'
!      if ( .not. VLIDORT_ModIn%MBool%TS_DO_FO_CALC ) then
!        OPEN(OutUnit,file='vlidort_v_test/results_thermal_lcs_tester_' &
!             // c2 // '.all',status='unknown')
!      else
!        OPEN(OutUnit,file='vlidort_v_test/results_thermal_lcs_tester_' &
!             // c2 // '_' // c3 //'.all',status='unknown')
