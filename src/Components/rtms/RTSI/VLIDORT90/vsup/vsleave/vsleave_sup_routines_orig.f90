...
! Absorption/scattering terms, and parameters for calculation of f
      real a_wat,b_wat,b_wat_all,a_chl,f,a_tot,b_tot,a_ph,a_cdom,v,bp,bbp,eta
...
!  Gaussian-quadrature calls
   ...
   call VSleave_GAULEG(zero,pio2,PolarQuads,PolarWeights,Max_PolarQuads)
   call VSleave_GAULEG(zero,pi2, AzimQuads, AzimWeights, Max_AzimQuads )
...
!Obtain solar data if necessary
      ...
      if (wvl(1) < 0.0d0) then
        if (solar_file == 1) then
          ...
          open(unit=50,file='vlidort_v_test/data/wehrli85.dat',&
          ...
          open(unit=50,file='vlidort_v_test/data/ref_solar_irradiance_whi-2008_ver2.dat',&
               status='old',action='read')
...
!  End module
      ...
      END MODULE vsleave_sup_routines_m
