...
! #                                                             #
! #              VLIDORT_LC_QBEAM_SOLUTION                      #
! #              VLIDORT_LC_UBEAM_SOLUTION                      #
! #                                                             #
! ###############################################################


      MODULE vlidort_lc_solutions

      PRIVATE
      PUBLIC :: VLIDORT_LC_QBEAM_SOLUTION, &
                VLIDORT_LC_UBEAM_SOLUTION

      CONTAINS

      SUBROUTINE VLIDORT_LC_QBEAM_SOLUTION ( &
...
!  Linearized output (Ostensibly out)
      ...
      DOUBLE PRECISION, INTENT (INOUT) ::  LC_BVEC &
...
!     &           m,ibeam,i,O1,BVEC(I,O1,N),LC_BVEC(I,O1,N,Q)
          ENDDO
         ENDDO
        ENDDO
...
!  Finish
      ...
      RETURN
      END SUBROUTINE VLIDORT_LC_QBEAM_SOLUTION

!

      SUBROUTINE VLIDORT_LC_UBEAM_SOLUTION ( &
        GIVEN_LAYER, FOURIER, IBEAM, &
...
!  Linearized (Ostensibly OUTPUT)
      ...
      DOUBLE PRECISION, INTENT (INOUT) :: L_UPAR_UP_1 &
          ( MAX_USER_STREAMS, MAXSTOKES, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, INTENT (INOUT) :: LC_UPAR_DN_2 &
          ( MAX_USER_STREAMS, MAXSTOKES, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, INTENT (INOUT) :: LC_UPAR_UP_2 &
          ( MAX_USER_STREAMS, MAXSTOKES, MAXLAYERS, MAX_ATMOSWFS )
          ...
          DO Q = 1, N_PARAMETERS
           IF ( DO_UPWELLING ) THEN
            DO O1 = 1, NSTOKES
              ...
              LC_UPAR_UP_2(IB,O1,N,Q) = ZERO
            ENDDO
           ENDIF
           IF ( DO_DNWELLING ) THEN
            DO O1 = 1, NSTOKES
              L_UPAR_DN_1(IB,O1,N,Q)  = ZERO
              LC_UPAR_DN_2(IB,O1,N,Q) = ZERO
            ENDDO
           ENDIF
          ENDDO
        ENDIF
        ...
        DO L = M, NMOMENTS
         DO O1 = 1, NSTOKES
          ...
          DO J = 1, NSTREAMS
           ...
           DO O2 = 1, NSTOKES
            HH = H * SGW(O2)
            SP = SP + H  * PI_XQP    (L,J,O1,O2)*LC_BVEC(J,O2,N,Q)
            SM = SM + HH * PI_XQM_PRE(L,J,O1,O2)*LC_BVEC(J1,O2,N,Q)
           ENDDO
           SPS = SPS + SP + SM
          ENDDO
...
!  Finish
      ...
      RETURN
      END SUBROUTINE VLIDORT_LC_UBEAM_SOLUTION

      END MODULE vlidort_lc_solutions
