...
!;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
! NAME
!    satellite_angles_from_space
!
! PURPOSE
!    For a satellite and a given target location,
!    calculate spacecraft Azimuth Viewing Angle VAA
!    and spacecraft Viewing Angle VA;
...
vangles = view_angles_from_target(latp,lonp,latss,lonss,hgtss)
phiv    = vangles(1)
thetav  = 90.0 - vangles(2)
...
subroutine satAngles_fromSpace(n,yr,mo,day,hr,min,sec,latp,lonp,latss,lonss,tz,hgtss, &
                     view_azimuth, view_angle,  &
                     solar_azimuth, solar_zenith )
...
real(kind=8), intent(out) :: view_azimuth(n)
real(kind=8), intent(out) :: view_angle(n)
real(kind=8), intent(out) :: solar_azimuth(n)
real(kind=8), intent(out) :: solar_zenith(n)
...
!                ---
  ...
  do i = 1, n
     ...
     sat_angles = satellite_angles_from_space(yr(i),mo(i),day(i),hr(i),min(i),sec(i),tz, &
                                   latp(i),lonp(i),latss(i),lonss(i),hgtss)

     view_azimuth(i)  = sat_angles(1)
     view_angle(i)   = sat_angles(2)
     solar_azimuth(i) = sat_angles(3)
     solar_zenith(i)  = sat_angles(4)

  end do

end subroutine satAngles_fromSpace
