    ...
    def computeMie(self):
        ...
        if self.simple_aerosol and self.aerosol:
            ...
            if self.emde:
                ...
                f = open('Expansion_Coefs/PM_GRASP_simple_aerosol_renorm.txt.expan_coeff','r')
                f.readline() #header
                pmom[:] = 0
                ich = 0
                iob = 0
                for l in f:
                    ...
                    pmom[:,ich,iob,n,1] = float(P12)
                    pmom[:,ich,iob,n,2] = float(P33)
                    pmom[:,ich,iob,n,3] = float(P34)
                    pmom[:,ich,iob,n,4] = float(P11)
                    pmom[:,ich,iob,n,5] = float(P33)

                f.close()


                # Read in reconstructed function calculated with Mischenko's code
                f = open('Expansion_Coefs/PM_GRASP_simple_aerosol_renorm.txt.expan_matr','r')
                p11 = []
                p22 = []
                p33 = []
                p44 = []
...
if __name__ == "__main__":
    ...
    simple_aerosol = False # true if exponential aerosol with constnat optical properties
    ...
    for channel in channels:
        ...
        if aerosol is False:
            ...
            if simple_aerosol:
                ...
                aname = 'simple_aerosol'
                if emde:
                    aname = 'simple_aerosol+emde'
                elif grasp:
                    aname = 'simple_aerosol+grasp'
                elif grasp_higher:
