...
!  $Id: qsat_Mod.F90,v 1.1 2012-11-01 13:20:36 adasilva Exp $
...
if(UTBL) then
   ...
   if(present(PL)) then
      ...
      if(present(DQ)) DQ = DDQ*(1.0/DELTA_T)
   end if
...
if(UTBL) then
   ...
   if    (TL(I)<=TMINLQU) then
      QS(I)=ESTLQU
      if(present(DQ)) DDQ = 0.0
   elseif(TL(I)>=TMAXTBL  ) then
   ...
   elseif(TL(I)>TMAXTBL) then
      ...
      elseif(TYPE==2) then   !  Fit used in CAM.
         TT = TS/TI
         QS(I) = 10.0**(  DL(1)*(TT - 1.0) + DL(2)*LOG10(TT)             + &
              DL(3)*(10.0**(DL(4)*(1.0 - (1.0/TT))) - 1.0)/10000000.0 + &
              DL(5)*(10.0**(DL(6)*(TT -   1.0    )) - 1.0)/1000.0     + &
              LOGPS + 2.0                                               )
      elseif(TYPE==3) then   !  Murphy and Koop (2005, QJRMS)
   ...
   if(present(PL)) then
      ...
      if(present(DQ)) DQ(I) = DDQ*(1.0/DELTA_T)
   end if
...
if(UTBL) then
   ...
   if    (TL(I,J)<=TMINLQU) then
      QS(I,J)=ESTLQU
      if(present(DQ)) DDQ = 0.0
   elseif(TL(I,J)>=TMAXTBL  ) then
   ...
   elseif(TL(I,J)>TMAXTBL) then
      ...
      if    (TYPE==1) then
         TT = TI-ZEROC       !  Starr polynomial fit
         QS(I,J) = (TT*(TT*(TT*(TT*(TT*(TT*B6+B5)+B4)+B3)+B2)+B1)+B0)
      elseif(TYPE==2) then   !  Fit used in CAM.
         TT = TS/TI
         QS(I,J) = 10.0**(  DL(1)*(TT - 1.0) + DL(2)*LOG10(TT)             + &
              DL(3)*(10.0**(DL(4)*(1.0 - (1.0/TT))) - 1.0)/10000000.0 + &
              DL(5)*(10.0**(DL(6)*(TT -   1.0    )) - 1.0)/1000.0     + &
              LOGPS + 2.0                                               )
      elseif(TYPE==3) then   !  Murphy and Koop (2005, QJRMS)
   ...
   if(present(PL)) then
      ...
      if(present(DQ)) DQ(I,J) = DDQ*(1.0/DELTA_T)
   end if
...
if(UTBL) then
   ...
   if    (TL(I,J,K)<=TMINLQU) then
      QS(I,J,K)=ESTLQU
      if(present(DQ)) DDQ = 0.0
   elseif(TL(I,J,K)>=TMAXTBL  ) then
   ...
   elseif(TL(I,J,K)>TMAXTBL) then
      ...
      if    (TYPE==1) then
         TT = TI-ZEROC       !  Starr polynomial fit
         QS(I,J,K) = (TT*(TT*(TT*(TT*(TT*(TT*B6+B5)+B4)+B3)+B2)+B1)+B0)
      elseif(TYPE==2) then   !  Fit used in CAM.
         TT = TS/TI
         QS(I,J,K) = 10.0**(  DL(1)*(TT - 1.0) + DL(2)*LOG10(TT)             + &
              DL(3)*(10.0**(DL(4)*(1.0 - (1.0/TT))) - 1.0)/10000000.0 + &
              DL(5)*(10.0**(DL(6)*(TT -   1.0    )) - 1.0)/1000.0     + &
              LOGPS + 2.0                                               )
      elseif(TYPE==3) then   !  Murphy and Koop (2005, QJRMS)
         QS(I,J,K) = exp(                     (CL(0) + CL(1)/TI + CL(2)*alog(TI) + CL(3)*TI) + &
              tanh(CL(4)*(TI-CL(5))) * (CL(6) + CL(7)/TI + CL(8)*alog(TI) + CL(9)*TI)   )
      endif
   ...
   if(present(PL)) then
      ...
      if(present(DQ)) DQ(I,J,K) = DDQ*(1.0/DELTA_T)
   end if
...
if(UTBL) then
   ...
   if( type==1) then     ! Use Starr formulation
      ...
      elseif(TT >= TSTARR1 .and. TT < TSTARR2) then
         ...
         QS =       W *(TT*(TT*(TT*(TT*(TT*(TT*S16+S15)+S14)+S13)+S12)+S11)+S10) &
      ...
      elseif(TT >= TSTARR3 .and. TT < TSTARR4) then
         ...
         QS =       W *(TT*(TT*(TT*(TT*(TT*(TT*S26+S25)+S24)+S23)+S22)+S21)+S20) &
...
if(UTBL) then
   ...
   if    (TL(I)<=TMINTBL) then
      QS(I)=ESTBLE(1)
      if(present(DQ)) DDQ = 0.0
   elseif(TL(I)>=ZEROC  ) then
      QS(I)=ESTFRZ
      if(present(DQ)) DDQ = 0.0
   else
      TT  = (TL(I) - TMINTBL)*DEGSUBS+1
   ...
   if( type==1) then     ! Use Starr formulation
      TT = TI - ZEROC
      if    (TT < TSTARR1                   ) then
         QS(I) = (TT*(TT*(TT*(TT*(TT*(TT*S16+S15)+S14)+S13)+S12)+S11)+S10)
      elseif(TT >= TSTARR1 .and. TT < TSTARR2) then
         W = (TSTARR2 - TT)/(TSTARR2-TSTARR1)
         QS(I) =       W *(TT*(TT*(TT*(TT*(TT*(TT*S16+S15)+S14)+S13)+S12)+S11)+S10) &
              + (1.-W)*(TT*(TT*(TT*(TT*(TT*(TT*S26+S25)+S24)+S23)+S22)+S21)+S20)
      elseif(TT >= TSTARR2 .and. TT < TSTARR3) then
         QS(I) = (TT*(TT*(TT*(TT*(TT*(TT*S26+S25)+S24)+S23)+S22)+S21)+S20)
      elseif(TT >= TSTARR3 .and. TT < TSTARR4) then
         W = (TSTARR4 - TT)/(TSTARR4-TSTARR3)
         QS(I) =       W *(TT*(TT*(TT*(TT*(TT*(TT*S26+S25)+S24)+S23)+S22)+S21)+S20) &
              + (1.-W)*(TT*(TT*(TT*(TT*(TT*(TT*BI6+BI5)+BI4)+BI3)+BI2)+BI1)+BI0)
      else
         QS(I) = (TT*(TT*(TT*(TT*(TT*(TT*BI6+BI5)+BI4)+BI3)+BI2)+BI1)+BI0)
      endif
   elseif(type==2) then !  Fit used in CAM.
      TT = ZEROC/TI
...
if(UTBL) then
   ...
   if    (TL(I,J)<=TMINTBL) then
      QS(I,J)=ESTBLE(1)
      if(present(DQ)) DDQ = 0.0
   elseif(TL(I,J)>=ZEROC  ) then
      QS(I,J)=ESTFRZ
      if(present(DQ)) DDQ = 0.0
   else
      TT  = (TL(I,J) - TMINTBL)*DEGSUBS+1
   ...
   if( type==1) then     ! Use Starr formulation
      TT = TI - ZEROC
      if    (TT < TSTARR1                   ) then
         QS(I,J) = (TT*(TT*(TT*(TT*(TT*(TT*S16+S15)+S14)+S13)+S12)+S11)+S10)
      elseif(TT >= TSTARR1 .and. TT < TSTARR2) then
         W = (TSTARR2 - TT)/(TSTARR2-TSTARR1)
         QS(I,J) =       W *(TT*(TT*(TT*(TT*(TT*(TT*S16+S15)+S14)+S13)+S12)+S11)+S10) &
              + (1.-W)*(TT*(TT*(TT*(TT*(TT*(TT*S26+S25)+S24)+S23)+S22)+S21)+S20)
      elseif(TT >= TSTARR2 .and. TT < TSTARR3) then
         QS(I,J) = (TT*(TT*(TT*(TT*(TT*(TT*S26+S25)+S24)+S23)+S22)+S21)+S20)
      elseif(TT >= TSTARR3 .and. TT < TSTARR4) then
         W = (TSTARR4 - TT)/(TSTARR4-TSTARR3)
         QS(I,J) =       W *(TT*(TT*(TT*(TT*(TT*(TT*S26+S25)+S24)+S23)+S22)+S21)+S20) &
              + (1.-W)*(TT*(TT*(TT*(TT*(TT*(TT*BI6+BI5)+BI4)+BI3)+BI2)+BI1)+BI0)
      else
         QS(I,J) = (TT*(TT*(TT*(TT*(TT*(TT*BI6+BI5)+BI4)+BI3)+BI2)+BI1)+BI0)
      endif
   elseif(type==2) then !  Fit used in CAM.
      TT = ZEROC/TI
...
if(UTBL) then
   ...
   if    (TL(I,J,K)<=TMINTBL) then
      QS(I,J,K)=ESTBLE(1)
      if(present(DQ)) DDQ = 0.0
   elseif(TL(I,J,K)>=ZEROC  ) then
      QS(I,J,K)=ESTFRZ
      if(present(DQ)) DDQ = 0.0
   else
      TT  = (TL(I,J,K) - TMINTBL)*DEGSUBS+1
   ...
   if( type==1) then     ! Use Starr formulation
      TT = TI - ZEROC
      if    (TT < TSTARR1                   ) then
         QS(I,J,K) = (TT*(TT*(TT*(TT*(TT*(TT*S16+S15)+S14)+S13)+S12)+S11)+S10)
      elseif(TT >= TSTARR1 .and. TT < TSTARR2) then
         W = (TSTARR2 - TT)/(TSTARR2-TSTARR1)
         QS(I,J,K) =       W *(TT*(TT*(TT*(TT*(TT*(TT*S16+S15)+S14)+S13)+S12)+S11)+S10) &
              + (1.-W)*(TT*(TT*(TT*(TT*(TT*(TT*S26+S25)+S24)+S23)+S22)+S21)+S20)
      elseif(TT >= TSTARR2 .and. TT < TSTARR3) then
         QS(I,J,K) = (TT*(TT*(TT*(TT*(TT*(TT*S26+S25)+S24)+S23)+S22)+S21)+S20)
      elseif(TT >= TSTARR3 .and. TT < TSTARR4) then
         W = (TSTARR4 - TT)/(TSTARR4-TSTARR3)
         QS(I,J,K) =       W *(TT*(TT*(TT*(TT*(TT*(TT*S26+S25)+S24)+S23)+S22)+S21)+S20) &
              + (1.-W)*(TT*(TT*(TT*(TT*(TT*(TT*BI6+BI5)+BI4)+BI3)+BI2)+BI1)+BI0)
      else
         QS(I,J,K) = (TT*(TT*(TT*(TT*(TT*(TT*BI6+BI5)+BI4)+BI3)+BI2)+BI1)+BI0)
      endif
   elseif(type==2) then !  Fit used in CAM.
      TT = ZEROC/TI
      QS(I,J,K) = DI(0) * exp( -(DI(1)/TT + DI(2)*log(TT) + DI(3)*TT) )
   elseif(type==3) then   !  Murphy and Koop (2005, QJRMS)
      QS(I,J,K) = exp( CI(0)+ CI(1)/TI + CI(2)*alog(TI) + CI(3)*TI )
   endif
