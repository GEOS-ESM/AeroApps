...
for base_date in $FLIGHT_DATES; do
    ...
    m2_in="${OUTPUT_DIR}/${CAMPAIGN}-MERRA2-aer-Nv-${PLANE}_Model_${base_date}_R0.nc"
    m21c_in="${OUTPUT_DIR}/${CAMPAIGN}-MERRA21C-aer-Nv-${PLANE}_Model_${base_date}_R0.nc"

    for wl in $WAVELENGTHS; do
        m2_out="${OUTPUT_DIR}/${CAMPAIGN}-MERRA2-ext${wl}-${PLANE}_Model_${base_date}_R0.nc"
        m21c_out="${OUTPUT_DIR}/${CAMPAIGN}-MERRA21C-ext${wl}-${PLANE}_Model_${base_date}_R0.nc"
        ...
        if [ -f "$m2_in" ]; then
            ...
            echo "  -> Warning: Input file missing for MERRA-2 aop: ${m2_in}"
        fi
        ...
        if [ -f "$m21c_in" ]; then
            ...
            echo "  -> Warning: Input file missing for MERRA-21C aop: ${m21c_in}"
        fi
    done
