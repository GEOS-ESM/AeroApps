...
def process_flight_data(date, config, target_variable, model_versions, wavelength):
    """
    ...
    obs_rev = config.get('obs_revision', '') 
    
    # Try different cases for the campaign name
    campaign_original = config['campaign']
    campaign_variations = [
        campaign_original, campaign_original.upper(), campaign_original.lower(),
        "DISCOVER-AQ", "discover-aq", "Discover-AQ"
    ]
    
    obs_files = []
    for camp_name in campaign_variations:
        if obs_rev:
            search_pattern = f"{camp_name}-{instrument}_{config['plane']}_{date}_{obs_rev}*.h5"
        else:
    ...
    # Added UserInput to catch HSRL2 DEM variables
    ...
    for obs_file in obs_files:
        ...
        # 1. Check for Observation Data Variable
        found_data = False
        obs_data = None
        if obs_var_name in ds_obs:
            ...
            for group in ['Data_Products', 'DataProducts']:
                try:
                    ...
                    if obs_var_name in ds_obs_group:
                        obs_data = ds_obs_group[obs_var_name].values
                        found_data = True
                        break
                except Exception:
                    pass
        ...
        if np.all(np.isnan(obs_data)):
            continue

        # 2. Check for Time Coordinate
        obs_time_var = None
        for group in groups_to_check:
            try:
                ds_temp = xr.open_dataset(obs_file, group=group, engine='h5netcdf', backend_kwargs=b_kwargs)
                for k in ['time', 'Midtime', 'gps_time', 'Time', 'Time_UTC', 'UTC_time', 
                          'AppTime', 'mid_time', 'Start_Time', 'profile_time', 'time_offset']:
                    if k in ds_temp:
        ...
        if obs_time_var is None:
            continue

        # 3. Check for Altitude Coordinate
        obs_alt_var = None
        for group in groups_to_check:
            try:
                ds_temp = xr.open_dataset(obs_file, group=group, engine='h5netcdf', backend_kwargs=b_kwargs)
                # Removed 'GPS_Alt' so it doesn't accidentally grab the 1D aircraft flight altitude array
                for k in ['Altitude', 'Altitudes', 'z', 'altitude', 'Height', 'height', 'MSL_Alt', 'Data_Altitude']:
                    if k in ds_temp:
        ...
        if surface_elev_var is not None:
            ...
            pass # Terrain not found, remains ASL
    ...
    rev_pattern = f"_{obs_rev}*" if obs_rev else "_R*"
    
    aer_file_pattern = f"{config['campaign']}-MERRA21C-aer-Nv-{config['plane']}_Model_{date}{rev_pattern}.nc*"
    aer_files = sorted(glob.glob(os.path.join(model_dir, aer_file_pattern)))
    
    if not aer_files:
        return None
        
    ds_aer = xr.open_mfdataset(aer_files, combine='nested', concat_dim='time')
    ...
    for mv in model_versions:
        model_file_pattern = f"{config['campaign']}-{mv}-{collection}-{config['plane']}_Model_{date}{rev_pattern}.nc*"
        model_files = sorted(glob.glob(os.path.join(model_dir, model_file_pattern)))
        
        if not model_files:
            continue
            
        ds_model = xr.open_mfdataset(model_files, combine='nested', concat_dim='time')
        ...
        model_shape_tracker[mv] = model_data.shape

        # Model data magnitude fallback check
        mean_mod = np.nanmean(model_data)
        mean_obs = np.nanmean(values)
        if mean_mod > 0 and mean_obs > 0:
            if mean_mod < mean_obs / 100.0:
                print(f"[{date} - {config['campaign']}] Warning: Auto-scaling model {mv} data by 1000.")
                model_data = model_data * 1000.0

        model_time_hours = standardize_time_to_hours(model_time, date)
        
        if len(model_time_hours) != Z_mod_km.shape[0]:
            continue
    ...
    if len(model_shape_tracker) > 1:
        shapes = list(model_shape_tracker.values())
        if not all(shape == shapes[0] for shape in shapes):
            raise ValueError(f"DIMENSION MISMATCH ERROR on date {date}")
            
    return flight_results

def get_campaign_profile_data(config_path, target_variable="EXT", model_versions=["MERRA2", "MERRA21C"], wavelength=532):
    """Extracts aggregate stats for an entire campaign using multiprocessing for speed."""
    ...
    # Use ProcessPoolExecutor to parallelize processing across different flight dates
    process_func = partial(process_flight_data, config=config, target_variable=target_variable, 
                           model_versions=model_versions, wavelength=wavelength)
    
    with ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
        futures = {executor.submit(process_func, date): date for date in flight_dates}
    ...
    obs_p15, obs_p85 = np.nanpercentile(obs_2d, [25, 75], axis=0)
    ...
    for mv in model_versions:
        if all_models_data[mv]:
            ...
            model_stats[mv] = {
                ...
                'p15': np.nanpercentile(mod_2d, 25, axis=0),
                'p85': np.nanpercentile(mod_2d, 75, axis=0)
            }
...
def main():
    ...
    campaigns = [
        {"config": "config_discoveraq11.yaml", "label": "a) DISCOVER-AQ (Baltimore, 2011)"},
        {"config": "config_discoveraq13.yaml", "label": "b) DISCOVER-AQ (Texas, 2016)"},
        {"config": "config_seac4rs.yaml", "label": "c) SEAC4RS (U.S., 2013)"},
        {"config": "config_korusaq.yaml", "label": "d) KORUS-AQ (Korea, 2016)"},
        {"config": "config_traceraq.yaml", "label": "e) Tracer-AQ (Texas, 2021)"},
        {"config": "config_asiaaqKorea.yaml", "label": "f) ASIA-AQ (Korea, 2024)"}
    ]
    ...
    if target_variable == "BSC":
        x_axis_title = f"{wavelength} nm Aerosol {var_display_name} (m$^{{-1}}$ sr$^{{-1}}$)"
        conversion_factor = 0.001
    else:
        x_axis_title = f"{wavelength} nm Aerosol {var_display_name} (km$^{{-1}}$)"
    ...
    for i, camp_info in enumerate(campaigns):
        ...
        # Plot labels applied correctly to bottom row and left column
        if i in [4, 5]: # Bottom row
            ax.set_xlabel(x_axis_title)
        if i in [0, 2, 4]: # Left column
    ...
    plt.tight_layout()
    output_fig = f"Urban_Campaign_Profiles_{target_variable}_{wavelength}.png"
