...
DEEP = ( "mean_AOD0412dpbl-l",
         ...
         "mean_QAfilteredAexp0412-0470dpbl-l",
         "mean_QAfilteredSSA0412dpbl-l",
         "mean_QAfilteredSSA0470dpbl-l",
         "mean_QAfilteredSSA0660dpbl-l",
         "mean_cfracdpbl-l",
         "mean_mref0412dpbl-l",
         "mean_mref0470dpbl-l",
         "mean_mref0660dpbl-l",
...
class GIANT(object):
  ...
  def __init__ (self,Path=None,syn_time=None,nsyn=8,Verb=False,doMeta=True,
     ...
     """
       Create an GIANT object defining the attributes corresponding
     ...
     if Path is None:
            self.nobs = 0
            print "WARNING: Empty GIANT object created"
            return         
...
#---
  def writeODS(self, syn_tyme, filename=None, dir='.', expid='avhrr', nsyn=8, doNNR=False):
        """
        Writes the un-gridded GIANT object to an ODS file.
        """
...
#---
  def writeGridded(self, syn_tyme,
       ...
       """
        Writes gridded GIANT AOD to a GFIO file.
...
#      ------------
       ...
       vtitle = [ 'GIANT Aerosol Optical Depth at 630nm (NOAA CDR)',]
       vname  = ['tau_630', ]
       vunits = [ '1',  ]
       kmvar  = [  0 ,  ]
       levs = array([630.,])
       ...
       if doNNR:
         vtitle += [ 'GIANT Aerosol Optical Depth at 550nm (NASA NNR)',]
         vname  += ['tau_550', ]
         vunits += [ '1',  ]
         kmvar  += [  0 ,  ]
         levs = array([550.,])
       
       title = 'Gridded GIANT Aerosol Retrievals'
...
def granules(sat, orb, tyme,
              bracket=True,
              RootDir='/Users/adasilva/data/GIANT/Level2B',
...
def _count_des():
  ...
  RootDir = '/nobackup/GIANT/Level2/PATMOSX'

  Files = sorted(glob(RootDir+'/????/???/*_des_*.hdf'))

  f = open('des_inventory.txt','w')
  for fname in Files:
    a = GIANT_L2B(fname,doMeta=False,Verb=False)
    if a.nobs>0:
...
if __name__ == "__main__":
    ...
    a = GIANT_L2B('/nobackup/GIANT/Level2/NPZ/2008/*.npz',Verb=True)
...
def later():
    ...
    a.sampleFile(gas_x)
    #b = GIANT_L2B(Files,Verb=True,doMeta=False)
    for syn_hour in range(0,24,3):
        syn_tyme = tyme + timedelta(seconds=syn_hour*60*60)
        #a.writeODS(syn_tyme)
        a.writeGridded(syn_tyme)
