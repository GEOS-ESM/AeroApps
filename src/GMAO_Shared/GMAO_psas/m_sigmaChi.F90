...
! !INTERFACE:
    ...
    module m_sigmaChi
      ...
      public :: sigmaChi_matvec
      public :: sigmaChi_adjvec

      interface sigmaChi_matvec; module procedure	&
	matvec_; end interface
      interface sigmaChi_adjvec; module procedure	&
...
!EOP ___________________________________________________________________
  ...
  character(len=*),parameter :: myname='m_sigmaChi'
...
! !INTERFACE:
	...
	nsize,ktab,jtab,nvecs,Cvec,Rvec		)
      ...
      use FEsigW_imat,only : FEsigV_imat
...
!EOP ___________________________________________________________________
  ...
  real,allocatable,dimension(:) :: sigChi
...
!-----------------------------------------------------------------------
	...
	allocate(sigChi(nsize),stat=ier)
  ...
  do inav=1,lsize(nav)
    ...
    case (ktUU,ktVV,ktUs,ktVs)
      ...
      sigChi(lc:le)=(/ (FEsigV_imat(ktab(i),jtab(i)), i=lc,le) /)

      do ivec=1,nvecs
	Rvec(lc:le,ivec)=sigChi(lc:le)*Cvec(lc:le,ivec)
      end do
...
!-----------------------------------------------------------------------
  ...
  do inav=1,lsize(nav)
    ...
    case (ktUU,ktVV,ktUs,ktVs)
      ...
      do ivec=1,nvecs
	Cvec(lc:le,ivec)=sigChi(lc:le)*Rvec(lc:le,ivec)
      end do
