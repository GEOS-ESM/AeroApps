...
void init_hash(hash_t *h, int nbuckets) {
  ...
  if(!h->bucket_list) {
    printf("hash.c $Name: GEOSadas-5_21_2_OPS $ line=%d : Could not allocate bucket list\n",__LINE__);
    exit(1);
  }
...
void init_bucket(bucket_t *b) {
  ...
  if(!b->entry_list) {
    printf("hash.c $Name: GEOSadas-5_21_2_OPS $ line=%d : Could not allocate entry list\n",__LINE__);
    exit(1);
  }
}
...
int create_hash(int nbuckets)
{
  ...
  if(!hash_heap) {
    ...
    if(!hash_heap) {
      printf("hash.c $Name: GEOSadas-5_21_2_OPS $ line=%d : Could not allocate hash_heap\n",__LINE__);
      exit(1);
    }
  ...
  if(newhash==hash_heap_size) {
    ...
    if(!hash_heap) {
      printf("hash.c $Name: GEOSadas-5_21_2_OPS $ line=%d : Could not expand hash_heap\n",__LINE__);
      exit(1);
    }
...
void destroy_hash(int h)
{
  ...
  if(!hash_heap) {
    printf("hash.c $Name: GEOSadas-5_21_2_OPS $ line=%d : Attempt to destroy hash from empty heap\n",__LINE__);
    exit(1);
  } else if(!hash_heap[h].bucket_list) {
    printf("hash.c $Name: GEOSadas-5_21_2_OPS $ line=%d : Attempt to destroy uninitalized hash\n",__LINE__);
    exit(1);
  } else { 
...
int increment_hash(int h, int i, int j, int k)
{
  ...
  if(!hash_heap) {
    ...
    printf("hash.c $Name: GEOSadas-5_21_2_OPS $ line=%d : Attempt to increment hash from empty heap\n",__LINE__);
    exit(1);

  } else if(!(hash_heap[h].bucket_list)) {

    printf("hash.c $Name: GEOSadas-5_21_2_OPS $ line=%d : Attempt to increment uninitalized hash %d i=%d j=%d %ld\n",
	...
	if(!bucket->entry_list) {
	  printf("hash.c $Name: GEOSadas-5_21_2_OPS $ %d : Could not reallocate entry list\n",__LINE__);
	  exit(1);
	}
      }
