...
!BOP
!
! !ROUTINE: ODS_GetR() --- Read data from file to native real format
! 
! !DESCRIPTION:
!    \label{ODS:GetR}
!     Reads the contents of a real variable for a full synoptic
...
! !OUTPUT PARAMETERS:
      real            values ( nval ) ! Values read from file
...
!                            made in other routines to create ODS version
!                            2.00
...
*     Determine the indices of the first and last batch
...
*     ------------------------------
      ...
      count ( 1 ) =   first_nval
      count ( 2 ) =   1
      call ODS_NCVGTR ( nc_id, varid, start, count, values, ierr )
      if ( ierr .ne. NCNoErr ) return
...
*     ---------------------------------------------------
      ...
      do 10, ibatch = first_batch + 1, last_batch - 1
         ...
         count ( 1 ) = batch_len
         count ( 2 ) = 1
         call ODS_NCVGTR ( nc_id, varid, start, count,
     .                     values ( ival ), ierr )
         if ( ierr .ne. NCNoErr ) return
         ival        = ival + batch_len
 10   continue
...
*     ------
 ...
 901  format ( /, ' ODS_GetR: File handle id number does not ',
     .         /, '           correspond to an opened ODS file' )
 902  format ( /, ' ODS_GetR: Inappropriate variable for this ',
     .         /, '           routine.  Variable name is: ', a ) 
 903  format ( /, ' ODS_GetR: The number of values to be read ',
